...
<html  lang="ru">
<head>
<title>Kotlin и Android | Однонаправленный поток данных</title>
<meta charset="utf-8" />
<meta name="description" content="Однонаправленный поток данных в Jetpack Compose на языке программирования Kotlin, unidirectional data flow, передача состояния из родительских компонентов в дочерние">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Однонаправленный поток данных</h2><div class="date">Последнее обновление: 04.04.2024</div>
	<div class="socBlock">
	...
	<p>Однонаправленный поток данных (unidirectional data flow) представляет подход, при котором состояние компонента не может напрямую изменяться дочерними, вложенными компонентами. Применение однонаправленного потока данных 
упрощает создание модульных, независимых друг от друга компонентов и, как следствие, упрощает разработку приложения и его тестирование. Например, возьмем следующий пример:</p>
<pre class="brush:kt;">
...
<p>Здесь у нас есть компонент Counter. Этот компонент определяет состояние clicksState, которое хранит количество нажатий, и функицю onClicksChange, которая изменяет это состояние:</p>
<pre class="brush:kt;">
val clicksState = remember{mutableStateOf(0)}
val onClicksChange = { value : Int -&gt;
    clicksState.value = value
...
<p><code>onClicksChange</code> представлет функцию, которая принимает один параметр типа Int - новое значение и передает его в состояние clicksState.</p>
<p>Внутри Counter для вывода состояния и его изменения определены два компонента:</p>
<pre class="brush:kt;">
Column{
    Text(text = "Clicks: ${clicksState.value}", fontSize = 28.sp )
    Increment(clicksState.value, onClicksChange)
...
<p>Первый компонент - Text просто выводит текущее состояние на экран устройства.</p>
<p>Второй компонент - Increment призван изменять значение состояния. В него как раз передаются текущее значение состояния и функция изменения состояния. Посмотрим на его определение:</p>
<pre class="brush:kt;">
@Composable
fun Increment(clicks: Int, onClicksChange : (Int) -&gt; Unit){
    Text(
...
<p>Данный компонент в качестве параметров принимает текущее количество нажатий (по сути текущее значение состояния) и функцию для изменения. Внутри этого компонента также определен компонент Text с 
примитивной стилизацией под кнопку увеличения. По нажатию на этот компонент будет вызываться переданная функция <code>onClicksChange()</code>, в которую передается новое значение нажатий.</p>
<p>Таким образом, при нажатии на компонент Increment произойдет вызов функции onClicksChange и изменение состояния"</p>
<img src="./pics/5.8.png" alt="Unidirectional data flow в приложении на Jetpack Compose на Kotlin на Android" />
<p>Таким образом, мы можем отдельно развивать компоненты Counter и Increment. При этом, хотя и Increment вызывает изменение состояния, но реальное изменение - определение функции onClicksChange 
все равно происходит в Counter. Increment напрямую <span class="b">не изменяет</span> состояние компонента Counter. </p>
...
<style>
	...
	<div class="nav"><p><a href="./5.2.php">Назад</a><a href="./">Содержание</a><a href="./5.4.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
