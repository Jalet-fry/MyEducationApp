...
<html  lang="ru">
<head>
<title>Kotlin и Android | AndroidViewModel</title>
<meta charset="utf-8" />
<meta name="description" content="Модель представления AndroidViewModel и ее отличие от ViewModel в мобильном приложении на Jetpack Compose на языке программирования Kotlin в Android, получение контекста приложения и его ресурсов">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>AndroidViewModel</h1><div class="date">Последнее обновление: 01.06.2025</div>
	<div class="socBlock">
...
<p>При работе с современной архитектурой Android-приложений, компоненты ViewModel из библиотеки Jetpack стали золотым стандартом для управления данными UI. Они решают классическую проблему потери 
состояния при изменении конфигурации (например, при повороте экрана). Однако, разработчики часто сталкиваются с выбором между двумя классами: ViewModel и его подклассом AndroidViewModel. 
Посмотрим, чем они различаются.</p>
<p>ViewModel — это класс, предназначенный для хранения и управления данными, связанными с пользовательским интерфейсом, с учётом жизненного цикла.  
Его основная задача - пережить изменения конфигурации. Когда объекты Activity или Fragment пересоздаются, экземпляр ViewModel, связанный с ними, остаётся в памяти, сохраняя все свои данные. 
Новый экземпляр Activity просто подключается к уже существующей ViewModel.</p>
<p>Но ViewModel ничего не знает о фреймворке Android Framework и не имеет доступа к Context или любым другим специфичным для Android классам.</p>
<p>Почему это хорошо?</p>
<ul>
<li><p>Разделение ответственности (Separation of Concerns): логика приложения (в ViewModel) полностью отделена от графического интерфейса (в Activity/Fragment/Composable).</p></li>
<li><p>Простота тестирования: поскольку ViewModel не зависит от Android, её можно легко протестировать с помощью обычных юнит-тестов. Вам не нужно имитировать (мокать) Context или другие сложные объекты Android.</p></li>
</ul>
<p>Простейший пример ViewModel:</p>
<pre class="brush:kt;">
...
<p>Этот GameViewModel содержит только чистую логику и данные, без каких-либо Android-зависимостей.</p>
<h3>Что такое AndroidViewModel?</h3>
<p>AndroidViewModel — это специальный подкласс ViewModel, который знает о существовании Application. Основная задача AndroidViewModel та же, что и у ViewModel — сохранять данные при изменении конфигурации, но с одним дополнением: он предоставляет доступ к Context приложения.
Но ключевое отличие заключается в том, что конструктор типа AndroidViewModel принимает объект Application, который можно использовать для получения applicationContext.</p>

<p>Пример AndroidViewModel:</p>
<pre class="brush:kt;">
...
<p>Зачем это нужно? Нередко приложению приходится обращаться к ресурсам строк, изображений и т.д., которые определены в проекте. Например, пусть в файле <span class="b">string.xml</span> 
определена некоторая строка с ключом "welcome_user". Благодаря передаче объекта Application в конструктор мы могли бы получить этот ресурс:</p>
<pre class="brush:kt;">
import android.app.Application
import androidx.lifecycle.AndroidViewModel
...
class UserProfileViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun getWelcomeMessage(userName: String): String {
        ...
        val welcomeFormat = getApplication&lt;Application&gt;().getString(R.string.welcome_user)
        return String.format(welcomeFormat, userName)
    }
}
</pre>
<p>Внутри класса AndroidViewModel доступен метод <code>getApplication&lt;Application&gt;()</code>, который возвращает значение <code>Context</code> и через который мы можем получить доступ к ресурсам проекта. Например, для получения строки применяется метод 
<code>getString()</code>, в который передается идентификатор ресурса строки.</p>
<p>То есть иными словами AndroidViewModel удобно применять, когда необходим доступ к приложению и его ресурсам, что по умолчанию не доступно из стандартного класса ViewModel.</p>

<p>Но здесь надо помнить, что даже при использовании <code>AndroidViewModel</code> не следует хранить <code>Context</code> в поле <code>ViewModel</code>. 
Всегда получайте его по требованию через вызов <code>getApplication()</code>, чтобы избежать потенциальных утечек и предупреждений от статического анализатора.</p>



<p>Суммируем сравнение ViewModel и AndroidViewModel</p>
<table class="tab"><tr class="tabhead"><td><p>Характеристика</p></td><td><p><code>ViewModel</code></p></td><td><p><code>AndroidViewModel</code></p></td></tr>

<tr><td><p>Базовый класс</p></td><td><p><code>androidx.lifecycle.ViewModel</code></p></td><td><p><code>androidx.lifecycle.AndroidViewModel</code> (наследуется от <code>ViewModel</code>)</p></td></tr>
<tr><td><p>Зависимость от Android</p></td><td><p>Нет. Полностью независим.</p></td><td><p>Да. Зависит от класса <code>android.app.Application</code>.</p></td></tr>
<tr><td><p>Конструктор</p></td><td><p>Пустой конструктор по умолчанию.</p></td><td><p>Требует <code>Application</code> в качестве параметра.</p></td></tr>
<tr><td><p>Доступ к <code>Context</code></p></td><td><p>Нет прямого доступа.</p></td><td><p>Да. Через метод <code>getApplication()</code>.</p></td></tr>
<tr><td><p>Тестируемость</p></td><td><p>Очень легко. Можно использовать стандартные юнит-тесты.</p></td><td><p>Сложнее. Требует имитации <code>Context</code> в юнит-тестах.</p></td></tr>
</table>
<h3>Что и когда использовать? Практические рекомендации</h3>
<p>Руководствуйтесь следующим простым правилом:</p>
<blockquote>
<p>Всегда начинайте с обычного <code>ViewModel</code>. Используйте <code>AndroidViewModel</code> только тогда, когда вам абсолютно необходим доступ к <code>Application Context</code>.</p>
</blockquote>
<h4>Используйте <code>ViewModel</code> (в 95% случаев), если:</h4><ul>
<li><p>Нужно управлять состоянием экрана.</p></li>
<li><p>Логика приложения не требует доступа к системным ресурсам или сервисам.</p></li>
<li><p>Надо, чтобы ваш код был максимально чистым и легко тестируемым.</p></li>
<li><p>Используется механизм внедрения зависимостей Dependency Injection (Hilt, Koin) (например, репозитория, который сам может иметь доступ к <code>Context</code>).</p></li>
</ul>
<h4>Используйте <code>AndroidViewModel</code>, если:</h4>
<ul>
<li><p>Нужно получить доступ к строковым (<code>R.string</code>), числовым (<code>R.dimen</code>) или другим ресурсам прямо в <code>ViewModel</code>.</p></li>
<li><p>Нужно получить доступ к системным сервисам (например, <code>ConnectivityManager</code>).</p></li>
<li><p>Необходимо взаимодействовать с базой данных (например, Room), для инициализации которой требуется <code>Context</code>.</p></li>
</ul>

<p>Таким образом, <code>AndroidViewModel</code> — это удобное, но более специализированное решение для тех редких случаев, когда <code>ViewModel</code> не может обойтись без <code>Context</code>.</p>
...
<style>
	...
	<div class="nav"><p><a href="./15.3.php">Назад</a><a href="./">Содержание</a><a href="./16.1.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
