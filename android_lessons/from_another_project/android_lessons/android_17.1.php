...
<html  lang="ru">
<head>
<title>Kotlin и Android | Введение в навигацию</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в навигацию в мобильном приложении на Jetpack Compose на языке программирования Kotlin в Android, класс NavController, компонент NavHost, функция composable, обработка маршрутов компонентами">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Навигация</h1><h2>Введение в навигацию</h2><div class="date">Последнее обновление: 01.05.2025</div>
	<div class="socBlock">
...
<p>Очень немногие мобильные приложения состоят только из одного экрана. На самом деле большинство приложений состоят из нескольких экранов, по которым пользователь перемещается 
с помощью экранных жестов, щелчков кнопок и выбора пунктов меню. Соответственно навигация является одним из ключевых моментов при построении приложения под Android.</p>
<p>Каждый экран обычно представляет собой отдельный компонент или объект Activity. Подобные экраны Google еще называет термином <span class="b">destination</span>, то пункт назначения, к которому может перейти пользователь. 
Каждое приложение имеет главный экран (home screen), который появляется после запуска приложения и после появления любого экрана-заставки. На этом главном экране пользователь обычно 
выполняет задачи, которые приводят к появлению других экранов или пунктов назначения. Эти экраны обычно принимают форму других компонентов проекта. Например, приложение для обмена сообщениями может иметь главный 
экран со списком текущих сообщений, с которого пользователь может перейти на другой экран для доступа к списку контактов или экрану настроек. Экран списка контактов, 
в свою очередь, может позволить пользователю переходить к другим экранам, где можно добавлять новых пользователей или обновлять существующие контакты.</p>
<p>Для отслеживания переходов по различным экранам Android использует стек навигации. При первом запуске приложения начальный экран помещается в стек. 
Когда пользователь переходит к другому экрану, то этот экран помещается на верхушку стека. Когда пользователь переходит к другим экранам, они также помещаются в стек. 
Когда пользователь перемещается обратно по экранам с помощью системной кнопки "Назад", каждый компонент извлекается из стека до тех пор, пока главный экран не окажется единственным элементом в стеке. Извлекать элементы из стеа можно также программно.</p>
<h3>Подключение функционала навигации в проект</h3>
<p>По умолчанию проект не содержит функционала навигации, и нам надо добавить все необходимые зависимости. Для этого можно 
вручную надо добавить соответствующую зависимость в файл <span class="b">build.gradle</span>. Мы можем сделать это напрямую, указав в секции 
<span class="b">dependencies</span> название зависимости:</p>
<pre class="brush:kt;">
dependencies {
...
<p>Или вместе с версией (если нужна какая-то определенная версия):</p>
<pre class="brush:kt;">
dependencies {
...
Для этого изменим файл <span class="b">libs.version.toml</span> - в секцию 
<code>[versions]</code> версию подключаемой зависимости, а в секцию <code>[libraries]</code> имя подключемой библиотеки:</p>
<pre class="brush:kt;">
[versions]
...
<p>Затем в файл <span class="b">build.gradle.kts</span> (Module :app) в секцию <code>dependencies</code> добавим следующую директиву:</p>
<pre class="brush:kt;">
dependencies {
...
<p>Для синхронизации и обновления проекта нажмем на кнопку <span class="b">Sync Now</span></p>
<h3>Создание хоста навигации</h3>
<p>Для управления стеком навигации и самой навигацией применяется <span class="b">контроллер навигации</span>, представленным классом <span class="b">NavHostController</span>. 
И первым шагом при добавлении навигации в проект приложения является создание экземпляра NavHostController:</p>
<pre class="brush:kt;">val navController = rememberNavController()</pre>
<p>Для создания объекта NavHostController применяется функция <span class="b">rememberNavController()</span>. Данная функция позволяет сохранить целостность стека навигации в процессе рекомпозиции компонентов пользовательского интерфейса.</p>
<p>После создания контроллера навигации его необходимо назначить хосту навигации - объекту <span class="b">NavHost</span>. 
<span class="b">Хост навигации</span> (NavHost) — это специальный компонент, который добавляется в макет пользовательского интерфейса действия и служит заполнителем для страниц, 
по которым будет перемещаться пользователь.</p>
<p>При вызове в NavHost передается объект NavHostController, компонент, который будет служить начальным экраном/начальным пунктом назначения, и граф навигации (navigation graph):</p>
<pre class="brush:kt;">
...
<p>Для передачи графа навигации применяется третий параметр - концевая лямбда. <span class="b">Граф навигации</span> состоит из всех компонентов, которые должны быть доступны в качестве пунктов назначения навигации в контексте контроллера навигации. 
Пункты назначения добавляются в граф навигации путем вызова функции <span class="b">composable()</span>, в который передается маршрут (<span class="b">route</span>) и пункт назначения. 
<span class="b">Маршрут</span> представляет обычную строку, которая однозначно идентифицирует пункт назначения в контексте текущего контроллера навигации. 
Пункт назначения — это компонент, который будет вызываться при выполнении навигации. Например:</p>
<pre class="brush:kt;">
...
<p>Здесь NavHost включает граф навигации, состоящий из трех пунктов назначения, где маршрут "home" настроен как начальный пункт назначения (начальный экран).</p>

<p>В качестве альтернативы жесткому кодированию строк маршрута в вызовах функции <code>composable()</code> можно определять маршруты в закрытом классе:</p>
<pre class="brush:kt;">
...
<p>Использование закрытого класса позволяет определить единую точку для управления маршрутами.</p>
...
<h3>Переход к точкам назначения</h3>
<p>Для перехода между точками назначения/экранами применяется метод <span class="b">navigation()</span> контроллера навигации. Данный метод определяет маршрут для пункта назначения. 
Например, в следующем коде по нажатию на кнопку происходит перехода к экрану "Contacts":</p>
<pre class="brush:kt;">
Button(onClick = {
...
<p>Метод <span class="b">navigation()</span> также с помощью концевой лямбды позволяет задать параметры навигации с помощью различных функций. Так, 
функция <span class="b">popUpTo()</span> позволяет очистить стек навигации вплоть до определенного элемента. Это упрощает навигацию, когда пользователь хочет вернуться не просто назад к экрану, 
который хранится на верхушке стека навигации, а в какое-то определенное место, например, на начальный экран. В этом случае мы могли бы сделать следующим образом:</p>
<pre class="brush:kt;">
Button(onClick = {
...
<p>Теперь, когда пользователь нажимает кнопку "To Contact Page", в стек очищается вплоть до главного экрана Home. И после посещения экрана Contacts при возвращении назад пользователь перейдет к экрану Home.</p> 
<p>Для определения начального пункта назначения также можно использовать метод <code>navController.graph.findStartDestination()</code>, а очистка стека вплоть до этого пункта будет представлять следующий вызов popUpTo:</p>
<pre class="brush:kt;">popUpTo(navController.graph.findStartDestination().id)</pre>
<p>Функция <span class="b">popUpTo()</span> также может принимать дополнительные параметры. Например, параметр <code>inclusive</code> указывает, надо ли извлечь из стека навигации также и тот пункт назначения, 
который передан в функцию. Так, значение <code>inclusive = true</code> в следующем коде также извлекает из стека навигации 
пункт назначения Home перед выполнением навигации:</p>
<pre class="brush:kt;">
Button(onClick = {
...
<p>По умолчанию попытка перехода от текущего пункта назначения к самому себе помещает в стек навигации дополнительный экземпляр пункта назначения. В большинстве ситуаций такое поведение вряд ли 
будет желательным. Чтобы этого предотвратить, для параметра <code>launchSingleTop</code> устанавливается значение <code>true</code>:</p>
<pre class="brush:kt;">
Button(onClick = {
...
<p>Если для параметров <code>saveState</code> и <code>restoreState</code> установлено значение <code>true</code>, они будут автоматически сохранять и восстанавливать состояние записей стека, 
когда пользователь повторно выбирает пункт назначения, который был выбран ранее.</p>
...
<style>
	...
	<div class="nav"><p><a href="./16.3.php">Назад</a><a href="./">Содержание</a><a href="./17.2.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
