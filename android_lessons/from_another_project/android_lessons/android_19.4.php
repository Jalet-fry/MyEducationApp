...
<html  lang="ru">
<head>
<title>Kotlin и Android | Передача данных между Activity</title>
<meta charset="utf-8" />
<meta name="description" content="Передача данных между Activity в Android и Jetpack Compose, Intent и Intent-фильтры, сериализация и интерфейс Serializable, взаимодействие между разными activity">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Передача данных между Activity</h1><div class="date">Последнее обновление: 30.05.2025</div>
	<div class="socBlock">
...
<p>Допустим, у нас в приложении есть два класса Activity: MainActivity (главный класс Activity) и SecondActivity (которую запускаем из MainActivity). Рассмотрим как передать данные из одной Activity в другую на примере 
передачи данных из MainActivity в SecondActivity.</p>
...
<p>Например, передача из текущей activity в SecondActivity строки "Hello World" с ключом "message":</p>
<pre class="brush:kt;">
// создание объекта Intent для запуска SecondActivity
val intent = Intent(context, SecondActivity::class.java)
// передача объекта с ключом "message" и значением "Hello World"
...
Char, массивы этих типов, либо объект интерфейса Serializable. Вообщем мы можем передать довольно большой диапазон типов данных.</p>

<h3>Получение данных</h3>
<p>При получении данных мы можем использовать разные подходы. Прежде всего в каждом классе Activity доступно свойство <span class="b">intent</span>, которое хранит объект <span class="b">Intent</span>, 
использованное для запуска текущей Activity. Значение этого свойства также доступно через метод <code> getIntent()</code>, который есть в классе Activity.</p>
<p>В зависимости от типа отправляемых данных при их получении мы можем использовать ряд методов объекта Intent. Все они называются по одному шаблону - <code>get[Тип_данных]Extra()</code> и в качестве параметра принимают 
ключ объекта. Основные из них:</p>
<ul>
<li><p><span class="b">getStringExtra()</span>: возвращает объект типа String</p></li>
<li><p><span class="b">getIntExtra()</span>: возвращает значение типа int</p></li>
<li><p><span class="b">getByteExtra()</span>: возвращает значение типа byte</p></li>
<li><p><span class="b">getCharExtra()</span>: возвращает значение типа char</p></li>
<li><p><span class="b">getShortExtra()</span>: возвращает значение типа short</p></li>
<li><p><span class="b">getLongExtra()</span>: возвращает значение типа long</p></li>
<li><p><span class="b">getFloatExtra()</span>: возвращает значение типа float</p></li>
<li><p><span class="b">getDoubleExtra()</span>: возвращает значение типа double</p></li>
<li><p><span class="b">getBooleanExtra()</span>: возвращает значение типа boolean</p></li>
<li><p><span class="b">getCharArrayExtra()</span>: возвращает массив объектов char</p></li>
<li><p><span class="b">getIntArrayExtra()</span>: возвращает массив объектов int</p></li>
<li><p><span class="b">getFloatArrayExtra()</span>: возвращает массив объектов float</p></li>
<li><p><span class="b">getSerializableExtra()</span>: возвращает объект интерфейса Serializable</p></li>
</ul>
<p>Например, чтобы получить строку с ключом "message", мы можем использовать следующую инструкцию:</p>
<pre class="brush:kt;">val message = intent.getStringExtra("message") ?: "Undefined"</pre>
<p>Стоит учитывать, что метод получения данных можем возвращать Null, поэтому мы можем на этот случай предоставить значение по умолчанию - в данном случае строка "Undefined".</p>
<p>Другой способ в принципе аналогичный и применялся в Java в аналогичных сценариях:</p>
<p>Чтобы получить отправленные данные при загрузке SecondActivity, можно воспользоваться методом <span class="b">get[Тип_данных]()</span>, в который передается ключ объекта:</p>
<pre class="brush:kt;">
val arguments = intent.getExtras()
val message = arguments?.getString("message") ?: ""
</pre>
<p>Здесь в начале с помощью метода <code>intent.getExtras()</code> получаем объект <code>android.os.Bundle</code>, из которого затем извлекаем собственно данные. 
В зависимости от типа отправляемых данных при их получении мы можем использовать ряд методов объекта Bundle. Все они в качестве параметра принимают 
ключ объекта. Основные из них:</p>
<ul>
...
<h3>Пример передачи данных</h3>
<p>РАссмотрим полный пример. Пусть в MainActivity будет определен следуюший код:</p>
<pre class="brush:kt;">
...
<p>Здесь по нажатию на кнопку запускаем SecondActivity, передавая ей строку с ключом "message". А в коде SecondActivity определим получение этих данных:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
class SecondActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            val message = intent.getStringExtra("message") ?: "Undefined"
            Text(message, fontSize = 22.sp)
        }
    }
}
</pre>
<p>В данном случае в SecondActivity получаем переданную строку и выводим ее в компоненте Text.</p>
<img src="./pics/124.png" alt="Передача данных между Activity в Jetpack Compose Kotlin Android" />
...
<p>Аналогичным способом можно передавать и набор разнотипных данных. Например, в MainActivity передадим несколько данных:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>В SecondActivity получим эти данные:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
class SecondActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            val userAge = intent.getIntExtra("age", 0) // второй параметр - значение по умолчанию
            val company = intent.getStringExtra("company") ?: ""
            Column {
                Text("Name: ${userName}", fontSize = 22.sp)
                Text("Age: ${userAge}", fontSize = 22.sp)
                Text("Company: ${company}", fontSize = 22.sp)
            }
        }
    }
}
...
<img src="./pics/125.png" alt="Передача сложных данных между Activity в Jetpack Compose Kotlin Android" />

<h4>Сериализация</h4>
<p>Однако передавать данные способом, который предложен выше, не очень оптимально, особенно когда данных много, но их можно оформить в один объект. Например:</p>
<pre class="brush:kt;">class User(val name: String, val age: Int, val company: String)</pre>
<p>И было бы неплохо передавать сразу объект класса вместо тысяч разрозненных данных.</p>
<p>В Android для передачи пользовательских объектов между Activity через Intent необходимо, чтобы класс реализовал один из двух интерфейсов: <span class="b">Parcelable</span> или 
<span class="b">Serializable</span>. Рекомендуемый способ (и более производительный): <span class="b">Parcelable</span>, поэтому рассмотрим только этот способ. 
<span class="b">Parcelable</span> специально разработан для передачи объектов через Intent и является более эффективным, чем Serializable, особенно для больших или сложных объектов, поскольку он быстрее и потребляет меньше памяти.</p>
<p>Прежде всего класс должен реализовать интерфейс <span class="b">Parcelable</span>:</p>
<pre class="brush:kt;">
import android.os.Parcel
import android.os.Parcelable

data class User(val name: String, val age: Int, val company: String) : Parcelable {

    // Вспомогательный конструктор для чтения из Parcel
    constructor(parcel: Parcel) : this(
        parcel.readString() ?: "", // Читаем имя
        parcel.readInt(),           // Читаем возраст
        parcel.readString() ?: ""   // Читаем компанию
    )

    // Записываем данные объекта в Parcel
    ...
    // Описываем тип содержимого (обычно 0)
    override fun describeContents(): Int {
        return 0
    }

    // Companion object для создания экземпляров Parcelable
    companion object CREATOR : Parcelable.Creator&lt;User&gt; {
        // Создает новый экземпляр класса Parcelable из Parcel
        override fun createFromParcel(parcel: Parcel): User {
            return User(parcel)
        }

        // Создает массив экземпляров класса Parcelable
...
<ul>
<li><p><code>data class User(...) : Parcelable</code>: класс должен реализовывать интерфейс <code>Parcelable</code>.</p></li>
<li><p><code>constructor(parcel: Parcel)</code>: вспомогательный конструктор, который используется для восстановления объекта <code>User</code> из <code>Parcel</code> при получении его в другой <code>Activity</code>. Порядок чтения данных должен соответствовать порядку записи.</p></li>
<li><p><code>writeToParcel(parcel: Parcel, flags: Int)</code>: отвечает за запись данных вашего объекта в <code>Parcel</code> перед отправкой.</p></li>
<li><p><code>describeContents(): Int</code>: в большинстве случаев возвращает <code>0</code>. Используется для специальных типов объектов.</p></li>
<li><p><code>companion object CREATOR : Parcelable.Creator&lt;User&gt;</code>: обязательный статический объект, который реализует <code>Parcelable.Creator</code>. Он содержит два метода:
<ul>
<li><p><code>createFromParcel(parcel: Parcel)</code>: создает новый экземпляр вашего класса из <code>Parcel</code>.</p></li>
<li><p><code>newArray(size: Int)</code>: создает массив экземпляров вашего класса.</p></li>
</ul>
</li>
</ul>

<p>Для отправки объекта <code>Parcelable</code> применяется тот же метод <code>putExtra()</code>, который имеет соответствующую перегрузку. 
А вот с получением данных немного сложнее. С API 33 (Android 13, Tiramisu) появился новый, типобезопасный способ получения Parcelable-объектов:</p>
<pre class="brush:kt;">getParcelableExtra("key", Class&lt;T&gt; clazz)</pre>
<p>Это предпочтительный способ. Для более старых версий Android (до API 33) используется устаревший метод <code>getParcelableExtra("key")</code>, который возвращает <code>Parcelable?</code>. 
При получении данных можно проверять текущий API устройства:</p>
<pre class="brush:kt;">
...
<p>Здесь в зависимости от версии API применяем определенный метод. А с помощью аннотации <code>@Suppress("DEPRECATION")</code> можно подавить предупреждение о deprecated, если нужно поддерживать старые версии.</p>

<p>Полный пример. Код MainActivity, который отправляет данные:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Код SecondActivity, который получает данные:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
class SecondActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            Column {
                if(user!=null) {
                    Text("Name: ${user.name}", fontSize = 22.sp)
                    Text("Age: ${user.age}", fontSize = 22.sp)
                    Text("Company: ${user.company}", fontSize = 22.sp)
                }
                else{
                    Text("User Undefined", fontSize = 22.sp)
                }
            }
        }
    }
...
<style>
	...
	<div class="nav"><p><a href="./19.3.php">Назад</a><a href="./">Содержание</a><a href="./11.1.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
