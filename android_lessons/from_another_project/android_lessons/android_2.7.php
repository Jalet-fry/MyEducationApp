...
<html  lang="ru">
<head>
<title>Kotlin и Android | Грид</title>
<meta charset="utf-8" />
<meta name="description" content="Контейнеры LazyHorizontalGrid и LazyHorizontalGrid и создание сетки компонентов в Jetpack Compose на языке программирования Kotlin на Android, GridCells, адаптивная и фиксированая ширниа и высота, табличное представление элементов, тип LazyHorizontalGrid и добавление элементов в грид">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Грид</h2><div class="date">Последнее обновление: 03.04.2024</div>
	<div class="socBlock">
...
<p>Компоненты <span class="b">LazyVerticalGrid</span> и <span class="b">LazyHorizontalGrid</span> обеспечивают отображение элементов в виде сетки. 
Эти компоненты похожи на списки LazyColumn/LazyRow, только для описания содержимого применяют интерфейс <span class="b">LazyGridScope</span></p>
<h3>LazyHorizontalGrid</h3>
<p><span class="b">LazyHorizontalGrid</span> отображает элементы в горизонтально прокручиваемом контейнере, распределенном по нескольким строкам, и имеет следующие параметры:</p>
<pre class="brush:kt;">
@Composable
...
<ul>
<li><p><code>rows</code>: представляет тип <span class="b">GridCells</span>, который описывает количество и размер строк</p></li>
<li><p><code>modifier</code>: применяемые к контейнеру модификаторы</p></li>
<li><p><code>state</code>: объект состояния LazyGridState, применяемый для управления состоянием контейнера</p></li>
<li><p><code>contentPadding</code>: отступы вокруг содержимого</p></li>
<li><p><code>reverseLayout</code>: при значении <code>true</code> располагает элементы в обратном порядке</p></li>
<li><p><code>verticalArrangement</code>: настройки расположения элементов по горизонтали</p></li>
<li><p><code>horizontalArrangement</code>: выравнивание элементов по вертикали</p></li>
<li><p><code>flingBehavior</code>: описывает поведение при таком типе прокрутки, когда пользователь быстро перетаскивает что-то и поднимает палец. 
Представляет объект <code>FlingBehavior</code></p></li>
<li><p><code>userScrollEnabled</code>: указывает, доступна ли прокрутка жестами либо через специальные инструменты управления доступом</p></li>
<li><p><code>content</code>: устанавливает содержимое контейнера с помощью функции типа <code>LazyGridScope.() -&gt; Unit</code>. </p></li>
</ul>
<p>Для установки содержимого интерфейс <span class="b">LazyGridScope</span> предоставляет несколько функций:</p>
<ul>
<li><p><code>LazyListScope.item()</code>: для добавления одного элемента</p></li>
<li><p><code>LazyListScope.items()</code>: для добавления нескольких элементов</p></li>
<li><p><code>LazyListScope.itemsIndexed()</code>: для добавления нескольких элементов с использованием индексов</p></li>
</ul>
<p>Для определения количества и размера ячеек применяется интерфейс <span class="b">GridCells</span>. В частности, он предоставляет следующие методы:</p>
<ul>
<li><p><code>GridCells.Adaptive</code>: определяет сетку с максимально возможным количеством строк или столбцов при условии, что каждая ячейка имеет как минимум пространство minSize 
и все дополнительное пространство распределено равномерно.</p></li>
<li><p><code>GridCells.Исправлено</code>: определяет сетку с фиксированным количеством строк или столбцов.</p></li>
<li><p><code>GridCells.FixedSize</code>: определяет сетку с максимально возможным количеством строк или столбцов при условии, что каждая ячейка занимает ровное пространство.</p></li>
</ul>
<p>Например, определим простейший грид из текстовых элементов:</p>
<pre class="brush:kt;">
...
<p>В данном случае для создания грида применяется список itemsList из 13 элементов. С помощью параметра</p>
<pre class="brush:kt;">rows = GridCells.Fixed(3)</pre>
<p>определяем три строки. Для заполнения этих строк вызывается функция <code>items()</code></p>
<pre class="brush:kt;">
items(itemsList) {item -&gt;
    Text("Item $item", fontSize = 28.sp,modifier= Modifier.padding(8.dp))
}
</pre>
<p>Таким образом, все элементы будут последовательно располагаться в первом столбце, когда последняя строка будет заполнена, создается новый столбец и т.д.</p>
<img src="./pics/4.81.png" alt="LazyHorizontalGrid в Jetpack Compose и Kotlin на Android" />
<p>Данный могут более сложными по композиции, и соответственно для их отображения могут использоваться более сложные композиции компонентов:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Здесь для представления данных применяется класс Language, который хранит название языка программирования и связанный с ним цвет. А список langs хранит набор объектов Language. 
Для вывода этого списка в грид создается компонент LazyHorizontalGrid с 2-мя столбцами. Внутри каждой ячейки отображаем Box, закрашенный в определенный цвет, и текст с названием языка.</p>
<img src="./pics/4.82.png" alt="Строки LazyHorizontalGrid в Jetpack Compose и Kotlin на Android" />
<p>Обратите внимание, что два элемента не поместились на экране смартфона, и для их просмотра надо использовать горизонтальную прокрутку.</p>
...
<h3>LazyVerticalGrid</h3>
<p><span class="b">LazyVerticalGrid</span> отображает элементы в вертикально прокручиваемом контейнере, распределенном по нескольким столбцам, и имеет следующие параметры:</p>
<pre class="brush:kt;">
@Composable
...
<ul>
<li><p><code>columns</code>: представляет объект <span class="b">GridCells</span>, который описывает количество и размер столбцов</p></li>
<li><p><code>modifier</code>: применяемые к контейнеру модификаторы</p></li>
<li><p><code>state</code>: объект состояния LazyGridState, применяемый для управления состоянием контейнера</p></li>
<li><p><code>contentPadding</code>: отступы вокруг содержимого</p></li>
<li><p><code>reverseLayout</code>: при значении <code>true</code> располагает элементы в обратном порядке</p></li>
<li><p><code>verticalArrangement</code>: настройки расположения элементов по вертикали</p></li>
<li><p><code>horizontalArrangement</code>: выравнивание элементов по горизонтали</p></li>
<li><p><code>flingBehavior</code>: описывает поведение при таком типе прокрутки, когда пользователь быстро перетаскивает что-то и поднимает палец. 
Представляет объект <code>FlingBehavior</code></p></li>
<li><p><code>userScrollEnabled</code>: указывает, доступна ли прокрутка жестами либо через специальные инструменты управления доступом</p></li>
<li><p><code>content</code>: устанавливает содержимое контейнера с помощью функции типа <code>LazyGridScope.() -&gt; Unit</code>. </p></li>
</ul>
<p><span class="b">LazyVerticalGrid</span> во многом аналогичен <code>LazyHorizontalGrid</code>. Так, перепишем предыдущий пример с использованием LazyVerticalGrid:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<img src="./pics/4.83.png" alt="LazyVerticalGrid в Jetpack Compose и Kotlin на Android" />
<p>Здесь практически то же самое, только в LazyVerticalGrid вместо строк здесь задаем 2 столбца. Соответственно сначала элементы заполняются 2 стобца первой строки. Если после этого еще есть элементы, то 
создается вторая строка и так далее. А чтобы просмотреть те элементы, которые не поместились на экране, надо использовать вертикальную прокрутку.</p>
<h3>Адаптивные размеры</h3>
<p>В примерах выше применялось точное количество строк/столбцов. Для этого использовался метод <code>GridCells.Fixed()</code>, в который передавалось количество строк/столбцов. Теперь рассмотрим другие возможности установки размеров. 
Так, метод <span class="b">GridCells.Adaptive()</span>. Он определяет сетку с максимально возможным количеством строк или столбцов при условии, что каждая ячейка имеет как минимум размер minSize и все дополнительное пространство распределено равномерно. Значение minSize, 
которе указывает на минимаьную высоту строки или минимальную ширину столбца, передается в метод <span class="b">GridCells.Adaptive()</span> в качестве единственного параметра. Например:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>В данном случае создается вертикальный грид, в котором минимальная ширина каждой ячейки равна 120 пикселей:</p>
<pre class="brush:kt;">columns = GridCells.Adaptive(minSize=120.dp)</pre>
<p>Минимальную ширину в данном случае я определил на основе содержимого ячейки. В итоге при вертикальной ориентации на моем устройстве будет создано 3 столбца</p>
<img src="./pics/4.84.png" alt="Настройка столбцов в LazyVerticalGrid в Jetpack Compose и Kotlin на Android" />
<p>Тогда как при горизонтальной ориентации будет создаваться аж 5 столбцов</p>
<img src="./pics/4.85.png" alt="Адаптивные размеры в LazyVerticalGrid в Jetpack Compose и Kotlin на Android" />
<p>Данный подход может показать лучше, чем установка фиксированного количества строк/столбцов, поскольку экранное пространство устройства расходуется более экономично.</p>
<h3>Установка точных размеров</h3>
<p>Еще один метод <span class="b">GridCells.FixedSize()</span> позволяет задать точный размер ячейки (ширину столбцов или высоту строк). В этом случае создается максимально возможное 
количество строк или столбцов при условии, что каждая ячейка имеет указанный размер. Значение размера передается в метод <span class="b">GridCells.FixedSize()</span> в качестве единственного параметра. Например:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>В данном случае создается вертикальный грид, в котором ширина каждой ячейки должна быть равна 140 пикселей:</p>
<pre class="brush:kt;">columns = GridCells.FixedSize(size=140.dp)</pre>
<p>В итоге при вертикальной ориентации на моем устройстве будет создано 2 столбца</p>
<img src="./pics/4.86.png" alt="Точные размеры столбцов в LazyVerticalGrid в Jetpack Compose и Kotlin на Android" />
<p>Тогда как при горизонтальной ориентации будет создаваться 4 столбца</p>
<img src="./pics/4.87.png" alt="GridCells в LazyVerticalGrid в Jetpack Compose и Kotlin на Android" />
...
<style>
	...
	<div class="nav"><p><a href="./2.6.php">Назад</a><a href="./">Содержание</a><a href="./2.12.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
