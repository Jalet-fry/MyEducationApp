...
<html  lang="ru">
<head>
<title>Kotlin и Android | Взаимодействие с кодом Kotlin</title>
<meta charset="utf-8" />
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Взаимодействие с кодом Kotlin</h2><div class="date">Последнее обновление: 30.03.2024</div>
	<div class="socBlock">
...
<p>Как и в общем случае переменные можно определять на уровне класса (например, на уровне класса MainActivity), либо на уровне отдельного метода/функции, в том числе и в функциях Composable. Например, используем пару переменных:</p>
<pre class="brush:kt;">
...
<p>В данном случае в компоненте Hello определяются две переменных, и их значения передаются параметрам компонента Text:</p>
...
<p>Здесь иерархию компонентов можно представить следующим образом: <code>setContent -&gt; Hello -&gt; Text</code>. На уровне компонента setContent определяется 
переменная <code>textSize</code> - она видна на уровне компонентов в setContent.</p>
<p>На уровне компонента Hello определена переменная <code>content</code> - она доступна во вложенных компонентах, в частности, в компоненте Text, но недоступна вне компонента Hello.</p>
...
<h3>Вызов функций Kotlin</h3>
<p>Стоит отметить, что <span class="b">@Composable</span>-функция 
может вызывать и стандартные функции Kotlin, однако стандартные функции не могут вызывать <span class="b">@Composable</span>-функции. Например, изменим код <span class="b">MainActivity.kt</span> 
следующим образом:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>В данном случае определена стандартная функция getTime, которая с помощью встроенного класса Calendar получает текущее время (часы и минуты) и возвращает его в виде строки. 
В компоненте Text эта функция используется для установки текста.</p>
<img src="./pics/5.5.png" alt="Переменные и функции в компонентах в Jetpack Compose и Kotlin и Android" />
<p>Подобным образом можно определять и вызывать лямбда-выражения:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Внутри компонентов также можно использовать все стандартные управляющие конструкции, например, условные конструкции, в том числе для создания интерфейса:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Здесь применяется встроенный компонент <span class="b">Column</span>, который располагает вложенные компоненты в столбик. Но эти компоненты здесь создаются динамически. В начале 
определяем данные в виде списке langs. В столбце Column пробегаемся по этому списку и создаем для каждого свой компонент Text</p>
...
<style>
	...
	<div class="nav"><p><a href="./1.5.php">Назад</a><a href="./">Содержание</a><a href="./1.6.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
