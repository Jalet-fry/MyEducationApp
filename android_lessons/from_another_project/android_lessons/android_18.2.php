...
<html  lang="ru">
<head>
<title>Kotlin и Android | Петаскивание</title>
<meta charset="utf-8" />
<meta name="description" content="Обработка петаскивания в мобильном приложении на Jetpack Compose на языке программирования Kotlin в Android, модификатор draggable и функция rememberDraggableState">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Петаскивание</h2><div class="date">Последнее обновление: 19.04.2024</div>
	<div class="socBlock">
...
<p>Модификатор <span class="b">draggable()</span> позволяет определить жесты перетаскивания на компоненте. Этот модификатор сохраняет смещение (или дельту) движения перетаскивания от 
исходной точки по мере его возникновения и сохраняет его в состоянии, которое создается с помощью функции <span class="b">rememberDraggableState()</span>. Это состояние затем можно использовать, 
например, для перемещения перетаскиваемого компонента в соответствии с жестом. Определение функции модификатора:</p>
<pre class="brush:kt;">
Modifier.draggable(
...
<ul>
<li><p><code>state<code>: состояние типа DraggableState, которое хранит информацию об операции перетаскивания.</p></li>

<li><p><code>orientation<code>: направоение пертаскивания. Может быть горизонтальным (значение <code>Orientation.Horizontal</code>), либо вертикальным (<code>Orientation.Vertical</code>)</p></li>

<li><p><code>enabled<code>: доступна ли операция перетаскивания</p></li>

<li><p><code>interactionSource<code>: объект MutableInteractionSource, который будет использоваться для генерации DragInteraction.Start при начале перетаскивания.</p></li>
...
<li><p><code>onDragStarted<code>: suspend-функция, которая вызывает при начале перетаскивания</p></li>

<li><p><code>onDragStopped<code>: suspend-функция, которая вызывает после завершения перетаскивания.</p></li>

<li><p><code>reverseDirection<code>: изменяет направление прокрутки на обратное (прокрутка сверху вниз будет вести себя как снизу вверх, а слева направо — как справа налево).</p></li>
</ul>
<p>При вызове модификатора <span class="b">draggable()</span> необходимо указать как минимум два параиметра - состояние перемещения и его тип - по горизонтали или по вертикали.</p>
...
<p>В данном случае применяется перетаскивание по горизонтали. Для его отслеживания определяем сначала состояние xOffset:</p>
<pre class="brush:kt;">var xOffset by remember { mutableStateOf(0f) }</pre>
<p>Для перетаскивания определяем компонент Box, x-координата которого будет привязана к состоянию xOffset:</p>
<pre class="brush:kt;">
Box(
    Modifier
        .offset { IntOffset(xOffset.roundToInt(), 20) }
</pre>
<p>Далее к этому компоненту Box применяется модификатор draggable, который использует горизонтальную ориентацию. Параметр состояния устанавливается путем вызова функции 
<code>rememberDraggableState()</code>, в которой концевая лямбда-выражение используется для получения текущего значения дельты перемещения для получения обновленного состояния xOffset. 
Это, в свою очередь, приводит к перемещению поля в направлении жеста перетаскивания:</p>
<pre class="brush:kt;">
.draggable(
    orientation = Orientation.Horizontal,
    state = rememberDraggableState { distance -&gt;
...
<p>И для большей наглядности определен компонент Text, который выводит значение xOffset:</p>
<img src="./pics/18.2.png" alt="Обработка перетаскивания модификатор draggable в Jetpack Compose Kotlin Android" />
...
<style>
	...
	<div class="nav"><p><a href="./18.1.php">Назад</a><a href="./">Содержание</a><a href="./18.3.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
