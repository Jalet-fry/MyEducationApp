...
<html  lang="ru">
<head>
<title>Kotlin и Android | Переопределение и объединение модификаторов</title>
<meta charset="utf-8" />
<meta name="description" content="Переопределение и объединение модификаторов компонентов в Jetpack Compose и Kotlin и Android, функция Modifier.then">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Переопределение и объединение модификаторов</h2><div class="date">Последнее обновление: 05.04.2024</div>
	<div class="socBlock">
...
<h3>Модификаторы по умолчанию и переопределение модификаторов</h3>
<p>Большинство (если не все) компонентов принимают модификатор через один из своих параметров. При этом для многих из подобных компонентов применяется некоторый модификатор по умолчанию. 
Если мы явным образом не передаем компоненту никаких функций-модификаторов, то применяется модификатор по умолчанию. И мы можем делать подобным образом при определении своих компонентов. Рассмотрим простейший пример:</p>
<pre class="brush:kt;">
...
<p>Здесь определен компонент Message, который по сути является оберткой над встроенным компонентом Text. Функция Message принимает два параметра: выводимый текст и функции модификатора:</p>
<pre class="brush:kt;">fun Message(text:String, textModifier:Modifier=Modifier.fillMaxWidth().padding(10.dp)){</pre>
<p>Второй параметр имеет значение по умолчанию - модификатор, который устанавливает растяжение по всей ширине экрана и отступ в 10 пикселей от границ контейнера. Таким образом, мы можем передавать модификатор через второй параметр, либо не передавать (если он не нужен).</p>
<p>В методе <code>setContent()</code> вызываем компонент Message, не передавая ему никаких модификаторов:</p>
<pre class="brush:kt;">Message("Hello METANIT.COM")</pre>
<p>В итоге будет применяться модификатор по умолчанию:</p>
<img src="./pics/3.22.png" alt="Определение модификатора по умолчанию для компонентов Jetpack Compose на Kotlin на Android" />

<p>Теперь переопределим модификатор по умолчанию:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>В данном случае в компонент Message передается модификатор myModifier, который устанавливает границу, фон и отступы от границ контейнера</p>
<img src="./pics/3.23.png" alt="Переопределение модификатора по умолчанию для компонентов Jetpack Compose на Kotlin на Android" />
...
<p>Может возникнуть ситуация, когда к компоненту необходимо применить несколько модификаторов. Например, если мы определяем набор модификаторов по умолчанию, которые в любом случае применяются к компоненту, 
но также хотим, чтобы динамически при вызове компонента в него также передавалиь еще какие-нибудь модификаторы. Для объединения модификаторов у объекта <code>Modifier</code> применяется метод 
<span class="b">then</span>. Например:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Здесь компонент Message также принимает два параметра: выводимый текст и функции модификатора, которые по умолчанию равны пустому модификатору.</p>
<pre class="brush:kt;">fun Message(text:String, textModifier: Modifier=Modifier){</pre>
<p>Таким образом, мы можем передавать модификатор через второй параметр, либо не передавать. 
Внутри функции Message создается модификатор по умолчанию, который растягивает компонент по всей ширине и устанавливает отступы от границ контейнера в 10 пикселей:</p>
<pre class="brush:kt;">val defaultModifier = Modifier.fillMaxWidth().padding(10.dp)</pre>
<p>Затем вызываем компонент Text, устанавливая у него текст, модификаторы и выравнивание текста по центру:</p>
<pre class="brush:kt;">Text(text, defaultModifier.then(textModifier), fontSize = 28.sp, textAlign = TextAlign.Center)</pre>
<p>При этом происходит объединение модификаторов - к defaultModifier присоединяются модификаторы из textModifier:</p>
<pre class="brush:kt;">defaultModifier.then(textModifier)</pre>
<p>В методе <span class="b">setContent()</span> вызываем компонент Message, передавая ему модификатор customModifier:</p>
<pre class="brush:kt;">
val customModifier = Modifier
    .border(width = 2.dp, color = Color.DarkGray, shape= CircleShape)
    .clip(shape= CircleShape)
...
<p>Кастомный модификатор customModifier устанавливает границу, получает округлый сегмент компонента и задает цвет и отступ от границы до содержимого. В конечном итоге defaultModifier объединяется с customModifier:</p>
<img src="./pics/3.24.png" alt="Объединение модификаторов для компонентов Jetpack Compose на Kotlin на Android" />
