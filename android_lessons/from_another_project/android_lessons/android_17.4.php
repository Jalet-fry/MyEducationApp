...
<html  lang="ru">
<head>
<title>Kotlin и Android | Панель навигации</title>
<meta charset="utf-8" />
<meta name="description" content="Панель навигации NavigationBar в мобильном приложении на Jetpack Compose на языке программирования Kotlin в Android, настройка NavigationBarItem для перехода между компонентами">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Панель навигации</h2><div class="date">Последнее обновление: 17.04.2024</div>
	<div class="socBlock">
...
<p>Для управления навигацией мы можем использовать стандартные встроенные компоненты типа кнопок, определять свои компоненты, однако специально для целей навигации Compose также предоставляет 
специальный компонент - <span class="b">NavigationBar</span>, который представляет навигационную панель. Каждый отдельный элемент этой панели представляет компонент <span class="b">NavigationBarItem</span>.</p>
<p>Реализация <span class="b">NavigationBar</span> обычно включает содержит цикл forEach, который проходит по списку и для каждого его элемента 
создает отдельный компонент <span class="b">BNavigationBarItem</span>. Для <code>NavigationBarItem</code> настраивается иконку и текст, а также обработчик onClick для перехода 
к соответствующему пункту назначения.</p>
<p>Рассмотрим небольшой примерчик:</p>
<pre class="brush:kt;">
...
<p>Рассмотрим основные моменты. Прежде всего для представления отдельного элемента панели навигации определяем класс BarItem:</p>
<pre class="brush:kt;">
data class BarItem(
    val title: String,
    val image: ImageVector,
...
<p>Данный класс будет хранить текст ссылки навигации, иконку и маршрут для перехода к другому экрану.</p>
<p>Для представления набора ссылок навигации определяем объект NavBarItems, который содержит список из трех навигационных ссылок:</p>
<pre class="brush:kt;">
object NavBarItems {
    val BarItems = listOf(
        BarItem(
...
<p>Для создания самой навигационной панели определяем компонент BottomNavigationBar:</p>
<pre class="brush:kt;">
@Composable
fun BottomNavigationBar(navController: NavController) {
    NavigationBar {
...
<p>Фактически этот компонент является оберткой над NavigationBar, в который передает объект NavController для выполнения навигации.</p>
<p>Внутри BottomNavigationBar мы можем идентифицировать текущий маршрут с помощью метода <code>currentBackStackEntryAsState()</code> контроллера навигации. Это позволит провести некоторую стилизацию 
элементов панели на основе текущего маршрута:</p>
<pre class="brush:kt;">
val backStackEntry by navController.currentBackStackEntryAsState()
val currentRoute = backStackEntry?.destination?.route
</pre>
<p>Далее проходим по всем  элементам в списке и выводим каждый элемент с помощью компонента <span class="b">NavigationBarItem</span>:</p>
<pre class="brush:kt;">
NavBarItems.BarItems.forEach { navItem -&gt;
    NavigationBarItem(
        selected = currentRoute == navItem.route,
...
<p>Обратите внимание, что при нажатии зедсь вызывается функция <code>popUpTo()</code>, чтобы гарантировать, что если пользователь нажмет кнопку "Назад", навигация вернется к начальному пункту 
назначения. Мы можем определить начальный пункт назначения, вызвав метод <code>findStartDestination()</code> в графе навигации:</p>
<pre class="brush:kt;">
onClick = {
    navController.navigate(navItem.route) {
        popUpTo(navController.graph.findStartDestination().id) {saveState = true}
...
<p>Запустим приложение, и нашему взору предстанет очаровательная панель с иконками, нажав на которые мы сможем переходить к различным компонентами:</p>
<img src="./pics/17.3.png" alt="Навигационная панель NavigationBar в приложении на Jetpack Compose на Kotlin в Android" />
...
<style>
	...
	<div class="nav"><p><a href="./17.3.php">Назад</a><a href="./">Содержание</a><a href="./18.1.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
