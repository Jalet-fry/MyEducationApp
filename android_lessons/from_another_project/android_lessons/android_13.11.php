...
<html  lang="ru">
<head>
<title>Kotlin и Android | Модификатор animateEnterExit()</title>
<meta charset="utf-8" />
<meta name="description" content="Модификатор animateEnterExit() в Jetpack Compose на языке программирования Kotlin в Android, настройка анимации видимости для отдельных компонентов в AnimatedVisibility">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Модификатор animateEnterExit()</h2><div class="date">Последнее обновление: 13.04.2024</div>
	<div class="socBlock">
...
<p>Когда к компоненту <span class="b">AnimatedVisibility</span> применяются анимации, они применяются ко всем вложенным компонентам. Однако специальная функция-модификатор <span class="b">animateEnterExit()</span> 
позволяет определить анимацию на уровне отдельных компонентов. Как и в случае с AnimatedVisibility, этот модификатор позволяет объявлять анимацию появления и исчезновения компонента на экране. Но важно отметить, что 
этот модификатор применяется не сам по себе в любом месте пользовательского интерфейса, а только в рамках <span class="b">AnimatedVisibility</span>.</p>
<p>Модификатор <span class="b">animateEnterExit()</span> имеет следующее определение:</p>
<pre class="brush:kt;">
...
<p>Как и AnimatedVisibility, модификатор использует параметры <code>enter</code> и <code>exit</code> для определения анимации появления и исчезновения компонента на экране. Кроме того с помощью параметра 
<code>label</code> можно назначить текстовую метку. Поскольку данный модификатор является экспериментальным то при его использовании к компоненту следует применять аннотацию <span class="b">@ExperimentalAnimationApi</span></p>
<p>Пример применения модификатора animateEnterExit:</p>
<pre class="brush:kt;">
...
<p>Здесь модификатор применяется к компоненту Box, который расположен внутри AnimatedVisibility:</p>
<pre class="brush:kt;">
AnimatedVisibility(
    visible = boxVisible.value,
    Modifier.padding(40.dp),
...
<p>Видимость компонента AnimatedVisibility привязана к переменной-состоянию boxVisible, значение которой переключается кнопками.</p>
<p>Поскольку для управления анимацией всецело используется модификатор animateEnterExit на компоненте Box, то для AnimatedVisibility наоборот отключаем анимацию:</p>
<pre class="brush:kt;">
enter = EnterTransition.None,
exit = ExitTransition.None
</pre>
<p>У компонента Box для анимации применяем функции <code>slideInVertically</code> и <code>slideOutVertically</code></p>
<pre class="brush:kt;">
 Box(Modifier.animateEnterExit(
    enter = slideInVertically(tween(durationMillis = 5000)),
    exit = slideOutVertically(tween(durationMillis = 5000))
...
<p>Таким образом при появлении компонент Box начнет медленно скользить сверху вниз в течение 5 секунд, а при изчезновении, наоборот, подниматься снизу вверх, пока не уйдет из области обзора.</p>
<img src="./pics/13.2.png" alt="Modifier.animateEnterExit и анимация в Jetpack Compose на Kotlin в Android" />
<p>Аналогично можно было бы определить внутри AnimatedVisibility несколько компонентов с разными типами анимаций:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>В данном случае в AnimatedVisibility два компонента Box с разным набором анимаций. Первый Box для создания анимации использует функции <code>slideInVertically()</code> и 
<code>slideOutVertically()</code>, тогда как второй Box - функции <code>fadeIn()</code> и <code>fadeOut()</code></p>
...
<style>
	...
	<div class="nav"><p><a href="./13.10.php">Назад</a><a href="./">Содержание</a><a href="./13.12.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
