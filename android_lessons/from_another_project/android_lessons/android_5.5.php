...
<html  lang="ru">
<head>
<title>Kotlin и Android | Производное состояние</title>
<meta charset="utf-8" />
<meta name="description" content="Производное состояние и функция derivedStateOf в Jetpack Compose на языке программирования Kotlin, автоматическое динамическое изменение состояния компонентов">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Производное состояние</h2><div class="date">Последнее обновление: 09.04.2024</div>
	<div class="socBlock">
...
<p>Какое-то состояние компонентов может зависеть от различных условий, в том числе и от другого состояния, то есть оно должно изменяться динамически при изменении соответствующих условий. Jetpack Compose 
позволяет определить такое состояние как <span class="b">вычисляемое</span> или <span class="b">производное</span>. 
Для определения такого состояния применяется функция <span class="b">derivedStateOf()</span>:</p>
<pre class="brush:kt;">
public fun &lt;T&gt; derivedStateOf(
    calculation: () -&gt; T
): State&lt;T&gt;
</pre>
<p>Эта функция принимает функцию, которая собственно и вычисляет некоторое значение, и это значение возвращается в виде состояния из <code>derivedStateOf</code></p>
...
<p>Здесь вначале определяется обычное состояние counter, которое представляет число и по умолчанию равно 0:</p>
<pre class="brush:kt;">val counter = remember{ mutableStateOf(0) }</pre>
<p>А далее определяется производное состояние double:</p>
<pre class="brush:kt;">val double = remember { derivedStateOf { counter.value * 2 } }</pre>
<p>Состояние double также представляет число, но это число вычисляется с помощью умножения значения counter на 2. В реальности, конечно, тут может быть более сложная логика, когда 
состояние одинх компонентов зависит от состояния других.</p>
<p>В компоненте Column определено два компонента Text, первый из которых отображает значение counter, а второй - значение double. Причем по нажатию на первый Text увеличивается значение counter на единицу. 
Однако поскольку значение double зависит от состояния counter, то изменение counter автоматически приведут к изменению в double:</p>
<img src="./pics/5.10.png" alt="Производное состояние и derivedStateOf в Jetpack Compose в Kotlin на Android" />
...
<style>
	...
	<div class="nav"><p><a href="./5.4.php">Назад</a><a href="./">Содержание</a><a href="./4.1.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
