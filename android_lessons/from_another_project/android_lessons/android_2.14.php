...
<html  lang="ru">
<head>
<title>Kotlin и Android | Интерфейс Edge-to-edge, enableEdgeToEdge и WindowInsets</title>
<meta charset="utf-8" />
<meta name="description" content="Интерфейс Edge-to-edge в Jetpack Compose на языке программирования Kotlin на Android, метод enableEdgeToEdge, объекты WindowInsets statusBars, navigationBars, systemBars, safeContent и safeDrawing">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Интерфейс Edge-to-edge, enableEdgeToEdge и WindowInsets</h1><div class="date">Последнее обновление: 28.05.2025</div>
	<div class="socBlock">
...
<p>Современные Android-приложения стремятся к максимально иммерсивному пользовательскому интерфейсу, где контент занимает весь экран, а системные элементы (такие как статус-бар и навигационная панель) 
гармонично интегрируются с дизайном приложения, иначе говоря к интерфейсу <b>edge-to-edge</b>. Что такое edge-to-edge? <b>Edge-to-edge</b> (от края до края) — это подход к дизайну интерфейсов, при котором контент приложения отображается под системными UI-элементами (статус-баром и навигационной панелью), создавая эффект "бесшовного" перехода между системным и прикладным интерфейсом.</p>
<p>В Jetpack Compose для реализации такого подхода используется метод <b>enableEdgeToEdge()</b>, который появивился в библиотеке 
<code>androidx.activity:activity-compose</code> начиная с версии 1.8.0. Этот метож реализован как метод класса <code>ComponentActivity</code>. До появления <code>enableEdgeToEdge()</code> разработчикам приходилось вручную настраивать системные флаги и вычислять отступы, что было довольно трудоемко. Сейчас же это сделать гораздо проще.</p>
<p>Стоит отметить, что Edge-to-edge применяется на Android 15 (уровень API 35) и выше, как только ваше приложение нацелено на SDK 35. Если ваше приложение еще не является edge-to-edge, части вашего приложения могут быть скрыты, и вам придется обрабатывать вставки. В зависимости от приложения эта работа может быть или не быть значительной.</p>
<h3>Как работает enableEdgeToEdge()</h3> 
<p>Метод <b>enableEdgeToEdge()</b> автоматически настраивает ваше приложение для работы в edge-to-edge режиме, выполняя следующие действия:</p>
<ul>
<li><p>Делает системные бары прозрачными</p></li>
<li><p>Настраивает цвет системного интерфейса (светлый/темный)</p></li>
<li><p>Обеспечивает корректное отображение контента под системными областями</p></li>
</ul>
<p>Если ваше приложение ориентировано SDK 35 и выше, то Edge-to-Edge автоматически включается для устройств Android 15 или более поздней версии. 
Если мы посмотрим на шаблон приложения, который генерируется в Android Studio по умолчанию, то мы в нем увидим вызов этого метода:</p>
<pre class="brush:kt;">
...
<p>Метод <code>enableEdgeToEdge()</code> должен вызываться до установки интерфейса.</p>

<p>Возьмем простейший пример и сравнивним с <code>enableEdgeToEdge()</code> и без него. Пусть у нас будет простейший интерфейс:</p>
<pre class="brush:kt;">
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            Column(Modifier.fillMaxSize()){
                Text("Верхняя строна", fontSize = 25.sp, modifier = Modifier.weight(1f))
                Text("Нижняя строна", fontSize = 25.sp)
            }
        }
    }
}
</pre>
<p>Для показательности примера здесь два компонента Text. Причем первый Text имеет модификатор <code>Modifier.weight(1f)</code> и таким образом занимает все пространство контейнера Column за вычетом 
высоты второго элемента Text, которая устанавливается автоматически исходя из его содержимого.</p>
<img src="./pics/2.55.png" alt="Метод enableEdgeToEdge и Edge-to-Edge в Jetpack Compose и Kotlin" />
<p>Здесь мы видим, что первый элемент <code>Text</code> располагается в самом верху в одной строке со статус-баром. 
Зато второй элемент Text располагается где-то за нижней панелью кнопок, то есть приложение фактически занимает всю площадь экрана вне зависимости от верхней и нижней панелей.</p>
<p>Уберем вызов <code>enableEdgeToEdge()</code>:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Теперь область приложения ограничена верхней и нижней панелями:</p>
<img src="./pics/2.56.png" alt="Метод enableEdgeToEdge в Jetpack Compose и Kotlin" />
<h3>Интеграция с Compose и WindowInsets</h3>
<p>Хотя Edge-to-Edge позволяет приложению более органично использовать пространство устройства, но в то же время это создает некоторые проблемы. 
Например, как видно в примере выше элементы интерфейса могутнакладываться на верхнюю панель или оказаться за нижней панелью. Для полной интеграции edge-to-edge с Compose применяется интерфейс 
<b>WindowInsets</b>, который с помощью специальных объектов описывает отступы для различных панелей:</p>
<ul>
<li><p><span class="b">WindowInsets.statusBars</span>: отступы для строки состояния. Это верхние системные панели пользовательского интерфейса, содержащие значки уведомлений и другие индикаторы.</p></li>
<li><p><span class="b">WindowInsets.statusBarsIgnoringVisibility</span>: отступы для строки состояния, когда она видна.</p></li>
<li><p><span class="b">WindowInsets.navigationBars</span>: отступы для панели навигации. Это системные панели пользовательского интерфейса на левой, правой или нижней стороне устройства, описывающие панель задач или значки навигации. 
Они могут меняться во время выполнения в зависимости от предпочитаемого пользователем метода навигации и взаимодействия с панелью задач.</p></li>
<li><p><span class="b">WindowInsets.navigationBarsIgnoringVisibility</span>: отступы для панели навигации, когда она видна.</p></li>
<li><p><span class="b">WindowInsets.captionBar</span>: отступы для панели заголовка.</p></li>
<li><p><span class="b">WindowInsets.captionBarIgnoringVisibility</span>: отступы для панели заголовка, когда она видна</p></li>
<li><p><span class="b">WindowInsets.systemBars</span>: Объединение вставок системной панели, которые включают строки состояния, панели навигации и строку заголовка.</p></li>
<li><p><span class="b">WindowInsets.systemBarsIgnoringVisibility</span>: отступы для системной панели, когда они видны.</p></li>
<li><p><span class="b">WindowInsets.ime</span>: отступы для пространства внизу, которое занимает программная клавиатура.</p></li>
<li><p><span class="b">WindowInsets.systemGestures</span>: отступы для области, где система будет перехватывать жесты для навигации. Позволяет избежать конфликта системных жестов с жестами приложений (например, для нижних листов, каруселей или в играх)</p></li>
<li><p><span class="b">WindowInsets.displayCutout</span>: отступы, необходимые для предотвращения перекрытия с вырезом дисплея (камеры, выемкой или отверстием).</p></li>
<li><p><span class="b">WindowInsets.waterfall</span>: отступы для изогнутых областей дисплея по краям экрана, где экран начинает заворачиваться по бокам устройства.</p></li>

<li><p><span class="b">WindowInsets.safeDrawing</span>: для контента, который не должен отображаться под системным пользовательским интерфейсом - для предотвращения рисования контента, 
который скрыт системным пользовательским интерфейсом (частично или полностью).</p></li>
<li><p><span class="b">WindowInsets.safeContent</span>: работает как комбинация <code>WindowInsets.safeDrawing</code> и <code>WindowInsets.safeGestures</code>, 
чтобы гарантировать отсутствие визуального перекрытия контента и перекрытия жестов.</p></li>
</ul>
...
<p>Каждая часть системного пользовательского интерфейса имеет соответствующий тип отступов. Для этих объектов доступен метод <code>asPaddingValues()</code>, который возвращает отступы для соответствующего объекта в виде значения 
<span class="b">PaddingValues</span>. Например, <code>WindowInsets.statusBars</code> предоставляет размер и положение строки состояния, тогда как в<code>WindowInsets.navigationBars</code> предоставляет размер и положение панели навигации. </p>
<p>Каждый из этих типов состоит из четырех размеров в пикселях: сверху, слева, справа и снизу. Эти размеры указывают, насколько далеко системный пользовательский интерфейс выступает от соответствующих сторон окна приложения. Чтобы избежать перекрытия с этим типом системного пользовательского интерфейса, пользовательский интерфейс приложения должен быть вставлен на эту величину. 
Для получения конкретных значений отступов в единицах <span class="b">Dp</span> у <code>PaddingValues</code> применяется ряд методов:</p>
<ul>
<li><p><code>calculateBottomPadding()</code>: отступ снизу</p></li>
<li><p><code>calculateTopPadding()</code>: отступ сверху</p></li>
<li><p><code>calculateLeftPadding(layoutDirection: LayoutDirection)</code>: отступ слева</p></li>
<li><p><code>calculateRightPadding(layoutDirection: LayoutDirection)</code>: отступ справа</p></li>
<li><p><code>calculateStartPadding(layoutDirection: LayoutDirection)</code>: отступ с начала</p></li>
<li><p><code>calculateEndPadding(layoutDirection: LayoutDirection)</code>: отступ с конца</p></li>
</ul>

<p>Например, получим отступ снизу с учетом навигационной панели и отступ сверху с учетом строки статуса:</p>
<pre class="brush:kt;">
package com.example.helloapp
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            Column(
                ...
                Text("Верхняя сторона", fontSize = 25.sp, modifier = Modifier.weight(1f))
                Text("Нижняя сторона", fontSize = 25.sp)
            }
        }
    }
}
</pre>
<img src="./pics/2.59.png" alt="WindowInsets и отступы в Jetpack Compose и Kotlin" />

<p>Учитывать все эти панели по отдельности нелегко, и проще использовать свойство <code>WindowInsets.safeContent</code>, которое объединяет большинство аспектов:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            val startPadding = safeContent.calculateStartPadding(layoutDirection)
            val endPadding = safeContent.calculateEndPadding(layoutDirection)
            val bottomPadding = safeContent.calculateBottomPadding()
            val topPadding = safeContent.calculateTopPadding()

            Column(
                Modifier.fillMaxSize().padding(bottom = bottomPadding, top=topPadding, start = startPadding, end= endPadding)
            ){
                Text("Верхняя сторона", fontSize = 25.sp, modifier = Modifier.weight(1f))
                Text("Нижняя сторона", fontSize = 25.sp)
            }
...
<p>Здесь опять же с помощью метода <code>asPaddingValues()</code> получаем объект <span class="b">PaddingValues</span>, а затем, используя методы <code>calculateStartPadding()</code>, 
<code>calculateEndPadding()</code>, <code>calculateBottomPadding()</code> и <code>calculateTopPadding()</code> получаем конкретные отступы, которые позволяют определить безопасную область для контента приложения. 
Причем нам не надо ничего дополнительно настраивать для альбомного или портретного режима - все отступы определяются автоматически как надо:</p>

<img src="./pics/2.62.png" alt="WindowInsets.safeContent в Jetpack Compose и Kotlin" />
<img src="./pics/2.63.png" alt="WindowInsets.safeDrawing в Jetpack Compose и Kotlin" />
<p>Причем нам не обязательно указывать по отдельности все отступы, мы можем указать сразу весь набор отступов</p>
<pre class="brush:kt;">
setContent {
    ...
    Column(
        Modifier.fillMaxSize().padding(safeContent)
    ){
        Text("Верхняя сторона", fontSize = 25.sp, modifier = Modifier.weight(1f))
        Text("Нижняя сторона", fontSize = 25.sp)
    }
...
<h3>Специальные модификаторы</h3>
<p>Поскольку использование подобных объектов в <code>WindowInsets</code> является обычной практикой, то для упрощения их использования в Jetpack Compose доступны специальные модификаторы:</p>
<ul>
<li><p><span class="b">statusBarsPadding()</span></p></li>
<li><p><span class="b">statusBarsIgnoringVisibilityPadding()</span></p></li>
<li><p><span class="b">navigationBarsPadding()</span></p></li>
<li><p><span class="b">navigationBarsIgnoringVisibilityPadding()</span></p></li>
<li><p><span class="b">captionBarPadding()</span></p></li>
<li><p><span class="b">captionBarIgnoringVisibilityPadding()</span></p></li>
<li><p><span class="b">systemBarsPadding()</span></p></li>
<li><p><span class="b">systemBarsIgnoringVisibilityPadding()</span></p></li>
<li><p><span class="b">imePadding()</span></p></li>
<li><p><span class="b">systemGesturesPadding()</span></p></li>
<li><p><span class="b">displayCutoutPadding()</span></p></li>
<li><p><span class="b">waterfallPadding()</span></p></li>
</ul>
<p>Например, используем модификатор <span class="b">systemBarsPadding()</span>, который применяет отступы в соответствии с системными панелями:</p>
        ...
        setContent {
            Column(
                Modifier.fillMaxSize()
                    .systemBarsPadding() // устанавливаем отступы с учетом системных панелей
            ){
                Text("Верхняя строна", fontSize = 25.sp, modifier = Modifier.weight(1f))
                Text("Нижняя строна", fontSize = 25.sp)
            }
...
<h3>Кастомизация</h3>
<p><b>enableEdgeToEdge()</b> предоставляет параметры для настройки:</p>
<pre class="brush:kt;">
...
<ul>
<li><p><code>statusBarStyle</code>: стиль панели статуса</p></li>
<li><p><code>navigationBarStyle</code>: стиль панели навигации</p></li>
</ul>
<p>Оба параметра представляют тип <span class="b">SystemBarStyle</span>, для создания которого может применяться ряд методов. Возьмем наиболее распространенный - метод <code>auto()</code>:</p>
<pre class="brush:kt;">
auto(
...
<p>Его параметры:</p>
<ul>
<li><p><code>lightScrim</code>: Цвет, который будет использоваться для фона, когда приложение находится в светлом режиме (только на уровне API 28 и ниже)</p></li>
<li><p><code>darkScrim</code>: Цвет, который будет использоваться для фона, когда приложение находится в темном режиме. (только на уровне API 28 и ниже)</p></li>
<li><p><code>detectDarkMode</code>: Необязательный параметр, который определяет, использует ли пользовательский интерфейс в данный момент темный режим или нет.</p></li>
</ul>
<p>Для примера используем этот метод:</p>
<pre class="brush:kt;">
package com.example.helloapp
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            Column(
                ...
                Text("Верхняя строна", fontSize = 25.sp, modifier = Modifier.weight(1f), color=Color.White)
                Text("Нижняя строна", fontSize = 25.sp, color=Color.White)
            }
        }
    }
}
</pre>
<p>Для параметров панели навигации непосредственно указываем прозрачный цвет - <code>Color.Transparent</code>. А поскольку параметры принимают цвет в виде числовых значений, то конвертируем цвет через метод 
<code>toArgb()</code>. </p>
<p>Результат:</p>
<img src="./pics/2.58.png" alt="Установка цвета системных панелей и SystemBarStyle в Jetpack Compose и Kotlin" />
<p>Также можно было бы применять отдельные настройки цвета:</p>
<pre class="brush:kt;">
enableEdgeToEdge(
            statusBarStyle = SystemBarStyle.auto(
                lightScrim = Color.Black.copy(alpha = 0.1f).toArgb(),
...
<p>В данном случае для установки цвета верхней панели применяются выражения типа <code>Color.Black.copy(alpha = 0.1f).toArgb()</code> - то есть черный цвет с почти полной прозрачностью.</p>
...
<style>
	...
	<div class="nav"><p><a href="./2.13.php">Назад</a><a href="./">Содержание</a><a href="./5.2.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
