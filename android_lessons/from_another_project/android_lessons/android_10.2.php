...
<html  lang="ru">
<head>
<title>Kotlin и Android | LaunchedEffect</title>
<meta charset="utf-8" />
<meta name="description" content="LaunchedEffect и запуск корутин в компонентах в Jetpack Compose на языке программирования Kotlin в Android, SideEffect">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>LaunchedEffect</h2><div class="date">Последнее обновление: 08.04.2024</div>
	<div class="socBlock">
...
<p>В предыдущей статье корутины запускались по нажатию на кнопку из обработчика события onClick. Но что, если мы хотим запускать корутину в каком-то другом месте, например, непосредственно в функции компонента:</p>
<pre class="brush:kt;">
suspend fun doWork() {
...
<p>При попытке скомпилировать этот код мы получим ошибку</p>
<pre class="browser">Calls to launch should happen inside a LaunchedEffect and not composition</pre>
<p>Суть ошибки заключается в том, что мы не можем где угодно запускать корутины таким образом внутри компонента, поскольку это может вызвать нежелательные побочные эффекты. 
В контексте Jetpack Compose побочный эффект возникает, когда асинхронный код вносит изменения в состояние компонента из другой области корутины без учета жизненного цикла этого компонента. 
И есть вероятность, что корутина может продолжить работу после завершения функции компонента.</p>
<p>Чтобы избежать этой проблемы, необходимо запускать корутины из компонентов <span class="b">LaunchedEffect</span> или <span class="b">SideEffect</span>. Эти два компонента считаются 
безопасными для запуска корутин, поскольку они знают о жизненном цикле родительского компонента. Когда вызывается функция компонента <span class="b">LaunchedEffect</span>, которая содержит 
код запуска корутины, корутина немедленно запускается и начинает выполнять асинхронный код. Как только родительский компонент завершается, экземпляр LaunchedEffect и корутина также завершаются.</p>
<p>Общий синтаксис запуска корутин из <span class="b">LaunchedEffect</span> выглядит следующим образом:</p>
<pre class="brush:kt;">
...
<p>Значения параметров key1, key2 и так далее управляют поведением корутины в течение рекомпозиции. Причем необходимо указать как минимум один такой параметр. 
Пока значения любого из этих параметров остаются неизменными, <span class="b">LaunchedEffect</span> будет продолжать выполнение одной и той же корутины в течение несколько рекомпозиций 
родительского компонента. Однако если значение параметра изменится, <span class="b">LaunchedEffect</span> завершит текущую корутину и запустит новую.</p>
<p>ПРименим компонент <span class="b">LaunchedEffect</span>:</p>
<pre class="brush:kt;">
...
<p>Здесь в компоненте HelloWork запускается корутина, которая выполняет функцию doWork. Эта функция для демонстрации просто логгирует на консоль две строки с задержкой в 5 секунд. 
Для запуска корутины определен компонент <span class="b">LaunchedEffect</span>. Обратите внимание, что в функцию этого компонента в качестве ключа передается значение Unit. Это значение указывает, что корутину не нужно воссоздавать 
в течение рекомпозиций. В остальном работа с корутиной протекает также. Также для простой демонстрации внутри HelloWork вызывыается компонент Text, который выводит надпись "Hello Work".</p>
<p>Запустим приложение, и при запуске компонента, и в окне <span class="b">Logcat</span> внизу Android Studio мы сможем увидеть сообщения из функции doWork:</p>
<img src="./pics/10.1.png" alt="Корутины и асинхронность в Jetpack Compose и Kotlin на Android" />
<p>Еще один пример - изменение состояния компонента:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Здесь определен компонент Counter, в котором вложенный компонент Text отображает значение состояния - переменной count. В корутине, которая запускается в <code>LaunchedEffect</code>, в цикле изменяем значение count с 1 до 5 с задержкой в 1 секунду. После этого выходим из цикла, и корутина завершается.</p>
<p>Запустим приложение, и при старте будет запущена корутина, которая будет изменять состояние переменной count:</p>
<img src="./pics/10.3.png" alt="Корутины и LaunchedEffect в Jetpack Compose и Kotlin на Android" />
<p>Кроме <code>LaunchedEffect</code> Jetpack Compose также предоставляет компонент <span class="b">SideEffect</span>. В отличие от LaunchedEffect, корутина в <span class="b">SideEffect</span> выполняется после завершения 
композиции родительского компонента. Кроме того <span class="b">SideEffect</span> не принимает параметрjd и перезапускается при каждой перекомпозиции родительского компонента.</p>
