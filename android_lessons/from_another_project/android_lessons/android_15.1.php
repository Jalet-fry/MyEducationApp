...
<html  lang="ru">
<head>
<title>Kotlin и Android | ViewModel</title>
<meta charset="utf-8" />
<meta name="description" content="Модель представления ViewModel в мобильном приложении на Jetpack Compose на языке программирования Kotlin в Android, Хранение состояния во ViewModel и взаимодействие с интерфейсом">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>ViewModel</h1><h2>Хранение состояния во ViewModel и взаимодействие с интерфейсом</h2><div class="date">Последнее обновление: 16.04.2024</div>
	<div class="socBlock">
...
<p>ViewModel позволяет отделить модель данных и логику приложения, связанную с пользовательским интерфейсом, от кода, который отвечает за отображение пользовательского интерфейса и 
управление им, а также взаимодействие с операционной системой. 
При таком подходе приложение будет состоять из одного или нескольких объектов Activity, которые представляют пользовательский 
интерфейс, а также ряда объектов ViewModel, которые будут отвечать за обработку данных. Классы Activity, которые представляют пользовательский 
интерфейс, отслеживают состояние модели во ViewModel, поэтому любые изменения данных вызывают рекомпозицию. События пользовательского интерфейса, например, нажатие кнопки, настраиваются для вызова соответствующей функции в ViewModel. По сути, это прямая реализация концепции однонаправленного потока данных:</p>
<img src="./pics/15.1.png" alt="ViewModel в Jetpack Compose в Kotlin на Android" />
<p>Такое разделение ответственности решает проблемы, связанные с жизненным циклом класса Activity. Независимо от того, сколько раз объект Activity воссоздается в течение жизненного цикла 
приложения, экземпляры ViewModel остаются в памяти, тем самым обеспечивая согласованность данных. Например, ViewModel, используемая объектом Activity, будет оставаться в памяти до тех пор, пока 
этот объект Activity не завершит свою работу. А если в приложении только один объект Activity, то ViewModel остается в памяти до конца работы приложения.</p>

<p>ViewModel реализуется как отдельный класс, который содержит данные состояния - данные модели и функции для управления этими данными:</p>
<pre class="brush:kt;">
...
<p>Основная цель ViewModel — обеспечить работу с данными с помощью пользовательского интерфейса. При этом сами данные могут браться из внешнего источника, например, из базы данных или веб-сервиса. 
(В этом случае обычно определяется отдельный модуль репозитория, который не связан с ViewModel). Пользовательский интерфейс же может реагировать на изменения данных во ViewModel. 
Существует два способа объявить данные внутри ViewModel, чтобы их можно было наблюдать:</p>
<ul>
<li><p>Определить данные как состояние</p></li>
<li><p>Использовать компонент <span class="b">LiveData</span></p></li>
</ul>
<p>При первом подходе данные определяются с помощью функции <span class="b">mutableStateOf()</span> (или аналогичных функциий, которые создают состояние типа <code>State&lt;T&gt;</code>)</p>
<pre class="brush:kt;">
class MyViewModel : ViewModel() {
    var count by mutableStateOf(0)  // состояние ViewModel
}
</pre>
<p>Вместе с состоянием могут обычно определяются функции, которые можно вызывать из пользовательского интерфейса для изменения состояния:</p>
<pre class="brush:kt;">
class MyViewModel : ViewModel() {
...
<p>Определив класс ViewModel, его можно использовать в компонентах пользовательского интерфейса. Однако для его создания нам еще понадобится функция <span class="b">viewModel()</span>, которая по умолчанию 
отсутствует в стандартном проекте Compose. Поэтому нам вручную надо добавить соответствующую зависимость в файл <span class="b">build.gradle</span>. Мы можем сделать это напрямую, указав в секции 
...
<p>Однако более предпочтительный подход в установке библиотеки и использование ее в качестве зависимости. Поэтому вначале откроем в проекте файл 
<span class="b">libs.version.toml</span> и в секцию <code>[libraries]</code> добавим определение библиотеки:</p>
<pre class="brush:kt;">
[libraries]
...
<p>Далее откроем файл <span class="b">build.gradle.kts</span> (Module :app) и в секцию <span class="b">dependencies</span> добавим соответствующую зависимость:</p>
<pre class="brush:kt;">implementation(libs.androidx.lifecycle.viewmodel.compose)</pre>
...
<p>Затем синхронизируем проект, нажав на кнопку <span class="b">Sync Now</span></p>
<p>После добавления зависимости мы можем использовать ViewModel. Для этого надо передать экземпляр ViewModel в качестве параметра компоненту, из которого можно 
получить доступ к значениям состояния и функциям. Причем рекомендуется передавать ViewModel в компонент, который находится на вершине иерархии компонентов. А из этого компонента при 
необходимости состояние и функции для управления состоянием могут быть переданы дочерним компонентам. Например:</p>
<pre class="brush:kt;">
...
<p>Здесь определен компонент верхнего уровня CounterView, который инкапсулирует весь остальной интерфейс. В этот компонент в качестве параметра передается объект CounterViewModel - наш ViewModel, 
который определяет состояние - переменную count и функцию increase для изменения этого состояния. Причем для создания CounterViewModel применяется не его конструктор, а функция <span class="b">viewModel()</span>. 
Если экземпляр ViewModel (в нашем случае CounterViewModel) уже создан в текущем контексте, то функция <span class="b">viewModel()</span> вернет ссылку на этот экземпляр. 
В противном случае будет создан и возвращен новый экземпляр ViewModel.</p>
<p>Внутри CounterView определяем текстовую метку, которая выводит состояние, и кнопку, которая вызывает функцию increase. Таким образом, при нажатии на кнопку 
произойдет вызов функции increase, которая, в свою очередь, увеличивает состояние count. Это изменение состояния вызывает рекомпозицию пользовательского интерфейса, 
в результате чего новое значение count отобразится в компоненте Text:</p>
<img src="./pics/15.3.png" alt="Управление состоянием ViewModel в Jetpack Compose на Kotlin на Android" />
...
<style>
	...
	<div class="nav"><p><a href="./14.11.php">Назад</a><a href="./">Содержание</a><a href="./15.2.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
