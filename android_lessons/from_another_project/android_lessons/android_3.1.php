...
<html  lang="ru">
<head>
<title>Kotlin и Android | Что такое модификаторы</title>
<meta charset="utf-8" />
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Модификаторы и визуальный интерфейс</h1><h2>Что такое модификаторы</h2><div class="date">Последнее обновление: 29.03.2024</div>
	<div class="socBlock">
	...
	<p>Для настройки внешнего вида и стилизации большинства встроенных компонентов в Jetpack Compose применяются так называемые <span class="b">модификаторы</span>. 
...
<span class="b">Modifier</span>. Его цель хранить конфигурационные настройки, которые применябтся к компонентами Composable. И тип <span class="b">Modifier</span> предоставляет огромное количество встроенных функций-модификаторов, 
которые изменяют (модифицируют) отдельные аспекты компонентов. 
Далее отдельно рассмотрим отдельные группы этих функций. Большинство из них более или менее универсальны, применяются ко многим компонентам. А пока посмотрим на примере компонента <span class="b">Text</span>, как применять модификаторы.</p>
<pre class="brush:kt;">
...
размер шрифта - 28 единиц. Третий параметр задает модификаторы, которые применяются к компоненту:</p>
<pre class="brush:kt;">modifier = Modifier.background(Color.LightGray)</pre>
...
цвета - здесь желтый цвет, предоставленный встроенным значением <code>Color.LightGray</code>.</p>
...
<pre class="brush:kt;">Modifier.background(Color.LightGray)</pre>
<p>Второй модификатор устанавливает отступы (30 единиц) от границ контейнера:</p>
...
<p>Стоит отметить, что порядок модификаторов может играть большую роль. Так, в примере выше сначала применяется фоновый цвет к элементу Text, затем его содержимое сдвигается на 30 единиц от границ контейнера. 
Но попробуем изменить порядок модификаторов:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Теперь сначала содержимое Text сдвигается на 30 единиц, а потом к нему применяется фоновый цвет. И в итоге мы получим несколько иной результат:</p>
<img src="./pics/3.11.png" alt="Порядок модификаторов в компонентах в Jetpack Compose и Kotlin в Android" />
...
<style>
	...
	<div class="nav"><p><a href="./1.8.php">Назад</a><a href="./">Содержание</a><a href="./3.2.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
