...
<html  lang="ru">
<head>
<title>Kotlin и Android | Трансформации</title>
<meta charset="utf-8" />
<meta name="description" content="Трансформации вращения, масштабирования и перемещения на компоненте Canvas с помощью функции rotate, scale и translate в мобильном приложении на Jetpack Compose на языке программирования Kotlin в Android">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Трансформации</h2><div class="date">Последнее обновление: 14.04.2024</div>
	<div class="socBlock">
...
<h3>Вращение</h3>
<p>Любой нарисованный на Canvas элемент можно повернуть с помощью функции <span class="b">rotate()</span>:</p>
<pre class="brush:kt;">
fun rotate(degrees: Float, pivot: Offset = center, block: DrawScope.() -&gt; Unit): Unit
</pre>
<p>Она принимает три параметра:</p>
<ul>
<li><p><code>degrees</code>: угол поворота по часовой стрелки в виде значения Float</p></li>
<li><p><code>pivot</code>: точка поворота, по умолчанию представляет центр Canvas</p></li>
<li><p><code>block</code>: функция типа <code>DrawScope.() -&gt; Unit</code>, внутри которой происходит отрисовка на повернутой плоскости</p></li>
</ul>
<p>Например, повернем прямоугольник на 45°:</p>
<pre class="brush:kt;">
...
<img src="./pics/14.9.png" alt="Canvas и поворот с помощью функции rotate в Jetpack Compose на Kotlin на Android" />

<h3>Масштабирование</h3>
<p>Для масштабирования элементов на Canvas применяется функция <span class="b">scale()</span>:</p>
<pre class="brush:kt;">
...
<p>Первая версия принимает три параметра:</p>
<ul>
<li><p><code>scale</code>: коэффициент масштабирования</p></li>
<li><p><code>pivot</code>: точка масштабирования, по умолчанию представляет центр Canvas</p></li>
<li><p><code>block</code>: функция типа <code>DrawScope.() -&gt; Unit</code>, внутри которой происходит отрисовка на отмасштабированной плоскости</p></li>
</ul>
<p>Вторая версия позволяет задать отдельные коэффициенты для масштабирования по оси X и Y.</p>
<p>Например, увеличим прямоугольник в полтора раза:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>В данном случае темно-сервым цветом отображен прямоугольник с оригинальными размерами, а синим цветом - с отмасштабированными размерами:</p>
<img src="./pics/14.24.png" alt="Canvas и масштабирование с помощью функции rotate в Jetpack Compose на Kotlin на Android" />


<h3>Перемещение</h3>
<p>Функция <span class="b">translate()</span> позволяет переместить сцену на определенное количество пикселей по горизонтали и/или вертикали:</p>
<pre class="brush:kt;">
fun translate(left: Float = 0.0f, top: Float = 0.0f, block: DrawScope.() -&gt; Unit): Unit
</pre>
<p>Функция принимает три параметра:</p>
<ul>
<li><p><code>left</code>: перемещение в пикселях по горизонтали</p></li>
<li><p><code>top</code>: перемещение в пикселях по вертикали</p></li>
<li><p><code>block</code>: функция типа <code>DrawScope.() -&gt; Unit</code>, внутри которой происходит отрисовка на перещенной плоскости</p></li>
</ul>
<p>Например, повернем прямоугольник на 45°:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Здесь происходит сдвиг прямоугольника на 250 пикселей по горизонтали и на 150 пикселей по вертикали. Для наглядности темно-серый прямоугольник представляет оригинальное положение, а синий прямоугольник - положение после сдвига:</p>
<img src="./pics/14.25.png" alt="Canvas и перемещение с помощью функции translate в Jetpack Compose на Kotlin на Android" />
...
<style>
	...
	<div class="nav"><p><a href="./14.9.php">Назад</a><a href="./">Содержание</a><a href="./14.11.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
