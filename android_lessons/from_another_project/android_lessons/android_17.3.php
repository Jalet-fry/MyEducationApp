...
<html  lang="ru">
<head>
<title>Kotlin и Android | Параметры навигации</title>
<meta charset="utf-8" />
<meta name="description" content="Параметры навигации и маршрутизации в мобильном приложении на Jetpack Compose на языке программирования Kotlin в Android, свойство arguments записи навигационного стека, функции getString и getInt, NavType">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Параметры навигации</h2><div class="date">Последнее обновление: 17.04.2024</div>
	<div class="socBlock">
...
<p>Иногда бывает необходимо при переходе от одного экрана к другому требуется передать аргумент в пункт назначения. Compose поддерживает передачу аргументов самых различных типов. 
Для этого сначала надо добавить имя аргумента к маршруту назначения:</p>
<pre class="brush:kt;">
...
<p>В данном случае в маршрут User передается аргумент "userId". Когда приложение запускает переход к месту назначения, значение аргумента сохраняется в соответствующей записи стека навигации. Запись стека передается 
в качестве параметра в концевую лямбду функции <code>composable()</code>, откуда ее можно извлечь и передать компоненту User.</p>
<p>По умолчанию предполагается, что аргумент навигации имеет тип String, соответственно для его извлечения из записи стека применяется метод <code>getString()</code>. Чтобы передать аргументы разных типов, тип необходимо указать с помощью перечисления <code>NavType</code> 
через параметр <span class="b">arguments</span> функции <code>composable()</code>. В следующем примере тип параметра объявлен как тип Int, соответственно для его извлечения используется метод <code>getInt()</code>:</p>
<pre class="brush:kt;">
composable(
...
<p>На своей стороне компонент User должен ожидать получения аргумент userId через одноименный параметр:</p>
<pre class="brush:kt;">
@Composable
...
<p>И на последнем шаге надо передать значение аргумента при вызове метода <code>navigation()</code>, например, добавив значение аргумента в конец маршрута назначения:</p>
<pre class="brush:kt;">
...
<p>В итоге при нажатии на кнопку произойдет следующая последовательность событий:</p>
<ol>
<li><p>Для текущего пункта назначения создается запись для стека</p></li>

<li><p>Текущее значение someId сохраняется в эту запись</p></li>

<li><p>Запись помещается в стек навигации</p></li>

<li><p>Вызывается функция composable() в объявлении NavHost</p></li>

<li><p>Концевая лямбда функции composable() извлекает значение аргумента из записи стека и передает его в компонент User</p></li>
</ol>
<h3>Пример использования параметров</h3>
<p>Рассмотрим простейший пример применения параметров при навигации:</p>
<pre class="brush:kt;">
...
<p>Здесь для описания данных используется класс Employee с тремя свойствами, из которых свойство id - идентификатор, уникально идентифицирует пользователя.</p>
<p>Компонент User применяется для отображения одного объекта Employee. Через параметры он получает идентификатор пользователя и список пользователей:</p>
<pre class="brush:kt;">
@Composable
fun User(userId:Int?, data: List&lt;Employee&gt;){
    val user = data.find { it.id==userId }
...
<p>Получив идентификатор, компонент извлекает из переданного списка нужного пользователя и выводит его данные на экран.</p>
<p>Компонент Users отображает список пользователей </p>
<pre class="brush:kt;">
@Composable
fun Users(data: List&lt;Employee&gt;, navController: NavController){
    LazyColumn {
...
<p>По нажатию на каждый элемент списка будет происходить переход к отображению соответствующего пользователя. И при переходе передается в качестве аргумента навигации идентификатор пользователя</p>
<p>В компоненте Main создаем список пользователей для отображения и разворачиваем NavHost для его отображения:</p>
<pre class="brush:kt;">
fun Main() {
    val navController = rememberNavController()
    val employees = listOf(
...
<p>В данном случае параметр userId определяется как значение типа Int, и для его получения применяется метод <code>getInt()</code></p>
<p>Таким образом, если мы запустим приложение, мы увидим список пользователей. При нажатии на одного из пользователей в списке произойдет переход к компоненту User, который отобразит информацию по данному пользователю:</p>

<img src="./pics/17.2.png" alt="Навигация и передача параметров в приложении на Jetpack Compose на Kotlin в Android" />
...
<style>
	...
	<div class="nav"><p><a href="./17.2.php">Назад</a><a href="./">Содержание</a><a href="./17.4.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
