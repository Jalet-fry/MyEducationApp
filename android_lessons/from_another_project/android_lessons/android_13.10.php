...
<html  lang="ru">
<head>
<title>Kotlin и Android | Настройка анимации в AnimatedVisibility</title>
<meta charset="utf-8" />
<meta name="description" content="Настройка анимации в AnimatedVisibility в Jetpack Compose на языке программирования Kotlin в Android, параметры enter и exit, функции expandHorizontally, expandVertically, expandIn, fadeIn, fadeOut, scaleIn, scaleOut">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Настройка анимации в AnimatedVisibility</h2><div class="date">Последнее обновление: 13.04.2024</div>
	<div class="socBlock">
...
<p>Для управления анимацией при появлении и исчезновении вложенных компонентов в AnimatedVisibility применяются параметры <span class="b">enter</span> и <span class="b">exit</span>. Этим параметрам 
передается одна из функций, которые создают эффект анимации. В частности, это следующие функции:</p>
<ul>
<li><p><code>expandHorizontally()</code>: содержимое отображается с использованием метода горизонтального отсечения. Позволяет указать, какая часть контента изначально отображается до начала анимации.</p></li>

<li><p><code>expandVertically()</code>: содержимое отображается с использованием техники вертикального отсечения. Позволяет указать, какая часть контента изначально отображается до начала анимации.</p></li>

<li><p><code>expandIn()</code>: содержимое отображается с использованием методов горизонтального и вертикального отсечения. Позволяет указать, какая часть контента изначально отображается до начала анимации.</p></li>

<li><p><code>fadeIn()</code>: постепенно делает прозрачное содержимое непрозрачным. Начальная прозрачность может быть объявлена с использованием значения с плавающей запятой от 0 до 1.0. 
Значение по умолчанию - 0.</p></li>

<li><p><code>fadeOut()</code>: постепенно делает непрозрачное содержимое прозрачным (невидимым). Целевая прозрачность перед исчезновением содержимого может быть объявлена с использованием значения с плавающей запятой от 0 до 1.0. Значение по умолчанию - 0.</p></li>

<li><p><code>scaleIn()</code>: содержимое постепенно увеличивается. По умолчанию содержимое начинается с нулевого размера и расширяется до полного размера, хотя это значение по умолчанию можно изменить, указав начальное значение масштаба как плавающее значение от 0 до 1.0.</p></li>

<li><p><code>scaleOut()</code>: содержимое постепенно уменьшается и в конце полностью исчезает. По умолчанию целевой масштаб равен 0, но его можно настроить с использованием плавающего значения от 0 до 1.0.</p></li>

<li><p><code>shrinkHorizontally()</code>: компонент постепенно сживается по горизонтали, пока польностью не исчезнет.</p></li>

<li><p><code>shrinkVertically()</code>: компонент постепенно сживается по вертикали, пока польностью не исчезнет.</p></li>

<li><p><code>shrinkOut()</code>: ккомпонент постепенно сживается по горизонтали и вертикали, пока польностью не исчезнет.</p></li>

<li><p><code>slideInHorizontally()</code>: содержимое перемещается в область обзора вдоль горизонтальной оси</p></li>

<li><p><code>slideInVertically()</code>: содержимое перемещается в область обзора вдоль вертикальной оси.</p></li>

<li><p><code>slideIn()</code>: содержимое появляется в области обзора под настраиваемым углом, определяемым через начальное значение смещения.</p></li>

<li><p><code>slideOut()</code>: содержимое исчезает из поля зрения под настраиваемым углом, определяемым через начальное значение смещения.</p></li>

<li><p><code>slideOutHorizontally()</code>: содержимое выходит из области обзора вдоль горизонтальной оси.</p></li>

<li><p><code>slideOutVertically()</code>: содержимое выходит из поля зрения вдоль вертикальной оси</p></li>
</ul>
<p>Например, применм пару из них:</p>
<pre class="brush:kt;">
...
<p>В данном случае при появлении компонент Box будет постепенно отображаться, пока не станет полностью непрозрачным (видимым). А при исчезновении он будет склользить по вертикали вверх, как бы уходя из 
поля зрения:</p>
<pre class="brush:kt;">
enter = fadeIn(),
exit = slideOutVertically()
...
<p>Стоит отметить, что с помощью операции сложения мы можем комбинировать несколько функций:</p>
<pre class="brush:kt;">
AnimatedVisibility(
    visible = boxVisible,
...
<h3>Настройка времени и плавности анимации</h3>
<p>Для настройки анимации вышеуказанные функции имеют параметр <code>animationSpec</code>, который представляет интерфейс <span class="b">AnimationSpec</span>. Так, для установки времени и сглаживания анимации в AnimatedVisibility может использоваться рассмотренная в одной из прошлых тем функция <span class="b">tween()</span>. Например:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>В данном случае для анимации появления на экране применяется функция <code>fadeIn()</code></p>
<pre class="brush:kt;">
enter = fadeIn(animationSpec = tween(durationMillis = 5000)),
</pre>
<p>То есть появление компонента на экране будет занимать 5000 миллисекунд.</p>
<p>Аналогичным образом можно настраивать и другие функции, например:</p>
<pre class="brush:kt;">
...
<p>В данном случае компонент будет появляется области обзора, скользя вдоль горизонтальной оси в течение 5 секунд.</p>
<p>Или</p>
<pre class="brush:kt;">
...
<p>В данном случае компонент будет постепенно появляться в течение 5 секунд, выдвигаясь из-за вертикальной границы. И будет ужиматься вдоль вертикальной оси в течение 5 секунд.</p>
<p>Также для установки специафикации анимации можно использовать и другие функции, которые возвращают объект AnimationSpec. Например, используем функцию 
<span class="b">repeatable()</span>, чтобы повторить анимацию 10 раз:</p>
<pre class="brush:kt;">
...
<style>
	...
	<div class="nav"><p><a href="./13.9.php">Назад</a><a href="./">Содержание</a><a href="./13.11.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
