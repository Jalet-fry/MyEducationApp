...
<html  lang="ru">
<head>
<title>Kotlin и Android | Рисование. Canvas</title>
<meta charset="utf-8" />
<meta name="description" content="Рисование на компоненте Canvas в мобильном приложении на Jetpack Compose на языке программирования Kotlin в Android, тип DrawScope, его свойства и функции">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Рисование. Canvas</h1><h2>Компонент Canvas и DrawScope</h2><div class="date">Последнее обновление: 14.04.2024</div>
	<div class="socBlock">
...
<p>Jetpack Compose предоставляет возможность для отрисовки графики в мобильном приложении. Для этого применяется компонент <span class="b">Canvas</span>. По сути данный компонент 
предоставляет поверхность для рисования 2D-графики и инструменты для управления состоянием графического контента.</p>
<p>Компонент <span class="b">Canvas</span> имеет следующие формы:</p>
<pre class="brush:kt;">
@Composable
...
<li><p><code>onDraw</code>: лямбда-выражение, которое будет вызываться для отрисовки графических примитивов. Обратите внимание, что это лямбда-выражение будет вызываться на этапе рисования, 
а это означает, что вызов функции Composable внутри этого выражения приведет к исключению во время выполнения.</p>
</li>
<li><p><code>contentDescription</code>: текст, используемый службами доступности для описания того, что представляет собой этот компонент.</p></li>
</ul>
<p>Например, определим простейший компонент Canvas без какой-либо графики:</p>
<pre class="brush:kt;">
...
<p>В данном случае для комопнента Canvas определяется прямоугольная область 300х300, окрашенная серым цветом:</p>
<img src="./pics/14.1.png" alt="Canvas в Jetpack Compose на Kotlin на Android" />
<h3>DrawScope</h3>
<p><span class="b">Canvas</span> имеет собственную область действия в виде объекта <span class="b">DrawScope</span>, который предоставляет доступ к свойствам поверхности рисования:</p>
<ul>
<li><p><code>center</code>: центральная точка поверхности рисования в виде объекта Offset</p></li>
<li><p><code>drawContext</code>: объект DrawContext, который представляет контекст отрисовки</p></li>
<li><p><code>layoutDirection</code>: направление отрисовки в виде объекта LayoutDirection</p></li>
<li><p><code>size</code>: размер поверхности рисования в виде объекта Size</p></li>
</ul>
<p><span class="b">DrawScope</span> также предоставляет функции для рисования фигур и управления графикой. Отмечу только основные из них:</p>
<ul>
<li><p><code>drawArc()</code>: отрисовывает дугу</p></li>
<li><p><code>drawCircle()</code>: отрисовывает круг</p></li>
<li><p><code>drawImage()</code>: выводит изображение</p></li>
<li><p><code>drawLine()</code>: рисует линию</p></li>
<li><p><code>drawOval()</code>: отрисовывает овал</p></li>
<li><p><code>drawPath()</code>: отрисовывает геометрический путь</p></li>
<li><p><code>drawPoints()</code>: отрисовывает последовательность точек</p></li>
<li><p><code>drawRect()</code>: рисует прямоугольник</p></li>
<li><p><code>drawRoundRect()</code>: рисует прямоугольник с закругленными углами</p></li>
<li><p><code>drawOutline()</code>: отрисовывает контур</p></li>
<li><p><code>drawText()</code>: выводит текст</p></li>
<li><p><code>clipPath()</code>: вырезает путь</p></li>
<li><p><code>clipRect()</code>: вырезает прямоугольник</p></li>
<li><p><code>rotate()</code>: применяет поворот</p></li>
<li><p><code>scale()</code>: применяет масштабирование</p></li>
<li><p><code>translate()</code>: применяет перемещение</p></li>
</ul>

<p>Свойства и функции DrawScope мы можем использовать внутри лямбда-выражения из последнего параметра Canvas. Например, получим ширину и высоту Canvas:</p>
<pre class="brush:kt;">
Canvas(Modifier.size(300.dp).background(Color.LightGray)) {
...
<p>В следущих статьях мы рассмотрим применение функций DrawScope для отрисвки и управления графикой.</p>
...
<style>
	...
	<div class="nav"><p><a href="./13.12.php">Назад</a><a href="./">Содержание</a><a href="./14.2.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
