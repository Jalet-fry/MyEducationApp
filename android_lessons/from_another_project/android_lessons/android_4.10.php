...
<html  lang="ru">
<head>
<title>Kotlin и Android | Панели приложения TopAppBar и BottomAppBar</title>
<meta charset="utf-8" />
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Панели приложения TopAppBar и BottomAppBar</h2><div class="date">Последнее обновление: 01.04.2024</div>
	<div class="socBlock">
...
<h3>TopAppBar</h3>
<p>Компонент <span class="b">TopAppBar</span> позволяет определить верхнюю панель приложения. Он имеет следующие параметры:</p>
<pre class="brush:kt;">
...
<ul>
<li><p><code>title</code>: задает заголовок панели.</p></li> 	
<li><p><code>modifier</code>: представляет объект <span class="b">Modifier</span>, который определяет модификаторы панели</p></li>
<li><p><code>navigationIcon</code>: задает иконку навигации (обычно компонент IconButton), которая отображается в начале панели</p></li>
<li><p><code>actions</code>: набор значков, отображаемых в конце панели. Обычно это кнопки <code>IconButton</code>. ДЛя расположения иконок по умолчанию применяется контейнер 
<code>Row</code>, поэтому иконки располагаются по горизонтали.</p></li>
<li><p><code>windowInsets</code>: отступы от сторон окна</p></li> 	
<li><p><code>colors</code>: цвета панели в виде объекта TopAppBarColors. Для установки применяется функция <code>TopAppBarDefaults.topAppBarColors()</code>.</p></li> 	
<li><p><code>scrollBehavior</code>: задает поведение при прокрутке с помощью объекта TopAppBarScrollBehavior.</p></li> 	
</ul>
<p>На момент написания текущей статьи данный компонент помечен как экспериментальный. Поэтому при его использовании к родительскому компоненту следует применять аннотацию 
<code>@OptIn(ExperimentalMaterial3Api::class)</code>. Определим простейшую панель TopAppBar:</p>
<pre class="brush:kt;">
...
<p>В данном случае в самом начале панели располагается иконка навигации <code>navigationIcon</code> (здесь в этом качестве выступает иконка <code>Icons.Filled.Menu</code>). 
Затем идет заголовок, который определяется функцией из <code>title</code>. И в правой части панели располагаются дополнительные иконки (здесь <code>Icons.Filled.Info</code> и 
<code>Icons.Filled.Search</code>), которые представляют дополнительные действия в приложении. В данном случае для каждого компонента IconButton параметр <code>onClick</code> 
представляет пустое лямбда-выражение, поэтому при нажатии естественно никаких реальных действий не происходит.</p>
<img src="./pics/4.36.png" alt="TopAppBar в Jetpack Compose и Kotlin в Android" />
<h4>Настройка цвета</h4>
...
<p>Для установки цвета верхней панели применяется объект <code>TopAppBarColors</code>. Для создания этого объекта удобно использовать функцию-компонент <code>TopAppBarDefaults.topAppBarColors</code>:</p>
<pre class="brush:kt;">
@Composable
fun topAppBarColors(
...
<p>Эта функция предполагает установку следующих цветов:</p>
<ul>
<li><p><code>containerColor</code>: цвет контейнера</p></li>
<li><p><code>scrolledContainerColor</code>: цвет контейнера при прокрутке содержимого за ним</p></li>
<li><p><code>navigationIconContentColor</code>: цвет содержимого, используемый для значка навигации</p></li>
<li><p><code>titleContentColor</code>: цвет содержимого, используемый для заголовка</p></li>
<li><p><code>actionIconContentColor</code>: цвет контента, используемый для действий</p></li>
</ul>
<p>По умолчанию все эти цвета не указаны, соответственно панель не имеет цвета. Теперь настроим цвета:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<img src="./pics/4.37.png" alt="Цвет в TopAppBar в Jetpack Compose и Kotlin в Android" />
<h3>BottomAppBar</h3>
<p>Компонент <span class="b">BottomAppBar</span> позволяет определить нижнюю панель. Он имеет ряд похожих версий. Возьмем одну из них:</p>
<pre class="brush:kt;">
@Composable
fun BottomAppBar(
    modifier: Modifier = Modifier,
...
<ul>
<li><p><code>modifier</code>: представляет объект <span class="b">Modifier</span>, который определяет модификаторы панели</p></li>
<li><p><code>containerColor</code>: значение типа <code>Color</code>, которое задает фоновый цвет панели</p></li>
...
По умолчанию предоставляет вызов <code>contentColorFor(containerColor)</code>, которое для установки использует значение параметра <code>bcontainerColor</code></p></li>
<li><p><code>windowInsets</code>: устанавливает отступы от границ</p></li> 	
<li><p><code>tonalElevation</code>: определяет эффект анимацию для содержимого панели</p></li> 
<li><p><code>contentPadding</code>: объект типа <span class="b">PaddingValues</span>, который устанавливает отступы между границами панели и ее содержимым.</p></li> 		
<li><p><code>windowInsets</code>: устанавливает отступы от границ окна</p></li> 	
<li><p><code>content</code>: задает контент панели. По умолчанию содержимое располагается в виде строки с помощью контейнера 
<code>Row</code>. Обычно здесь располагаются компоненты  IconButton, по нажатию на которые выполняются определенные действия.</p></li> 	
</ul>
<p>Определим простейший <span class="b">BottomAppBar</span>:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>В качестве формального содержимого здесь определен компонент Text с некоторым текстом, а внизу столбца располагается нижняя панель приложения с тремя иконками.</p>
<img src="./pics/4.38.png" alt="BottomAppBar в Jetpack Compose и Kotlin в Android" />
<p>По умолчанию все вложенные компоненты панели располагаются рядом друг с другом. Но иногда требуется сделать между ними некоторый зазор. Так, в примере выше мы, возможно, захотим разместить две последние иконки в правой части панели (как в случае с TopAppBar). Чтобы установить пустое пространство 
между компонентами Compose предоставляет специальный компонент <span class="b">Spacer</span>. Этот компонент просто определяет пустую область, которая соответствует его размеру. 
Например:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Здесь в Spacer растягивается на все незанятое другими компонентами пространство с помощью установки модификатора <code>Modifier.weight(1f, true)</code>.</p>
<img src="./pics/4.66.png" alt="BottomAppBar и Spacer в Jetpack Compose и Kotlin в Android" />
<p>С помощью параметров <code>containerColor</code> и <code>contentColor</code> можно настроить цвет саой панели BottomAppBar и ее компонентов соответственно:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<img src="./pics/4.67.png" alt="Цет BottomAppBar в Jetpack Compose и Kotlin в Android" />
<p>Стоит отметить, что Jetpack Compose также предоставляет компоненты, которые  позволяют автоматически разместить верхнюю и нижнюю панель соответственно вверху и внизу окна приложения и которые мы далее рассмотрим.</p>
...
<style>
	...
	<div class="nav"><p><a href="./4.9.php">Назад</a><a href="./">Содержание</a><a href="./4.11.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
