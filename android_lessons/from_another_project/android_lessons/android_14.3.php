...
<html  lang="ru">
<head>
<title>Kotlin и Android | Отрисовка прямоугольников</title>
<meta charset="utf-8" />
<meta name="description" content="Отрисовка прямоугольников на компоненте Canvas с помощью функции drawRect в мобильном приложении на Jetpack Compose на языке программирования Kotlin в Android">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Отрисовка прямоугольников</h2><div class="date">Последнее обновление: 14.04.2024</div>
	<div class="socBlock">
...
<p>Для отрисовки прямоугольников применяется функция <span class="b">drawRect()</span>:</p>
<pre class="brush:kt;">
fun drawRect(
    color: Color,
...
<ul>
<li><p><code>color</code>: цвет прямоугольника</p></li>
<li><p><code>topLeft</code>: точка верхнего левого угла (значение типа Offset)</p></li>
<li><p><code>size</code>: размер прямоугольника</p></li>
<li><p><code>alpha</code>: прозрачность, применяемая к цвету прямоугольника в виде значения из диапазона от 0.0f до 1.0f</p></li>
<li><p><code>style</code>: стиль заполнения прямоугольника (значение DrawStyle)</p></li>
<li><p><code>colorFilter</code>: цветовой фильтр ColorFilter, который применяется к цвету прямоугольника</p></li>
<li><p><code>blendMode</code>: алгоритм смешивания, применяемый к цвету</p></li>
</ul>
<p>Пример прямоугольника:</p>
<pre class="brush:kt;">
...
<img src="./pics/14.4.png" alt="Canvas и отрисовка прямоугольников в Jetpack Compose на Kotlin на Android" />
<p>Обратите внимание, что в примере выше размеры Canvas составляют 300 x 300, а размер прямоугольника — 700 x 300, и как в случае с отрисовкой линий, которая рассматривалась в прошлой статье, для отрисовки прямоугольников применяются обычные пиксели <span class="b">px</span></p> 

<p>Вместо указания размеров размер прямоугольника также можно определить относительно размера Canvas. Например, следующий код рисует квадрат размером в половину размера Canvas:</p>
<pre class="brush:kt;">
Canvas(Modifier.size(300.dp).background(Color.LightGray)) {
    drawRect(
        color = Color.Blue,
...
<img src="./pics/14.5.png" alt="Canvas и отрисовка прямоугольников по относительным размерам в Jetpack Compose на Kotlin на Android" />
<h4>Начальная позиция</h4>
<p>Начальная позиция прямоугольника определяется с помощью координат верхнего левого угла через параметр <code>topLeft</code>:</p>
<pre class="brush:kt;">
...
<img src="./pics/14.6.png" alt="Canvas и установка позиции прямоугольника в Jetpack Compose на Kotlin на Android" />
<p>В качестве альтернативы можно использовать функцию <span class="b">inset(</span>) для изменения границ внутри Canvas:</p>
<pre class="brush:kt;">
...
<p>Первая версия принимает общий отступ в пикселях со всех четырех сторон. Вторая версия устанавливает отдельные отступы по горизонтали и вертикали. И третья версия устанавливает отдельные отступы от каждой из четырех сторон. Например:</p>
<pre class="brush:kt;">
...
<h4>Заполнение прямоугольника</h4>
<p>Параметр <span class="b">style</span> отвечает за то, как цвет заполняет прямоугольник. Он представляет объект класса <span class="b">DrawStyle</span>. По умолчанию применяется 
объект <span class="b">Fill</span>, который заполняет цветом всю площадь фигуры. Но в качестве альтернативы мы также можем использовать подкласс <span class="b">Stroke</span>, 
который отрисовывает только границу фигуры, не заполняя ее содержимого.</p>
<p>Класс <span class="b">Stroke</span> имеет следущий конструктор:</p>
<pre class="brush:kt;">
Stroke(
    width: Float,
    miter: Float,
...
<p>Его параметры:</p>
<ul>
<li><p><code>cap</code>: устанавливает тип начала и конца линий</p></li>
<li><p><code>join</code>: устанавливает тип соединения линии</p></li>
<li><p><code>miter</code>: устанавливает форму для соединения линий линий</p></li>
<li><p><code>pathEffect</code>: устанавливает тип линии</p></li>
<li><p><code>width</code>: устанавливает ширину линии в пикселях</p></li>
</ul>
<p>Например, зададим для прямоугольника только контур в 3.dp без заливки:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<img src="./pics/14.8.png" alt="Canvas и Stroke в Jetpack Compose на Kotlin на Android" />
<h4>Прямоугольник с округленными углами</h4>
<p>Аналогично мы можем рисовать прямоугольники с закругленными углами. Для этого применяется функция <span class="b">drawRoundRect()</span></p>
<pre class="brush:kt;">
fun drawRoundRect(
    color: Color,
    topLeft: Offset = Offset.Zero,
...
<p>Эта функция принимает все те же параметры, что и <code>drawRoundRect()</code> за тем исключением, что здесь добавляется параметр <code>cornerRadius</code>, который задает радиус закругления углов в виде объекта 
<code>CornerRadius</code>. Для его создания может применяться одноименная функция:</p>
<pre class="brush:kt;">fun CornerRadius(x: Float, y: Float = x): CornerRadius</pre>
<p>В качестве параметров она принимает значения радиуса по оси x и y.</p>
<p>Простейший пример создания закругленного прямоугольника:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<img src="./pics/14.7.png" alt="Canvas и drawRoundRect в Jetpack Compose на Kotlin на Android" />
