...
<html  lang="ru">
<head>
<title>Kotlin и Android | Прикрепленные заголовки</title>
<meta charset="utf-8" />
<meta name="description" content="Прикрепленные заголовки контейнеров LazyColumn и LazyRow и функция stickyHeader в Jetpack Compose на языке программирования Kotlin на Android">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Прикрепленные заголовки</h2><div class="date">Последнее обновление: 09.04.2024</div>
	<div class="socBlock">
...
<p>LazyColumn и LazyRow позволяют использовать прикрепленные заголовки или sticky headers. То есть мы можем сгруппировать элементы списка под соответствующим заголовком. И припрепленные заголовки 
остаются видимыми на экране во время прокрутки текущей группы. Как только группа прокручивается из поля зрения, ее место занимает заголовок следующей группы.</p>
<p>Для создания прикрепленных заголовков применяется функция <span class="b">LazyListScope.stickyHeader()</span>. А содержимое списка должно храниться в массиве или списке, 
который трансформируется в группы с помощью функции Kotlin <span class="b">groupBy()</span>. Функция <code>groupBy()</code> принимает лямбду-селектор, 
которая определяет, как данные должны быть сгруппированы. Этот селектор затем служит ключом для доступа к элементам каждой группы.</p>
<p>Рассмотрим, например, следующий список, содержащий модели смартфонов:</p>
<pre class="brush:kt;">
...
<p>Прежде всего стоит отметить, что на момент написания текущей статьи функциональность <span class="b">LazyListScope.stickyHeader()</span> является экспериментальной, 
поэтому компонент, который использует данную функцию, надо предварять аннотацией <code>@OptIn(ExperimentalFoundationApi::class)</code></p>
<pre class="brush:kt;">
setContent @OptIn(ExperimentalFoundationApi::class){
.....
}
</pre>
<p>Сами выводимые данные представляют список phones, который содержит названия моделей смартфонов. С помощью функцию <code>groupBy()</code> группируем элементы этого списка по начальной подстроке:</p>
<pre class="brush:kt;">val groups = phones.groupBy { it.substringBefore(" ") }</pre>
<p>Здесь <code>it</code> - это каждая строка из списка, соответственно выражение <code>it.substringBefore(" ")</code> получает в каждой строке списка подстроку, которая идет до пробела. Например, из строки 
...
<code>Map&lt;String, List&lt;String&gt;&gt;</code>, где ключами являются подстрокам-производители, а значениями - подсписок смартфонов конкретно данного производителя.</p>
<p>Далее перебираем весь этот словарь:</p>
<pre class="brush:kt;">
groups.forEach { (brand, models) -&gt;
    stickyHeader {
        Text(
...
<p>И для каждого элемента словаря для ключа используем функцию <code>stickyHeader()</code> - в ней создаем компонент Text, который будет представлять собственно заголовок и будет отображать ключ -производителя смартфонов. 
И также для каждого элемента словаря применяется функция <code>items()</code>, которая отображает список смартфонов конкретного данного производителя. В итоге мы получим следующее приложение:</p>
<img src="./pics/2.46.png" alt="stickyHeader в LazyColumn в мобильном приложении Jetpack Compose на Kotlin на Android" />
<p>Обратите внимание, что хотя группа Apple частично прокручивается вне поля зрения, заголовок остается в верхней части экрана.</p>
<p>Другой пример:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Здесь данные представляют сложные данные объекта Person, который определяет два свойства: name и company. Группировка идет по свойству company:</p>
<img src="./pics/2.47.png" alt="stickyHeader в LazyColumn в Jetpack Compose на Kotlin на Android" />
...
<style>
	...
	<div class="nav"><p><a href="./2.10.php">Назад</a><a href="./">Содержание</a><a href="./2.13.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
