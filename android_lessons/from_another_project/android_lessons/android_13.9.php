...
<html  lang="ru">
<head>
<title>Kotlin и Android | AnimatedVisibility. Управление видимостью компонента</title>
<meta charset="utf-8" />
<meta name="description" content="AnimatedVisibility. Управление видимостью компонента в Jetpack Compose на языке программирования Kotlin в Android">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>AnimatedVisibility. Управление видимостью компонента</h2><div class="date">Последнее обновление: 13.04.2024</div>
	<div class="socBlock">
...
<p>Компонент <span class="b">AnimatedVisibility</span> управляет видимостью вложенного содержимого и позволяет анимировать его появление и исчезновение, сделать переход к видимой или невидимой форме более плавным. <span class="b">AnimatedVisibility</span> имеет следующие параметры:</p>
<pre class="brush:kt;">
@Composable
...
<ul>
<li><p><code>visible<code>: определяет, видимо ли вложенное содержимое (если true. то видимое)</p></li>
<li><p><code>modifier<code>: применяемые к компоненту функции-модификаторы</p></li>
<li><p><code>enter<code>: настройки перехода при появлении содержимого на экране</p></li>
<li><p><code>exit<code>: настройки перехода при исчезновении содержимого с экрана</p></li>
<li><p><code>content<code>: функция типа <code>AnimatedVisibilityScope.() -&gt; Unit</code>, которая устанавливает вложенное содержимое</p></li>
</ul>
<p>Этот компонент также имеет еще одну версию:</p>
<pre class="brush:kt;">
@Composable
fun AnimatedVisibility(
...
<p>Единственное отличие от предыдущей версии состоит в том, что здесь для управления видимостью применяется параметр <code>visibleState</code>, который представляет значение типа <code>MutableTransitionState</code></p>
<p>Итак, компонент <span class="b">AnimatedVisibility</span> управляет видимостью вложенного содержимого. Рассмотрим простейший пример:</p>
<pre class="brush:kt;">
...
<p>Здесь прежде всего для управления видимостью определяем состояние boxVisible:</p>
<pre class="brush:kt;">val boxVisible = remember { mutableStateOf(true) }</pre>

<p><span class="b">AnimatedVisibility</span> содержит компонент Box и соответственно управляет видимостью этого компонента. Для управления видимостью параметр <span class="b">visible</span> 
привязан к состоянию boxVisible</p>
<pre class="brush:kt;">
AnimatedVisibility(visible = boxVisible.value, Modifier.padding(40.dp)) {
    Box(Modifier.size(200.dp).background(Color.DarkGray))
}
</pre>
<p>С помощью кнопок переключаем видимость компонента:</p>
<img src="./pics/13.1.png" alt="AnimatedVisibility в Jetpack Compose на Kotlin в Android" />
<p>При этом появление и исчезновение комопнента Box будет происходить не многовенно, а плавно. И именно в это плавности и суть компонента, потому что само переключение видимости (мгновенное) 
мы могли бы сделать и без AnimatedVisibility, например, следующим образом:</p>
<pre class="brush:kt;">
val boxVisible = remember { mutableStateOf(true) }
...
<style>
	...
	<div class="nav"><p><a href="./13.8.php">Назад</a><a href="./">Содержание</a><a href="./13.10.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
