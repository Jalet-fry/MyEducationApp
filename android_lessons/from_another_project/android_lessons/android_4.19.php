...
<html  lang="ru">
<head>
<title>Kotlin и Android | Аннотированные строки</title>
<meta charset="utf-8" />
<meta name="description" content="Компонент Text и Аннотированные строки (Annotated strings) в Jetpack Compose на языке программирования Kotlin, функция buildAnnotatedString, стилизация отдельных сегментов текста, SpanStyle и высота и цвет шрифта, TextStyle, TextAlign, TextDecoration">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Аннотированные строки</h2><div class="date">Последнее обновление: 04.04.2024</div>
	<div class="socBlock">
...
<p>Аннотированные строки (Annotated strings) позволяют разбить текст на сегменты, каждый из которых будет иметь свою собственную стилизацию. Аннотированные строки представляют 
объект <span class="b">AnnotatedString</span>. Для его создания применяется функция <span class="b">buildAnnotatedString()</span>, которой передается отображаемый текст и стили.</p>
<p>Поддерживаются два типа стилей: <b>SpanStyle</b> и <b>ParagraphStyle</b></p>

<h3>SpanStyle</h3>
<p>Первый тип стилей представляет объект <span class="b">SpanStyle</span> и используется для стилизации отдельных символов в строке. <b>SpanStyle</b> имеет следующий конструктор:</p>
<pre class="brush:kt;">
SpanStyle(
...
<p>Из конструктора мы видим, какие параметры текста устанавливает SpanStylе:</p>
<ul>
<li><p><code>color</code>: цвет текста</p></li></p></li>

<li><p><code>fontSize</code>: высота шрифта</p></li>

<li><p><code>fontWeight</code>: вес шрифта.</p></li>

<li><p><code>fontStyle</code>: стиль шрифта.</p></li>

<li><p><code>fontSynthesis</code>: устанавливает синтез толщины и/или стиля шрифта, 
если запрошенную толщину или стиль невозможно найти в предоставленном семействе шрифтов.</p></li>

<li><p><code>fontFamily</code>: семейство шрифтов.</p></li>

<li><p><code>fontFeatureSettings</code>: настройки типографии шрифта</p></li>

<li><p><code>letterSpacing</code>: расстояние между буквами.</p></li>

<li><p><code>baselineShift</code>: величина, на которую сдвигается текст вверх относительно базовой линии.</p></li>

<li><p><code>textGeometricTransform</code>: применяемые к тексту геометрические трансформации.</p></li>

<li><p><code>localeList</code>: список локалей, который используется для выбора специфичных для языка глифов</p></li>

<li><p><code>background</code>: фоновый цвет текста.</p></li>

<li><p><code>textDecoration</code>: декорации текста.</p></li>

<li><p><code>shadow</code>: тень текста.</p></li>

<li><p><code>platformStyle</code>: платформозависимые параметры стиля.</p></li>

<li><p><code>drawStyle</code>: стиль заполнения текста.</p></li>
</ul>
<p>Для создания и применения SpanStyle используется следующий синтаксис:</p>
<pre class="brush:kt;">
...
<p>Для добавления сегмента в текст применяется функция <code>append()</code>, в которую передается добавляемая подстрока. Если <code>append()</code> вызывается в контексте функции <code>withStyle()</code>, 
то к добавляемому тексту применяется стилизация определенная с помощью SpanStyle.</p>
...
<p>В данном случае текст разбивается на три сегмента. Первый сегмент состоит из одной буквы "H", к которой применяется некоторое форматирование. Второй сегмент - "ello" не использует никакого форматирования и добавляется в текст как есть. 
И затем добавляется третий сегмент, к которому также применяется некоторая стилизация:</p>
<img src="./pics/4.88.png" alt="Аннотированные строки и SpanStyle в Jetpack Compose на Kotlin и Android" />

<h3>ParagraphStyle</h3>
<p>Второй способ стилизации предоставляет класс <b>ParagraphStyle</b>. Он применяется к отдельным параграфам и имеет следующие параметры:</p>
<pre class="brush:kt;">
ParagraphStyle(
...
<ul>
<li><p><code>textAlign</code>: выравнивание текста в параграфе</p></li>

<li><p><code>textDirection</code>: направление текста (слева направо или справо налево)</p></li>

<li><p><code>lineHeight</code>: высота строк</p></li>

<li><p><code>textIndent</code>: настройки отступа в параграфе</p></li>

<li><p><code>platformStyle</code>: платформо-зависимые параметры</p></li>

<li><p><code>lineHeightStyle</code>: конфигурация для установки высоты строк. Применяется, если параметр <code>lineHeight</code> установлен</p></li>

<li><p><code>lineBreak</code>: настройка разрыва строк</p></li>

<li><p><code>hyphens</code>: настройки переносов</p></li>

<li><p><code>textMotion</code>: настройки размещения текстовых символов, оптимизация для анимированного или статического текста.</p></li>
</ul>
<p>Для создания и применения ParagraphStyle используется следующий синтаксис:</p>
<pre class="brush:kt;">
buildAnnotatedString {
...
<p>Применение <b>ParagraphStyle</b>:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Здесь определены два идентифных параграфа/абзаца, но ко второму применяется дополнительная стилизация, в частности, отступы в 30 пикселей для первой строки и 8 пикселей для остальных строк, а также 25 пикселей 
высота строки.</p>
<img src="./pics/4.89.png" alt="Аннотированные строки и ParagraphStyle в Jetpack Compose на Kotlin и Android" />
...
<style>
	...
	<div class="nav"><p><a href="./4.1.php">Назад</a><a href="./">Содержание</a><a href="./4.2.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
