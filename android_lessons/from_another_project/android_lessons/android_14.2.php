...
<html  lang="ru">
<head>
<title>Kotlin и Android | Отрисовка линий</title>
<meta charset="utf-8" />
<meta name="description" content="Отрисовка линий на компоненте Canvas с помощью функции drawLine в мобильном приложении на Jetpack Compose на языке программирования Kotlin в Android">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Отрисовка линий</h2><div class="date">Последнее обновление: 14.04.2024</div>
	<div class="socBlock">
...
<p>Для отрисовки линий в DrawScope определена функция <span class="b">drawLine()</span>:</p>
<pre class="brush:kt;">
fun drawLine(
    color: Color,
    start: Offset,
...
<p>Она имеет следующие параметры:</p>
<ul>
<li><p><code>color</code>: цвет линии</p></li>
<li><p><code>start</code>: начальная точка линии (значение типа Offset)</p></li>
<li><p><code>end</code>: конечная точка линии (объект Offset)</p></li>
<li><p><code>strokeWidth</code>: ширина линии в Float</p></li>
<li><p><code>cap</code>: тип окончания линии в виде значения типа <code>StrokeCap</code></p></li>
<li><p><code>pathEffect</code>: эффект, применяемый к линии</p></li>
<li><p><code>alpha</code>: прозрачность, применяемая к цвету линии в виде значения из диапазона от 0.0f до 1.0f</p></li>
<li><p><code>colorFilter</code>: цветовой фильтр ColorFilter, который применяется к цвету линии</p></li>
<li><p><code>blendMode</code>: алгоритм смешивания, применяемый к цвету</p></li>
</ul>
<p>Пример линии:</p>
<pre class="brush:kt;">
...
<p>Здесь рисуем синию линию шириной в 12 единиц от точки с координатами (x=0, y=0) до точки в правом нижнем углу.</p>
<img src="./pics/14.2.png" alt="Canvas и отрисовка линии в Jetpack Compose на Kotlin на Android" />
<p>Стоит учитывать, что для отрисовки линий (как и всех остальных графических примитивов) для определения координат используются не независимые от плотности экрана единицы <span class="b">Dp</span>, 
а обычные пиксели. Например:</p>
<pre class="brush:kt;">
Canvas(Modifier.size(300.dp).background(Color.LightGray)) {
    drawLine(
        start = Offset(x= 0f, y = 0f),
...
<p>Здесь Canvas имеет размер 300х300, а координата крайней точки линии имеет координаты (x=400, y=400). Казалось бы, линия должна уйти за пределы Canvas, но этого не происходит:</p>

<img src="./pics/14.26.png" alt="Canvas и пиксели в Jetpack Compose на Kotlin на Android" />
<p>Дело в том, что размер Canvas объявляется в независимых от плотности/устройства пикселях (dp), а координаты для отрисовки указывается в пикселях (px). Независимые от плотности пиксели — 
...
пикселям на экране. Чтобы работать исключительно в пикселях (а не в dp), значения <code>dp</code> можно преобразовать в пиксели с помощью функции <span class="b">Px()</span>:</p>
<pre class="brush:kt;">
Canvas(Modifier.size(300.dp).background(Color.LightGray)) {
    drawLine(
        start = Offset(x= 0f, y = 0f),
...
<h3>Тип линий</h3>
<p>С помощью параметра <code>pathEffect</code>, который представляет значение типа PathEffect, 
можно настроить тип линии. Чтобы создать пунктирную линию, нам нужно вызвать метод <span class="b">DashPathEffect()</span> объекта PathEffect и передать ему массив чисел с плавающей запятой. 
Числа с плавающей запятой обозначают интервалы в пикселях внутри линии. Должно быть четное количество интервальных значений, минимум 2 значения. Например:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Здесь рисуем линию, которая начинается с длинного штриха в 40 пикселей, после которого идет пробел в 10 пикселей, черточка в 10 пикселей и пробел в 10 пикселей. Далее вся эта комбинация 
повторяется до конца линии:</p>
<img src="./pics/14.3.png" alt="Canvas и отрисовка пунктирной линии в Jetpack Compose на Kotlin на Android" />
