...
<html  lang="ru">
<head>
<title>Kotlin и Android | LiveData</title>
<meta charset="utf-8" />
<meta name="description" content="Модель представления ViewModel и LiveData в мобильном приложении на Jetpack Compose на языке программирования Kotlin в Android, функция observeAsState и подписка на измнение состояния">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>LiveData</h2><div class="date">Последнее обновление: 16.04.2024</div>
	<div class="socBlock">
...
<p>Компонент <span class="b">LiveData</span> представляет еще один способ управления дангными в ViewModel и может использоваться в качестве обертки для данных во ViewModel. После помещения в 
LiveData данные становятся доступными для компонентов внутри класса Activity. Объекты <span class="b">LiveData</span> можно объявить как изменяемые с помощью класса 
<span class="b">MutableLiveData</span>, что позволяет функциям ViewModel вносить изменения в базовые значения данных.</p>
<p>Для работы с <span class="b">LiveData</span> в файле 
<span class="b">libs.versions.toml</span> в секции <code>[libraries]</code> укажем саму библиотеку, а в секции <code>[versions]</code> укажем версию этой библиотеки:</p>
<pre class="browser">
[versions]
<b>runtimeLivedata = "1.6.5"</b>
.......................


[libraries]
<b>androidx-runtime-livedata = { module = "androidx.compose.runtime:runtime-livedata", version.ref = "runtimeLivedata" }</b>
...
<p>Затем в файл <span class="b">build.grade.kts</span> добавим соответствующую зависимость:</p>
...
<p>Пример ViewModel, которая использует LiveData:</p>
<pre class="brush:kt;">
...
<p>Здесь у нас есть переменная count, которая представляет тип <code>MutableLiveData&lt;Int&gt;</code> - обертку над числом. И есть функция increase, которая увеличивает значение в count. Поскольку теоретически 
значение в MutableLiveData может представлять null, однако в нашем случае это невозможная ситуация, так как значение в любом случае представляет некоторое число, то при увеличении с помощью оператора <code>!!</code> 
говорим, что это значение не равно null.</p>
<p>Так, перепишем прошлый пример, использовав LiveData:</p>
<pre class="brush:kt;">
...
<p>Здесь определен компонент верхнего уровня CounterView, который инкапсулирует весь остальной интерфейс. В этот компонент в качестве параметра передается объект LiveCounterViewModel - наш ViewModel, 
...
Если экземпляр ViewModel (в нашем случае LiveCounterViewModel) уже создан в текущем контексте, то функция <span class="b">viewModel()</span> вернет ссылку на этот экземпляр. 
В противном случае будет создан и возвращен новый экземпляр ViewModel.</p>
<p>Для отслеживания изменения значения в LiveData компонент определяет дополнительную переменную countValue:</p>
<pre class="brush:kt;">val countValue by vm.count.observeAsState(0)</pre>
<p>Вызов функции <code>observeAsState()</code> преобразует значение из LiveData в объект состояния и присваивает его переменной countValue. После преобразования это состояние будет отслеживать, 
и при его изменении будет запускаться рекомпозиция при компонента.</p>
...
<img src="./pics/15.3.png" alt="Управление состоянием ViewModel и LiveData в Jetpack Compose на Kotlin на Android" />
