...
<html  lang="ru">
<head>
<title>Kotlin и Android | CompositionLocal</title>
<meta charset="utf-8" />
<meta name="description" content="Определение состояния с помощью CompositionLocal в Jetpack Compose на языке программирования Kotlin, передача состояния из родительских компонентов в дочерние, изменение состояния, функции staticCompositionLocalOf() и compositionLocalOf()">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>CompositionLocal</h2><div class="date">Последнее обновление: 04.04.2024</div>
	<div class="socBlock">
	...
	<p><span class="b">CompositionLocal</span> позволяет объявлять состояние на самом высоком уровне, а затем получать к нему доступ на любых нижних уровнях без необходимости 
передавать состояние через промежуточные дочерние элементы. Подобная организация позволяет сделать доступным состояние для определенных веток дереава компонентов.</p>
<p>Объявление состояния с использованием <code>CompositionLocal</code> начинается с создания объекта <span class="b">ProvidableCompositionLocal</span>, который можно получить с 
помощью вызова функции <span class="b">compositionLocalOf()</span> или <span class="b">staticCompositionLocalOf()</span>. В каждом случае функция принимает лямбда-выражение, 
которое определяет для состояния значение по умолчанию (если состоянию не передается конкретное значение). Например:</p>
<pre class="brush:kt;">
val LocalState1 = compositionLocalOf { значение }
val LocalState2 = staticCompositionLocalOf { значение }
</pre>
<p>Функция <span class="b">staticCompositionLocalOf()</span> рекомендуется для хранения состояния, которое вряд ли будет меняться очень часто. Это связано с тем, что любые изменения 
состояния приведут к перекомпоновке всей ветки дерева компонентов, для которой определяется состояние. Функция <span class="b">compositionLocalOf()</span> применяется при работе с состояниями, которые часто меняются - 
она вызывает рекомпозицию только тех компонентов, которые обращаются к состоянию.</p>
<p>Далее в вызове <span class="b">CompositionLocalProvider</span> объекту <code>ProvidableCompositionLocal</code> передается некоторое значение с помощью слова <span class="b">provides</span>, 
и выполняется вызов непосредственного дочернего компонента:</p>
<pre class="brush:kt;">
CompositionLocalProvider(LocalState1 provides новое_значение) {
    ChildComposable()
}
</pre>
<p>В итоге любые вложенные компоненты внутри компонента ChildComposable будут иметь прямой доступ к состоянию CompositionLocal через свойство <span class="b">current</span> объекта ProviderCompositionLocal:</p>
<pre class="brush:kt;">val переменная = LocalState1.current</pre>
...
<p>Здесь на верхнем уровне определено состояние LocalMessage (название произвольное):</p>
<pre class="brush:kt;">
val LocalMessage = staticCompositionLocalOf { "Hello" }
</pre>
<p>В методе setContent вызываем два компонента Composable11 и Composable21. Причем для второго компонента (и соответственно для всех его <span class="b">дочерних компонентов</span>) переустанавливаем значение состояния:</p>
<pre class="brush:kt;">
Column {
    Composable11()
    CompositionLocalProvider(LocalMessage provides "World") {
...
<p>После этого во вложенных компонентах через <code>LocalMessage.current</code> можно получить состояние:</p>
<pre class="brush:kt;">
@Composable
fun Composable22(){ Text(LocalMessage.current, fontSize = 28.sp) }
...
<img src="./pics/5.9.png" alt="Состояние CompositionLocal и его передача в дочерние компоненты в Jetpack Compose в Kotlin на Android" />
