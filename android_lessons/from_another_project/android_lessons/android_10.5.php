...
<html  lang="ru">
<head>
<title>Kotlin и Android | SharedState</title>
<meta charset="utf-8" />
<meta name="description" content="SharedState, хранение и получение состояния потока Flow в мобильном приложении Android на Jetpack Compose на языке программирования Kotlin, MutableSharedState">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>SharedState</h2><div class="date">Последнее обновление: 20.04.2024</div>
	<div class="socBlock">
...
<p><span class="b">SharedFlow</span> предоставляет более универсальный вариант потоковой передачи, чем <code>StateFlow</code>. Ключевые различия между StateFlow и SharedFlow 
заключаются в следующем:</p>
<ul>
<li><p>При создании объекта SharedFlow начальное значение не указывается</p></li>
<li><p>SharedFlow позволяет получить значения, которые были отправлены до начала сбора данных</p></li>
<li><p>SharedFlow генерирует значения с помощью <code>emit()</code> вместо использования свойства <code>value</code></p></li>
</ul>
<p>Для создания объекта SharedFlow сначала вызывается функция <span class="b">MutableSharedFlow()</span>, которая создает значение типа MutableSharedFlow:</p>
<pre class="brush:kt;">
...
<ul>
<li><p><code>replay</code>: количество значений, воспроизводимых новым подписчикам</p></li>
<li><p><code>extraBufferCapacity</code>: количество значений, буферизуемых в дополнение к значению параметра <code>replay</code>. 
emit не приостанавливается, пока остается свободное пространство в буфере.</p></li>
<li><p><code>onBufferOverflow</code>: настраивает действие отправки при переполнении буфера. Значения, отличные от BufferOverflow.SUSPEND, поддерживаются только в том случае, 
...
При отсутствии подписчиков сохраняются только самые последние значения воспроизведения, а поведение переполнения буфера никогда не срабатывает и не имеет никакого эффекта.</p>
<p>Может принимать следующие значения:</p>
<ul>
<li><p><code>DROP_LATEST</code>: последнее значение удаляется, когда буфер заполнен, оставляя буфер неизменным при обработке новых значений.</p></li>
<li><p><code>DROP_OLDEST</code>: рассматривает буфер как стек, где самое старое значение отбрасывается, чтобы освободить место для нового значения, когда буфер заполнен.</p></li>
<li><p><code>SUSPEND</code>: поток приостанавливается, когда буфер заполнен.</p></li>
</ul>
</li>
</ul>
<p>После создания у значения MutableSharedFlow вызывается метод <span class="b">asSharedFlow()</span> для получения ссылки SharedFlow.</p>
<pre class="brush:kt;">
private val _sharedFlow = MutableSharedFlow&lt;Int&gt;()
val sharedFlow = _sharedFlow.asSharedFlow()
</pre>
<p>Значения передаются в поток SharedFlow путем вызова метода <span class="b">emit()</span> объекта MutableSharedFlow:</p>
<pre class="brush:kt;">
...
<p>Например, возьмем пример с StateFlow из прошлой статьи и изменим его, заменив StateFlow на SharedFlow:</p>
<pre class="brush:kt;">
...
<p>Здесь вначале собственно определяем состояние SharedFlow:</p>
<pre class="brush:kt;">
val _sharedFlow = MutableSharedFlow&lt;Int&gt;()
val sharedFlow = _sharedFlow.asSharedFlow()
</pre>
<p>Для эммитирования значений в поток определяем контекст корутины:</p>
<pre class="brush:kt;">val coroutineScope = rememberCoroutineScope()</pre>
<p>И чтобы при запуске приложения началась генерация значений в поток, определяем компонент LaunchedEffect:</p>
<pre class="brush:kt;">
LaunchedEffect(Unit) {
    coroutineScope.launch() {
        for (i in 1..5) {
...
<p>В данном случае в поток поступают числа от 1 до 5 с задержкой в 2 секунды.</p>
<p>Для определения интерфейса определен компонент Main, который принимает поток SharedFlow. В этом компоненте сначала определяем состояние:</p>
<pre class="brush:kt;">
val count by flow.collectAsState(0)
val messages = remember { mutableStateListOf&lt;Int&gt;()}
</pre>
<p>Переменная count получает текущее значение потока. А переменная messages хранит список полученных из потока значений. Для сбора значений в messages определен компонент LaunchedEffect:</p>
<pre class="brush:kt;">
LaunchedEffect(Unit) {
    flow.collect {
        messages.add(it)
...
<p>Для отображения текущего значения из потока определен компонент Text, а для вывода всех полученных значений - компонент LazyList:</p>
<img src="./pics/10.7.png" alt="SharedFlow и хранение состояния потока в Jetpack Compose в Kotlin" />
...
<style>
	...
	<div class="nav"><p><a href="./10.4.php">Назад</a><a href="./">Содержание</a><a href="./12.1.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
