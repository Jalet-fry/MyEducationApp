...
<html  lang="ru">
<head>
<title>Kotlin и Android | Потоки Flow</title>
<meta charset="utf-8" />
<meta name="description" content="Определение и использование потоков Flow в мобильном приложении Android на Jetpack Compose на языке программирования Kotlin, операции с потоками">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Потоки Flow</h2><div class="date">Последнее обновление: 20.04.2024</div>
	<div class="socBlock">
	...
	<p>Потоки являются частью языка программирования Kotlin и предназначены для последовательного возврата нескольких значений из асинхронных задач на основе корутин. Подробнее про потоки 
можно прочитать на этом в соответствующей главы из руководства по Kotlin: <a href="https://metanit.com/kotlin/tutorial/9.1.php">Асинхронные потоки</a>. В данном же случае мы рассмотрим применение потоков в контексте мобильного приложения Android.</p>
<h4>Создание потока Flow</h4>
<p>Самую базовую форму потока в Kotlin представляет тип <span class="b">Flow</span> Каждый поток может отправлять данные только одного типа, который должен быть указан при объявлении потока. 
Например, для потоковой передачи строк можно использовать тип <code>Flow&lt;String&gt;</code>.</p>
<p>При объявлении потока нам нужно определить код для генерации данных. Для этого можно использовать специальную функцию-строитель <span class="b">flow()</span>, 
которая принимает в качестве параметра блок корутин с кодом генерации данным. Например:</p>
<pre class="brush:kt;">
...
<p>В данном случае определяется поток myFlow, предназначенный для генерации значений String.</p>
<p>В качестве альтернативы можно использовать функцию-строитель потока <span class="b">flowOf()</span> для преобразования фиксированного набора значений в поток:</p>
<pre class="brush:kt;">val myFlow2 = flowOf("Tom", "Bob", "Sam")</pre>
<p>Кроме того, многие типы коллекций Kotlin имеют функцию расширения <span class="b">asFlow()</span>, которую можно вызывать для преобразования данных коллекции в поток. Например, 
следующий код преобразует массив строковых значений в поток:</p>
<pre class="brush:kt;">val myFlow3 = arrayOf&lt;String&gt;("Tom", "Bob", "Sam").asFlow()</pre>
<h4>Генерация данных</h4>
<p>После создания потока следует предоставить механизм для передачи данных во вне. Из трех построителей потоков, которые мы рассмотрели в предыдущем разделе, 
только построители <code>flowOf()</code> и <code>asFlow()</code> создают потоки, которые автоматически отправляют данные, как только потребитель-внешний код начинает извлекать из потока данные. 
Однако в случае с функцией <code>flow()</code> нам нужно самим написать код, который будет вручную выдавать каждое значение по мере его появления. Для этого применяется 
функция <span class="b">emmit()</span>, в которую в качестве аргумента передается текущее значение для передачи из потока. Например:</p>
<pre class="brush:kt;">
...
<p>Здесь myFlow в цикле перебирает числа от 0 до 9 и через вызов <code>emit()</code> возвращает текущее перебираемое число. 
Для имитации долгой работы на каждой итерации цикла выполняется 2-секундная задержка, что позволяет продемонстрировать асинхронный характер потока.</p>
<h3>Поток как состояние и collectAsState()</h3>
<p>Jetpack Compose предоставляет функцию <span class="b">collectAsState()</span>, которая позволяет получить текущее значение из потока в виде состояния. ПРи получении нового значения произойдет рекомпозиция 
интерфейса. Например:</p>
<pre class="brush:kt;">
...
<p>Здесь в лямбда-выражении в методе setContent() создается простейший поток Flow&lt;Int&gt;, который генерирует числа от 0 до 9 с интервалом в 2 секунды. Этот поток передается в кастомный компонент Main, где текущее значение этого потока 
получаем в переменную состояния count. А компонент Text выводит ее значение.</p>
<p>Таким образом при запуске приложения каждые 2 секунды будет генерироваться новое число и помещаться в состояние count, а компонент Text будет его отображать:</p>
<img src="./pics/10.4.png" alt="асинхронные потоки Flow в мобильном приложении Android на Jetpack Compose на Kotlin" />
...
<h3>Преобразование потока</h3>
<p>И как в общем случае, к потокам Flow в приложении на Jetpack Compose можно применять различные преобразования. Основные из них были рассмотрены на этом сайте в руководстве по Kotlin в статье 
<a href="https://metanit.com/kotlin/tutorial/9.3.php">Операции с потоками</a>. Например, используем ряд операций:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>На уровне класса MainActivity определяется поток myFlow, который содержит числоа от 0 до 9.</p>
<p>В лямбда-выражении из метода setContent этот поток изменяется - из него выбираются только четные числа, которые трансформируются в строки</p>
<pre class="brush:kt;">val newFlow = myFlow.filter {it % 2 == 0}.map{ "Current value = $it"}</pre>
<p>И этот измененный поток передается в компонент Main.</p>
<p>Или другой пример - используем функцию <span class="b">reduce()</span>, которая сводит все значения потока к одному значению:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Функция <code>reduce()</code> принимает два параметра в виде аккумулятора и текущего значения из потока. Первое значение из потока помещается в аккумулятор, и между аккумулятором и 
текущим значением выполняется указанная операция (с сохранением результата в аккумуляторе). В данном случае мы просто складываем все числа из потока. Поскольку функция <code>reduce()</code> является suspend-функцией, она выполняется в контексте корутины. 
И для ее выполнения определяем компонент LaunchedEffect.</p>
...
<style>
	...
	<div class="nav"><p><a href="./10.2.php">Назад</a><a href="./">Содержание</a><a href="./10.4.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
