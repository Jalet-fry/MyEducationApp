...
<html  lang="ru">
<head>
<title>Kotlin и Android | Введение в пагинацию</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в пагинацию и постраничный вывод в мобильном приложении Jetpack Compose на языке программирования Kotlin в Android, HorizontalPager и VerticalPager, состояние PagerState и функция rememberPagerState">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Пагинация</h1><h2>Введение в пагинацию</h2><div class="date">Последнее обновление: 10.04.2024</div>
	<div class="socBlock">
...
<p>Пагинация или постраничный вывод в приложении является одной из распространенных задач, и для упрощения ее реализации Compose предоставляет ряд компонентов-пагинаторов, в частности, компоненты <span class="b">VerticalPager</span> и 
<span class="b">HorizontalPager</span>. Эти компоненты позволяют перелистывать контент по горизонтали или по вертикали. Поведение по умолчанию — перелистывание страниц с помощью свайпа пальцем  
влево и вправо для HorizontalPager и вверх и вниз для VerticalPager. Пагинаторы также предоставляют интерфейс для перехода к определенным страницам и различные варианты управления и контроля 
поведением компонента пагинации. Стоит отметить, что эти компоненты используют ленивую загрузку (как LazyColumn/LazyRow), то есть по умолчанию непосредственно компонуются только те страницы, которые находятся в видимой области.</p>
<p>Компонент <span class="b">HorizontalPager</span> применяется для создания горизонтальной пагинации. Он имеет следующие параметры:</p>
<pre class="brush:kt;">
@Composable
fun HorizontalPager(
    state: PagerState,
...
<ul>
<li><p><code>state</code>: состояние пагинатора в виде объекта <span class="b">PagerState</span></p></li>

<li><p><code>modifier</code>: применяемые к компоненту функции модификатора</p></li>

<li><p><code>contentPadding</code>: отступ вокруг контента в виде объекта <code>PaddingValues</code>. Может применяться для добавления отступов перед первой страницей или 
после последней страницы.</p></li>

<li><p><code>pageSize</code>: определяет, как страницы будут выглядеть внутри компонента. Представляет значение типа <code>PageSize</code>.</p></li>

<li><p><code>beyondViewportPageCount</code>: количество страниц, которые располагаются до или после видимой части и которые надо предварительно скомпоновать. Служит для цели оптимизации для ускорения 
загрузки страниц.</p></li>

<li><p><code>pageSpacing</code>: отступы в пикселях между отдельными страницами в компоненте</p></li>

<li><p><code>verticalAlignment</code>: вертикальное выравнивание страниц внутри компонента.</p></li>

<li><p><code>flingBehavior</code>: определяет поведение для момента, когда выполнена прокрутка пальцем.</p></li>

<li><p><code>userScrollEnabled</code>: указывает, будет ли доступна прокрутка.</p></li>

<li><p><code>reverseLayout</code>: указывает, надо ли размещать содержимое (страницы) в обратном порядке.</p></li>

<li><p><code>key</code>: определяет уникальный ключ для каждого элемента с помощью функции типа <code>(index: Int) -&gt; Any</code>. Ключ применяется для определения позиции при прокрутке. Если передается null, 
то позиция при прокрутке определяется с помощью позиции в списке.</p></li>

<li><p><code>pageNestedScrollConnection</code>: NestedScrollConnection = PagerDefaults.pageNestedScrollConnection(state,Orientation.Horizontal ) A NestedScrollConnection that dictates how this Pager behaves with nested lists. The default behavior will see Pager to consume all nested deltas.</p></li>

<li><p><code>snapPosition</code>: определяет, как компонент будет выполнять привязку страниц.</p></li>

<li><p><code>pageContent</code>: определяет содержимое страницы с помощью функции-компонента <code>PagerScope.(page: Int) -&gt; Unit</code>.</p></li>
</ul>
<p><span class="b">VerticalPager</span> располагает страницы по вертикали и имеет аналогичный набор параметров:</p>
<pre class="brush:kt;">
@Composable
fun VerticalPager(
    state: PagerState,
...
<p>Единственное отличие, что для страниц здесь используется выравнивание по вертикали.</p>
...
<h4>Состояние PagerState</h4>
<p>Для определения HorizontalPager или VerticalPager прежде всего необходимо определить их состояние. Для управления состоянием пагинатора применяется объект <span class="b">PagerState</span>. Класс <span class="b">PagerState</span> определяет ряд свойств и методов, которые позволяют получить информацию о состоянии пагинатора, либо управлять его поведением, например, прокручивать контент. 
Наиболее распространенные свойства:</p>
<ul>
<li><p><code>pageCount</code>: количество страниц</p></li>
<li><p><code>currentPage</code>: "текущая" страница (страница, наиболее близкая к текущей точке прокрутки)</p></li>
</ul>
<p>Среди методов PagerState следует отметить следующие:</p>
<ul>
<li><p><code>scrollToPage(page)</code>: мгновенный переход к странице с номером page</p></li>
<li><p><code>animateScrollToPage(page)</code>: плавный переход к странице с номером page</p></li>
</ul>
<p>Для определения состояния применяется функция <span class="b">rememberPagerState()</span></p>
<pre class="brush:kt;">
...
<p>Эта функция принимает три параметра:</p>
<ul>
<li><p><code>initialPage</code>: номер начальной страницы, которая будет отображаться первой.</p></li>
<li><p><code>initialPageOffsetFraction</code>: смещение начальной страницы в виде доли от размера страниц</p></li>
<li><p><code>pageCount</code>: количество страниц в пагинаторе.</p></li>
</ul>
<p>Например, определим состояние для пагинатора, в котормом будет 10 страниц:</p>
<pre class="brush:kt;">val pagerState = rememberPagerState { 10 }</pre>
<h4>Определение содержимого</h4>
<p>Последний параметр компонентов-пагинаторов устанавливает содержимое, в качестве которого может выступать любой компонент. Для установки содержимого применяется функция <code>(page: Int) -&gt; Unit</code>. 
Ее единственный параметр представляет индекс страницы (начиная с 0). Например, определим вертикальный пагинатор, где содержимое, то есть каждая страница представляет компонент Text, который 
отображает текущий номер страницы:</p>
<pre class="brush:kt;">
...
<p>В итоге создается 10 страниц, и с помощью свайпа пальцем вверх-вниз мы можем перемещаться по ним:</p>
<img src="./pics/12.1.png" alt="VerticalPager в Jetpack Compose на Kotlin в Android" />
...
<style>
	...
	<div class="nav"><p><a href="./10.5.php">Назад</a><a href="./">Содержание</a><a href="./12.2.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
