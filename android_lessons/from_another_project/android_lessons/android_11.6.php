...
<html  lang="ru">
<head>
<title>Kotlin и Android | Локализация приложений</title>
<meta charset="utf-8" />
<meta name="description" content="Локализация мобильноых приложении Android на Jetpack Compose на языке программирования Kotlin, строк и других ресурсов, роль файла strings.xml и функции stringResource, создание ресурсов для разных языков, тестирование локализации, параметр locale, изменение языка устройства">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Локализация приложений</h1><div class="date">Последнее обновление: 31.08.2025</div>
	<div class="socBlock">
...
<p>Локализация — это процесс адаптации приложения для различных языковых и культурных предпочтений пользователей. В контексте Android-разработки это означает создание ресурсов, которые система сможет автоматически выбирать в зависимости от настроек языка устройства. Правильная локализация делает приложение доступным для более широкой аудитории по всему миру, что является критически важным для глобального успеха.</p>
<h3>Ресурсы для локализации: strings.xml</h3>
<p>В Android локализация текста происходит через строковые ресурсы, которые хранятся в XML-файлах. Основной файл — <b>strings.xml</b>, расположенный в проекте в папке <span class="b">res/values/</span>. 
Для каждого дополнительного языка создается отдельный каталог с постфиксом-локалью, например, <code>values-ru/</code> для русского языка, <code>values-fr/</code> для французского языка и т.д.</p>
<p>Пример структуры папок при локализации на русский и испанский языки:</p>
<pre class="browser">
res/
├── values/
│   └── strings.xml      (строки по умолчанию, например, на английском)
├── values-ru/
│   └── strings.xml      (строки на русском языке)
└── values-es/
    └── strings.xml      (строки на испанском языке)
</pre>
<p>Рассмотрим небольшой пример локализации <code>strings.xml</code>. Так, каждый файл <b>strings.xml</b> содержит пары "ключ-значение". Ключи (идентификаторы) должны быть одинаковыми для всех языков, 
а значения — переведенными строками.</p>
<p>Например, пусть файл <b>res/values/strings.xml</b> будет выглядеть следующим образом:</p>
<pre class="brush:xml;">
&lt;resources&gt;
...
<p>Допустим, нам надо добавить локализацию для русского языка. Для этого в проекте в папке <b>res</b> надо создать новую папку <b>values-ru</b>, а в ней - файл <b>strings-ru.xml</b>. 
Итак, в Android Studio нажмем на папку <b>res/values</b> правой кнопкой мыши и в выпадающем меню выберем пункт <span class="b">New -> Values Resource File</span>:</p>
<img src="./pics/127.png" alt="Локализация в Android и Kotlin" />
<p>И далее в окне добавления нового файла ресурсов для имени файла укажем <span class="b">strings-ru.xml</span>, а для папки - <span class="b">values-ru</span></p>
<img src="./pics/128.png" alt="Локализация строк в Android и Kotlin" />
<p>Причем даже если в проекте изначально нет папки "values-ru", она автоматически будет создана.</p>
<p>После этого в структуре проекта в Android Studio мы увидим добавленный файл с суффиксом "ru" (который будет означать, что файл в реальности располагается в папке "values-ru"):</p>
<img src="./pics/129.png" alt="Локализация приложения Android и Kotlin и Jetpack Compose" />
<p>Изменим файл <b>res/values-ru/strings-ru.xml</b> следующим образом:</p>
<pre class="brush:xml;">
&lt;resources&gt;
...
<p>То есть файл <b>values-ru/strings-ru.xml</b> должен иметь все то же содержимое, что и <b>values/strings.xml</b>, только вместо надписей на языке по умолчанию (которым обычно является английский) указываем перевод на русский.</p>
<p>Теперь нам надо применить этот перевод в коде Kotlin. В Jetpack Compose мы можем напрямую обращаться к строковым ресурсам с помощью функции <b>stringResource()</b>, которая считывает 
строковый ресурс из <code>strings.xml</code> и автоматически обновляет контент при смене языка, что идеально подходит для реактивной природы Compose.</p>
<p>Например, пусть у нас файл <b>MainActivity.kt</b> будет выглядеть следующим образом:</p>
<pre class="brush:kt;">
package com.metanit.helloapp
...
<p>При запуске приложения будет вибираться та локализация, которая соответствует системному языку устройства. Например, в моем случае на устройства текущий язык - русский, поэтому будет выбираться локализация ресурсов из 
файла <code>strings-ru.xml</code></p>
<img src="./pics/130.png" alt="Локализация строк в Android и Kotlin и Jetpack Compose" />
<p>Когда пользователь сменит язык на устройстве, система автоматически загрузит нужный файл <code>strings.xml</code>, и приложение отобразит текст на соответствующем языке.</p>
...
<h3>Локализация строк с переменными</h3>
<p>Иногда в строках нужно использовать переменные, например, для отображения имени пользователя или количества предметов. Для этого в <code>strings.xml</code> используются плейсхолдеры. Например:</p>
<pre class="brush:xml;">
&lt;resources&gt;
    &lt;string name="app_name"&gt;Hello App&lt;/string&gt;
...
<p>Здесь число 1 в "%1" указывает на номер аргумента, который передается в функцию <code>stringResource</code>, а буква "s" в "$s" указывает на тип аргумент - строка, равно как "d" в "$d" указывает, что аргумент - число. 
То есть <code>%1$s</code>— это плейсхолдер для строки (string), а <code>%1$d</code>— для целого числа (decimal integer).</p>
<p>В локализованной версии плейсхолдеры должны сохраняться. Например:</p>
<pre class="brush:xml;">
&lt;resources&gt;
    &lt;string name="app_name"&gt;Привет&lt;/string&gt;
...
<p>Использование с <code>stringResource</code>:</p>
<pre class="brush:kt;">
@Composable
...
<p><code>stringResource()</code>автоматически подставит переданные аргументы в соответствующие плейсхолдеры.</p>
<h3>Другие типы ресурсов для локализации</h3>
<p>Локализация не ограничивается только текстом. Также можно локализовать:</p>
<ul>
<li><p><span class="b">Изображения</span>: переведенные изображения помещаются в папки с названиями по шаблону "drawable-код_языка", такие как <code>res/drawable-ru/</code>. Например, <code>flag.png</code> для английского языка в <code>drawable/</code>и <code>flag.png</code> с изображением российского флага в <code>drawable-ru/</code>.</p></li>
<li><p><span class="b">Размеры (<code>dimens.xml</code>)</span>: полезно, когда размеры макетов должны быть разными для разных языков (например, для языков с длинными словами).</p></li>
<li><p><span class="b">Иконки (<code>mipmap</code>)</span>: для иконок приложени, аналогично работе с изображениями.</p></li>
</ul>
<h3>Тестирование локализации</h3>
<p>Для тестирования локализации можно изменить язык на устройстве или эмуляторе:</p>
<ol>
<li><p>Перейдите в <span class="b">Settings</span> -&gt; <span class="b">System</span> -&gt; <span class="b">Languages & input</span> -&gt; <span class="b">Languages</span>.</p></li>
<li><p>Добавьте нужный язык и переместите его на первое место.</p></li>
</ol>
<p>Также можно использовать функционал <span class="b">Android Studio Preview</span> для просмотра макетов с разными локалями. Просто добавьте параметр <code>locale</code>к аннотации <code>@Preview</code>.
...
<img src="./pics/131.png" alt="Тестирование локализации приложения в Android Studio и Jetpack Compose" />
...
<style>
	...
	<div class="nav"><p><a href="./11.5.php">Назад</a><a href="./">Содержание</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
