...
<html  lang="ru">
<head>
<title>Kotlin и Android | Пример приложения с VieModel</title>
<meta charset="utf-8" />
<meta name="description" content="Модель представления ViewModel и взаимодействие с пользовательским интерфейсом в мобильном приложении на Jetpack Compose на языке программирования Kotlin в Android">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Пример приложения с VieModel</h2><div class="date">Последнее обновление: 16.04.2024</div>
	<div class="socBlock">
...
<p>Рассмотрим примере применения ViewModel на простом примере:</p>
<pre class="brush:kt;">
...
<p>Прежде всего здесь определяется класс User - модель данных, с которыми мы будем работать:</p>
<pre class="brush:kt;">data class User(val name: String, val age: Int)</pre>
<p>Данный класс представляет пользователя. Свойство name представляет его имя , а age - возраст.</p>
<p>Для работы с данными определена модель представления UserListViewModel. Она управляет списком пользователей, в который мы можем добавить других пользователей или, наоборот, удалить из него пользователей. 
Непосредственно сам список пользователей хранится в переменной users. По умолчанию в этот список добавлено 2 объекта:</p>
<pre class="brush:kt;">val users = mutableStateListOf&lt;User&gt;(User("Tom", 39), User("Bob", 43))</pre>
<p>Для добавления нового пользователя определяются две переменных - для имени и возраста:</p>
<pre class="brush:kt;">
var userName by mutableStateOf("")
var userAge by mutableStateOf(0)
</pre>
<p>Для управления данными также определяется ряд функций:</p>
<pre class="brush:kt;">
fun addUser(){
    users.add(User(userName, userAge))
}
...
<p>В функции addUser добавляем нового пользователя, используя переменные name и age. В функции deleteUser получаем удаляемого пользователя и удаляем его из списка. И функции changeName и changeAge предназначены для 
изменения переменных name и age соответственно. Обратите внимание, что в changeAge передается строка, которую надо преобразоваь в число.</p>
<p>Для создания формы ввода данных нового пользователя определен компонент UserData с двумя текстовыми полями для имени и возраста:</p>
<pre class="brush:kt;">
fun UserData(userName: String, userAge:Int, changeName:(String)-&gt;Unit, changeAge:(String)-&gt;Unit, add:()-&gt;Unit){
    Column{
        OutlinedTextField(value = userName, modifier=Modifier.padding(8.dp), label = {Text("Enter a name")}, onValueChange = {changeName(it)})
...
<p>При изменении введенных данных в текстовых полях будут вызываться функции changeName и changeAge, которые получают введенное значение. А при нажатии на кнопку будет вызываться функция addUser, которая добавит нового пользователя.</p>
<p>Для вывода списка предназначен компонент UserList:</p>
<pre class="brush:kt;">
@Composable
fun UserList(users:List&lt;User&gt;, delete:(User)-&gt;Unit) {
    LazyColumn(Modifier.fillMaxWidth()) {
...
<p>Здесь для каждого пользователя в списке создается пара текстовых меток для вывода имени и возраста с кнопкой удаления, по нажатию на которую будет вызываться функция deleteUser.</p>
<p>И оба этих компонента вызываются в компоненте верхнего уровня Main, который получает ViewModel и передает ее данные в компоненты:</p>
<pre class="brush:kt;">
@Composable
fun Main(vm: UserListViewModel = viewModel()) {
    Column {
...
<p>В итоге приложения отобразит список, в который мы сможем добавлять новые данные:</p>
<img src="./pics/15.4.png" alt="ViewModel в мобильном приложении на Jetpack Compose на Kotlin на Android" />
...
<style>
	...
	<div class="nav"><p><a href="./15.2.php">Назад</a><a href="./">Содержание</a><a href="./15.4.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
