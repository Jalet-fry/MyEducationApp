...
<html  lang="ru">
<head>
<title>Kotlin и Android | Программная прокрутка</title>
<meta charset="utf-8" />
<meta name="description" content="Программная прокрутка контейнеров Column, Row, LazyColumn и LazyRow с помощью ScrollState и LazyListState в Jetpack Compose на языке программирования Kotlin на Android, функции animateScrollTo, animateScrollToItem, scrollTo и scrollToItem">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Программная прокрутка</h2><div class="date">Последнее обновление: 09.04.2024</div>
	<div class="socBlock">
...
<p>Кроме автоматической прокрутки с помощью пролистывания пальцев на экране устройства Jetpack Compose также поддерживает программную прокрутку, которую можно выполнить из кода Kotlin. 
Это может быть полезно, когда нам надо изменить текущую позицию в списке, перейти к какому-то определенному элементу в этом списке.</p>
<h3>Column и Row</h3>
<p>При работе со контейнерами Column и Row программную прокрутку можно выполнить с помощью функций объекта <span class="b">ScrollState</span>:</p>
<ul>
<li><p><span class="b">animateScrollTo(value: Int)</span> - плавная прокрутка до указанной позиции пикселя в списке с использованием анимации</p></li>
<li><p><span class="b">scrollTo(value: Int)</span> - мгновенная прокрутка до указанной позиции пикселя</p></li>
</ul>
<p>Печальная сторона этих функций заключается в том, что их параметры представляют позицию в пикселях, а не номер элемента. То есть начало списка представлено позицией пикселя 0, но позиция пикселя, представляющая конец списка, может 
быть менее очевидной. К счастью, максимальную позицию прокрутки можно определить, обратившись к свойству maxValue:</p>
<pre class="brush:kt;">val maxScrollPosition = scrollState.maxValue</pre>
<p>Также стоит учитывать, что это suspend-функции, поэтому их необходимо вызывать из корутин.</p>
<p>Например, определим код для прокрутки в последнему элементу в контейнере Column:</p>
<pre class="brush:kt;">
...
<p>В начале контейнера Column здесь определен компонент Text, по нажатию на который вызывается корутина, которая выполняет функцию <code>scrollState.animateScrollTo()</code></p>
<img src="./pics/2.48.png" alt="программная прокрутка в Column в Jetpack Compose на Kotlin на Android" />
<h3>LazyColumn и LazyRow</h3>
<p>Для программной прокрутки списков LazyColumn и LazyRow применяются функции объекта <span class="b">LazyListState</span>, который можно получить с помощью вызова функции <span class="b">rememberLazyListState()</span>:</p>
<pre class="brush:kt;">val listState = rememberLazyListState()</pre>
<p>Затем этои объект передается параметру <span class="b">state</span>:</p>
<pre class="brush:kt;">
LazyColumn(state = listState....
</pre>
<p>Для собственно прокрутки вызываются следующие методы LazyListState:</p>
<ul>
<li><p><span class="b">animateScrollToItem(index: Int)</span> — плавная прокрутка к указанному элементу списка (где 0 — первый элемент)</p></li>
<li><p><span class="b">scrollToItem(index: Int)</span> — мгновенная прокрутка к указанному элементу списка (где 0 — первый элемент)</p></li>
</ul>
<p>Это тоже suspend-функции, которые должны запускаться из корутин. Применение:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>В данном случае создается аналогичный список с 20 компонентами Text плюс начальный Text, который выполняет роль кнопки перехода к 19-му элементу в списке.</p>
...
<style>
	...
	<div class="nav"><p><a href="./2.9.php">Назад</a><a href="./">Содержание</a><a href="./2.11.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
