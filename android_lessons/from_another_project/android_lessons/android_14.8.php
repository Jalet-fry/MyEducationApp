...
<html  lang="ru">
<head>
<title>Kotlin и Android | Вывод текста</title>
<meta charset="utf-8" />
<meta name="description" content="Вывод текста на компоненте Canvas с помощью функции drawText в мобильном приложении на Jetpack Compose на языке программирования Kotlin в Android, роль класса TextMeasurer">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Вывод текста</h2><div class="date">Последнее обновление: 14.04.2024</div>
	<div class="socBlock">
...
<p>Для вывода текста на Canvas применяется функция <span class="b">drawText()</span>. Она имеет ряд версий, рассмотрим одну из них:</p>
<pre class="brush:kt;">
...
<ul>
<li><p><code>textMeasurer</code>: объект TextMeasurer, который применяется для расположения текста</p></li>
<li><p><code>text</code>: выводимый текст</p></li>
<li><p><code>topLeft</code>: определяет смещение текста от верхней левой точки текущей системы координат</p></li>
<li><p><code>style</code>: стиль заполнения текста</p></li>
<li><p><code>overflow</code>: указывает, как следует обрабатывать выход текста за границы контейнера</p></li>
<li><p><code>softWrap</code>: указывает, должен ли текст разрываться при разрывах строк</p></li>
<li><p><code>maxLines</code>: максимальное количество строк</p></li>
<li><p><code>size</code>: размер прямоугольной области, в которую вписывается текст</p></li>
<li><p><code>blendMode</code>: алгоритм смешивания, применяемый к цвету текста</p></li>
</ul>
<p>Еще одна версия:</p>
<pre class="brush:kt;">
fun DrawScope.drawText(
...
<p>Эта версия функции принимает следующие параметры:</p>
<ul>
<li><p><code>textLayoutResult</code>: параметры расположения текста</p></li>
<li><p><code>color</code>: цвет пути</p></li>
<li><p><code>topLeft</code>: определяет смещение текста от верхней левой точки текущей системы координат</p></li>
<li><p><code>alpha</code>: прозрачность, применяемая к тексту</p></li>
<li><p><code>shadow</code>: параметры тени текста</p></li>
<li><p><code>textDecoration</code>: декорации текста (например, подчеркивание)</p></li>
<li><p><code>drawStyle</code>: стиль заполнения текста</p></li>
<li><p><code>blendMode</code>: алгоритм смешивания, применяемый к цвету текста</p></li>
</ul>


<p>Для установки расположения текста применяется объект <span class="b">TextMeasurer</span>, который применяется для расчета размера текста в зависимости от семейства и размера шрифта. 
Для получения объекта <span class="b">TextMeasurer</span> применяется функция <span class="b">rememberTextMeasurer()</span>:</p>
<pre class="brush:kt;">
val textMeasurer = rememberTextMeasurer()
</pre>
<p>Получив объект TextMeasurer, его можно передать его в функцию <code>drawText()</code> вместе с текстом, который нужно отрисовать:</p>
<pre class="brush:kt;">
...
<img src="./pics/14.21.png" alt="Canvas и вывод текста в Jetpack Compose на Kotlin на Android" />
<p>Однако по умолчанию получается довольно маленький текст. И с помощью TextMeasurer мы можем настроить его параметры с помощью функции <code>TextMeasurer.measure()</code>:</p>
<pre class="brush:kt;">
fun measure(
    text: String,
    style: TextStyle = TextStyle.Default,
...
<p>Эта функция позволяет задать текст, его стиль, семейство шрифтов и т.д. Например:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>В данном случае для шрифта задается высота в 28 единиц:</p>
<img src="./pics/14.22.png" alt="Canvas и вывод текста и drawText в Jetpack Compose на Kotlin на Android" />
<p>Кроме обычного текста <code>drawText()</code> позволяет выводить аннотированные строки с более сложным форматированием отдельных сегментов текста. Для создагния аннотированных строк применяется 
функция <span class="b">buildAnnotatedString()</span>. Подробнее про аннотированные строки рассказывалось в статье <a href="https://metanit.com/kotlin/jetpack/4.19.php">Аннотированные строки</a>. 
Здесь же рассмотрим их вывод на Canvas:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>В данном случае определяется два сегмента текста с разным форматированием:</p>
<img src="./pics/14.23.png" alt="Canvas и вывод аннотированного текста и drawText в Jetpack Compose на Kotlin на Android" />
...
<style>
	...
	<div class="nav"><p><a href="./14.7.php">Назад</a><a href="./">Содержание</a><a href="./14.9.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
