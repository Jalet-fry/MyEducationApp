...
<html  lang="ru">
<head>
<title>Kotlin и Android | Перетаскивание с помощью PointerInputScope</title>
<meta charset="utf-8" />
<meta name="description" content="Перетаскивание с помощью PointerInputScope в мобильном приложении на Jetpack Compose на языке программирования Kotlin в Android, модификатор pointerInput, функция detectDragGestures">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Перетаскивание с помощью PointerInputScope</h2><div class="date">Последнее обновление: 19.04.2024</div>
	<div class="socBlock">
...
<p>В прошлой теме был рассмотрен модификатор <code>draggable()</code>, который позволяет нам реализовать перетаскивание компонента. Однако этот модификатор имеет ограничение: одновременно он может использовать 
либо перетаскивание по горизонтали, либо по вертикали. Функция <span class="b">detectDragGestures()</span> типа <code>PointerInputScope</code> решает эту проблему и позволяет нам одновременно поддерживать 
операции горизонтального и вертикального перетаскивания. Эта функция имеет следующие параметры:</p>
<pre class="brush:kt;">
...
<ul>
<li><p><code>onDragStart</code>: функция, которая вызывается при начале перетаскивания</p></li>
<li><p><code>onDragEnd</code>: функция, которая вызывается после завершения перетаскивания</p></li>
<li><p><code>onDragCancel</code>: функция, которая вызывается при отмене перетаскивания</p></li>
<li><p><code>onDrag</code>: функция, которая вызывается при перетаскивания</p></li>
</ul>
<p>Нам достаточно задать последний параметр для управления перетаскиваемым компонентом. И обработка перетаскивания в общем случае будет выглядеть примерно следующим образом:</p>
<pre class="brush:kt;">
...
<p>Для отслеживания перетаскивания как по горизонтали, так и по вертикали, определяются две переменных состояния для хранения смещений по оси X и Y:</p>
<pre class="brush:kt;">
var xOffset by remember { mutableStateOf(0f) }
var yOffset by remember { mutableStateOf(0f) }
</pre>
<p>К их значениям привязаны координаты компонента Box:</p>
<pre class="brush:kt;">
Box(
    Modifier.offset { IntOffset(xOffset.roundToInt(), yOffset.roundToInt()) }
</pre>
<p>В лямбда-выражение в функции detectDragGestures передается параметр distance, который представляет объект Offset и из которого мы можем получить последние значения смещения перетаскивания 
по осям x и y. Они добавляются к состояниям xOffset и yOffset соответственно, в результате чего компонент Box следует за движением перетаскивания по экрану:</p>
<pre class="brush:kt;">
.pointerInput(Unit) {
    detectDragGestures { _, distance -&gt;
        xOffset += distance.x
...
<p>И для наглядности координаты перемещения выводятся на экран ыв компоненте Text:</p>
<img src="./pics/18.3.png" alt="Обработка перетаскивания и модификатор pointerInput в Jetpack Compose Kotlin Android" />
...
<style>
	...
	<div class="nav"><p><a href="./18.2.php">Назад</a><a href="./">Содержание</a><a href="./18.4.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
