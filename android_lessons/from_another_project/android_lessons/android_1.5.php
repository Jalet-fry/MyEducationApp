...
<html  lang="ru">
<head>
<title>Kotlin и Android | Создание компонентов Composable</title>
<meta charset="utf-8" />
<meta name="description" content="Создание компонентов Composable в проекте на Jetpack Compose на языке программирования Kotlin в среде разработки Android Studio, роль аннотации @Composable, установка корневого компонента в MainActivity, настройка предварительного просмотра">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Создание компонентов Composable</h2><div class="date">Последнее обновление: 30.03.2024</div>
	<div class="socBlock">
...
<p>Основой пользовательского интерфейса в Jetpack Compose являются компоненты, которые оформлены в виде функции  с аннотацией <span class="b">@Composable</span>. 
При вызове <span class="b">@Composable</span>-компонента обычно передаются некоторые данные и набор свойств, которые определяют отображение и поведение определенной части 
пользовательского интерфейса. По сути, функции <span class="b">@Composable</span> преобразуют данные в элементы пользовательского интерфейса. 
Эти функции не возвращают значения в традиционном смысле функции Kotlin, а вместо этого определяют элементы пользовательского интерфейса для рендеринга.</p>
<p><span class="b">@Composable</span>-функции могут вызывать другие подобные компоненты для создания иерархии компонентов. При разработке приложений с помощью Compose мы можем как определять свои собственные компоненты, так и использовать встроенные. 
Собственно типичный пользовательский интерфейс на основе Compose обычно состоит из комбинации встроенных и кастомных <span class="b">@Composable</span>-компонентов.</p>
<p>Встроенные компоненты, которые по умолчанию входят 
в состав Compose, можно разделить на три категории: <span class="b">Layout</span>, <span class="b">Foundation</span> и <span class="b">Material Design</span>.</p>
<p>В категорию <span class="b">Layout</span> входят компоненты, которые определяют макет приложения, позволяют определить расположение других компонентов на экране. В частности, 
это следуюшие компоненты:</p>
<ul>
<li><p><code>Box</code><p></li>
<li><p><code>BoxWithConstraints</code><p></li>
<li><p><code>Column</code><p></li>
<li><p><code>ConstraintLayout</code><p></li>
<li><p><code>Row</code><p></li>
</ul>
<p>Компоненты <span class="b">Foundation</span> — это набор минимальных компонентов, которые обеспечивают базовую функциональность пользовательского интерфейса. Например, это:</p>
<ul>
<li><p><code>BaseTextField</code><p></li>
<li><p><code>Canvas</code><p></li>
<li><p><code>Image</code><p></li>
<li><p><code>LazyColumn</code><p></li>
<li><p><code>LazyRow</code><p></li>
<li><p><code>Shape</code><p></li>
<li><p><code>Text</code><p></li>
</ul>
<p>Компоненты <span class="b">Material Design</span> разработаны так, чтобы они соответствовали рекомендациям визуальному стилю Google Material. Например, это:</p>
<ul>
<li><p><code>AlertDialog</code><p></li>
<li><p><code>Button</code><p></li>
<li><p><code>Card</code><p></li>
<li><p><code>CircularProgressIndicator</code><p></li>
<li><p><code>DropdownMenu</code><p></li>
<li><p><code>Checkbox</code><p></li>
<li><p><code>FloatingActionButton</code><p></li>
<li><p><code>LinearProgressIndicator</code><p></li>
<li><p><code>ModalDrawer</code><p></li>
<li><p><code>RadioButton</code><p></li>
<li><p><code>Scaffold</code><p></li>
<li><p><code>Slider</code><p></li>
<li><p><code>Snackbar</code><p></li>
<li><p><code>Switch</code><p></li>
<li><p><code>TextField</code><p></li>
<li><p><code>TopAppBar</code><p></li>
<li><p><code>BottomNavigation</code><p></li>
</ul>
<h3>Создание своего компонента</h3>
<p>Аналогично мы могли бы создать свой компонент на основе имеющихся и использовать его в качестве корневого компонента. Например, изменим код следующим образом:</p>
<pre class="brush:kt;">
...
<p>Здесь мы определили свой компонент Hello, внутри которого по сути используется встроенный компонент Text. Чтобы функция Hello рассматривалась как компонент, к ней применяется аннотация <span class="b">@Composable</span> 
из пакета <code>androidx.compose.runtime</code>. Причем функции компонентов естественно можно вынести в отдельные файлы и даже пакеты и оттуда подключать.</p>
...
<h3>Настройка предварительного просмотра</h3>
<p>И также для наших компонентов можно задать предварительный просмотр в Android Studio, что позволяет уже до запуска увидеть, как будет выглядеть приложение. Так, изменим код <span class="b">MainActivity.kt</span> 
следующим образом:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Здесь добавлен компонент HelloPreview, который использует компонент Hello. К компоненту HelloPreview применяется аннотация 
<span class="b">@Preview</span> из пакета <code>androidx.compose.ui.tooling.preview</code>. Свойство <code>showSystemUi = true</code> этой аннотации позволяет увидеть интерфейс приложения в целом в области предпросмотра в Android Studio:</p>
<img src="./pics/1.17.png" alt="Предварительный просмотр Jetpack Compose в Android Studio" />
<h3>Механика функций Composable</h3>
<p>Исходный код аннотации <span class="b">Composable</span> выглядит следующим образом:</p>
<pre class="brush:kt;">
@MustBeDocumented
...
<p>Здесь мы видим, что к классу аннотации, в совю очередь, применяются три других аннотации:</p>
<ul>
<li><p><code>@MustBeDocumented</code>: указывает, что аннотация является частью общедоступного API и должна быть включена в создаваемую документацию.</p></li>
<li><p><code>@Retention</code>: сообщает компилятору, как долго должна сохраняться аннотация. Значение <code>AnnotationRetention.BINARY</code> позволяет сохранить код в бинарном файле 
во время компиляции.</p></li>
<li><p><code>@Target</code>: описывает контексты, в которых применяется этот тип. @Composable можно применять к типам, параметрам, функциям и свойствам.</p></li>
</ul>
<p>Последняя аннотация указывает, что мы можем применять аннотацию <span class="b">@Composable</span> гораздо шире, чем просто к функции верхнего уровня, как в примерах выше. 
Например:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Здесь аннотация <code>@Composable</code> применяется к функции printMessage внутри класса Message. Эта функция определяет компонент Text, в который передается значение свойства messageText. То есть функция 
printMessage выступает в качестве компонента. И мы можем использовать этот компонент, вызывав данную функцию:</p>
<pre class="brush:kt;">
setContent {
    Message("Hello METANIT.COM!").printMessage()
}
</pre>
<p>Соответственно фактически интерфейс будет определяться компонентом Text, который выведет строку "Hello METANIT.COM!":</p>
<img src="./pics/1.13.png" alt="Аннотация Composable в Kotlin и Jetpack Compose" />
<h3>setContent</h3>
<p>Теперь пришло время, чтобы сказать чуть подробнее, как устанавливается, корневой компонент. Это происходит в методе <span class="b">setContent</span>. Сигнатура метода <span class="b">setContent()</span> выглядит следующим образом:</p>
<pre class="brush:kt;">
...
<p>Как можно видеть, <code>setContent()</code> является функцией расширения типа <code>ComponentActivity</code>. 
Функции расширения добавляют классу дополнительную функциональность без изменения его исходного кода. Это означает, что метод <span class="b">setContent()</span> можно использовать 
для любого ComponentActivity или его подклассов, например AppCompatActivity.</p>
<p>Вызов <span class="b">setContent()</span> устанавливает функцию из параметра content в качестве корневого компонента, 
...
Обратите внимание, что функция content также аннотируется с помощью аннотации @Composable. Так как аннотация <code>@Target</code> позволяет применять аннотацию <span class="b">@Composable</span> к параметрам функции. 
Конкретно в данном случае аннотация <span class="b">@Composable</span> помечает передаваемую параметру <code>content</code> функцию как <span class="b">Composable</span>-функцию.</p>
<p>Еще один параметр метода <span class="b">setContent()</span> представляет объект <span class="b">CompositionContext</span>, который предствляет контекст композиции интерфейса и используется для рекомпозиции - обновления пользовательского интерфейса.</p>
<h3>Компонент как параметр</h3>
<p>Определение метода <code>setContent</code> демонстрирует один из распространенных способов передачи компонентов в другие компоненты. 
Подобная организация очень удобна, поскольку мы можем в функции получить компонент, как-то настроить его, выполнить с ним некоторые действия.</p>
...
<p>Здесь определен компонент Message, который через параметр принимает некоторую функцию-компонент. Внутри компонента Message мы просто вызываем переданную функцию.</p>
<p>Методе <span class="b">setContent</span> вызываем компонент Message. А в функции, которая передается в Message через параметр content, вызываем компонент Text:</p>
<pre class="brush:kt;">
Message{
    Text(text = "Hello World", fontSize = 28.sp)
}
...
<style>
	...
	<div class="nav"><p><a href="./1.3.php">Назад</a><a href="./">Содержание</a><a href="./5.1.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
