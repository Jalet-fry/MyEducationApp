...
<html  lang="ru">
<head>
<title>Kotlin и Android | SQLite и Room</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с базой данных SQLite с помощью библиотеки Room в мобильном приложении на Jetpack Compose на языке программирования Kotlin в Android, ORM, подключение Room в проект в gradle">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Работа с базой данных</h1><h2>SQLite и Room</h2><div class="date">Последнее обновление: 16.04.2024</div>
	<div class="socBlock">
...
<p>База данных является наиболее распространенным местом, где мобильное приложение хранит свои данные. Наиболее популярной с системой управления базами данных в мобильном приложении является 
SQLite. И Android SDK предоставляет необходимый функционал для работы с SQLite. Конечно, прежде чем углубляться в особенности SQLite в контексте разработки под Android, 
необходимо иметь базовое понимание запросов SQL. Для этого можно обратиться к соответствующему руководству на данном сайте - <a href="https://metanit.com/sql/sqlite/">Руководство по SQLite</a>. 
Здесь же мы будет рассматривать работу с SQLite непосредственно в контексте приложения на Jetpack Compose на Android без отсылки к основам SQLite.</p>
<p>Базы данных SQLite размещаются в файлах (обычно с раширением <code>*.db</code>), которые зачастую расположены по следующему пути:</p>
<pre class="brush:kt;">/data/data/&lt;имя_пакета&gt;/databases/&lt;имя_базы_данных&gt;.db</pre>
<p>Например, если пакет приложения называется "com.example.helloapp", и оно использует базу данных "mydatabase.db", то путь к файлу базы данных будет следующим:</p>
<pre class="brush:kt;">/data/data/com.example.helloapp/databases/mydatabase.db</pre>
<p>Для работы с SQLite Jetpack Compose имееет специальную библиотеку <span class="b">Room</span>, которая предоставляет высокоуровневый интерфейс поверх системы базы данных SQLite и которая 
значительно упрощает взаимодействие с базами данными. Вся схема взаимодействия с базой данных выглядит следующим образом:</p>
<img src="./pics/16.1.png" alt="SQLite и Room в мобильном приложении на Jetpack Compose на Kotlin на Android" />
<p>Прежде всего для получения данных из базы данных обычно определяется отдельный модуль репозитория. Однако код приложения, в частности, репозиторий не обращается  напрямую к базе данных. Вместо этого все операции с базой данных выполняются с использованием комбинации базы данных Room, DAO и сущностей. 
DAO (Data Access Object - объект доступа к данным) содержит операторы SQL, необходимые репозиторию для добавления, извлечения и удаления данных в базе данных SQLite. 
Эти инструкции SQL сопоставляются с методами, которые затем вызываются из репозитория для выполнения соответствующего запроса.</p>
<p>Объект базы данных Room предоставляет интерфейс к базовой базе данных SQLite. Он также предоставляет репозиторию доступ к объекту DAO. Приложение должно иметь только один экземпляр 
базы данных Room, который можyj использовать для доступа к нескольким таблицам базы данных.</p>
<p>Для определения данных, которые хранятся в отдельной таблице, применяются специальные классы - <span class="b">сущности</span> (entity). Сущности определяют схему таблицы в базе данных, имя таблицы, ее столбцы и сопутствующюю информацию. 
Когда репозиторий получает данные базы данных через DAO, то эти данные как раз представляют объекты классов сущностей. Аналогично, когда репозиторию необходимо добавить в базу данных новые данные, 
он создает объект сущности и затем вызывает методы добавления, определенные в DAO.</p>
<p>В итоге все взаимодействие с базой данных выглядит следующим образом:</p>
<ol>
<li><p>Репозиторий взаимодействует с базой данных Room для получения объекта базы данных, который, в свою очередь, используется для получения ссылок на объект DAO</p></li>

<li><p>Репозиторий создает объекты сущностей и настраивает их с данными перед передачей их в DAO</p></li>

<li><p>Репозиторий вызывает методы DAO, передавая в них объекты сущностей, которые надо добавить и т.д.</p></li>

<li><p>Когда объект DAO получает результат запроса для передачи в репозиторий, он упаковывает эти результаты в объекты сущностей</p></li>

<li><p>Объект DAO взаимодействует с базой данных Room для выполнения операций с базой данных и обработки результатов</p></li>

<li><p>База данных Room обрабатывает все низкоуровневые взаимодействия с базой данных SQLite, отправляя запросы и получая результаты</p></li>
</ol>
<h3>Добавление Room в проект</h3>
<p>Перед началом работы с Room, необходимо настроить конфигурацию проекта и добавить все необходимые зависимости. Первым шагом является добавление плагина ksp и 
дополнительных библиотек в конфигурацию сборки Gradle. Для этого перейдем в преокте к файлу <span class="b">libs.versions.toml</span> и изменим его следующим образом:</p>
<pre class="brush:kt;">
[versions]
...................
roomRuntime = "2.6.1"
...
<p>Далее для добавления плагина ksp изменим файл <span class="b">build.gradle.kts</span> уровня проекта ("build.gradle.kts (Project: HelloApp)") следующим образом:</p>
<pre class="browser">
// Top-level build file where you can add configuration options common to all sub-projects/modules.
plugins {
    alias(libs.plugins.androidApplication) apply false
    alias(libs.plugins.jetbrainsKotlinAndroid) apply false
    <b>alias(libs.plugins.devtoolsKsp)</b>
}
</pre>
<p>И далее отредактируем файл <span class="b">build.gradle.kts</span> уровня модуля ("build.gradle.kts (Module :app)") следующим образом:</p>
<pre class="browser">
plugins {
...
dependencies {
    ...
    <b>implementation(libs.androidx.room.runtime)
    implementation(libs.androidx.room.ktx)
    implementation (libs.androidx.runtime.livedata)
    implementation(libs.androidx.lifecycle.viewmodel.compose)
    annotationProcessor(libs.androidx.room.room.compiler)
    ksp(libs.androidx.room.room.compiler)</b>

    ..........................................
}
</pre>
<p>Затем синхронизируем проект, нажав на кнопку <span class="b">Sync Now</span></p>
<img src="./pics/16.2.png" alt="Добавление зависимости Room в проект Jetpack Compose на Kotlin на Android" />
...
<style>
	...
	<div class="nav"><p><a href="./15.4.php">Назад</a><a href="./">Содержание</a><a href="./16.2.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
