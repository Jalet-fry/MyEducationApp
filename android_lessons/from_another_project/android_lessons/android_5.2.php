...
<html  lang="ru">
<head>
<title>Kotlin и Android | Введение в состояние компонентов</title>
<meta charset="utf-8" />
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Введение в состояние компонентов</h2><div class="date">Последнее обновление: 03.04.2024</div>
	<div class="socBlock">
	...
	<p>Jetpack Compose применяет декларативный подход для создания интерфейса, поэтому единственный способ обновить визуальный компонент представляет повторный вызов функции этого компонента, 
...
<p>Когда значение состояния изменяется, происходит рекомпозиция (<span class="b">recomposition</span>) или обновление пользовательского интерфейса. Перекомпоновка всего пользовательского интерфейса каждый раз при изменении значения состояния была бы крайне неэффективным подходом к рендерингу и 
обновлению пользовательского интерфейса. Compose позволяет избежать этих накладных расходов, используя метод, называемый интеллектуальной рекомпозицией, который включает в себя 
перекомпоновку только тех компонентов, на которые непосредственно влияет изменение состояния. Как только Compose обнаруживает изменение состояния, он перекомпоновывает любые компоненты, на которые повлияло изменение состояния. 
Перекомпоновка просто означает, что функция компонента вызывается снова, и ей передается новое значение состояния.</p>
<h3>MutableState</h3>
...
<code>MutableState&lt;String&gt;</code>. С помощью свойства <code>value</code> можно получить отслеживаемое значение: <code>message.value</code>. В данном случае 
это значение передается в компонент Text для отображения.</p>
<p>Однако просто определение состояния - это еще не все. B вполне возможно, что Android Studio даже не сможет скомпилировать выше приведенный код. Потому что нам нужен еще ожин элемент - функция <span class="b">remember</span>.  
Эта функция применяется для сохранения некоторого объекта в памяти. Она может хранить как 
...
храняться во время обновлений интерфейса (рекомпозиции).</p>

<p>Есть несколько способов применения функции <code>remember</code> в сочетании с функцией <code>mutableStateOf</code>. Самый распространенный:</p>
<pre class="brush:kt;">
...
<p>Здесь в модификаторе <code>clickable</code> параметру <code>onClick</code> передается обработчик нажатия, который изменяет отслеживаемое значение. 
То есть мы ожидаем, что по нажатию на текст он изменится с "Hello METANIT.COM" на "Hello Work!".</p>
<img src="./pics/5.2.png" alt="Функции mutableStateOf и remember в Kotlin и Jetpack Compose в Android" />
<p>Стоит отметить, что есть еще два способа использования функции <span class="b">remember</span>:</p>
<pre class="brush:kt;">
var value by remember { mutableStateOf(значение) }
</pre>
<p>В этом случае применятся делегат <span class="b">by</span>, для работы с которым необходимо импортировать следующие функции:</p>
<pre class="brush:kt;">
import androidx.compose.runtime.getValue
import androidx.compose.runtime.setValue
</pre>
<p>Применение второго варианта:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>И последний третий вариант выглядит следующим образом:</p>
<pre class="brush:kt;">
val (value, setValue) = remember { mutableStateOf(значение) }
</pre>
<p>В этом случае возвращается собственно отслеживаемое значение (value) и функция обработки изменения этого значения (setValue). Для примера применим этот вариант:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
также принимает объект String. И в данном случае мы передаем подобный объект: <code>setValue("Hello Word")</code>. А обаботчик сам установит данное значение.</p>

<h3>Сохранение состояния при изменении ориентации</h3>
<p>Выше мы рассмотрели сохранение состояния, его получение и изменение. Однако даже если мы изменим состояние, при изменении ориентации мобильного устройства (например, при переходе от портретного режима к 
алюбомному) наше состояние пропадет:</p>
<img src="./pics/5.3.png" alt="mutableStateOf и remember в Kotlin и Jetpack Compose в Android" />
<p>То же самое касается и других конфигурационныхт изменений, например, измненения системных настроек. Подобные изменения вызывают не просто рекомпозицию отдельных компонентов, а уничтожение и пересоздание всего класса 
MainActivity. В итоге вновь созданный объект MainActivity не понмнит никаких изменений состояния.</p>
<p>Для сохранения состояния между поворотами экрана и другими подобными системными изменениями конфигурации применяется обертка над функцией <code>remember</code> - функцию <span class="b">rememberSaveable</span>, которая сохраняет данные в объект 
<span class="b">Bundle</span>. Правда, не все объекты можно сохранить, а только данные примитивных типов, либо классов, которые реализуют интерфейс Parcelable. Например, изменим предыдущий пример применив функцию 
rememberSaveable:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<img src="./pics/5.7.png" alt="mutableStateOf и rrememberSaveable в Kotlin и Jetpack Compose в Android" />
...
<style>
	...
	<div class="nav"><p><a href="./2.14.php">Назад</a><a href="./">Содержание</a><a href="./5.3.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
