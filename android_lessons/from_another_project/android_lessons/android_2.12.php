...
<html  lang="ru">
<head>
<title>Kotlin и Android | LazyVerticalStaggeredGrid и LazyHorizontalStaggeredGrid</title>
<meta charset="utf-8" />
<meta name="description" content="Контейнеры LazyVerticalStaggeredGrid и LazyHorizontalStaggeredGrid и создание зубчатых гридов в Jetpack Compose на языке программирования Kotlin на Android, тип LazyStaggeredGridScope">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>LazyVerticalStaggeredGrid и LazyHorizontalStaggeredGrid</h2><div class="date">Последнее обновление: 09.04.2024</div>
	<div class="socBlock">
...
<p>В прошлой статье были рассмотрены гриды - компоненты <code>LazyVerticaldGrid</code> и <code>LazyHorizontalGrid</code>, которые позволяют табличные представления со строками и столбцами. 
...
<span class="b">LazyVerticalStaggeredGrid</span> и <span class="b">LazyHorizontalStaggeredGrid</span></p>
<h3>LazyVerticalStaggeredGrid</h3>
<p>Компонент <span class="b">LazyVerticalStaggeredGrid</span> представляет вертикальную сетку. Как и другие lazy-контейнеры, этот компонент компонует и размещает только элементы, которые 
видимые в данный момент на экране. Он имеет следующие параметры:</p>
<pre class="brush:kt;">
@Composable
...
<ul>
<li><p><code>columns</code>: представляет размер и количество столбцов в виде объекта StaggeredGridCells</p></li>
<li><p><code>modifier</code>: применяемые к компоненту модификаторы</p></li>
<li><p><code>state</code>: состояние в виде объекта <code>LazyStaggeredGridState</code>. Для его создания применяется функция <code>rememberLazyStaggeredGridState()</code></p></li>
<li><p><code>contentPadding</code>: отступы вокруг контента</p></li>
<li><p><code>reverseLayout</code>: указывает, будут ли элементы грида располагаться в обратном порядке</p></li>
<li><p><code>horizontalArrangement</code>: определяет расположение элементов по горизонтали</p></li>
<li><p><code>verticalItemSpacing</code>: вертикальные отступы между элементами</p></li>
<li><p><code>flingBehavior</code>: описывает поведение при таком типе прокрутки, когда пользователь быстро перетаскивает что-то и поднимает палец.</p></li>
<li><p><code>userScrollEnabled</code>: указывает, будет ли доступна прокрутка</p></li>
<li><p><code>content</code>: представляет функцию типа <code>LazyStaggeredGridScope.() -&gt; Unit</code>, которая задает вложенные компоненты. 
Для добавления элементов в грид можно использовать функции <code>LazyStaggeredGridScope.items()</code> (для списка объектов) или <code>LazyStaggeredGridScope.item()</code> (для одного элемента)</p></li>
</ul>
<p>Рассмотрим на примере. Возьмем следующее приложение:</p>
<pre class="brush:kt;">
...
<p>Для LazyVerticalStaggeredGrid устанавливаем три столбца:</p>
<pre class="brush:kt;">columns = StaggeredGridCells.Fixed(3)</pre>
<p>Расстояние от границ контейнера до содержимого будет составлять 8 пикселей:</p>
<pre class="brush:kt;">contentPadding = PaddingValues(8.dp)</pre>
<p>Расстояние между столбцами по горизонтали будет также составлять 8 пикселей:</p>
<pre class="brush:kt;">horizontalArrangement = Arrangement.spacedBy(8.dp)</pre>
<p>И также 8 пикселей будет составлять вертикальные расстояния между элементами в столбце:</p>
<p>verticalItemSpacing = 8.dp</p>
<p>Для создания вложенных элементов здесь применяется функция <code>LazyStaggeredGridScope.items()</code>. Ей передается количество создаваемых элементов - в данном случае 50. Причем каждый элемент представляет компонент Box:</p>
<pre class="brush:kt;">
items(50) {_ -&gt;
    Box(Modifier
        .fillMaxWidth()
...
<p>Для показательности высота и цвет компонентов Box устанавливается на основе случайных чисел. В итоге мы получим зубчатое расположение элементов наподобие следующего:</p>
<img src="./pics/2.49.png" alt="LazyVerticalStaggeredGrid в Jetpack Compose на Kotlin на Android" />
...
<h3>LazyHorizontalStaggeredGrid</h3>
<p>Компонент <span class="b">LazyHorizontalStaggeredGrid</span> представляет горизонтальную сетку. Как и другие lazy-контейнеры, этот компонент компонует и размещает только элементы, которые 
видимые в данный момент на экране. Он имеет следующие параметры:</p>
<pre class="brush:kt;">
@Composable
...
<ul>
<li><p><code>rows</code>: представляет размер и количество строк в виде объекта StaggeredGridCells</p></li>
<li><p><code>modifier</code>: применяемые к компоненту модификаторы</p></li>
<li><p><code>state</code>: состояние в виде объекта <code>LazyStaggeredGridState</code>. Для его создания применяется функция <code>rememberLazyStaggeredGridState()</code></p></li>
<li><p><code>contentPadding</code>: отступы вокруг контента</p></li>
<li><p><code>reverseLayout</code>: указывает, будут ли элементы грида располагаться в обратном порядке</p></li>
<li><p><code>verticalArrangement</code>: определяет расположение элементов по вертикали</p></li>
<li><p><code>horizontalItemSpacing</code>: горизонтальные отступы между элементами</p></li>
<li><p><code>flingBehavior</code>: описывает поведение при таком типе прокрутки, когда пользователь быстро перетаскивает что-то и поднимает палец.</p></li>
<li><p><code>userScrollEnabled</code>: указывает, будет ли доступна прокрутка</p></li>
<li><p><code>content</code>: представляет функцию типа <code>LazyStaggeredGridScope.() -&gt; Unit</code>, которая задает вложенные компоненты. 
Для добавления элементов в грид можно использовать функции <code>LazyStaggeredGridScope.items()</code> (для списка объектов) или <code>LazyStaggeredGridScope.item()</code> (для одного элемента)</p></li>
</ul>
<p>Изменим предыдущий пример, использовав LazyHorizontalStaggeredGrid:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Здесь все почти аналогично, только теперь создаем три строки.</p>
<pre class="brush:kt;">
LazyHorizontalStaggeredGrid(
    rows = StaggeredGridCells.Fixed(3),
</pre>
<p>Кроме того, у компонентов Box в качестве ширины устанавливаем случайное значение. В итоге мы получим интерфейс наподобие следующего:</p>
<img src="./pics/2.49.png" alt="LazyHorizontalStaggeredGrid в Jetpack Compose на Kotlin на Android" />
...
<style>
	...
	<div class="nav"><p><a href="./2.7.php">Назад</a><a href="./">Содержание</a><a href="./2.8.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
