...
<html  lang="ru">
<head>
<title>Kotlin и Android | Пример работы с SQLite и Room</title>
<meta charset="utf-8" />
<meta name="description" content="Пример работы с SQLite и Room в мобильном приложении на Jetpack Compose на языке программирования Kotlin в Android, DAO, сущности, репозитории, ViewModel, RoomDatabase">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Пример работы с SQLite и Room</h2><div class="date">Последнее обновление: 16.04.2024</div>
	<div class="socBlock">
...
<p>Рассмотрим примитивный практически пример работы с базой данных SQLite через библиотеку Room с использованием ViewModel. Финальный проект будет выглядеть следующим образом:</p>
<img src="./pics/16.3.png" alt="работа с SQLite через Room в приложении на Jetpack Compose и Kotlin в Android" />
<p>Пусть в файле <span class="b">User.kt</span> будет располагаться класс User - сущность, с которой мы будем работать:</p>
<pre class="brush:kt;">
...
<p>Класс User будет сопоставляться с таблицей "users".</p>
<p>В файле <span class="b">UserDao.kt</span> располагается интерфейс UserDao, который определяет методы для взаимодействия с базой данных:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Здесь определены три метода. Метод <code>getUsers()</code> будет выполнять SELECT-запрос и возвращает список всех объектов User из базы данных в виде объекта <code>LiveData&lt;List&lt;User&gt;&gt;</code>. 
Метод <code>addUser()</code> принимает добавляемый объект User и выполняет INSERT-запрос. И метод <code>deleteUser()</code> удаляет объект User из базы данных по id.</p>
<p>В файле <span class="b">UserRoomDatabase.kt</span> располагается класс базы данных Room:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>В данном случае UserRoomDatabase определяет объект-синглтон, поскольку в приложении база данных Room должна существовать в одном в единственном виде. В качестве имени базы данных устанавливается "usersdb".</p>
<p>В файле <span class="b">UserRepository.kt</span> расположен класс репозитория, через который приложение будет взаимодействовать с базой данных через объект UserDao:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Класс репозитория определяет ряд методов, которые обращаются к методам объекта UserDao, который передается через конструктор. Для этого используются корутины, чтобы избежать выполнения операций с базой данных в основном потоке.</p>
<p>Отдельно стоит сказать про свойство userList, которое хранит список всех объектов из БД. Для его получения вызывается метод <code>userDao.getUsers()</code>. Причем репозиторию необходимо 
...
<p>В файле <span class="b">UserViewModel.kt</span> расположен класс UserViewModel - модель представления, который будет выполнять роль посредника между репозиторием и графическим интерфейсом:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Класс ViewModel принимает экземпляр контекста приложения, который представлен классом Android Context и который используется в коде приложения для получения доступа к ресурсам приложения 
во время выполнения. Кроме того, в контексте приложения можно вызывать широкий спектр методов для сбора информации и внесения изменений в среду приложения. 
В нашем случае контекст приложения необходим при создании базы данных.</p>
<p>ViewModel определяет ряд переменных.</p>
<pre class="brush:kt;">
val userList: LiveData&lt;List&lt;User&gt;&gt;
private val repository: UserRepository
var userName by mutableStateOf("")
...
<p><code>userList</code> прелставляет список пользователей, полученный из базы данных. для взаимодействия с БД определяется переменная репозитория - <code>repository</code>. И для управления 
вводом новых данных для имени и возраста пользователя определяются 
две переменных состояния - userName и userAge.</p>
<p>Блок инициализатора создает базу данных, которая используется для создания объекта UserDao. Затем мы используем UserDao для инициализации репозитория и получения данных в userList:</p>
<pre class="brush:kt;">
init {
    val userDb = UserRoomDatabase.getInstance(application)
    val userDao = userDb.userDao()
...
<p>И также UserViewModel определяет ряд методов, которые будут вызываться при изменении ввода в текстовом поле или при нажатии на кнопку.</p>
<p>И наконец определим сам интерфейс в <span class="b">MainActivity.kt</span>:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Для создания модели представления UserViewModel ей необходимо передать ссылку на объект Application (контекст приложения). Стандартная функция <code>viewModel()</code>, которая обычно применяется 
для создания моделей представления, не позволяет этого сделать. Поэтому вместо использования <code>viewModel()</code> мы создаем свой собственный класс <code>ViewModelProvider.Factory</code>, 
предназначенный для передачи ссылки на объект Application и возврата объекта UserViewModel.</p>
<pre class="brush:kt;">
class UserViewModelFactory(val application: Application) :
    ViewModelProvider.Factory {
    override fun &lt;T : ViewModel&gt; create(modelClass: Class&lt;T&gt;): T {
...
<p>Кроме фабрики типа <code>ViewModelProvider.Factory</code> функция <span class="b">viewModel()</span> также требует ссылки на текущий объект <span class="b">ViewModelStoreOwner</span>. 
...
можно получить нужную модель представления:</p>
<pre class="brush:kt;">
setContent {
    val owner = LocalViewModelStoreOwner.current
...
<p>Здесь вначале получаем текущий объект <span class="b">ViewModelStoreOwner</span>, используя свойство <code>LocalViewModelStoreOwner.current</code>. Провереяем полученный объект на null и 
вызываем функцию <code>viewModel()</code>, в которую передаем объект <code>ViewModelStoreOwner</code> (<code>it</code>), идентифицирующую строку модели представления ("UserViewModel") 
и фабрику модели представления UserViewModelFactory (которой передается ссылка на контекст приложения).</p>
<p>Получив объект ViewModel, передаем его в компонент Main, который определяет интерфейс приложения:</p>
<pre class="brush:kt;">
@Composable
fun Main(vm: UserViewModel = viewModel()) {
    val userList by vm.userList.observeAsState(listOf())
...
<p>Компонент получает из модули представления список пользователей в переменную userList. Причем благодаря получению с помощью функции <code>vm.userList.observeAsState()</code> компонент будет отслеживать все изменения в 
списке, и если в список будет добавлены новые объекты или из него будут удалены ранее существовавшие, то компонент будет обновлен, чтобы отразить эти изменения.</p>
<p>Внутри компонента определен простейший интерфейс. Прежде всего это два текстовых поля для ввода данных для имени и возраста пользователя, при изменении которых срабатывают функции changeName и changeAge из 
UserViewModel. Также определена кнопка, при нажатии на которую вызывается функция addUser, которая добавляет нового пользователя.</p>
<p>И также вызывается кастомный компонент UserList, который выводит список пользователей:</p>
<pre class="brush:kt;">
@Composable
fun UserList(users:List&lt;User&gt;, delete:(Int)-&gt;Unit) {
    LazyColumn(Modifier.fillMaxWidth()) {
...
<p>Для вывода списка применяется LazyColumn, который для отображения заголовка использует кастомный компонент UserTitleRow, а для вывода данных каждого пользователя - UserRow.</p>
<pre class="brush:kt;">
@Composable
fun UserRow(user:User, delete:(Int)-&gt;Unit) {
    Row(Modifier .fillMaxWidth().padding(5.dp)) {
...
<p>Здесь выводятся все свойства объекта User, а последнbq компонент Text выстпает в качестве кнопки, по нажатию на которую срабаывает метод deleteUser в UserViewModel.</p>
<p>Запустим приложение и добавим объект в базу данных:</p>
<img src="./pics/16.4.png" alt="база данных SQLite и Room в приложении на Jetpack Compose и Kotlin в Android" />
<p>Причем данные автоматически отобразяться в списке объектов. Аналогичным образом можно добавить больше объектов или удалять их из базы данных:</p>
<img src="./pics/16.5.png" alt="операции с бозой данных SQLite в Room в приложении на Jetpack Compose и Kotlin в Android" />
...
<style>
	...
	<div class="nav"><p><a href="./16.2.php">Назад</a><a href="./">Содержание</a><a href="./17.1.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
