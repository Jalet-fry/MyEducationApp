...
<html  lang="ru">
<head>
<title>Kotlin и Android | Всплывающие сообщения и Snackbar</title>
<meta charset="utf-8" />
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Всплывающие сообщения и Snackbar</h2><div class="date">Последнее обновление: 01.04.2024</div>
	<div class="socBlock">
...
<li><p><code>action</code>: вложенный компонент (обычно текст), по нажатию на который компонент уведомляет систему, что необходимо выполнить некоторое действие.</p></li> 	
<li><p><code>dismissAction</code>: дополнительный компонент, по нажатию на который выполняется некоторое действие.</p></li> 	
<li><p><code>actionOnNewLine</code>: указывает, будет ли действие располагаться на новой строке.</p></li> 	
<li><p><code>shape</code>: объект <span class="b">Shape</span>, который задает форму компонента.</p></li>
<li><p><code>containerColor</code>: фоновый цвет</p></li>
<li><p><code>contentColor</code>: цвет содержимого.</p></li>
<li><p><code>actionContentColor</code>: цвет компонента, который представляет основное действие и устанавливается через параметр <code>action</code></p></li>
<li><p><code>dismissActionContentColor</code>: цвет компонента, который устанавливается через <code>dismissAction</code></p></li>
...
<ul>
<li><p>Свойство <code>visuals</code> позволяет установить общую информацию с помощью следующих свойств:</p>
<ul>
...
<li><p><code>withDismissAction</code>: указывает, надо ли отображать кнопку отмены. Если равно <code>true</code>, то Snackbar отображает кнопку отмены в виде крестика</p></li>
...
<p>Фактически все эти параметры аналогичны свойствам объекта <code>SnackbarDat.visuals</code>.</p>
...
<p>Здесь надо отметить несколько моментов. Во-первых, определяем оббъект состояния в виде <span class="b">SnackbarHostState</span>:</p>
<pre class="brush:kt;">val snackbarHostState = remember { SnackbarHostState() }</pre>
...
применяется другая встроенная функция - <span class="b">rememberCoroutineScope()</span>:</p>
<pre class="brush:kt;">val scope = rememberCoroutineScope()</pre>
<p>В Scaffold используем SnackbarHostState для установки параметра snakbarHost:</p>
<pre class="brush:kt;">snackbarHost = { SnackbarHost(snackbarHostState) }</pre>
...
<p>По нажатию на FloatingActionButton с помощью контекста корутины scope запускаем корутину и в ней 
...
<p>В самой функции showSnackbar увеличиваем значение в переменной <code>count</code>. Вызов данной функции приведт к отображению сообщения в приложении:</p>
...
<p>В данном случае текстовая метка в сообщении, которая приглашает к некоторому действию, имеет текст "Подтвердить" и отображается в правой части окна сообщения.</p>
...
Если нажатия не было, и сообщение само по себе пропало, либо если мы нажали на кнопку отмены (на крестик), то возвращается значение <code>SnackbarResult.Dismissed</code></p>
...
В данном случае для простоты при одном результате к переменной count добавляется единица, а при другом результате - отображаем сообщение об отмене действия.</p>

<img src="./pics/4.68.png" alt="Результат showSnackbar в Scaffold в Jetpack Compose и Kotlin в Android" />
...
<p>Далее в SnackbarHost определяется объект <code>Snackbar</code>, 
...
<p>В данном случае полностью переопределяем внутреннее содержимое и метку действия у Snackbar. При этом параметры из функции <code>showSnackbar()</code> могут игнорироваться. 
...
<style>
	...
	<div class="nav"><p><a href="./4.11.php">Назад</a><a href="./">Содержание</a><a href="./4.13.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
