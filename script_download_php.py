...
# Конфигурация
BASE_URL = "https://metanit.com/java/tutorial/"
OUTPUT_DIR = "java_lessons"
os.makedirs(OUTPUT_DIR, exist_ok=True)

# HTML оглавления (скопируй сюда содержимое страницы)
TOC_HTML = """
...
def parse_links_from_toc(html_content):
    """Извлекает все ссылки на уроки из оглавления"""
    soup = BeautifulSoup(html_content, 'html.parser')
    links = set()

    # Ищем в тегах <ol class="content"> — там все главы
    content_list = soup.find('ol', class_='content')
    if content_list:
        for a in content_list.find_all('a', href=True):
            href = a['href']
            if href.endswith('.php') and not href.startswith('http'):
                links.add(href)

    # Дополнительно проверяем древовидное меню (на всякий случай)
    for a in soup.find_all('a', href=True):
        href = a['href']
        if 'java/tutorial/' in href and href.endswith('.php'):
            filename = href.split('/')[-1]
            links.add(filename)

    return sorted(links)

def download_lesson(filename):
    """Скачивает одну страницу урока"""
    url = urljoin(BASE_URL, filename)
    output_path = os.path.join(OUTPUT_DIR, filename)

    if os.path.exists(output_path):
        print(f"⏭️  Пропускаю (уже есть): {filename}")
        return
...
def main():
    print("🔍 Парсим ссылки из оглавления...")
    lessons = parse_links_from_toc(TOC_HTML)
    print(f"📚 Найдено уроков: {len(lessons)}")

    print("📥 Скачиваю уроки...")
