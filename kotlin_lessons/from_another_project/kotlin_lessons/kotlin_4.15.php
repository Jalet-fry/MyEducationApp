...
<html  lang="ru">
<head>
<title>Kotlin | Companion-объекты</title>
<meta charset="utf-8" />
<meta name="description" content="Companion-объекты и определение статических свойств и методов в языке программирования Kotlin">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Companion-объекты</h2><div class="date">Последнее обновление: 28.03.2024</div>
	<div class="socBlock">
...
<p>Класс в языке Kotlin может содержать так называемые <span class="b">companion-объекты</span>. <span class="b">companion-объект</span> определяется внутри некоторого класса и 
...
То есть <span class="b">companion-объекты</span> определяют свойства и методы <span class="b">класса</span> в целом, а не объекта.</p> 
<p>Общий синтаксис опредедения companion-объекта:</p>
<pre class="brush:kt;">
class ClassName{
...
<p>Например, нам надо подсчитать, сколько было создано объектов определенного класса. Для этого определим следующую программу:</p>
<pre class="brush:kt;">
...
<p>Здесь в классе Person определен следующий companion-объект:</p>
<pre class="brush:kt;">
companion object {
    var counter = 0
}
</pre>
<p>Для его определения применяется ключевое слово <span class="b">companion</span>. Внутри этого объекта определено свойство counter - счетчик, который по умолчанию равен 0.</p>
<p>В теле класса Person мы можем напрямую обращаться к свойствам и методам companion-объекта. В данном случае при создании объекта в инициализаторе мы увеличиваем этот счетчик на 1:</p>
<pre class="brush:kt;">
init{
    counter++
}
</pre>
<p>То есть при создании каждого объекта счетчик увеличивается на единицу. И поскольку companion-объекты определяют свойства и методы, общие для <span class="b">всех</span> объектов класса, 
то этот счетчик будет один общий для <span class="b">всех</span> объектов класса Person. И для обращения к нему вне класса применяется имя класса, а не объекта:</p>
<pre class="brush:kt;">
println(Person.counter) // 0
Person("Tom")
println(Person.counter) // 1
...
<p>И здесь мы видим, что изначально счетчик равен 0. После создания одного объекта он равен 1, а после создания еще двух объектов - 3.</p>
<p>Если же мы попробуем обратиться к переменной counter через имя объекта, то мы получим ошибку:</p>
<pre class="brush:kt;">
val tom = Person("Tom")
println(tom.counter) // Ошибка
</pre>
<p>Теоретически мы, конечно, можем определить и для объектов одноименное свойство. Например:</p>
<pre class="brush:kt;">
class Person(val name: String){
...
<p>Здесь в классе Person определено свойство counter, которое по умолчанию равно 22:</p>
<pre class="brush:kt;">var counter = 22</pre>
<p>И для обращения к этому свойству мы можем использовать имя объекта:</p>
<pre class="brush:kt;">
val tom = Person("Tom")
println(tom.counter) // 22
</pre>
<p>В этом случае, если внутри класса мы хотим обратиться не к этому свойству, а к свойству companion-объекта, то мы также должны указывать имя класса:</p>
<pre class="brush:kt;">
init{
    Person.counter++
}
...
<p>Companion-объекты также могут определять методы, общие для всего класса и для обращения к которым также используется имя класса, а не объекта. Например, в примерах выше у нас 
определенно есть проблема с целостностью данных, так как мы можем из внешнего кода присвоить счетчику любое значение:</p>
<pre class="brush:kt;">Person.counter = -100500</pre>
<p>Естественно это нежелательная ситуация. Поэтому опосредуем доступ к свойству counter:</p>
<pre class="brush:kt;">
class Person(val name: String){
...
<p>Итак, переменная counter теперь приватная, она недоступна вне своего класса. Однако внутри класса мы по прежнему можем к ней обращаться.</p>
<p>А для того, чтобы внешний код также мог получить ее значение, в companion-объекте определен метод printCounter:</p>
<pre class="brush:kt;">fun printCounter() = println(counter)</pre>
<p>Для доступа к этому методу применяется имя класса:</p>
<pre class="brush:kt;">Person.printCounter()</pre>
<h3>Наследование</h3>
<p>Методы и свойства companion-объекта не наследуются, поэтому для обращения к ним применяется имя базового класса, в котором определен companion-объект:</p>
<pre class="brush:kt;">
...
<style>
	...
	<div class="nav"><p><a href="./4.14.php">Назад</a><a href="./">Содержание</a><a href="./6.1.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
