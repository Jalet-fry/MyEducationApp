...
<html  lang="ru">
<head>
<title>Java | Компактные файлы кода и метод main</title>
<meta charset="utf-8" />
<meta name="description" content="Компактные файлы кода и метод main в языке программирования Java, правила определения и выполнения метода main, неявно определяемые классы">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Компактные файлы кода и метод main</h1><div class="date">Последнее обновление: 23.05.2025</div>
	<div class="socBlock">
	...
	<p>С выходом JDK 21 появилась экспериментальная возможность - компактные файлы кода и метод main, которая позволяет упростить программу на Java. А в JDK 25 эта возможность перестает быть экспериментальной 
и стала полноценной для применения. Суть этой возможности заключается в упрощении файла программы и метода main. В частности, теперь можно писать код без объявления класса с сокращением определения кода <b>main</b>. 
<h3>Компактный метод main</h3>
<p>Прежде всего мы можем определить более компактный метод <span class="b">main</span>. Например, возьмем стандартную программу, которая размещена в файле <span class="b">Program.java</span></p>
...
<p>Таким образом, раньше для построения и запуска программы на Java требовалось:</p>
<ul>
<li><p>Объявить класс (<code>public class Program</code>).</p></li>
<li><p>Создать статический метод main с параметром-массивом строк (<code>public static void main (String args[])</code>).</p></li>
</ul>
<p>Теперь можно определить метод <code>main</code> без <code>public static</code> и параметра <code>String args[]</code>:</p>
<pre class="brush:java;">
class Program{
    void main() {
        System.out.println("Hello METANIT.COM");
...
<p>Версии JDK 21, 22, 23 и 24 поддерживали такое определение метода main в качестве экспериментального, а для компиляции программы надо 
было указыть дополнительные опции. Например, для компиляции с помощью JDK 22 применялась команда:</p>
<pre class="sh">javac --release 22 --enable-preview Program.java</pre>
<p>Здесь 22 указывает на версию JDK, которую соответственно можно заменить на 21, 23, 24. И для запуска программы - также надо было указать дополнительную опцию <code>--enable-preview</code>:</p>
<pre class="sh">java --enable-preview Program</pre>
<p>Начиная с версии JDK 25 никаких дополнительных опций ни при компиляции, ни при запуске прогарммы указывать не нужно:</p>
<pre class="sh">
...
<p>Рассмотрим общую механику вызова метода main. При запуске приложения лаунчер Java (программа запуска приложения) выбирает и вызывает основной метод запущенного класса:</p>
<ul>
<li><p>Если класс определяет или наследует метод <code>main</code> с параметром <code>String[]</code>, то лаунчер выбирает этот метод.</p></li>
<li><p>В противном случае, если класс определяет или наследует метод <code>main</code> без параметров, то лаунчер выбирает этот метод.</p></li>
<li><p>В противном случае лаунчер сообщает об ошибке и завершает работу.</p></li>
</ul>
<p>После выбора метода дальнейшие действия зависят от того, выбран статический или не статический метод <code>main</code>:</p>
<ul>
<li><p>Если выбранный метод - <span class="b">статический</span>, то лаунчер вызывает его.</p></li>
<li><p>Иначе класс должен иметь не закрытый (без модификатора <code>sealed</code>) конструктор без параметров. Лаунчер вызывает этот конструктор, 
а затем у созданного объекта класса вызывает выбранный метод <code>main</code>.</p>
<p>Если же класс не имеет конструктора без параметров, то лаунчер сообщает об ошибке и завершает работу.</p></li>
</ul>
<p>Для примера возьмем файл программы со следующим кодом:</p>
<pre class="brush:java;">
public class Program{
...
<p>Здесь два определения метода main, оба нестатические, но один из них принимает параметр <code>String[]</code>, поэтому именно этот метод и будет выбран для выполнения программы.</p>
<h3>Компактные файлы</h3>
<p>Если компилятор Java обнаружит исходный файл с полями и методами, которые не заключены в объявление класса, он будет считать, что исходный файл неявно объявляет класс, членами которого являются эти поля и методы. Такой исходный файл называется компактным исходным файлом.</p>
<p>Таким образом, раньше для построения и запуска программы на Java требовалось:</p>
<ul>
<li><p>Объявить класс (<code>public class Program</code>).</p></li>
<li><p>Создать статический метод main с параметром-массивом строк (<code>public static void main (String args[])</code>).</p></li>
</ul>
<p>Теперь в файле программы можно писать код без объявления класса:</p>
<pre class="brush:java;">
void main() {
    System.out.println("Hello METANIT.COM!");
}
...
<p>Для подобного файла неявно определяется класс верхнего уровня в неименованном пакете. То есть весь код файла фактически будет представлять этот класс, который:</p>
<ul>
<li><p>Расширяет java.lang.Object и не реализует никаких интерфейсов</p></li>
<li><p>Имеет конструктор по умолчанию без параметров и никаких других конструкторов</p></li>
<li><p>Имеет в качестве своих членов поля и методы в компактном исходном файле</p></li>
<li><p>Должен иметь запускаемый метод main; в противном случае выдается ошибка времени компиляции</p></li>
</ul>
<p>Поскольку поля и методы, объявленные в компактном исходном файле, интерпретируются как члены неявно объявленного класса, мы можем рядом с методом <code>main</code> определеть и использовать другие методы и поля, 
которые автоматически будут являться методами и полями класса. Например, определение и вызов метода:</p>
<pre class="brush:java;">
String sayHello(){ 
    return "Hello";
}
...
<p>Или определение и использование полей:</p>
<pre class="brush:java;">
...
<p>Компактный исходный файл объявляет класс неявно, поэтому у класса нет имени, которое можно использовать в исходном коде. 
Компилятор Java генерирует имя класса при компиляции компактного исходного файла, но это имя зависит от реализации и не должно использоваться ни в каком исходном коде — даже в исходном коде 
в самом компактном исходном файле. Мы можем ссылаться на текущий экземпляр класса через this, явно или, как выше, неявно, но мы не можем создать экземпляр класса с помощью оператора <span class="b">new</span>.</p>
<h3>Определение других типов в файле</h3>
<p>В том же файле мы можем определять и другие типы, например, классы:</p>
<pre class="brush:java;">
void main(String[] args) {  
...
<p>В данном случае это будет аналогично тому, что класс Person был определн внутри класса, который представляет текущий файл.</p>
<p>Если вложенность класса нас не устраивает, мы можем явным образом определить для метода <code>main</code> конкретный класс, а рядом с ним остальные типы:</p>
<pre class="brush:java;">
class Program{
    void main(String[] args) {  
...
<style>
	...
	<div class="nav"><p><a href="./3.3.php">Назад</a><a href="./">Содержание</a><a href="./3.22.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
