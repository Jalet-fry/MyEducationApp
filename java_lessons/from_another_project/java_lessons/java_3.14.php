...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Объекты как параметры методов</h1><div class="date">Последнее обновление: 16.09.2025</div>
	<div class="socBlock">
	...
	<p>В прошлой теме была рассмотрена передача параметров в метод. Объекты классов, как и данные примитивных типов, также могут передаваться в методы. Однако в данном случае есть одна особенность 
- при передаче объектов в качестве значения передается копия ссылки на область в памяти, где расположен этот объект.</p>
<p>Сначала рассмотрим более детально, что у нас происходит при передача параметров примитивных типов - числовых типов как <code>int</code>, <code>double</code> и т.д., типа <code>char</code> и <code>boolean</code>:</p>
...
<p>Здесь в методе main значение переменной num передается в метод <code>calc.twice()</code> параметру n:</p>
<pre class="brush:java;">
calc.twice(num);
</pre>
<p>В методе <code>calc.twice()</code> значение параметра n (которому передается значение переменной num) увеличивается в два раза:</p>
<pre class="brush:java;">
void twice(int n){
...
<p>И тут возникает главный вопрос: увеличится ли значение переменной num или нет после выполнения метода <code>calc.twice()</code> и почему? Посмотрим на консольный вывод программы:</p>
<pre class="sh">
n in twice: 44
num in main (after twice): 22
</pre>
<p>По результату программы мы видим, что значение параметра n в методе twice увеличилось в два раза (как и ожидалось), а значение переменной num после выполнения метода twice не изменилось. 
При вызове метода <code>calc.twice()</code> параметр n в этом методе получает 
<span class="b">копию значения</span> переменной num, то есть число 22:</p>
<img src="./pics/3.13.png" alt="Передача параметров по значению в Java" />
<p>Когда вызывается метод <code>calc.twice()</code>, в область памяти, которая предназначена для параметра n, копируется значение переменной num - то есть число 22. После этого параметр n и переменная num 
никак не связаны - у них разные участки памяти, и изменение значениия одного из них никак не сказывается на другом. Поэтому изменение параметра n никак не влияет на переменную num.</p>

<p>Особенностью переменных других типов является то, что они хранят ссылку на область в памяти, где расположен объект. Рассмотрим небольшой пример:</p>
<pre class="brush:java;">
class Program{
...
<p>Здесь метод twice класса Calculator теперь принимает объект типа Person, у которого есть целочисленное поле age. И в методе twice аналогичным образом это поле увеличивается в два раза.</p>
<pre class="brush:java;">
 void twice(Person p){
...
<p>В методе <code>main</code> определяем переменную bob типа Person и передаем ее значение в метод twice:</p>
<pre class="brush:java;">
Person bob = new Person();
.......................
calc.twice(bob);
</pre>
<p>Посмотрим на консольный вывод программы:</p>
<pre class="sh">
bob.age in main (before twice): 22
p.age in twice: 44
bob.age in main (after twice): 44
</pre>
<p>Здесь мы видим, что значение поля age у объекта, на который указывает переменная bob, изменилась. Почему? Так как в метод twice по сути передавается копия ссылки на область памяти, 
в которой находится объект Person, то переменная bob и параметр p метода twice будут указывать на один и тот же объект в памяти:</p>

<img src="./pics/3.14.png" alt="Передача объектов в метод в Java" />
<p>Поэтому 
после выполнения метода у объекта, адрес которого хранит переменная bob и который передается в метод, будет изменено значение age c 22 на 44.</p>
...
<p>В метод twice также передается копия ссылки на объект Person. Однако в самом методе мы изменяем не отдельные значения объекта, а пересоздаем объект 
...
<p>То есть после создания нового объекта Person параметр p и переменная bob в методе main будут хранить ссылки на разные объекты. 
Переменная bob, которая передавалась в метод, продолжит хранить ссылку на старый объект в памяти. Поэтому ее значение не меняется, что мы и увидим по консольному выводу:</p>
<pre class="sh">
bob.age in main (before twice): 22
p.age in twice: 44
bob.age in main (after twice): 22
</pre>
...
<style>
	...
	<div class="nav"><p><a href="./2.16.php">Назад</a><a href="./">Содержание</a><a href="./2.17.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
