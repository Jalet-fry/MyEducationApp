...
<html  lang="ru">
<head>
<title>Java | Конструкторы и инициализаторы</title>
<meta charset="utf-8" />
<meta name="description" content="Конструкторы и инициализаторы в приложении на языке программирования Java, конструктор по умолчанию, вызов цепочки конструкторов">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Конструкторы и инициализаторы</h1><div class="date">Последнее обновление: 22.09.2025</div>
	<div class="socBlock">
...
при создании нового объекта данного класса. Обычно конструкторы выполняют инициализацию объекта.</p>
<h3>Конструктор по умолчанию</h3>
<p>Если в классе не определено ни одного конструктора, то для этого класса автоматически создается конструктор без параметров. Например:</p>
...
использовать для создания объекта Person.</p>
...
а для типа String и классов - это значение <code>null</code> (то есть фактически отсутствие значения).</p>
...
определить в классе свои конструкторы. При этом если в классе определяются свои конструкторы, то этот класс <span class="b">лишается</span> конструктора по умолчанию.</p>
<p>На уровне кода конструктор представляет метод, который называется по имени класса, который может иметь параметры, но для него не надо определять возвращаемый тип. Например, определим в классе Person простейший конструктор:</p>
<pre class="brush:java;">
class Program{
...
<p>Итак, здесь в классе Person определен конструктор, который выводит на консоль некоторое сообщение и инициализирует поля класса:</p>
<pre class="brush:java;">
Person(){
...
<p>В данном случае выражение <code>Person()</code> как раз представляет вызов определенного в классе конструктора (это больше не автоматический конструктор по умолчанию, которого у класса теперь нет). Соответственно при его выполнении на консоли будет выводиться строка "Создание объекта Person"</p>
...
<p>Подобным образом мы можем определять и другие конструкторы в классе. Например:</p>
<pre class="brush:java;">
class Program{
...
<p>Из выше определенного кода несложно увидеть ряд преимуществ конструкторов по сравнению с версией программы без конструкторов:</p>
<ul>
<li><p><span class="b">Автоматическая инициализация</span>: конструкторы обеспечивают корректную инициализацию объектов при их создании, снижая вероятность ошибок из-за неинициализированных переменных.</p></li>
<li><p><span class="b">Краткость кода</span>: код инициализации сосредоточен в конструкторе, что делает основной код более лаконичным и удобочитаемым.</p></li>
<li><p><span class="b">Объектно-ориентированный дизайн</span>: конструкторы способствуют более объектно-ориентированному дизайну, инкапсулируя логику инициализации внутри самого класса.</p></li>
</ul>


<h3>Ключевое слово this</h3>
<p>Ключевое слово <span class="b">this</span> представляет ссылку на текущий экземпляр (объект) класса. Через это ключевое слово мы можем обращаться к переменным, 
...
<p>Кроме конструктора начальную инициализацию объекта вполне можно было проводить с помощью инициализатора объекта. Инициализатор выполняется <span class="b">до любого 
конструктора</span>. То есть в инициализатор мы можем поместить код, общий для всех конструкторов:</p>
<pre class="brush:java;">
class Program{
...
<h2>Порядок выполнения инициализации полей</h2>
<p>Таким образом, у нас есть три способа инициализации полей:</p>
<ul>
<li><p>Инициализация по умолчанию, когда мы явным образом не присваиваем полям никаких значений. В этом случае числовым полям присваивается число 0, булевым полям - значение <code>false</code>, а полям других классов 
- значение <code>null</code></p></li>
<li><p>Явная инициализация полей - полям при определении явным образом присваиваются некоторые значения</p></li>
<li><p>Инициализация в инициализаторе</p></li>
<li><p>Инициализация в конструкторе</p></li>
</ul>
<p>Теперь рассмотрим, в каком порядке все это происходит:</p>
<ol>
<li><p>Поля получают значения, которые им присваиваются при определении. Если при определении полям не присваивается никаких значений, то они получают значения по умолчанию</p></li>
<li><p>Поля получают значения, которые им присваиваются в инициализаторе</p></li>
<li><p>Поля получают значения, которые им присваиваются в конструкторе</p></li>
</ol>
...
<p>В данном случае в классе State есть поле value, которому присвоено некоторое начальное значение. Кроме того, в инициализаторе и конструкторе присваиваем этому полю новое значение, при этом выводя старое значение. 
В итоге мы получим следующий консольный вывод:</p>
<pre class="sh">
Initializator. Old value: Default
Constructor. Old value: Initializator
Final value: Constructor
</pre>
<p>Здесь мы види, что сначала полю value присваивается начальное значение, затем выполняется инициализатор, а затем конструктор.</p>
<p>Можно также отловить самое начальное значение:</p>
<pre class="brush:java;">
class Program{
...
<style>
	...
	<div class="nav"><p><a href="./3.20.php">Назад</a><a href="./">Содержание</a><a href="./3.4.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
