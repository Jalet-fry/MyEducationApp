...
<html  lang="ru">
<head>
<title>Java | Виртуальные потоки</title>
<meta charset="utf-8" />
<meta name="description" content="Виртуальные потоки в языке программирования Java, отличие от платформенных потоков, методы Thread.startVirtualThread и Thread.Builder.OfVirtual.ofVirtual">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Виртуальные потоки</h1><div class="date">Последнее обновление: 16.10.2025</div>
	<div class="socBlock">
...
<p>По умолчанию поток Java выполняется в потоке платформы, предоставляемом операционной системой, в которой выполняется виртуальная машина Java. 
Для многих рабочих нагрузок это приемлемое решение. Однако потоки платформы не являются легковесными. На создание потока платформы выделяется множество ресурсов и времени. 
Это ограничивает количество потоков платформы, которые может обработать процессор. А приложениях с интенсивной нагрузкой это становится узким местом, ограничивая производительность приложения. 
Так, нередко программа выполняет такие операции, которые могут занять продолжительное время, например, обращение к сетевым ресурсам, чтение-запись файлов, обращение к базе данных и т.д. 
Такие операции могут серьезно нагрузить приложение. Особенно это актуально в высоконагруженных веб-приложениях, которые должны быть готовы обслуживать тысячи запросов в секунду, и 
где продолжительные операции могут блокировать интерфейс пользователя и негативно 
повлиять на опыт пользователей. В случае с обычными потоками платформы при выполнении продолжительных операций поток просто бы блокировался на время выполнения операции. 
<span class="b">Виртуальные потоки</span> представляют решение этой проблемы.</p>


<p><span class="b">Виртуальные потоки</span>  — это легковесные потоки, управляемые виртуальной машиной Java (JVM), а не операционной системой. Виртуальным потокам обычно требуется мало ресурсов, и одна виртуальная машина Java может 
поддерживать миллионы виртуальных потоков. Виртуальные потоки подходят для выполнения задач, которые большую часть времени блокируются, например, операции ввода-вывода - обращение к файлам, к базе данных, операции отправки сетевых запросов.  Однако виртуальные потоки не предназначены для длительных операций, интенсивно использующих процессор.</p>

<p>
Как работает виртуальный поток? Виртуальный поток прикрепляется к потоку платформы и начинает выполнять некоторый код. То есть для выполнения виртуального потока все равно нужен платформенный поток, который в 
данном случае еще нызвается потоком-носителем (carrier), так как условно "несет" в себе виртуальный поток. Когда код виртуального потока вызывает блокирующую операцию (например, чтение файла), JVM не блокирует несущий платформенный поток, к которому прикреплен виртуальный. 
Вместо этого среда выполнения открепляет виртуальный поток с несущего потока платформы и сохраняет его состояние в куче, пока выполняется блокирующая операция  (например, чтение файла).
В это время несущий поток платформы освобождается и может выполнять другие виртуальные потоки, другие действия. Как только блокирующая операция завершается (например, чтение файла завершено), 
JVM снова прикрепляет виртуальный поток обратно на доступный поток платформы для продолжения выполнения. Этот механизм позволяет эффективно утилизировать системные ресурсы, так как платформенные потоки не простаивают в ожидании, а постоянно выполняют полезную работу.</p>


<p>В общем случае это выглядит так. Например, приходит запрос к веб-приложению, и для обработки запроса надо считать файл. Для обработки запроса запускается виртуальный поток, который прикрепляется к потоку платформы. 
Далее этот поток видит, что надо считать файл, соответственно запускается операция чтения файла, которая может занять долгое время. Виртуальный поток ждет, пока будет считан файл, и открепляется от 
потока платформы, к которому он был прикреплен и в рамках которого он выполнялся. Однако поток платформы не ждет завершения чтения файла, а переключается на выполнение другой работы (при ее наличие) - к нему может быть прикрпелен другой виртуальный поток. 
А когда файл будет считан, виртуальный поток возобновляет обработку и для этого снова прикрепляется к потоку платформы. Таким образом, приложение может более эффективно использовать ресурсы процессора, представленные 
потоками платформы.</p>

<p>Основные преимущества виртуальных потоков</p>
<ul>
<li><p><span class="b">Масштабируемость</span>: поскольку виртуальные потоки очень легковесны (занимают всего несколько сотен байт памяти по сравнению с мегабайтами для платформенных потоков), можно создавать миллионы таких потоков без существенной нагрузки на систему.</p></li>
<li><p><span class="b">Простота кода</span>: виртуальные потоки позволяют писать код в привычном стиле, используя тот же API</p></li>
<li><p><span class="b">Совместимость</span>: виртуальные потоки являются реализацией класса <code>java.lang.Thread</code>, что обеспечивает их совместимость с существующим кодом и библиотеками, которые 
работают с потоками.</p></li>
</ul>

<h3>Создание виртуального потока</h3>
<p>Виртуальные потоки имеют тот же API, что и потоки платформы. Для создания виртуальных потоков можно использовать ряд способов:</p>
<ul>
<li><p>С помощью статического метода <b>Thread.startVirtualThread(Runnable task)</b>, в который передается выполняемая задача <code>Runnable</code></p></li>
<li><p>С помощью статического метода <b>Thread.Builder.OfVirtual.ofVirtual()</b>, который создает строитель виртуального потока / фабрики потока</p></li>
</ul>
<p>Рассмотрим оба этих способа.</p>
<h4>Метод Thread.startVirtualThread</h4>
<p>Самый простой способ создать и запустить виртуальный поток представляет статический метод <b>Thread.startVirtualThread()</b>, в который передается 
объект <code>Runnable</code>:</p>
<pre class="brush:java;">public static Thread startVirtualThread(Runnable task)</pre>
...
<p>В данном случае в виртуальном потоке просто выводим сообщение на консоль. Однако вполне возможно при выполнении программы мы не увидим этого сообщения:</p>
<pre class="sh">
Main thread started...
Main thread finished...
</pre>
<p>По подобному консольному выводу не видно, чтобы виртуальный поток выполнялся. Все потому что, в отличие от платформенных потоков виртуальные потоки по умолчанию определяются как потоки-демоны, 
которые выполняются в фоновом режиме. Однако поскольку виртуальные потоки используют тот же API, что и обычные, мы можем вызвать метод <code>join()</code>, 
чтобы дождаться завершения виртуального потока:</p>
<pre class="brush:java;">
class Program{
...
<p>И теперь мы получим следующий консольный вывод:</p>
<pre class="sh">
Main thread started...
...
<h4>Метод ofVirtual()</h4>
<p>Метод <code>ofVirtual()</code> возвращает строитель виртуального потока, который позволяет выполнить более тонкую настройку:</p>
<pre class="brush:java;">public static Thread.Builder.OfVirtual ofVirtual()</pre>
<p>Сам строитель представляет объект типа <b>Thread.Builder.OfVirtual</b>, у которого мы можем вызвать ряд методов:</p>
<ul>
<li><p><code>Thread.Builder.OfVirtual name(String name)</code></p>
<p>Устанавливает имя для потока</p></li>
<li><p><code>Thread.Builder.OfVirtual name(String prefix, long start)</code></p>
<p>Также устанавливает имя для потока и плюс принимает значение счетчика, которое добавляется к имени потока</p></li>
<li><p><code>Thread start(Runnable task)</code></p>
<p>Запускает виртуальный поток, возвращая его в виде объекта Thread</p></li>
<li><p><code>Thread unstarted(Runnable task)</code></p>
<p>Создает новый поток, но без его запуска. Для планирования выполнения потока у объекта Thread необходимо явным образом вызвать метод <code>start()</code>.</p></li>
<li><p><code>ThreadFactory factory()</code></p>
<p>Возвращает фабрику создания потоков в виде объекта <code>ThreadFactory</code></p></li>
</ul>

<p>Используем этот метод для создания потока:</p>
<pre class="brush:java;">
class Program{
...
<p>Или можно сначала создать, а потом отдельно запустить:</p>
<pre class="brush:java;">
class Program{
...
<h4>Особенности свойств виртуального потока</h4>

<p>Для виртуальных потоков метод <code>isVirtual()</code> возвращает <code>true</code></p>
<p>Стоит отметить, что по умолчанию у виртуальных потоков нет имени потока. Метод <code>getName()</code> возвращает пустую строку, если имя потока не задано. 
Поэтому если необходимо разграничить несколько потоков на основе имени, следует для них устанавливать имя явным образом.</p>

<p>Виртуальные потоки являются потоками-демонами и, следовательно, не препятствуют началу последовательности завершения работы. Поэтому для них метод <code>isDaemon()</code> возвращает <code>true</code></p>
<p>Виртуальные потоки имеют фиксированный приоритет потока, который нельзя изменить. Посмотрим на эти свойства потока:</p>
<pre class="brush:java;">
class Program{
...
<h3>Когда стоит и не стоит использовать виртуальные потоки?</h3>
<p>Идеальные кандидаты для использования виртуальных потоков:</p>
<ul>
<li><p><span class="b">I/O-bound задачи</span> (работа с подсистемой ввода-вывода): приложения с большим количеством блокирующих операций ввода-вывода, такие как веб-серверы, микросервисы, приложения для работы с базами данных</p></li>
<li><p><span class="b">Высоконагруженные системы</span>: системы, которым необходимо обрабатывать тысячи или миллионы одновременных подключений</p></li>
</ul>
<p>Когда лучше воздержаться от виртуальных потоков:</p>
<ul>
<li><p><span class="b">CPU-bound задачи</span>: для задач, интенсивно использующих процессор (например, сложные математические вычисления, обработка больших объемов данных в памяти), преимущества виртуальных потоков не так заметны. В этом случае количество потоков должно быть соизмеримо с количеством ядер процессора, и классические платформенные потоки подойдут лучше</p></li>
<li><p><span class="b">Работа с synchronized-блоками</span>: длительное удержание блокировок в <b>synchronized</b>-блоках (рассматриваются в следующих статьях) может "прибить" (pin) виртуальный поток к его несущему платформенному потоку, что не позволит последнему выполнять другие задачи и может привести к снижению производительности, поэтому следует с осторожностью использовать синхронизацию с <b>synchronized</b></p></li>
<li><p><span class="b">Вызовы нативных методов (JNI)</span>: аналогично <code>synchronized</code>, вызовы нативного кода могут блокировать несущий поток</p></li>
</ul>
...
<style>
	...
	<div class="nav"><p><a href="./8.4.php">Назад</a><a href="./">Содержание</a><a href="./8.3.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
