...
<html  lang="ru">
<head>
<title>Java | Асинхронные задачи FutureTask. Callable и Future</title>
<meta charset="utf-8" />
<meta name="description" content="Возвращение результата из потока в языке программирования Java, асинхронная задача FutureTask, интерфейсы Callable и Future, отмена задачи">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Асинхронные задачи FutureTask. Callable и Future</h1><div class="date">Последнее обновление: 18.10.2025</div>
	<div class="socBlock">
...
<p>Метод <code>run()</code> интрефейcа <code>Runnable</code>, который выполняется потоком, не возвращает никакого результата и просто выполняет некоторые действия. Однако иногда в потоке необходимо выполнять действия, 
которые производят некоторый результат. И после завершения необходимо получить из потока этот результат и использовать его каким-то образом. Для этой задачи язык Java предоставляет интерфейсы <b>Callable</b> и  <b>Future</b>, которые определены в пакете  <b>java.util.concurrent</b>.</p>
<p><b>Callable</b> представляет некоторое действие, которое возвращает некоторый результат. Этот интерфейс имеет параметр типа и один метод - <b>call()</b>, который возвращает значение этого типа:</p>
...
<p>То есть, грубо говоря, мы запускаем в потоке объект этого интерфейса, а результат метода <code>call()</code> - это и будет то значение, которое мы получим из потока.</p>
<p>Интерфейс <b>Future</b> представляет результат асинхронной задачи. Используя объект <b>Future</b>, можно получить результат, например, в результате выполнения метода <code>call()</code> 
интерфейса <code>Callable</code>. Причем <b>Future</b> позволяет получить результат именно тогда, когда он будет готов.</p>
<p>Этот интерфейс предоставляет функционал для проверки завершения вычисления, ожидания его завершения и получения результата:</p>
<ul>
<li><p><code>boolean cancel(boolean mayInterruptIfRunning)</code></p><p>Пытается отменить выполнение задачи</p></li>
<li><p><code>default Throwable exceptionNow()</code></p><p>Возвращает исключение, сгенерированное задачей, без ожидания</p></li>
<li><p><code>V get()</code></p><p>При необходимости ожидает завершения задачи, а затем извлекает его результат</p></li>
<li><p><code>V get(long timeout, TimeUnit unit)</code></p><p>При необходимости ожидает завершения задачи не более заданного времени, а затем извлекает ее результат, если он доступен</p></li>
<li><p><code>boolean isCancelled()</code></p><p>Возвращает <code>true</code>, если задача была отменена до ее нормального завершения</p></li>
<li><p><code>boolean isDone()</code></p><p>Возвращает <code>true</code>, если задача завершена</p></li>
<li><p><code>default V resultNow()</code></p><p>Возвращает вычисленный результат без ожидания, неблокирующий вариант метода <code>get()</code></p></li>
<li><p><code>default Future.State state()</code></p><p>Возвращает состояние задачи. Представляет перечисление <code>Future.State</code> со следующими константами:</p>
<ul>
<li><p><code>CANCELLED</code>: задача отменена.</p></li>
<li><p><code>FAILED</code>: задача завершилась неудачно, сгенерировав исключение.</p></li>
<li><p><code>RUNNING</code>: задача выполняется и еще не завершена.</p></li>
<li><p><code>SUCCESS</code>: задача успешно завершена.</p></li>
</ul>
</li>
</ul>
<h3>FutureTask</h3>
<p>Один из способов запуска асинхронных задач в виде Callable и Future представляет класс <b>FutureTask</b>. Этот класс представляет асинхронную задачу, которую можно отменить. 
Этот класс предоставляет базовую встроенную реализацию интерфейса <code>Future</code> с методами для запуска и отмены вычисления, запроса на завершение вычисления и получения результата вычисления.</p>
<p>Кроме того, поскольку <b>FutureTask</b> реализует <code>Runnable</code>, то объекты <code>FutureTask</code> могут быть использованы для создания потоков.</p>
<p>Обычно <code>FutureTask</code> применяется для обертки объекта <code>Callable</code> или <code>Runnable</code> для запуска задачи, которая должна быть выполнена в будущем. Для этого класс предоставляет пару конструкторов:</p>
<ul>
<li><p><code>FutureTask(Runnable runnable, V result)</code></p>
<p>Создает FutureTask, который при запуске выполнит указанный Runnable и обеспечит возврат указанного результата методом get при успешном завершении.</p></li>
<li><p><code>FutureTask(Callable&lt;V&gt; callable)</code></p>
<p>Создает FutureTask, который при запуске выполнит указанный Callable.</p></li>
</ul>
<p>Функциональность класса <code>FutureTask</code> реализуется следующими методами:</p>
<ul>
<li><p><code>boolean cancel(boolean mayInterruptIfRunning)</code></p><p>Пытается отменить выполнение задачи</p></li>
<li><p><code>Throwable exceptionNow()</code></p><p>Возвращает исключение, сгенерированное задачей</p></li>
<li><p><code>V get()</code></p><p>При необходимости ожидает завершения задачи и возвращает результат задачи</p></li>
<li><p><code>V get(long timeout, TimeUnit unit)</code></p><p>При необходимости ожидает завершения задачи не более заданного времени, а затем извлекает его результат, если он доступен</p></li>
<li><p><code>boolean isCancelled()</code></p><p>Возвращает true, если задача была отменена до ее нормального завершения</p></li>
<li><p><code>boolean isDone()</code></p><p>Возвращает true, если задача завершена</p></li>
<li><p><code>V resultNow()</code></p><p>Возвращает результат задачи без ожидания ее завершения</p></li>
<li><p><code>void run()</code></p><p>Устанавливает используемый объект Future в качестве результата задачи, если только она не была отменена</p></li>
<li><p><code>Future.State state()</code></p><p>Возвращает состояние задачи</p></li>
</ul>
<p>Ключевым является метод <code>get()</code>, который блокирует выполнение, если задача еще не завершена. И после завершения возвращает ее результат. Посмотрим на примере:</p>
<pre class="brush:java;">
...
<p>Разберем эту программу. Прежде всего определяем задачу <code>Callable</code>, которая будет выполняться, в виде лямбда-выражения (мы можем так сделать, потому что Callable имеет только один метод):</p>
<pre class="brush:java;">
Callable&lt;Integer&gt; task = () -&gt; {
...
<p>Здесь простейший код выполнения факториала для числа number, которое определено во внешнем коде (лямбда-выражения могут обращаться в нешним переменным). В конце лямбда-выражение возвращает результат - значение переменной 
result.</p>
<p>Далее определяем задачу, которая будет выполнена в будущем - объект <b>FutureTask</b>, передавая в его конструктор выше определенную задачу Callable:</p>
<pre class="brush:java;">
var futureTask = new FutureTask&lt;Integer&gt;(task);
</pre>
<p>Поскольку задача возвращает значение типа <code>int</code>, преобразуемое в <code>Integer</code>, объект <code>FutureTask</code> (как и <code>Callable</code>) типизируется типом <code>Integer</code></p>
<p>Далее для выполнения задачи создаем и запускаем поток:</p>
<pre class="brush:java;">
var t = new Thread(futureTask); // futureTask - это объект Runnable
t.start();
</pre>
<p>Поскольку объект <code>FutureTask</code> реализует интерфейс <code>Runnable</code>, то мы можем передать его в конструктор класса Thread.</p>
<p>Пока дочерний поток t выполняет свою задачу - вычисляет факториала, основной поток также может выполнять некоторые действия. В данном случае для примера просто выводим сообщение на консоль:</p>
<pre class="brush:java;">
System.out.println("Main thread works...");
</pre>
<p>Затем для получения результата задачи у FutureTask вызываем метод <code>get()</code></p>
<pre class="brush:java;">
try{
    // ожидаем выполнения задачи
    Integer factorial = futureTask.get();
...
<p>Этот метод генерирует ряд исключений, поэтому оборачиваем его вызов в конструкцию <code>try..cath</code>. В итоге с помощью метода <code>get()</code> мы получим результат и выведем его на консоль.</p>
<p>Стоит отметить, что при вызове метода <code>get()</code></p>
<pre class="brush:java;">Integer factorial = futureTask.get();</pre>
<p>Главный поток - метод main приостанавливает выполнение и ждет получения результата (либо пока не произовдет ошибка)</p>


<h3>Генерация и обработка ошибки</h3>
<p>Но выше определенный способ вычисления факториала имеет большой недостаток - если переданное число будет меньше 1, то задача возвратит число 1, что будет неправильно: для вычисления факториала число должно быть больше 0. 
И мы можем предупредить эту ситуацию, сгенерировав ошибку. И стоит отметить, что при метод <code>call()</code> интерфейса <code>Callable</code> уже в своем определение предусматривает возможную генерацию ошибки:</p>
<pre class="brush:java;">V call() throws Exception;</pre>
<p>И также стоит отметить, что если FutureTask генерирует исключение, то оно имеет тип <b>ExecutionException</b>. Перехватывая исключения этого типа, мы можем понять, что исключение произошло в методе <code>call</code> 
при выполнении FutureTask.</p>
<p>Например, сгенерируем ошибку, если входное значение для факториала меньше 1:</p>
<pre class="brush:java;">
import java.util.concurrent.*;
...
<p>Здесь перед вычислением факториала проверяем число и при необходимости генерируем ошибку:</p>
<pre class="brush:java;">
if(number &lt; 1) throw new Exception("Number must be greater than 0");
</pre>
<p>Для обработки ошибки выполнения во FutureTask определяем специальный блок <code>catch</code>:</p>
<pre class="brush:java;">
catch(ExecutionException ex){
...
<p>И поскольку здесь для вычисления факториала передается заведомо некорректное число - -5, то мы столкнемся с ошибкой:</p>
<pre class="sh">
Main thread started...
Main thread works...
...
<p>Стоит отметить, что информацию о возникшем исключении мы можем получить и из объекта FutureTask через метод <b>exceptionNow()</b>. Например:</p>
<pre class="brush:java;">
import java.util.concurrent.*;
...
<p>Это может быть полезно, когда у нас несколько объектов FutureTask, и каждый из которых может сгенерировать исключения, которые надо как-то разграничить.</p>
<h3>Отмена задач</h3>
<p>Одним из достоинств интерфейса Future является то, что подобную задачу можно отменять. Для этого предназначен метод <b>cancel()</b>:</p>
<pre class="brush:java;">
boolean cancel(boolean mayInterruptIfRunning)
</pre>
<p>Если задача уже запущена, то параметр <code>mayInterruptIfRunning</code> определяет, прерывается ли поток задачи.
Возвращаемое значение: <code>false</code>, если задачу нельзя отменить (обычно потому, что она уже завершена), или <code>true</code>, если можно отменить. Но данный результат 
не обязательно указывает, отменена ли задача - для этой цели у Future используется метод <b>isCancelled()</b>.</p>
<p>Этот метод не действует, если задача уже завершена или отменена или не может быть отменена по какой-либо другой причине. В противном случае, если эта задача не запустилась при вызове отмены, 
эта задача никогда не должна запускаться.</p>
...
<p>Здесь для демонстрации в коде задачи Callable делаем небольшую задержку в 200 миллисекунд:</p>
<pre class="brush:java;">
while(n &gt; 0) {
...
<p>В основном потоке main останавливаем задачу, причем в прекращением работы потока, и проверяем ее статус остановки:</p>
<pre class="brush:java;">
// отменяем задачу
futureTask.cancel(true);
...
<p>Здесь мы видим, что после двух итераций цикла в задаче факториала задача была отменена и прекратила свое выполнение.</p>
...
<style>
	...
	<div class="nav"><p><a href="./8.12.php">Назад</a><a href="./">Содержание</a><a href="./8.14.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
