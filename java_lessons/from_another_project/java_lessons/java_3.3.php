...
<html  lang="ru">
<head>
<title>Java | Модификаторы доступа</title>
<meta charset="utf-8" />
<meta name="description" content="Модификаторы доступа в языке программирования Java, ключевые слова public, private, protected, уровень доступа по умолчанию">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Модификаторы доступа</h1><div class="date">Последнее обновление: 18.09.2025</div>
	<div class="socBlock">
	...
	<p>Все компоненты класса в языке Java - поля, методы и конструкторы имеют <span class="b">модификаторы доступа</span>. 
Модификаторы доступа управляют видимостью классов, методов и переменных, тем самым определяя, как к компонентам класса можно получить доступ из других частей программы. 
Они управляют тем, откуда можно получить доступ к конкретному классу, методу или переменной.</p>
<p>В Java используются следующие модификаторы доступа:</p>
<ul>
<li><p><span class="b">public</span>: публичный, общедоступный класс или компонент класса. Поля и методы, объявленные с модификатором public, 
видны другим классам из текущего пакета и из внешних пакетов, вообще в любом месте программы.</p>
<p>Стоит отметить, что в исходном файле может быть только один класс с модификатором <b>public</b>, но количество классов с другими модификаторами (или без них) может быть любым. 
И если в файле есть класс с модификатором <b>public</b>, то имя файла должно совпадать с именем этого класса.</p>
</li>
<li><p><span class="b">private</span>: закрытый класс или компонент класса, противоположность модификатору public. Закрытый класс или компонент класса доступен только из кода в том же классе.</p></li>
<li><p><span class="b">protected</span>: такой класс или компонент класса доступен из любого места в текущем классе или пакете или в производных классах, даже если они находятся в других пакетах</p></li>
...
<p>Вкратце модификаторы доступа можно суммировать в виде следующей таблицы:</p>
<img src="./pics/3.15.png" alt="Модификаторы доступа в языке Java" />
<p>Рассмотрим некоторые примеры применения модификаторов</p>

<h3>Модификаторы в рамках текущего пакета</h3>
...
<p>Так как класс State не имеет явного модификатора, поэтому он будет доступен из любого места данного пакета, однако не будет доступен из других пакетов.</p> 
<p>Класс State имеет четыре поля для каждого уровня доступа. А также определено 4 методов с разными модификаторами, 
...
<p>Теперь посмотрим, как мы сможем использовать переменные класса State в другом классе, который расположен <span class="b">в том же пакете</span>:</p>
<pre class="brush:java;">
class Program {
...
<p>Таким образом, в классе Program мы смогли только обратиться к переменным defaultVar, protectedVar и publicVar, 
...
<p>Здесь нам оказались доступны только три метода: printDefault, printProtected, printPublic, которые имееют соответственно модификатор по умолчанию и модификаторы protected и public.</p>
<h3>Модификаторы в рамках пакетов</h3>
<p>Теперь мосмотрим, что будет, если классы State и Program расположены в разных пакетах. Допустим, у нас есть проект со следующей структурой:</p>
<ul>
<li><p>Файл <span class="b">Program.java</span></p></li>
<li><p>Папка <span class="b">types</span></p>
<ul>
<li><p>Файл <span class="b">State.java</span></p></li>
</ul>
</li>
</ul>
<p>Пусть файл <span class="b">State.java</span> будет содержать определение аналогичного класса State:</p>
<pre class="brush:java;">
package types;
...
<p>Здесь только два отличие по сравнению с предыдущим определением класса State:</p>
<ul>
<li><p>Класс State принадлежит пакету "types" (поэтому и располагается в соответствующем каталоге)</p></li>
<li><p>Класс State определен с модификатором <code>public</code>, благодаря чему он виден из других пакетов</p></li>
</ul>

<p>Применим этот класс в классе Program:</p>
<pre class="brush:java;">
import types.State;
...
<p>И здесь мы видим, что мы можем обратиться только к тем полям класса, которые имеют модификатор <code>public</code>. То же самое касается методов, пример с которыми я не буду приводить, так как все аналогично.</p>
...
<style>
	...
	<div class="nav"><p><a href="./3.2.php">Назад</a><a href="./">Содержание</a><a href="./3.19.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
