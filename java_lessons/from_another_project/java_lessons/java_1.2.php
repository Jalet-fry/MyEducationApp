...
<html  lang="ru">
<head>
<title>Java | Первая программа в Windows</title>
<meta charset="utf-8" />
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Первая программа на Java</h2><div class="date">Последнее обновление: 20.03.2024</div>
	<div class="socBlock">
...
<p>И чтобы превратить код программы в исполняемое приложение необходим компилятор. Компилятор устанавливается вместе с JDK.</p>
...
<p>Теперь скомпилируем написанную программу. Откроем командную строку (в Windows) или терминал в Linux/MacOS и введем там соответствующие команды. Первым делом перейдем в каталог, где лежит наш файл с программой с помощью команды <span class="b">cd</span>:</p>
<pre class="sh">cd C:\Java</pre>
<p>В данном случае файл находится в каталоге C:\Java.</p>
<img src="./pics/1.20.png" alt="Первая программа на Java на Windows" />
<p>Затем cкомпилируем программу с помощью команды</p>
<pre class="sh">javac Program.java</pre>
<p>Компилятор языка Java представляет утилиту <b>javac</b>. Через пробел название нашего файла, который содержит класс программы.</p>
<p>После этого программа компилируется в байт-код, и в каталоге C:\Java можно будет найти новый файл <span class="и">Program.class</span>. 
Это и будет файл с байт-кодом программы. Теперь нам надо его запустить с помощью утилиты <b>java</b>:</p>
<pre class="sh">java Program</pre>
<p>Здесь уже расширение у файла не надо использовать.</p>
<p>Для ОС Windows весь процесс будет выглядеть следующим образом:</p>
<img src="./pics/1.1.png" alt="Первая программа на Java" style="width:600px;" />
<p>В примере выше весь процесс разбивался на два этапа:</p>
<ol>
<li><p>Компилятор javac создает из программы на языке Java файл с расширением <code>*.class</code>, который содержит байт-код для виртуальной машины Java или JVM</p></li>
<li><p>Утилита java запускает скомпилированный байт-код из файла с расширением <code>*.class</code>. То есть происходит выполнение программы</p></li>
</ol>
<p>Но в последних версиях Java также стало возможно сразу выполнить код программы на языке Java:</p>
<pre class="sh">C:\Java&gt;java Program.java
Hello METANIT.COM!

C:\Java&gt;
</pre>
<p>В этом случае утилите <b>java</b> передается файл с исходным кодом. При этом файла с расширением <code>*.class</code> не создается.</p>
...
<style>
	...
	<div class="nav"><p><a href="./1.6.php">Назад</a><a href="./">Содержание</a><a href="./1.5.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
