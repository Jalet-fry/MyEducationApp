...
<meta charset="utf-8" />
<meta name="description" content="Использование интерфейсов в языке программирования Java, их определение и реализация в классах, ключевое слово implements, методы по умолчанию и константы в интерфейсах">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Интерфейсы</h1><div class="date">Последнее обновление: 30.09.2025</div>
	<div class="socBlock">
...
<p>Но суть интерфейса не только и не столько в этом. Ключевая мысли интерфейса состоит в том, чтобы определить некий набор требований, которым должен соответствовать класс. 
Интерфейс определяет, <span class="b">что</span> должен делать класс, а класс уже определяет, <span class="b">как</span> он это делает.</p>
...
<p>Данный интерфейс называется Movable и далее будет представлять некое траспортное средство, нечто, что может перещаться. Согласно условностям названия интерфейсов в Java начинаются с большой буквы и обычно заканчиваются на суффикс "-able", хотя в принципе это необязательно.</p>
<h3>Применение интерфейса</h3>
<p>Как было сказано выше, интерфейс определяет набор требований к классу, а класс применяет эти требования. Чтобы указать, что класс <span class="b">применяет</span> или <span class="b">реализует</span> интерфейс, 
после названия класса указывается ключевое слово <span class="b">implements</span>, после которого идет название интерфейса:</p>
<pre class="brush:java;">
interface Movable{ }
class Car implements Movable{ }
</pre>
<p>То есть в данном случае мы можем сказать, что класс Car применяет или реализует интерфейс Movable. Грубо говоря, мы говорим, что машина (Car) - это транспортное средство, нечто, что функционал для 
перемещения.</p>
<h3>Переменные интерфейса</h3>
<p>Интерфейс фактически представляет новый тип. И мы можем определять переменные интерфейса:</p>
<pre class="brush:java;">
...
<p>Однако все же интерфейсы - это не классы. В частности, мы не можем использовать оператор <code>new</code> для создания экземпляра интерфейса:</p>
<pre class="brush:java;">
public class Program{
...
<p>Но переменной интерфейса можно присвоить ссылку на объект класса, который применяет данный интерфейс. Например:</p>
<pre class="brush:java;">
public class Program{
...
<p>Поскольку класс Car применяет интерфейс Movable, то переменной этого интерфейса мы можем присвоить объект Car.</p>

<h2>Определение компонентов интерфейса</h2>
<p>Выше определенный интерфейс Movable был пустой и ничего не определял. Но интерфейс может определять ряд компонентов:</p>
<ul>
<li><p>Методы без реализации</p></li>
<li><p>Методы по умолчанию (с реализацией)</p></li>
<li><p>Приватные методы</p></li>
<li><p>Статические методы (как приватные, так и публичные)</p></li>
<li><p>Константы</p></li>
</ul>
<h3>Определение методов в интерфейсе</h3>
<p>Как говорилось выше, интерфейс определяет ряд требований или контракт, которым должен соответствовать класс. И эти требования определяются в интерфейсе в виде методов. Методы интерфейса могут иметь, а могут и не иметь реализации. 
Если некоторый класс применяет определенный интерфейс, то этот класс должен 
реализовать все методы применяемого интерфейса (исключение касается методов, для которых в интерфейсе определена реализация по умолчанию).</p> 
<p>Например, изменим интерфейс Movable, добавив в него метод:</p>
<pre class="brush:java;">
interface Movable{
...
<p>В данном случае интерфейс Movable представляет функционал транспортного средства и определяет один метод без реализации - метод <code>move()</code>, который условно предназначен 
для передвижения транспортного средства. Методы без реализации похожи на абстрактные методы абстрактных классов. Все методы интерфейса не имеют модификаторов доступа, но фактически по умолчанию доступ <span class="b">public</span>, так как цель 
...
<p>Таким образом, у нас еть интерфейс Movable, которое представляет непонятно какое транспортное средство, и есть функция move, которая предназначена для перемещения транспортного средства, но как именно это перемещение осуществляется - неизвестно.</p>
<p>Однако класс, который будет применять интерфейс, <span class="b">должен</span> реализовать все его методы (которые не имеют реализации). Теперь реализуем интерфейс Movable:</p>
<pre class="brush:java;">
public class Program{
...
<p>В данном случае класс Car применяет интерфейс Movable и поэтому должен реализовать его метод <code>move()</code>. Если класс не реализует какие-то методы интерфейса, то такой класс должен быть 
определен как <span class="b">абстрактный</span>, но его неабстрактные классы-наследники затем все равно должны будут реализовать эти методы.</p>
<p>Таким образом, интерфейс Movable определяет требования - реализовать метод перемещения (метод move). Класс Car применяет эти требования, но сам решает, как конкретно реализовать метод перемещения.</p>

<p>Подобным образом мы можем реализовать интерфейс в серии классов. Например, добавим класс, который будет представлять самолет:</p>
<pre class="brush:java;">
public class Program{
...
<p>Интерфейсы в языке Java могут иметь <span class="b">методы по умолчанию</span> (default methods) - методы с реализацией по умолчанию, которая используется, если класс, реализующий данный 
интерфейс, не реализует метод.</p>
<p>Зачем нужны методы по умолчанию? Допустим, ранее был создан интерфейс, который затем реализовало множество классов. Если мы захотим внести изменения в этот интерфейс в силу каких-то новых условий, 
то нам также придется изменять все те методы, которые его применяют. Когда речь идет о паре классов, которые определены в одном файле с интерфейсов, проблема может показаться надуманной. Но когда 
Но когда было определено множество классов множеством разработчиков по всему миру в течение многих лет, как в случае с интерфейсами стандартной библиотеки Java, 
проблем становится более актуальной и трудно решаемой. Поскольку изменение интерфейса приведет к тому, что классы больше не будут компилироваться, поскольку не реализуют новые методы.</p>
<p>При наличии же метода по умолчанию класс все равно будет загружаться, даже с отсутствующим методом. Программы по-прежнему могут создавать экземпляры классов, и ничего страшного не произойдет.</p>


<p>Например, ранее использованный интерфейс Movable и добавим в него новый метод, который будет методом по умолчанию:</p>
<pre class="brush:java;">
interface Movable{
...
<p>В данном случае метод <code>stop()</code> определен как метод по умолчанию. Метод по умолчанию - это обычный метод без модификаторов (формально имеет модификатор <code>public</code>), который помечается ключевым словом <span class="b">default</span>. В данном 
случае метод stop выводит строку на консоль. В принципе можно было вообще ничего не делать и оставить пустые фигурные скобки (<code>default void stop() { }</code>)</p>
<p>Затем в классе нам необязательно этот метод реализовать:</p>
<pre class="brush:java;">
public class Program{
...
<p>При желании также можно переопределить этот метод:</p>
<pre class="brush:java;">
public class Program{
...
<p>В интерфейсах также можно определять статические методы - они аналогичны методам класса:</p>
<pre class="brush:java;">
interface Printable {
...
<style>
	...
	<div class="nav"><p><a href="./3.10.php">Назад</a><a href="./">Содержание</a><a href="./3.28.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
