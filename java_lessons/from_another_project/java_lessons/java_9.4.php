...
<html  lang="ru">
<head>
<title>Java | Функциональные интерфейсы и ссылки на методы</title>
<meta charset="utf-8" />
<meta name="description" content="Функциональные интерфейсы и ссылки на методы в языке программирования Java, формат определения ссылок на статические и нестатические методы, методы как параметры и результаты других методов">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Функциональные интерфейсы и ссылки на методы</h1><div class="date">Последнее обновление: 02.10.2025</div>
	<div class="socBlock">
...
<h3>Функциональный интерфейс</h3>
<p>В языке Java есть особый тип интерфейсов, который называется <span class="b">функциональным интерфейсом</span>. 
<span class="b">Функциональный интерфейс</span> - это интерфейс, в котором определен только один абстрактный метод (метод без реализации). Простейший пример:</p>
<pre class="brush:java;">
interface Message{
    void print();
}
</pre>
<p>Данный интерфейс Message можно назвать функциональным.</p>
<p>И как и в случае с любым интерфейсом, мы можем определить переменную этого интерфейса:</p>
<pre class="brush:java;">
Message mes;
</pre>

<h3>Ссылки на метод</h3>
<p>Хотя функциональный интерфейс, на первый взгляд, кажется обычным интерфейсом, но у него есть особенность в том, что он может представлять <span class="b">ссылки на методы</span>, то есть на некоторое действие.</p>
<p><span class="b">Ссылка на метод</span> (method reference) представляет синтаксическую конструкцию следующего вида:</p>
<pre class="brush:java;">
...
<p>Здесь переменной mes передаем ссылку на метод <code>hello</code>, который определен в классе Program как статический. Поэтому ссылка на метод выглядит следующим образом:</p>
<pre class="brush:java;">Program::hello</pre>
<p>И тут также важно, что ссылка на метод из ссылки (hello) должен соответствовать единственному методу функционального интерфейса по типами параметров и типу результат. Так, в нашем случае метод 
<code>print</code> в интерфейсе Message имеет тип <code>void</code> и не принимает никаких параметров. Соответственно и статический метод <code>hello</code>, ссылка на который присваивается переменной интерфейса, 
также не принимает параметров и имеет тип <code>void</code></p>
<p>Далее мы можем вызвать метод по ссылке:</p>
<pre class="brush:java;">
public class Program {
...
<p>То есть при вызове</p>
<pre class="brush:java;">mes.print();</pre>
<p>фактически вызывается метод по ссылке - метод <code>hello()</code></p>
<p>Ссылка на метод предписывает компилятору создать экземпляр функционального интерфейса, переопределяя единственный метод интерфейса для его вызова. То есть в данном случае мы получаем некоторый объект интерфейса Message, 
где метод <code>hello()</code> выступает в качестве реализации метода <code>print()</code> из интерфейса.</p>
...
<p>В данном случае метод <code>hello()</code> определен как нестатический. Поэтому для формирования ссылки на него создаем объект класса Program и передаем ссылку переменной mes:</p>
<pre class="brush:java;">
Program prog = new Program();
Message mes = prog::hello;
</pre>
<h4>Пример метода с параметрами</h4>
<p>Рассмотрим другой пример, когда переменная ссылается на метод с параметрами и некоторым возвращаемым результатом:</p>
<pre class="brush:java;">
public class Program {
...
<p>Здесь мы определяем функциональный интерфейс Operation с методом <code>execute()</code></p>
<pre class="brush:java;">
interface Operation{
    int execute(int x, int y);
}
</pre>
<p>В программе в методе <code>main()</code> создаем переменную <code>op</code>, которая представляет данный интерфейс, и присваиваем ей ссылку на статический метод <code>Operations.add</code>:</p>
<pre class="brush:java;">
Operation op = Operations::add;
</pre>
<p>Мы можем так сделать, поскольку типы параметров и возвращаемый тип метода <code>Operations.add</code> соответствуют типам параметров и результата абстрактного метода <code>execute()</code> из интерфейса Operation.</p>
<p>Затем через переменную <code>op</code> вызываем метод по ссылке:</p>
<pre class="brush:java;">
System.out.println(op.execute(5, 4));
</pre>
<p>Далее присваиваем переменной ссылку на другой метод и повторно вызываем и соответственно получаем другой результат при тех же аргументах:</p>
<pre class="brush:java;">
op = Operations::sub;
System.out.println(op.execute(5, 4));       // 1
</pre>
...
<p>Возможность определять ссылки на методы открываем нам дополнительные возможности при написании программ. Так, мы можем определить параметр, который представляет некоторое действие, и передать ему ссылку на метод. Рассмотрим на примере:</p>
<pre class="brush:java;">
public class Program {
...
<p>Здесь возможные вычисления вынесены в отдельный класс Operations и представляют статические методы. Поэтому для передачи этих методов применяются выражения типа <code>Operations::add</code></p>
<pre class="brush:java;">doOperation(10, 4, Operations::add);  // 14</pre>
...
<h3>Ссылки на метод как результаты методов</h3>
<p>Кроме того, мы можем возвращать ссылки на методы из других методов. Например:</p>
<pre class="brush:java;">
...
<p>Здесь метод <code>select()</code> в зависимости от значения параметра возвращает ссылку на один из методов: sum, subtract или multiply.</p>
<pre class="brush:java;">
static Operation select(int choice){
...
<p>В методе <code>main()</code> получаем результат метода <code>select()</code> в переменную:</p>
<pre class="brush:java;">Operation action = select(1);</pre>
<p>То есть в данном случае переменная <code>action</code> фактически будет представлять ссылку <code>Program::sum</code></p>
...
<p>Аналогично выражение <code>Person[]::new</code> представляет создание массива объектов Person.</p>
...
<style>
	...
	<div class="nav"><p><a href="./3.18.php">Назад</a><a href="./">Содержание</a><a href="./9.1.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
