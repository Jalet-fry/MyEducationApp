...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Завершение и прерывание потока</h1><div class="date">Последнее обновление: 16.10.2025</div>
	<div class="socBlock">
	...
	<p>Примеры потоков в прошлых статьях представляли поток как последовательный набор операций. То есть поток по сути представлял последовательность инструкций, которые последовательно выполнялись одна за другой. 
А после выполнения последней операции завершался и поток. Однако нередко имеет место и другая организация потока в виде бесконечного цикла. Например, поток сервера в бесконечном 
...
<p>Переменная <code>isActive</code> указывает на активность потока: пока эта переменная равна <code>true</code>, поток будет продолжать работать. Но с помощью метода <code>disable()</code> 
мы можем сбросить состояние этой переменной в <code>false</code> и таким образом завершить поток.</p>
...
<p>Итак, вначале запускается дочерний поток:</p>
<pre class="brush:java;">
var myTask = new MyTask();
new Thread(myTask,"MyTask").start();
</pre>
<p>Затем на 1100 миллисекунд останавливаем главный поток "Main thread" и 
потом вызываем метод <code>disable()</code>, который переключает в потоке флаг <code>isActive</code>, и дочерний поток завершается.</p>
<pre class="brush:java;">myTask.disable();  // завершаем поток</pre>
...
<h3>Прерывание потока и метод interrupt</h3>
<p>В языке Java нельзя принудительно завершить поток. Однако с помощью метода <b>interrupt()</b> можно направить запроса на прерывание потока. 
При вызове метода <code>interrupt()</code> для потока устанавливается статус <b>INTERRUPTED</b>.</p>
<p>При выполнении работы с помощью метода <b>isInterrupted()</b> поток может периодически проверять, был ли он прерван, и, если прерван, завершать выполняемую работу:</p>
<pre class="brush:java;">
...
<p>Однако, если поток заблокирован, он не может проверить статус с помощью вызова <code>isInterrupted()</code>. Такая ситуация, например, возникает, когда метод <code>interrupt()</code> вызывается для потока, который блокируется вызовом <code>sleep()</code> (а также ряд других, например, методом <code>wait()</code>). 
В этом случае блокирующий вызов (например, <code>sleep()</code>) завершается исключением <code>InterruptedException</code>. 
Существуют блокирующие вызовы ввода-вывода, которые не могут быть прерваны. Тогда при завершении операции ввода-вывода также генерируется исключение <code>InterruptedException</code>. И в этом случае мы можем 
в коде потока обрабатывать исключение:</p>
<pre class="brush:java;">
...
<p>Хотя, если поток прерван, он не обязан завершаться - он может продолжать работать. Но гораздо чаще поток просто интерпретирует прерывание как запрос на завершение. Например, проэмулируем данную ситуацию:</p>
<pre class="brush:java;">
class MyTask implements Runnable {
...
<p>В методе <code>run()</code> класса MyTask в цикле <code>while</code> проверяем результат метода <code>isInterupted()</code>:</p>
<pre class="brush:java;">
try{
...
<p>В цикле делаем задержку на 1 секунду с помощью <code>Thread.sleep()</code>. Однако если во время этой задержки во вне будет вызван метод <code>interrupt()</code> для этого потока, то мы столкнемся с 
исключением <code>InterruptedException</code>. И с помощью конструкции <code>try..catch</code> перехватываем и обрабатываем это исключение.</p>
<p>В методе main запускаем поток MyTask, приостанавливаем главный поток, чтобы MyTask успел немного отработать, и далее вызыаем у MyTask метод <code>interrupt()</code>:</p>
<pre class="brush:java;">
var t = new Thread(new MyTask(), "MyTask");
t.start();
...
<p>В итоге при вызове метода <code>interrupt()</code> во время приостановки MyTask обработает исключение <code>InterruptedException</code> и завершит работу:</p>
<pre class="sh">
Main thread started...
MyTask started...
MyTask works
...
<style>
	...
	<div class="nav"><p><a href="./8.11.php">Назад</a><a href="./">Содержание</a><a href="./8.12.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
