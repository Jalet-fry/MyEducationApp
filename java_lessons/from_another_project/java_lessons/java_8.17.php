...
<html  lang="ru">
<head>
<title>Java | Атомарность и Atomics. Потокобезопасность без блокировок</title>
<meta charset="utf-8" />
<meta name="description" content="Атомарность и Atomics в многопточных приложениях на языке программирования Java, потокобезопасность без блокировок, классы AtomicLong и LongAdder">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Атомарность и Atomics. Потокобезопасность без блокировок</h1><div class="date">Последнее обновление: 23.10.2025</div>
	<div class="socBlock">
...
<p>В многопоточном программировании, в том числе и на языке Java, одной из самых частых проблем является "состояние гонки" (race condition), когда несколько потоков пытаются одновременно читать и изменять одни и те же данные. 
Рассмотрим классический пример - простой счетчик:</p>
...
<p>Итак, здесь у нас есть класс Counter - условного счетчика, где есть переменная <code>counter</code></p>
<pre class="brush:java;">private volatile long counter = 0;</pre>
<p>Чтобы все потоки вдели изменения этой переменной, она определена с ключевым словом <code>volatile</code>. А в методе <code>increment()</code> увеличиваем на 1 значение этой переменной:</p>
<pre class="brush:java;">void increment() { counter++;  }</pre>
<p>В методе <code>main()</code> используем этот класс, создая его объект:</p>
<pre class="brush:java;">Counter counter = new Counter();</pre>
<p>Далее создаем массив из 1000 потоков, которые 1000 раз выполняют метод <code>increment()</code> у объекта Counter:</p>
<pre class="brush:java;">
Thread[] threads = new Thread[numThreads];
...
<p>После завершения потоков выводим финальное значение счетчика:</p>
<pre class="brush:java;">System.out.println("Counter: " + counter.getCounter());</pre>
<p>Если мы запускаем 1000 потоков, каждый из которых будет 1000 раз инкрементировать счетчик на единицу, то было бы разумно ожидать, что финальное значение счетчика будет равно 1000 * 1000 = 1000 000 (то есть 1 миллион). 
Но посмотрим, какой на самом деле будет результат. Он может быть таким:</p>
<pre class="sh">
Counter: 944695
</pre>
<p>А может быть и таким</p>
<pre class="sh">
Counter: 870973
</pre>
<p>А может быть и совсем другим. Результат в данном случае недетерминарован, но он может (и скорее всего будет) отличаться от ожидаемых 1000 000. Почему так происходит? Дело в том, что операция инкремента, 
которую выполняет в примере выше 
метод <code>increment()</code>, на самом деле она состоит из трех шагов:</p>

<ol>
<li><p><span class="b">Чтение</span> текущего значения поля counter</p></li>
<li><p><span class="b">Увеличение</span> прочитанного значения на 1</p></li>
<li><p><span class="b">Запись</span> нового значения обратно в counter</p></li>
</ol>
<p>И если два и более потоков выполняют инкремент одновременно, то они могут считать одно и то же значение.</p>
<p>Для решения этой проблемы мы могли бы использовать блоки/функции с оператором <code>synchronized</code> или использоваnm блокировокb <code>ReentrantLock</code>. Однако оба этих способа представляют 
<span class="b">блокирующий</span> механизм. И если поток А держит блокировку, поток Б (и В, Г, Д...) вынуждены остановиться и ждать. В системах с высокой нагрузкой 
это приводит к простоям потоков, частым переключениям контекста и резкому падению производительности.</p>
...
<h3>Атомарность и пакет java.util.concurrent.atomic</h3>
<p>Для решения этой задачи язык Java  предоставляет более эффективный инструмент - классы из пакета <span class="b">java.util.concurrent.atomic</span>, такие как 
<b>AtomicInteger</b>, <b>AtomicBoolean</b>, <b>AtomicReference</b>, <b>AtomicLong</b> и т.д. Эти классы используют эффективные машинные инструкции для гарантии атомарности других операций без 
использования блокировок. Предоставляемые этими классами операции над одиночными переменными <span class="b">неблокирующие</span> и <span class="b">потокобезопасные</span> (thread-safe)</p>
<p>Рассмотрим применение подобных классов на примере <b>AtomicLong</b>, который представляет обертку над значением типа <code>long</code> с атомарными методами для его изменения. Для создания объекта применяется один из конструкторов класса:</p>
<pre class="brush:java;">
AtomicLong()
AtomicLong(long initialValue)
</pre>
<p>Первая версия инициализирует объект числом 0. Второй конструктор принимает начальное значение для <code>AtomicLong</code>.</p>
<p>Ключевые методы <code>AtomicLong</code>:</p>
<ul>
<li><p><code>long get()</code>: возвращает текущее значение (аналогично чтению <code>volatile long</code>)</p></li>
<li><p><code>void set(long newValue)</code>: устанавливает новое значение (аналогично записи в <code>volatile long</code>)</p></li>
</ul>
<p>И также класс предоставляет ряд атомарных операций, которые собственно и представляют суть класса:</p>
<ul>
<li><p><code>long incrementAndGet()</code>: атомарно увеличивает на 1 и возвращает новое значение (префиксный инкремент, <code>++i</code>)</p></li>
<li><p><code>long getAndIncrement()</code>: атомарно увеличивает на 1 и возвращает старое значение (постфиксный инкремент, <code>i++</code>)</p></li>
<li><p><code>long decrementAndGet()</code>: атомарно уменьшает на 1 и возвращает новое (<code>--i</code>)</p></li>
<li><p><code>long getAndDecrement()</code>: атомарно уменьшает на 1 и возвращает старое (<code>i--</code>)</p></li>
<li><p><code>long addAndGet(long delta)</code>: атомарно добавляет <code>delta</code> и возвращает новое (<code>i += delta</code>)</p></li>
<li><p><code>long getAndAdd(long delta)</code>: атомарно добавляет <code>delta</code> и возвращает старое</p></li>
<li><p><code>boolean compareAndSet(long expect, long update)</code>: пытается установить значение <code>update</code>, только если текущее значение равно <code>expect</code>. Возвращает <code>true</code> в случае успеха</p></li>
</ul>
<p>Для применения атомарности и <code>AtomicLong</code> изменим предыдущий пример следующим образом:</p>
<pre class="brush:java;">
...
<p>Здесь изменен только код класса Counter - вместо типа <code>long</code> в качестве типа переменной применяется класс <b>AtomicLong</b>. Для создания объекта этого класса применяется конструктор, 
в который передается начальное значение переменной:</p>
<pre class="brush:java;">AtomicLong counter = new AtomicLong(0);</pre>
<p>То есть по умолчанию переменная counter равна 0.</p>
<p>В методе <code>getCounter()</code> с помощью метода <code>get()</code> возвращаем текущее значение счетчика:</p>
<pre class="brush:java;">long getCounter() { return counter.get(); }</pre>
<p>А в методе <code>increment()</code> с помощью вызова <code>getAndIncrement()</code> увеличиваем значение счетчика на 1:</p>
<pre class="brush:java;">void increment() { counter.getAndIncrement();  }</pre>
<p>Таким образом, при запуске потоков и выполнении метода <code>increment</code>:</p>
<pre class="brush:java;">
for (int i = 0; i &lt; numThreads; i++){
...
<p>будет выполняться атомарная операция, даже если несколько потоков подряд попробуют вызвать метод <code>increment()</code></p>
<p>И теперь консольный вывод будет детерминирован и собственно тем, каким и должен быть:</p>
<pre class="sh">
Counter: 1000000
</pre>
<p>Как видно из примера, <code>AtomicLong</code> дает корректный результат, в то время как <code>volatile long</code> теряет инкременты из-за состояния гонки.</p>
<h3>LongAdder</h3>
<p>Атомарные типы, например, тот же <code>AtomicLong</code>, очень удобно использовать для создания и управления счетчиками производительности, для генерации идентфиикаторов. 
...
использовать классы <code>LongAdder</code> и <code>LongAccumulator</code>.</p>
<p>Например, класс <b>LongAdder</b> используется для сценария исключительно подсчета, когда не нужно мгновенно знать точное значение, а важен только итоговый результат (сумма всех значений). 
Это эффективно в распространtнной ситуации, когда значение суммы не требуется до завершения всей работы. Повышение в этом случае производительности может быть существенным.</p>
<p>Под капотом <b>LongAdder</b>  хранит не одно значение, а массив значений (ячеек), общая сумма которых является текущим значением. Каждый поток работает со своей ячейкой, и коллизии (конкуренция) 
возникают гораздо реже. А при вызове метода <code>sum()</code> <b>LongAdder</b> просто складывает значения всех ячеек.</p>
<p>Вкратце пробежимся по функционалу класса <b>LongAdder</b>:</p>
<ul>
<li><p><code>void add(long x)</code>: добавляет заданное значение.</p></li>
<li><p><code>void decrement()</code>: эквивалентно add(-1).</p></li>
<li><p><code>double doubleValue()</code>: возвращает <code>sum()</code> как значение типа double после расширяющего преобразования.</p></li>
<li><p><code>float floatValue()</code>: Возвращает <code>sum()</code> как значение типа float после расширяющего преобразования.</p></li>
<li><p><code>void increment()</code>: эквивалентно add(1).</p></li>
<li><p><code>int intValue()</code>: возвращает <code>sum()</code> как значение типа int после сужающего преобразования.</p></li>
<li><p><code>long longValue()</code>: эквивалентно <code>sum()</code>.</p></li>
<li><p><code>void reset()</code>: сбрасывает переменные, сохраняя сумму равной нулю.</p></li>
<li><p><code>long sum()</code>: возвращает текущую сумму.</p></li>
<li><p><code>long sumThenReset()</code>: эквивалентно <code>sum()</code>, за которым следует <code>reset()</code>.</p></li>
</ul>
<p>Так, в примере выше не требовалось получать промежуточное значение после каждого инкремента, поэтому мы можем заменить <code>AtomicLong</code> на <b>LongAdder</b>:</p>
<pre class="brush:java;">
...
<p>Фактически здесь все то же самое, только вместо <code>AtomicLong</code> для определения счетчика применяется <b>LongAdder</b>. С помощью метода <code>longValue()</code> возвращаем значение счетчика (по сути сумму значений всех потоков). 
 А с помощью метода <code>increment()</code> каждый поток будет увеличивать свое значение в счетчике.</p>
<p>А консольный вывод также будет детерминирован, каким и должен быть:</p>
<pre class="sh">
Counter: 1000000
</pre>
