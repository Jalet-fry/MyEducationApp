...
<html  lang="ru">
<head>
<title>Java | Поля класса и класс Field</title>
<meta charset="utf-8" />
<meta name="description" content="Получение полей класса в программе на языке программирования Java с помощью методов getFields, getDeclaredFields, класс Field, получение и изменение значения поля произвольного объекта">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Поля класса и класс Field</h1><div class="date">Последнее обновление: 29.09.2025</div>
	<div class="socBlock">
...
<p>За работу с полями классов в пакете <code>java.lang.reflect</code> отвечает класс <b>Field</b>. Для получения информации о полях и управления ими этот класс определеяет ряд методов, из которых отмечу основные:</p>
<ul>
<li><p><span class="b">String getName()</span>: возвращает имя поля</p></li>
<li><p><span class="b">Class getType()</span>: возвращает тип поля в виде объекта Class</p></li>
<li><p><span class="b">int getModifiers()</span>: возвращает целое число с различными включенными и выключенными битами, которые описывают используемые модификаторы.</p></li>
<li><p><span class="b">Set&lt;AccessFlag&gt; accessFlags()</span>: возвращает набор флагов доступа для этого поля</p></li>
<li><p><span class="b">Object get(Object obj)</span>: возвращает имя поля</p></li>
<li><p><span class="b">void set(Object obj, Object value)</span>: возвращает имя поля</p></li>
</ul>
<h3>Получение полей</h3>
<p>Для манипулирования полями нам прежде всего надо получить их. Для получения полей типа применяется ряд методов класса <code>Class</code>:</p>
<ul>
<li><p><code>Field[] getFields()</code>:  возвращает массив объектов <code>Field</code> для открытых полей (с модификатором <code>public</code>) данного класса или его суперклассов</p></li>
...
<li><p><code>Field getField(String name)</code>:  возвращает объект <code>Field</code> для публичного поля, имя которого передается в качестве параметра. Поиск также идет среди полей суперклассов. Если подобное публичное поле отсутствует, то метод генерирует исключение <code>NoSuchFieldException </code></p></li>
<li><p><code>Field getDeclaredField(String name)</code>: возвращает объект <code>Field</code> для поля, имя которого передается в качестве параметра. Если подобное поле отсутствует, то метод генерирует исключение <code>NoSuchFieldException </code></p></li>
</ul>
<p>Например, получим поля класса:</p>
...
<p>Теперь получим по отдельности поля, используя их имя:</p>
<pre class="brush:java;">
import java.lang.reflect.*;
...
<p>В данном случае с помощью метода <code>getDeclaredField()</code> получаем поля name и age класса Person. Обратите внимание, что при передаче имени поля учитывается регистр имени. В итоге на консоль будет выведена 
информация о полученных полях:</p>
<pre class="sh">
...
<h2>Манипуляции с полями</h2>
<p>Манипуляции обычно включают получение значения поля или его установку. для этого класс <code>Field</code> предоставляет методы <code>get()/set()</code>.</p>
<h3>Обращение к приватным полям</h3>

<p>По умолчанию механизм рефлексии учитывает контроль доступа Java. Так, при обращении к закрытым полям методы <code>get()</code> и <code>set()</code> по умолчанию вызываюют исключение <code>IllegalAccessException</code> - исключение недопустимого доступа. 
...
значения этих полей без соответствующего разрешения.</p>
<p>Но мы можем переопределить контроль доступа, вызвав метод <code>setAccessible()</code> для объекта Field и передав в метод <code>true</code>:</p>
<pre class="brush:java;">
field.setAccessible(true);
</pre>
<p>Вызов setAccessible вызывает исключение, если доступ не предоставлен. Доступ может быть запрещён модульной системой.</p>

<h3>Получение значения поля</h3>
<p>Для получения значения поля применяется метод <code>get()</code>, который имеет следующее определение:</p>
<pre class="brush:java;">
public Object get(Object obj) throws IllegalArgumentException, IllegalAccessException
</pre>
<p>В качестве параметра в метод передается объект, значение поля которого надо получить. Например, получим значения полей объекта Person:</p>
<pre class="brush:java;">
import java.lang.reflect.*;
...
<h3>Изменение поля</h3>
<p>Для изменения значения поля применяется метод <code>set()</code>:</p>
<pre class="brush:java;">
public void set(Object obj, Object value) throws IllegalArgumentException, IllegalAccessException
</pre>
<p>ПЕрвый параметр (<code>obj</code>) представляет объект, поле которого надо изменить, а второй параметр (<code>value</code>) - присваемое полю новое значение. Посмотрим на примере:</p>
<pre class="brush:java;">
import java.lang.reflect.*;
...
<p>Таким образом, благодаря рефлексии приватность полей является преградой для их изменения.</p>
...
<style>
	...
	<div class="nav"><p><a href="./14.2.php">Назад</a><a href="./">Содержание</a><a href="./14.4.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
