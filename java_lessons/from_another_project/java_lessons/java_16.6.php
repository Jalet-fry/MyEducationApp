...
<html  lang="ru">
<head>
<title>Java | Поиск и вызов внешних функций</title>
<meta charset="utf-8" />
<meta name="description" content="Поиск и вызов внешних функций в Foreign Functions и Memory API в языке программирования Java, типы Linker, MethodHandle, FunctionDescriptor, получение дескриптора функций и методов, downcallHandle, SymbolLookup.libraryLookup">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Поиск и вызов внешних функций</h1><div class="date">Последнее обновление: 21.12.2025</div>
	<div class="socBlock">
...
<p>Для поиска библиотечных функций языка Си и их подготовке для вызова в API FFM применяется класс <span class="b">java.lang.foreign.Linker</span>. Для создания линкера, связанного с текущей нативной платформой ОС применяется 
статический метод <span class="b">Linker.nativeLinker()</span>:</p>
<pre class="brush:java;">Linker linker = Linker.nativeLinker();</pre>
<p>Фактически, часть "поиска" делегируется экземпляру <span class="b">SymbolLookup</span>, который определяется с помощью метода <code>defaultLookup()</code> линкера:</p>
<pre class="brush:java;">
Linker linker = Linker.nativeLinker();
SymbolLookup stdlib = linker.defaultLookup();
</pre>
<p>Библиотеки, о которых знает поиск по умолчанию, зависят от платформы. В Linux это libc и libm. Этот поиск достаточен для таких известных функций, как printf и malloc.</p>
<p>Для использования со специализированной библиотекой (или своей) в метод <code>SymbolLookup.libraryLookup()</code> передается путь к файлу библиотеки (в виде строки или объекта <code>Path</code>) и арена:</p>
<pre class="brush:java;">
SymbolLookup somelib = SymbolLookup.libraryLookup("/lib/x86_64-linux-gnu/libsxxx.so", arena);
</pre>
<p>Метод <code>libraryLookup</code> загружает библиотеку. А когда арена закрывается, библиотека выгружается.</p>

<h3>Поиск функций</h3>
<p>Получив объект <code>SymbolLookup</code>, мы можем перейти к поиску функций. Для этого <code>SymbolLookup</code> предоставляет два метода:</p>
<ul>
<li><p><code>Optional&lt;MemorySegment&gt; find(String name)</code>: возвращает адрес символа с заданным именем</p></li>
<li><p><code>MemorySegment findOrThrow(String name)</code>: возвращает адрес символа с заданным именем или генерирует исключение, если функция не найдена</p></li>
</ul>
<p>В обоих случаях указатель на полученную функцию возвращается через объект <code>MemorySegment</code>, работа с которым была рассмотрена в предыдущеихз статьях. Например, поиск функции <code>printf</code>:</p>
<pre class="brush:java;">
Linker linker = Linker.nativeLinker();
SymbolLookup stdlib = linker.defaultLookup();
MemorySegment printf_addr = stdlib.findOrThrow("printf");
</pre>
<h3>Дескриптор функции/метода</h3>
<p>Получив адрес вызываемой функции, можн создать дескриптор метода для ее вызова. <span class="b">Дескриптор метода</span> - это объект, через который можно передавать аргументы, вызывать метод и 
получать возвращаемое значение.</p>
<p>Дескриптор метода представляет тип <span class="b">MethodHandle</span>. Для его создания линкер - объект <code>Linker</code> предоставляет метода <span class="b">downcallHandle()</span></p>
<ul>
<li><p><code>downcallHandle(FunctionDescriptor function, Linker.Option... options)</code></p>
<p>Создает дескриптор метода, используемый для вызова внешней функции с заданной сигнатурой</p></li>
<li><p><code>downcallHandle(MemorySegment address, FunctionDescriptor function, Linker.Option... options)</code></p>
<p>Создает дескриптор метода, используемый для вызова внешней функции с заданной сигнатурой и адресом</p></li>
</ul>

<p>Этот метод принимает до трех параметров:</p>
<ul>
<li><p><code>address</code>: сегмент памяти, базовый адрес которого совпадает с адресом вызываемой внешней функции</p></li>
<li><p><code>function</code>: дескриптор вызываемой внешней функции</p></li>
<li><p><code>options</code>: параметры линкера, связанные с этим запросом на компоновку</p></li>
</ul>
<p>Ключевой здесь параметр - это десткриптор функции, представленный интерфейсом <span class="b">FunctionDescriptor</span>. Для определения этого объекта обычно применяется статический метод 
<code>FunctionDescriptor.of</code></p>
<pre class="brush:java;">FunctionDescriptor.of(MemoryLayout resLayout, MemoryLayout... argLayouts)</pre>
<p>Его первый параметр представляет компоновку памяти типа результата функции, а второй параметр - компоновки памяти для параметров функции.</p>

<p>Например, получим десткриптор метода для функции <code>printf</code>:</p>
<pre class="brush:java;">
...
<p>При передаче указателя в функцию C или получении результата указателя используется <code>ValueLayout.ADDRESS</code>. Тип блока памяти не имеет значения. Кроме того, необходимо точно указать тип 
возвращаемого значения функции и типы параметров. Если предоставить неверную информацию, виртуальная машина может завершиться с ошибкой.</p>
<p>Так, функция <code>printf</code> в C возвращает целое число <code>int</code>, а в качестве параметра принимает строку (по сути указатель на символы), поэтому ее дескриптор имеет форму</p>
<pre class="brush:java;">FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS)</pre>
<h3>Вызов функции</h3>
<p>Получив дескриптор метода, наконец мы можем вызвать функцию. Для этого у типа <code>MethodHandle</code> применяется метод <span class="b">invoke()</span></p>
<pre class="brush:java;">public final Object invoke(Object... args) throws Throwable</pre>
<p>В качестве аргументов в вызов метода передаются значения для параметров внешней функции. ПРи этом параметры должны соответствовать типам языка Си, а не Java. Например, мы не можем написать так:</p>
<pre class="brush:java;">printf.invoke("Hello");</pre>
<p>Хотя, казалось бы функция <code>printf()</code> принимает строку. Однако это должна быть строка языка Си. Соответственно нам надо сконвертировать строку Java в строку Си (UTF-8). 
Пример такого преобразования и вызова функции <code>printf</code>:</p>
<pre class="brush:java;">
import java.lang.foreign.*;
import java.lang.invoke.*;
...
<h4>Возвращаемый тип void</h4>
<p>Отдельно стоит сказать про функции, который возвращают <span class="b">void</span>. В этом случае для определения дескриптора функции применяется метод <span class="b">FunctionDescriptor.ofVoid</span></p>
<pre class="brush:java;">
FunctionDescriptor ofVoid(MemoryLayout... argLayouts)
</pre>
<p>В качестве параметров методу также передаются значения, которые передаются вызываемой внешней функции. Указатель на <code>void</code> (<code>void*</code>), как и все указатели, представляют тип <code>ValueLayout.ADDRESS</code>. 
Например, получим дескрипторы методов для функций <code>malloc</code> (выделение памяти) и <code>free</code> (освобождение памяти)</p>
<pre class="brush:java;">
import java.lang.foreign.*;
import java.lang.invoke.*;
...
<style>
	...
	<div class="nav"><p><a href="./16.5.php">Назад</a><a href="./">Содержание</a><a href="./16.7.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
