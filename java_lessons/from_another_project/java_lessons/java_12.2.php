...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Большие числа BigInteger и BigDecimal</h1><div class="date">Последнее обновление: 16.09.2025</div>
	<div class="socBlock">
...
программе очень большие числа, которые выходят за пределы допустимых значений для типов long и double. Кроме того, при вычислениями с числами с плавающей точкой мы можем столкнуться с рядом не очень приятных особенностей.</p>
<h3>Особенности при вычислениях с числами с плавающей точкой</h3>

<p>Все вычисления с плавающей точкой следуют спецификации IEEE 754. В частности, существует три специальных значения с плавающей точкой для обозначения переполнений и ошибок:</p>
<ul>
<li><p>Положительная бесконечность (Positive infinity)</p></li>
<li><p>Отрицательная бесконечность (Negative infinity)</p></li>
<li><p>NaN (не число)</p></li>
</ul>

<p>Для представления этих значений в Java есть специальные константы:</p>
<ul>
<li><p><code>Double.POSITIVE_INFINITY</code> (для типа <code>double</code>) и <code>Float.POSITIVE_INFINITY</code> (для типа <code>float</code>)</p></li>
<li><p><code>Double.NEGATIVE_INFINITY</code> и <code>Float.NEGATIVE_INFINITY</code> </p></li>
<li><p><code>Double.NaN</code> и <code>Float.NaN</code></p></li>
</ul>

<p>Например, результатом деления положительного числа с плавающей точкой на 0 является положительная бесконечность:</p>
...
<p>ПРи делении на <code>-0.0</code> образуется отрицательная бесконечность (например, <code>1.0 / -0</code>).</p>
<p>Деление 0,0 на 0 или квадратный корень отрицательного числа дает <code>NaN</code>:</p>
<pre class="brush:java;">
public class Program{
...
<p>Мы можем использовать эти константы для проверки результата вычислений. Например:</p>
<pre class="brush:java;">
public class Program{
...
<p>Для проверки на NaN также можно использовать встроенный метод <code>Double.isNaN()</code>:</p>
<pre class="brush:java;">
public class Program{
...
<p>В целом следует отметить, что числа с плавающей точкой не подходят для финансовых вычислений, где ошибки округления недопустимы. Например:</p>
<pre class="brush:java;">
public class Program{
...
<p>Данная программа выведет число <code>0.8999999999999999</code>, а не <code>0.9</code>. Такие ошибки округления возникают из-за того, что числа с плавающей точкой представлены в двоичной системе счисления. 
А точного двоичного представления дроби 1/10 не существует. Поэтому если требуются точные вычисления без ошибок округления, то лучше использовать класс <code>BigDecimal</code>.</p>
...
<p>Пример использования BigDecimal:</p>
<pre class="brush:java;">
import java.math.*;
...
<h3>BigInteger</h3>
<p>Для работы с очень большими целочисленными значениями числовыми данными можно использовать тип <span class="b">BigInteger</span> из 
пакета <span class="b">java.math</span>. Основные методы класса BigInteger:</p>
<ul>
...
<p>Пример использования класса BigInteger:</p>
<pre class="brush:java;">
import java.math.*;
...
<style>
	...
	<div class="nav"><p><a href="./12.1.php">Назад</a><a href="./">Содержание</a><a href="./12.3.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
