...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Классы и объекты</h1><div class="date">Последнее обновление: 22.09.2025</div>
	<div class="socBlock">
...
<p>В принципе ранее уже использовались классы. Например, в предыдущих темах мы определяли класс Program, в котором определялся весь остальной код приложения. Например:</p>
...
<p>Здесь у нас класс Program, в котором определен метод main, и этот метод выводит на консоль некоторое сообщение. Для вывода на консоль мы используем еще один класс - <code>System</code>. 
Теперь же посмотрим, как мы можем определять и использовать другие классы.</p>
<h3>Определение классов</h3>
<p>Класс определяется с помощью ключевого слова <span class="b">сlass</span>, после которого идет название класса:</p>
<pre class="brush:java;">
class имя_класса{

}
</pre>
<p>Название класса подчиняется тем же правилам, что и названия переменных. Но в целом в Java есть условность, что обычно названия классов начинаются с большой буквы.</p>
<p>После названия класса идут фигурные скобки, между которыми помещается тело класса - то есть его компоненты (поля и методы).</p>

<p>Например, определим класс, который назовем Person:</p>
<pre class="brush:java;">
// класс Program
class Program{
...
<p>После имени класса Person идут пустые фигурные скобки, то есть класс Person не имеет никаких компонентов. Стоит отметить, что, как правило, классы определяются в разных файлах, но в данном случае для простоты мы определяем два класса в одном файле. </p>
<h3>Объекты классов</h3>
<p>Классы в Java представляют отдельные типы данных. И когда мы определяем новый класс, который даже ничего не делает и не имеет никакого содержимого, то фактически мы определяем новый тип данных. 
И как в случае со стандартными и уже имеющимися в Java типами данных мы можем определить переменные этого типа. Например:</p>
<pre class="brush:java;">
class Program{
...
<p>В данном случае в методе main класса Program определена переменная типа Person, которая называется "tom". Пока такая переменная не имеет никакого значения, то есть она <span class="b">не инициализирована</span>. 
По большому счету мы ее пока не можем использовать, поэтому вначале необходимо создать <b>объект</b> класса Person.</p>
<p>Для создания объекта применяются <b>конструкторы</b>. По сути конструкторы представляют специальные методы, которые называются так же как и класс, и которые вызываются при создании нового объекта класса и выполняют инициализацию объекта. Общий синтаксис вызова конструктора:</p>
<pre class="brush:java;">new конструктор_класса(параметры_конструктора);</pre>
<p>Сначала идет оператор <b>new</b>, который выделяет память для объекта, а после него идет вызов <b>конструктора</b>.</p>
<h4>Конструктор по умолчанию</h4>
<p>Если в классе не определено ни одного конструктора (как в случае с нашим пустым классом Person), то для этого класса автоматически создается пустой <b>конструктор по умолчанию</b>, который не принимает 
никаких параметров. Поэтому применим конструктор по умолчанию для создания объекта класса Person:</p>
<pre class="brush:java;">
class Program{
...
<p>Для создания объекта Person используется выражение <code>new Person()</code>. В итоге после выполнения данного выражения в памяти будет выделен участок, где будут храниться все данные объекта Person. А переменная tom получит ссылку на созданный объект, и через эту переменную мы можем использовать данный объект и обращаться к его функциональности.</p>
<p>Но пока в классе Person мы не определили никакой функциональности, тем не менее Java позволяет нам вывести объект на консоль:</p>
<pre class="brush:java;">
class Program{
...
<p>В данном случае Java выводит на консоль некоторое текстовое представление объекта типа "Person@1dbd16a6".</p>
...
<p>Для хранения состояния объекта в классе применяются поля или переменные класса.</p>
<p>Например, выше мы определили пустой класс Person. Добавим в него поля. Что за поля мы будем в него добавлять? Класс Person у нас будет представлять некоторого человека. У человека же мы можем выделить 
такие признаки как имя и возраст (при желании можно выделить и больше признаков). Поэтому добавим для этих признаков в класс поля:</p>
...
<p>Поле name представляет имя человека и представляет строку, а age - его возраст и представляет число.</p>
...
<p>Например, обратимся к полям объекта Person::</p>
<pre class="brush:java;">
class Program{
...
<p>Для обращения к полю name объекта tom применяется выражение <code>tom.name</code>, а для обращения к полю age - выражение <code>tom.age</code>. В программе мы сначала получаем значение полей и выводим их на консоль:</p>
<pre class="brush:java;">
System.out.println(tom.name);
System.out.println(tom.age);
</pre>
<p>А затем устанавливаем их значения. Так, полю name передаем строку, поскольку это поле представляет тип String, а полю age передаем число, так как оно представляет целочисленный тип int. 
В итоге мы получим следующий консольный вывод программы:</p>
<pre class="sh">
null
0
Tom
...
<h3>Инициализация полей</h3>
<p>Если мы посмотрим на вывод предыдущей программы, то увидим, что до присвоения значения полям они принимают некоторые значения по умолчанию. 
...
<code>null</code>. В этом состоит важное различие между полями и локальными переменными: локальные переменные в методе необходимо явно инициализировать, тогда как в классе, 
если вы не инициализируете поле, оно автоматически инициализируется значением по умолчанию (0, false или null).</p>
<p>ОДнако поля можно явным образом инициализировать некоторыми значениями. Например:</p>
<pre class="brush:java;">
class Program{
...
<h3>Создание нескольких объектов</h3>
<p>Подобным образом мы можем создавать большее количество объектов. Например:</p>
<pre class="brush:java;">
...
<p>Здесь создается два объекта класса Person - tom и bob. Для каждого из этих объектов будет выделяться своя область в памяти, и каждый из этих объектов будет иметь свой набор полей name и age.</p>
...
<style>
	...
	<div class="nav"><p><a href="./2.20.php">Назад</a><a href="./">Содержание</a><a href="./2.7.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
