...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Оператор return. Результат метода</h1><div class="date">Последнее обновление: 16.09.2025</div>
	<div class="socBlock">
	...
	<p>Методы могут возвращать некоторое значение. В примерах в предыдущих статьях применялись методы, которые имели тип <code>void</code>: методы с таким типом не возвращают никакого значения. Они просто выполняют некоторые действия. 
Для возвращения некоторого результата в методе применяется оператор <span class="b">return</span>:</p>
<pre class="brush:java;">return возвращаемое_значение;</pre>
<p>После оператора <b>return</b> указывается возвращаемое значение, которое является результатом метода. Это может быть литеральное значение, значение переменной или 
...
<p>Метод <code>sayHello</code> имеет тип <code>String</code>, следовательно, он должен возвратить строку. Поэтому в теле метода 
...
<p>Метод <code>sayHello()</code> возвращает значение типа <code>String</code>. Поэтому мы можем присвоить это значение какой-нибудь 
переменной типа String:</p>
<pre class="brush:java;">String message = sayHello();</pre>
<p>Либо даже передать в качестве значения параметру другого метода:</p>
<pre class="brush:java;">
System.out.println(tom.sayHello());
</pre>

<p>После оператора return также можно указывать сложные выражения или вызовы других методов, которые возвращают определенный результат. Например, определим метод, который возвращает сумму чисел:</p>
<pre class="brush:java;">
class Program{
...
<p>Метод может использовать несколько вызовов оператора <b>return</b> для возвращения разных значений в зависимости от некоторых условий:</p>
<pre class="brush:java;">
class Program{
...
<p>Здесь метод asString класса Time принимает текущий час через параметр hour и возвращает значение типа String, то есть строку, то есть фактически преобразует час в строку. И в зависимости от значения параметра hour возвращаемая строка будет различаться.</p>
<h3>Выход из метода</h3>
<p>Оператор <b>return</b> применяется для возвращаения значения из метода, но и для выхода из метода. sВ подобном качестве оператор return применяется в методах, которые ничего не возвращают, то есть имеют тип <code>void</code>:</p>
<pre class="brush:java;">
class Program{
    ...
    public static void main(String[] args) {  
        ...
        Time time = new Time();
        time.print(7);     // Good morning
        time.print(13);    // Good after noon
        time.print(32);    // 
        time.print(56);    // 
        time.print(2);     // Good night
    }
}
class Time{
    // метод получает текущий час
...
<p>Если переданное в метод print значение больше 24 или меньше 0, то просто выходим из метода. Возвращаемое значение после return указывать в этом случае не нужно.</p>
...
<style>
	...
	<div class="nav"><p><a href="./3.14.php">Назад</a><a href="./">Содержание</a><a href="./2.18.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
