...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Модульность</h1><h2>Создание модуля</h2><div class="date">Последнее обновление: 02.11.2025</div>
	<div class="socBlock">
...
модуля. Поддержка модулей реализована через подсистему <b>Java Platform Module System</b> (система модулей платформы Java), которая является частью языка Java и виртуальной машины. </p>
...
<p>Перед созданием модуля прежде всего надо определиться с его именем. В качестве имени модуля в принципе может использоваться произвольный идентификатор из алфавитно-цифровых символов и знаков подчеркивания. Обычно, как и в случае с пакетами, имена модулей следуют условности "reverse domain name" (обратного доменного имени). 
И в целом рекомендуется (хотя это и необязательно), чтобы название модуля соответствовало названию, c которого начинаются пакеты этого модуля. Это соглашение предотвращает конфликты имён пакетов в модулях. Любой пакет может быть размещtн только в одном модуле. Если имена модулей уникальны и начинаются с имени модуля, то имена пакетов также будут уникальными.</p>
<p>Определим и используем простейший модуль. Допустим, файлы с исходными кодами помещаются в папку "C:\java" или "Home/java" (либо какую-нибудь другую папку на жестком диске). 
Создадим в этой папке каталог, который назовем <span class="b">demo</span>. Этот каталог будет представлять модуль. По соглашению, имя базового каталога совпадает с именем модуля.</p>
<p>В каталоге demo определим новый файл <span class="b">module-info.java</span> со следующим кодом:</p>
<pre class="brush:java;">
module demo {
}
</pre>
<p>Этот файл представляет <span class="b">дескриптор модуля</span> (module descriptor). Этот файл может содержать только определение модуля.</p>
...
<p>Далее в каталоге demo создадим папку <span class="b">com</span>. В папке com создадим папку <span class="b">metanit</span>, а в папке <span class="b">com/metanit</ - 
...
<p>Параметр <span class="b">--module-path</span> указывает на путь к модулю, а <span class="b">--module</span> - на главный класс модуля.</p>
<p>При наборе команды вместо параметра <span class="b">--module-path</span> можно указать его сокращение <span class="b">-p</span>, а вместо 
параметра <span class="b">--module</span> - сокращение <span class="b">-m</span>.</p>
<p>И на консоли отобразится сообщение "Hello METANIT.COM!"</p>
<pre class="sh">
eugene@Eugene:~$ cd java
...
<style>
	...
	<div class="nav"><p><a href="./10.10.php">Назад</a><a href="./">Содержание</a><a href="./11.2.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
