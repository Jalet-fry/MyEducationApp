...
<html  lang="ru">
<head>
<title>Java | Нативные функции обратного вызова</title>
<meta charset="utf-8" />
<meta name="description" content="Нативные функции обратного вызова в Foreign Functions и Memory API в языке программирования Java, типы Linker, MethodHandle, FunctionDescriptor, получение дескриптора функций и методов, downcallHandle, создание заглушки функции с помощью upcallStub">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Нативные функции обратного вызова</h1><div class="date">Последнее обновление: 22.12.2025</div>
	<div class="socBlock">
...
<p>Иногда одна функция в качестве параметра принимает другую функцию - функцию обратного вызова или callback и в процессе своей работы вызывает ее. Например, возьмем функцию быстрой сортировки <code>qsort</code>, которая есть в 
стандартной библиотеке языка C:</p>
<pre class="brush:java;">
void qsort(void *base, size_t nmemb, size_t size, int (*compare)(const void *, const void *));
</pre>
<p>При вызове этой функции ей передаются:</p>
<ul>
<li><p><code>base</code>: массив сортируемых элементов</p></li>
<li><p><code>nmemb</code>: длина массива</p></li>
<li><p><code>size</code>: размер каждого элемента</p></li>
<li><p><code>compar</code>: ​​адрес функции обратного вызова, которая собственно сравнивает два элемента</p></li>
</ul>
<p>Последний параметр представляет указатель на функцию, которая вызывается при сравнении двух элементов. С точки зрения API FFM, как и любой указатель, указатель на функцию передается через значение 
<code>ValueLayout.ADDRESS</code>. Однако может возникнуть вопрос, как определить подобную функцию и как передать ее в вызов. Рассмотрим на примере следующей программы:</p>
<pre class="brush:java;">
...
public class Program {
    ...
    public static void main(String[] args) throws Throwable{
        ...
        // получаем адрес функции sqort
        MemorySegment qsortAddr = stdlib.findOrThrow("qsort");
        // определяем дескриптор функции
        FunctionDescriptor qsortDesc = FunctionDescriptor.ofVoid(ADDRESS, JAVA_LONG, JAVA_LONG, ADDRESS);
        // void qsort(void*, size_t, size_t, int (*)(const void *, const void *))
        MethodHandle qsort = linker.downcallHandle(qsortAddr, qsortDesc);

        // получаем тип метода
        MethodType compareType = MethodType.methodType(int.class, MemorySegment.class, MemorySegment.class);
        // получаем дескриптор статического метода
        MethodHandle compareHandle = MethodHandles.lookup().findStatic(Program.class, "compareInt", compareType);

        // определяем заглушку для функции
        ...
        int[] numbers = {3, 5, 1, 8, 0, 5, 2};

        try (Arena arena = Arena.ofConfined()) {

            // определяем для чисел numbers сегмент памяти
            MemorySegment numbers_seg = arena.allocateFrom(JAVA_INT, numbers);
            qsort.invoke(numbers_seg, numbers.length, JAVA_INT.byteSize(), compareStub);

            // преобразуем сегмент памяти в массив
            numbers = numbers_seg.toArray(JAVA_INT);
            System.out.println(Arrays.toString(numbers));   // [0, 1, 2, 3, 5, 5, 8]
        }
    }

    static int compareInt(MemorySegment a, MemorySegment b) {

        return Integer.compare(a.get(JAVA_INT, 0), b.get(JAVA_INT, 0));
    }
}
</pre>
<p>Рассмотрим поэтапно программный код:</p>
<ol>
<li><p>Допустим, мы хотим, чтобы нативная функция <code>qsort</code> для сортировки использовала следующий метод Java:</p>
<pre class="brush:java;">
static int compareInt(MemorySegment a, MemorySegment b) {
...
<p>Обратите внимание, что метод предназначен для сортировки целочисленных значений типа <code>int</code>, однако параметры представляют значения <code>MemorySegment</code></p>
</li>

<li><p>Для начала нам нам, как и в общем случае, надо получить дескриптор метода внешней функции:</p>
<pre class="brush:java;">
// получаем линкер/компоновщик 
Linker linker = Linker.nativeLinker();
// получаем стандартную библиотеку
...
MemorySegment qsortAddr = stdlib.findOrThrow("qsort");
// определяем дескриптор функции
FunctionDescriptor qsortDesc = FunctionDescriptor.of(ADDRESS, JAVA_LONG, JAVA_LONG, ADDRESS);
// void qsort(void*, size_t, size_t, int (*)(const void *, const void *))
MethodHandle qsort = linker.downcallHandle(qsortAddr, qsortDesc);
</pre></li>

<li>
<p>Чтобы получить адрес этого метода, сначала получаем объект <code>MemoryHandle</code> для статического метода Java:</p>
<pre class="brush:java;">
// получаем тип метода
MethodType compareType = MethodType.methodType(int.class, MemorySegment.class, MemorySegment.class);

// получаем дескриптор статического метода (Program - текущий класс, где располагается метод)
MethodHandle compareHandle = MethodHandles.lookup().findStatic(Program.class, "compareInt", compareType);
</pre>
<p>Метод <code>MethodType.methodType</code> принимает тип возвращаемого значения и далее типы параметров метода. В нашем случае метод сортировки <code>compareInt</code> возвращает значение <code>int</code> и 
принимает два значения <code>MemorySegment</code></p>
</li>

<li><p>С помощью метода <code>upcallStub()</code> линкера сгенерируем заглушку для функции C, которая затем вызывает дескриптор метода обратного вызова:</p>
<pre class="brush:java;">
MemorySegment compareStub = linker.upcallStub(
    compareHandle,
    FunctionDescriptor.of(JAVA_INT,
...
<p>Метод <code>upcallStub()</code> создает заглушку вызова, которую можно передать другим внешним функциям в виде указателя на функцию. Вызов такого указателя на функцию из внешнего кода приведет к выполнению предоставленного дескриптора метода:</p>
<pre class="brush:java;">
...
<p>Метод принимает до 4 параметров:</p>
<ul>
<li><p><code>target</code>: дескриптор целевого метода</p></li>
<li><p><code>function</code>: дескриптор функции-заглушки вызываемой функции</p></li>
<li><p><code>arena</code>: арена, связанная с возвращаемым сегментом-заглушкой вызываемой функции</p></li>
<li><p><code>options</code>: параметры линкера, связанные с этим запросом на компоновку</p></li>
</ul>
<p>
Обратите внимание на вызовы <code>withTargetLayout()</code>. Они настраивают структуру памяти, чтобы линкер мог сгенерировать код для настройки экземпляров MemorySegment, которые получает обратный вызов 
Java. Если этого не сделать, вызовы <code>a.get(JAVA_INT, 0)</code> и <code>b.get(JAVA_INT, 0)</code> в Java-коллбэке завершатся ошибкой, поскольку сегменты памяти a и b будут иметь нулевой размер.
</p></li>

<li><p>И в конце можно вызвать внешнюю функцию с адресом коллбэка:</p>
<pre class="brush:java;">
float[] samples = . . .;
...
<p>При выполнении программы на вход функции <code>qsort</code> передаем массив <code>{3, 5, 1, 8, 0, 5, 2}</code> и после выполнения мы увидим консольный вывод отсортированного массива:</p>
<pre class="sh">[0, 1, 2, 3, 5, 5, 8]</pre>
...
<style>
	...
	<div class="nav"><p><a href="./16.6.php">Назад</a><a href="./">Содержание</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
