...
<html  lang="ru">
<head>
<title>Java | Интерфейсы и полиморфизм</title>
<meta charset="utf-8" />
<meta name="description" content="Интерфейсы и полиморфизм в языке программирования Java, интерфейсы как параметры и результаты методов, преобразование между объектами и интерфейсами">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Интерфейсы и полиморфизм</h1><div class="date">Последнее обновление: 30.09.2025</div>
	<div class="socBlock">
...
<p>Все сказанное в отношении преобразования типов характерно и для интерфейсов. Например, так как класс реализует некоторый интерфейс, то объект этого класса также является объектом реализуемого интерфейса. Например, 
...
<p>Здесь класс Сar реализует интерфейс Movable, соответственно переменная типа Movable может хранить ссылку на объект типа Car.</p>

<p>И если мы хотим обратиться к методам класса Car, которые определены не в интерфейсе Movable, а в самом классе Car, то нам надо 
явным образом выполнить преобразование типов:</p>
<pre class="brush:java;">
public class Program{
...
<p>И также как и в случае с классами, интерфейсы могут использоваться в качестве типа параметров метода или в качестве возвращаемого типа. В качестве тпараметров методов и возвращаемых результатов, которые представляют интерфейс, 
можно использовать объекты классов, которые применяют данный интерфейс:</p>
<pre class="brush:java;">
public class Program{
...
<p>В данном случае метод travel (условная функция путешествия на транспорте) в качестве параметра получает объект Movable. Это может быть машина, и самолет, и любой другой объект, класс которого реализует интерфейс Movable. 
Какой конкретно это будет объект, мы не знаем. Мы лишь знаем, что этот объект должен реализовать интерфейс Movable, а значит, иметь метод move.</p>
...
<p>ПРимер с возвращением объекта интерфейса:</p>
<pre class="brush:java;">
public class Program{
...
<p>Метод <code>createMovable()</code> в качестве результата возвращает объект интерфейса Movable. В качестве параметра метод принимает булевое значение, которое указывает, объект какого именно класса 
будет создаваться: если <code>true</code>, то создается объект Car, иначе создается объект Aircraft..</p>
...
<style>
	...
	<div class="nav"><p><a href="./3.7.php">Назад</a><a href="./">Содержание</a><a href="./3.29.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
