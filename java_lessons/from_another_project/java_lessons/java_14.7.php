...
<html  lang="ru">
<head>
<title>Java | Proxy (Прокси)</title>
<meta charset="utf-8" />
<meta name="description" content="Proxy (Прокси) в программе на языке программирования Java, прокси-классы и прокси-объекты, Обработчик вызовов (invocation handler), интерфейс InvocationHandler, динамическая реализация интерфейсов и перехват вызова метода интерфейса">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Proxy (Прокси)</h1><div class="date">Последнее обновление: 04.10.2025</div>
	<div class="socBlock">
...
<p><span class="b">Proxy (Прокси)</span> в языке Java применяются для создания таких объектов, которые действуют как экземпляры интерфейсов, 
но при этом допускают настройку вызова методов этих интерфейсов. Прокси представляют класс <b>java.lang.reflect.Proxy</b> и не являются часто используемым инструментом и, как привило, применяются в системном программирования, например, при создании плагинов. 
...
для отладки</p>
<p>Прокси-класс, как и все другие, имеет методы, унаследованные от базового класса <code>Object</code> (как <code>toString()</code>, <code>equals()</code> и тд.). Но кроме того прокси-класс также 
имеет методы, которые определены в применяемых интерфейсах.</p>
<h3>Обработчик вызовов (invocation handler)</h3>
<p>Главное отличие по сравнению со стандартной реализацией интерфейсов заключается в том, что во время выполнения нельзя определить новый код для методов реализуемых интерфейсов. 
Вместо этого необходимо предоставить <span class="b">обработчик вызовов</span> (invocation handler). <span class="b">Обработчик вызовов</span> представляет объект произвольного класса, 
который реализует интерфейс <span class="b">InvocationHandler</span>. Этот интерфейс имеет единственный метод:</p>
<pre class="brush:java;">
Object invoke(Object proxy, Method method, Object[] args) throws Throwable
</pre>
<p>Метод принимает следующие параметры:</p>
<ul>
<li><p><code>proxy</code>: объект прокси-класса, на котором был вызван метод</p></li>
<li><p><code>method</code>: экземпляр метода, который соответствует методу вызываемого интерфейса</p></li>
<li><p><code>args</code>: массив аргументов, передаваемых в вызываемый метод при вызове, или null, если метод интерфейса не принимает аргументов. 
Аргументы примитивных типов обертываются в соответствующие класса-обертки, например, <code>int</code> в <code>java.lang.Integer</code> или <code>boolean</code> в <code>java.lang.Boolean</code>.</p></li>
</ul>
<p>Метод возвращает значение, возвращаемое классом-прокси при вызове метода реализуемого интерфейса.</p>
<p>В итоге, при каждом вызове метода прокси-объекта у обработчика вызовов вызывается метод <code>invoke()</code>, в который передается объект <code>Method</code> и аргументы для методы.</p>

<p>Рассмотрим небольшой пример. Допустим, мы хотим отслеживать вызовы методов. И для этого определим следующий класс:</p>
<pre class="brush:java;">
...
<p>Здесь определяется класс-реализация интерфейса <code>InvokeHandler</code> - класс <code>DebugHandler</code>. Через конструктор он принимает объект, на котором вызывается метод реализуемого интерфейса. 
А его метод <code>invoke()</code> просто выводит имя и аргументы вызываемого метода, а затем вызывает этот метод, передавая в него объект и аргументы.</p>
<h3>Создание прокси-объекта</h3>
<p>Для создания прокси-объекта применяется статический метод <code>Proxy.newProxyInstance()</code>:</p>
<pre class="brush:java;">
static Object newProxyInstance (ClassLoader loader, Class&lt;?&gt;[] interfaces, InvocationHandler h)
</pre>
<p>Этот метод принимает три параметра:</p>
<ul>
<li><p><code>loader</code>: загрузчик классов для определения прокси-класса. В качестве загрузчика можно использовать системный загрузчик, который можно получить с помощью метода <code>ClassLoader.getSystemClassLoader()</code></p></li> 
<li><p><code>interfaces</code>: список интерфейсов, реализуемых прокси-классом</p></li> 
<li><p><code>h</code>: обработчик вызовов для диспетчеризации вызовов методов</p></li>
</ul>
<p>Рассмотрим, как создать и использовать прокси-объект:</p>
<pre class="brush:java;">
import java.lang.reflect.InvocationHandler;
import java.lang.reflect.Method;
...
<p>Итак, у нас есть некоторый интерфейс Movable с одним методом <code>move()</code>:</p>
<pre class="brush:java;">
interface Movable{
    void move(int distance);
}
</pre>
<p>Метод <code>move()</code> в качестве параметра принимает условное расстояние, на которое надо переместить объект.</p>
<p>Есть класс Point, который представляет точку на плоскости и реализует этот интерфейс:</p>
<pre class="brush:java;">
class Point implements Movable{
...
<p>В реализации метода <code>move()</code> просто прибавляем расстояние distance к координатам x и y у точки и выводим некоторое диагностическое сообщение.</p>
<p>Теперь посмотрим на применение прокси-объекта в методе main. Но прежде всего создаем объект Point, для которого будет затем вызываться метод <code>move()</code></p>
<pre class="brush:java;">
Object start = new Point(10, 15);</pre>
<p>Затем создаем объект нашего обработчика вызовов - класса DebugHandler, которому передаем целевой объект:</p>
<pre class="brush:java;">
var handler = new DebugHandler(start);
</pre>
<p>И далее создаем сам прокси-объект:</p>
<pre class="brush:java;">
Movable proxy = (Movable) Proxy.newProxyInstance(ClassLoader.getSystemClassLoader(), new Class[] { Movable.class }, handler);
</pre>
<p>В данном случае прокси-объект будет вести себя как объект интерфейса Movable, поэтому преобразуем объект к этому интерфейсу.</p>
<p>Для создания прокси в качестве загрузчика применяем системный загрузчик, который получаем с помощью вызова <code>ClassLoader.getSystemClassLoader()</code></p>
<p>В качестве второго параметра передаем массив отслеживаемых классов интерфейсов. В нашем случае у нас один интерфейс - Movable:</p>
<pre class="brush:java;">new Class[] { Movable.class }</pre>
<p>И третий параметр - наш обработчик вызовов handler.</p>
<p>Итак, мы создали прокси-объект, который ведет себя как объект интерфейса Movable. А обработчик вызовов может перехватить вызовы методов этого интерфейса. Далее для демонстрации несколько раз вызываем на прокси-объекте метод 
<code>move()</code>:</p>
<pre class="brush:java;">
proxy.move(25);
proxy.move(15);
...
<p>При каждом вызове <code>proxy.move()</code> обработчик вызовов через метод <code>invoke()</code> получит вызванный метод и его аргументы, выведет данные на консоль и затем вызовет этот метод на объекте Point с помощью выражения</p>
<pre class="brush:java;">
m.invoke(target, args)
</pre>
<p>Таким образом, выражение</p>
<pre class="brush:java;">proxy.move(25)</pre>
<p>фактически превратится в </p>
<pre class="brush:java;">
...
<style>
	...
	<div class="nav"><p><a href="./14.6.php">Назад</a><a href="./">Содержание</a><a href="./15.1.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
