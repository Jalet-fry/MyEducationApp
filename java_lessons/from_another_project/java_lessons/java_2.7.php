...
<meta charset="utf-8" />
<meta name="description" content="Определение и использование методов в языке программирования Java, процедуры и функции, возвращаемые значения">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Методы</h1><div class="date">Последнее обновление: 16.09.2025</div>
	<div class="socBlock">
	...
	<p>Если переменные класса хранят состояние объекта, то <b>методы</b> представляют его поведение. Методы содержат собой набор операторов, которые выполняют определенные действия. И если нам надо 
определить в классе некоторые действия, нам надо определить методы.</p>
...
<p>Для примера создадим класс и определим в нем метод:</p>
<pre class="brush:java;">      
class Person{ 
    String name;
    int age;
...
<p>Здесь определен класс Person с двумя полями - name и age и одним методом print. Иетод print в качестве возвращаемого типа имеет тип <code>void</code> (то есть по сути ничего не возвращает) и не принимает никаких параметров. 
Все что делает этот метод - это выводит на консоль значения переменных name и age.</p>
<p>Но чтобы применить метод, его надо <b>вызвать</b>.</p>
<h3>Вызов метода</h3>
...
<p>Обращаю внимание, что в классе Program определен метод main, и нам не надо было его никак вызывать. Дело в том, что этот метод автоматически вызывается средой выполнения Java, и нам не надо его явным образом вызывать. 
Остальные методы нам надо явным образом вызвать. 
И поскольку при запуске программы запускает метод <code>main()</code>, то все остальные надо вручную явным образом вызывать в методе main (или в других методах, которые были также вызваны ранее). 
Например, вызовем выше определенный метод print:</p>
<pre class="brush:java;">
class Program{
...
<p>Для вызова метода print в методе main используется выражение <code>tom.print()</code>. То есть как и в случае с полями, после имени переменной класса идет точка и затем имя метода. 
Поскольку метод print не имеtт никаких параметров, то после его названия при вызове ставятся пустые скобки. В итоге при выполнении программы мы увидим на консоли информацию о полях объекта:</p>
<pre class="sh">
Name: Tom		Age: 41
</pre>
...
<p>Одно из преимуществ методов заключается в том, что мы можем вынести некоторые общие действия в 
отдельный метод и затем вызывать многократно их в различных местах программы. Например:</p>
<pre class="brush:java;">
class Program{
...
<p>Здесь мы три раза устанавливаем имя человека и с помощью метода print проверяем его установку. В итоге мы получим следующий консольный вывод:</p>
<pre class="sh">
Name: Tom		Age: 41
Name: Tomas		Age: 41
Name: Tommy		Age: 41
</pre>
<h3>Вызов методов в других методах класса</h3>

<p>Одни методы класса могут вызывать другие методы этого класса:</p>
<pre class="brush:java;">
class Program{
...
<p>Здесь в методе print вызываются два других метода класса - printName и printAge. Консольный вывод программы:</p>
<pre class="sh">
Name: Tom
Age: 41
</pre>
...
<style>
	...
	<div class="nav"><p><a href="./3.1.php">Назад</a><a href="./">Содержание</a><a href="./2.16.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
