...
<html  lang="ru">
<head>
<title>Java | Арена и сегменты памяти MemorySegment</title>
<meta charset="utf-8" />
<meta name="description" content="Арена Arena и сегменты памяти MemorySegment в Foreign Functions и Memory API в языке программирования Java, методы global, ofAuto, ofConfined, ofShared, allocate и allocateFrom">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Арена и сегменты памяти MemorySegment</h1><div class="date">Последнее обновление: 21.12.2025</div>
	<div class="socBlock">
	...
	<h2>Арена</h2>
<p>Одним из ключевых компонентов при взаимодействии с нативными (внешними) функциями является <span class="b">арена</span> (arena). 
При обращении к нативным (внешним) функциями код Java должен работать с памятью вне кучи (хипа), то есть с памятью, которая не была выделена с помощью оператора <code>new</code>. 
И арена как раз управляет жизненным циклом сегментов памяти вне кучи, позволяя легко выделять и автоматически освобождать используемую память.</p>
<h3>Создание и закрытие арены</h3>
<p>На платформе Java арена представлена классом <span class="b">Arena</span>. Для создания арены применяется один из статических методов класса:</p>
<ul>
<li><p><code>global()</code>: возвращает "глобальную" арену. Сегменты памяти, выделенные в рамках глобальной арены, доступны любому потоку.</p></li>
<li><p><code>ofAuto()</code>: cоздает новую арену, которая автоматически управляется сборщиком мусора. Сегменты арены доступны любому потоку</p></li>
<li><p><code>ofConfined()</code>: возвращает новую "ограниченную" арену. Сегменты, выделенные для ограниченной арены, доступны потоку, который создал арену, то есть потоку-владельцу арены.</p></li>
<li><p><code>ofShared()</code>: возвращает новую общую арену. Сегменты, выделенные для общей арены, доступны любому потоку.</p></li>
</ul>
<p>Для закрытия арены предназначен метод <span class="b">close()</span>. Но в зависимости от типа арены принцип закрытия может различаться:</p>
<ul>
<li><p>"Ограниченная" арена (<code>ofConfined()</code>) должна быть закрыта в том же потоке, в котором она была создана. Иначе будет выброшено исключение.</p></li>
<li><p>"Глобальная" арена (<code>global()</code>) никогда не закрывается, и выделенные ею сегменты памяти никогда не освобождаются.</p></li>
<li><p>"Автоматическую" арену (<code>ofAuto()</code>) закрыть нельзя. Сборщик мусора сам в конечном итоге освободит сегменты памяти, когда они, как и арена, перестанут использоваться. Это удобно, но менее предсказуемо.</p></li>
<li><p>"Общая" арена (<code>ofShared()</code>) работает до тех пор, пока не будет вызван метод <code>close</code> в любом потоке</p></li>
</ul>
<p>Таким образом, явно вызывать метод <span class="b">close()</span> необходимо только для ограниченной и общей арен, а для глобальной и автоматической вызов метода <code>close()</code> сгенерирует исключение.</p>
<p>После успешного выполнения метода <span class="b">close()</span>  доступ ко всем связанным с ареной сегментам памяти станет невозможным, а область памяти вне кучи, связанная с ареной, будет освобождена.:</p>
<pre class="brush:java;">
Arena arena = Arena.ofConfined();   // создаем арену
// неокторая работа с ареной
arena.close();  // закрываем арену
</pre>
<p>Но поскольку класс арены реализует интерфейс <code>AutoCloseable</code> (в виде метода <code>close()</code>), то арену удобно создавать и использовать через конструкцию <code>try-с ресурсами</code>, которая автоматически 
освобождает память арены:</p>
<pre class="brush:java;">
...
<h2>Сегменты памяти MemorySegment</h2>
<p>Сегмент памяти представляет непрерывный блок памяти. На уровне кода Java сегмент памяти представлен интерфейсом <span class="b">MemorySegment</span>. Сегмент памяти может находиться в куче Java, управляемой сборщиком мусора, или быть выделен вне кучи Java. Сегмент памяти вне кучи имеет 
фиксированный физический адрес. Сегмент кучи, как и любой объект Java, имеет ссылку, по которой его можно найти, но не имеет фиксированного адреса, поскольку сборщик мусора мог его переместить.</p>
<p>Для использования с внешними/нативными функциями выделяются сегменты памяти вне кучи.</p>

<p>Для выделения памяти у класса <code>Arena</code> есть ряд методов (в основном реализаций интерфейса <span class="b">java.lang.foreign.SegmentAllocator</span>). 
Некоторые из них:</p>
<ul>
<li><p><code>allocate(long byteSize, long byteAlignment)</code></p>
<p>Этот метод принимает два параметра:</p>
<ul>
<li><p><code>byteSize</code>: размер выделяемой памяти в байтах</p></li>
<li><p><code>byteAlignment</code>: выравнивание памяти в байтах</p></li>
</ul>
<p>Метод возвращает сегмент памяти заданного размера (первый параметр), адрес которого выравнивается исходя из значения второго параметра.</p>
</li>

<li><p><code>allocateFrom(String str)</code></p>
<p>Преобразует строку Java в строку C, завершающуюся нулевым символом, используя кодировку UTF-8, и сохраняет результат в сегмент памяти.</p>
</li>

<li><p><code>allocateFrom(ValueLayout.of<i>Type</i> elementLayout, Type[] elements)</code></p>
<p>Выделяет память для хранения элементов заданного примитивного типа <code>Type</code> (это может быть <code>int</code>, <code>float</code> и т.д.).</p>
</li>
<li><p><code>allocateFrom(ValueLayout.of[Type] elementLayout, Type element)</code></p>
<p>Аналогичен предыдущему варианту, только выделяет память для хранения одного элемента element заданного примитивного типа <code>Type</code> и сохраняет значение element в выделенной памяти</p>
</li>

<li><p><code>allocate(MemoryLayout layout)</code></p>
<p>Выделяет память вне кучи с заданным макетом layout.</p>
</li>

<li><p><code>allocate(MemoryLayout layout, long count)</code></p>
<p>Выделяет память вне кучи с заданным макетом layout с количеством элементов равным count</p>
</li>
</ul>
<p>Все эти методы возвращают объект типа <span class="b">MemorySegment</span>. Пример применения метода <span class="b">allocate()</span>:</p>
<pre class="brush:java;">
...
<p>Метод <code>allocateFrom()</code> является наиболее простым способом для сохранения в сегменте памяти вне кучи какой-нибудь строки:</p>
<pre class="brush:java;">
import java.lang.foreign.*;
...
<p>Здесь происходит кодирование строки Java в UTF-8 и выделение сегмент памяти, который содержит байты строки, включая нулевой терминатор.</p>


<h3>Осовобождение памяти</h3>
<p>Для освобождения сегмента памяти в ограниченной или общей арене необходимо закрыть эту арену с помощью метода <code>close()</code>.</p>
<p>В автоматической арене для освобождения сегмента сегменту передается значение <code>null</code>, и затем в какой-то момент времени сборщик мусора освободит память:</p>
<pre class="brush:java;">
...
<p>Наконец, сегменты памяти глобальной арены в процессе работы программы никогда не освобождаются.</p>
<p>Стоит также отметить, что освободить отдельные сегменты памяти невозможно.</p>
...
<style>
	...
	<div class="nav"><p><a href="./16.2.php">Назад</a><a href="./">Содержание</a><a href="./16.4.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
