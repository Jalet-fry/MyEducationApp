...
<html  lang="ru">
<head>
<title>Java | Конструкция и выражение switch</title>
<meta charset="utf-8" />
<meta name="description" content="Конструкция и выражение switch в языке программирования Java, блоки case">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Конструкция и выражение switch</h1><div class="date">Последнее обновление: 16.09.2025</div>
	<div class="socBlock">
...
<p>Конструкция <span class="b">switch/case</span>  оценивает некоторое выражение и сравнивает его значение с набором значений. И при совпадении значений выполняет определенный код. 
Конструкция <span class="b">switch/case</span> в некоторой степени аналогична конструкции <code>if/else</code>, так как позволяет обработать сразу несколько условий.</p>
<p>В классическом виде конструкция <code>switch</code> имеет следующее формальное определение: </p>
...
<p>В конце каждого блока <code>сase</code> должен ставиться один из операторов перехода: 
<span class="b">break</span>, <span class="b">return</span> или <span class="b">throw</span>. 
...
<pre class="sh">Ваше имя - Tom</pre>

<p>Если бы мы не использовали бы в данном случае операторы <code>break</code>, как в следующем случае:</p>
<pre class="brush:java;">
String name = "Tom";
...
<p>То после выполнения блока</p>
<pre class="brush:java;">
case "Tom":
    System.out.println("Ваше имя - Tom");
</pre>
<p>Стал бы выполняться блок</p>
case "Sam":
    System.out.println("Ваше имя - Sam");
</pre>
<p>И мы бы получили следующий консольный вывод:</p>
<pre class="sh">
Ваше имя - Tom
Ваше имя - Sam
</pre>
...
<h4>Новая форма конструкции switch</h4>
<p>В классической версии конструкции <code>switch</code> каждый блок <code>case</code>, если мы хотим закончить на нем выполнение, завершается оператором <code>break</code>. И обычно так и есть, редко когда требуется выполнять 
другие блоки case. И поэтому в результате развития Java в язык была добавлена новая форма конструкции <code>switch</code>:</p>
<pre class="brush:java;">
switch (выражение)
{
    case значение1 -&gt;
...
<p>В данной форме блок <code>default</code> также необязателен. Например, перепишем первый пример из текущей статьи:</p>
<pre class="brush:java;">
class Program {
...
<p>Если в блок <code>case</code> надо поместить несколько инструкций или более сложный код, то он оформляется в блок кода:</p>
<pre class="brush:java;">
case "Tom" -&gt; {
  System.out.println("Ваше имя - Tom");
...
<p>Ключевое слово <span class="b">switch</span> также применяется для создания выражения <code>switch</code>, которое позволяет возвращать некоторое значение - то есть получить результат выполнения конструкции switch. 
Для возвращения значения в блоках <span class="b">case</span> может применятся 
оператор <span class="b">return</span>. Например, определим следующую программу:</p>
<pre class="brush:java;">
class Program {
...
<p>Здесь у нас есть номер операции - переменная operation, два числа для выполнения операции - a и b и переменная result для хранения результата операции. В конструкции switch в зависимости от номера операции 
присваиваем переменной result соответствующее значение.</p>
<p>С помощью новой формы конструкции switch мы можем слегка упростить код:</p>
<pre class="brush:java;">
class Program {
...
<p>Тем не менее ключевым моментом в данном случае, является то, что во всех случаях нам надо установить переменную result - передать ей нужный результат операции над числами. И выражения <code>switch</code> 
позволяют упростить эту задачу:</p>
<pre class="brush:java;">
class Program {
...
<p>В данном случае мы напрямую присваиваем переменной result то значение, которое вычисляет выражение switch. Вычисляемое значение ставится после стрелки в блоке <code>case</code></p>
<pre class="brush:java;">case 1 -> a + b;</pre>
<p>Благодаря этому код стал проще и понятнее.</p>
<p>Но, следует отметить, что блоки <code>case</code> должны охватывать все потенциальные значения. Например, в данном случае случае мы получаем значение в переменную result - переменную типа <code>int</code>. 
Тип <code>int</code> охватывает 32<sup>2</sup> значений. Естественно в данном случае для каждого из них определять свой блок <code>case</code> нерационально, тем более нам нужны только три значения - для каждой операции. 
И в этом случае для всех остальных значений применяется выражение <code>default</code>, которое в данном случае просто возвращает 0.</p>
<p>Если же нам надо выполнить несколько инструкций в каждом блоке <code>case</code>, то инструкции оформляются в отдельный блок кода:</p>
<pre class="brush:java;">
class Program {
...
<p>При этом для возвращения значения из блока кода применяется оператор <span class="b">yield</span>, после которого ставится возвращаемое значение:</p>
<pre class="brush:java;">yield a - b;</pre>

<p>Стоит отметить, что также существует другая форма выражения switch, где после операторов <code>case</code> ставится двоеточие:</p>
<pre class="brush:java;">
class Program {
...
<p>Но в этом случае в конце блока <code>case</code> надо обязательно использовать оператор <code>yield</code> для возвращения значения</p>
...
<style>
	...
	<div class="nav"><p><a href="./2.4.php">Назад</a><a href="./">Содержание</a><a href="./3.1.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
