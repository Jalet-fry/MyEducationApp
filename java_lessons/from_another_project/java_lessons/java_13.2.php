...
<html  lang="ru">
<head>
<title>Java | Создание и подключение библиотеки JAR</title>
<meta charset="utf-8" />
<meta name="description" content="Создание библиотеки JAR и ее и подключение в программе на языке программирования Java, утилита jar и создание JAR-файлов">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Создание и подключение библиотеки</h1><div class="date">Последнее обновление: 21.09.2025</div>
	<div class="socBlock">
...
<p>Если мы хотим некоторый функционал повторно использовать в различных проектах или распространять для использования другими разработчиками в их проектах, то мы можем оформить этот функционал в виде библиотеки. 
В Java популярным форматом распространения библиотеки является <b>JAR</b>-файл. Рассмотрим, как мы можем создать такую библиотеку и подключить ее в свой проект.</p>
<h3>Компиляция классов</h3>
<p>Создадим простейшую библиотеку с одним классом, который будет выполнять математические операции. Сначала определим на диске папку для проекта. Например, в моем случае это папка 
<code>workspace/java/mathlib</code>. В папке проекта создадим каталог <span class="b">math</span>, а в нем - файл <span class="b">Operation.java</span>. То есть у нас будет класс Operation, который будет принадлежать 
пакету "math" (поэтому внутри проекта мы помещаем файл <span class="b">Operation.java</span> в папку "math").</p>
<p>В файле <span class="b">Operation.java</span> определим следующий код:</p>
...
<p>Сначала нужно скомпилировать этот файл в класс - файл с расширением <code>.class</code>. Для этого перейдем в консоли в корневой каталог проекта (в моем случае <code>workspace/java</code>) и 
выполним следующую команду:</p>

<pre class="sh">
javac -d . math/Operation.java
</pre>
<p>Параметр <span class="b">-d .</span> указывает, что скомпилированные файлы с расширением <code>.class</code> должны быть помещены в текущий каталог, сохраняя структуру пакетов.</p>
<h3>Создание JAR-файла</h3>

<p>Теперь, когда у нас есть скомпилированные классы, можно упаковать их в JAR-файл. Для этого применяется утилита <b>jar</b>:</p>

<pre class="sh">
jar -cvf mathlib.jar math/
</pre>
<p>Параметры утилиты jar:</p>
<ul>
<li><p><code>-c</code>: создает новый JAR-файл</p></li>
<li><p><code>-v</code>: выводит подробную информацию о процессе (verbose)</p></li>
<li><p><code>-f</code>: устанавливает имя создаваемого файла (<code>mathlib.jar</code>)</p></li>
<li><p><code>math/</code>: каталог, который нужно добавить в архив (содержит скомпилированные классы)</p></li>
</ul>
<p>В результате выполнения этой команды в каталоге появится файл <code>mathlib.jar</code>, который содержит скомпилированные классы (класс Operaton). Полный вывод команды:</p>
<pre class="sh">
...
<h3>Подключение JAR-файла к программе на Java</h3>
<p>Теперь, когда у нас есть библиотека <code>mathlib.jar</code>, создадим отдельный проект и используем ее. Для проекта определим на диске какую-нибудь новую папку с файлом <span class="b">Program.java</span>. 
Пусть в этом файле будет следующий код:</p>
<pre class="brush:java;">
...
<p>Здесь с помощью выражения импорта подключаем класс "math.Operation" (класс Operation располагается в пакете "math") и затем используем его в программе, вызывая один из его методов.</p>
<p>Также скопируем файл библиотеки <span class="b">mathlib.jar</span> в папку проекта, чтобы не прописывать полный путь к библиотеке. То есть в папке проекта у нас будет два файла:</p>
<ul>
<li><p><span class="b">Program.java</span></p></li>
<li><p><span class="b">mathlib.jar</span></p></li>
</ul>
<p>Чтобы скомпилировать файл <span class="b">Program.java</span>, компилятору нужно знать, где найти класс Operation. Для этого при компиляции можно использовать опцию <span class="b">-cp</span>:</p>

<pre class="sh">
javac -cp mathlib.jar Program.java
</pre>
<p>Параметр <code>-cp mathlib.jar</code> указывает, что компилятор должен искать классы не только в текущем каталоге, но и в файле <code>mathlib.jar</code> (jar-файл выступает в роли каталога).</p>
...
<p>После успешной компиляции можно запустить программу, также указав путь к JAR-файлу:</p>
...
<p>Для выполнения программы здесь применяется параметр <code>-cp mathlib.jar:.</code> (для Linux) или <code>-cp mathlib.jar;.</code> (для Windows), который указывает, что путь к классам должен включать "mathlib.jar" и текущий каталог ("."). Это важно, так как 
JVM должна найти и <code>Program.class</code> (который располагается в текущем каталоге), и <code>Operation.class</code> (который располагается в архиве mathlib.jar).</p>
<p>И после запуска мы увидим результат работы программы:</p>
<pre class="sh">
...
<h2>Использование в IDE (IntelliJ IDEA, Eclipse) </h2>
<p>Большинство разработчиков используют интегрированные среды разработки (IDE), где процесс подключения JAR-файлов значительно проще.</p>
<p>Установка в IntelliJ IDEA:</p>
<ol>
<li><p>Откройте пункт меню <span class="b">File -&gt; Project Structure...</span></p></li>
<li><p>Перейдите в раздел <span class="b">Modules</span> и выберите ваш модуль.</p></li>
<li><p>Нажмите на вкладку <span class="b">Dependencies</span>.</p></li>
<li><p>Нажмите <code>+</code> и выберите <span class="b">JARs or directories...</span>.</p></li>
<li><p>Найдите файл jar-библиотеки и нажмите "OK".</p></li>
</ol>

<p>Установка в Eclipse:</p>
<ol>
<li><p>Кликните правой кнопкой мыши по вашему проекту в <span class="b">Package Explorer</span>.</p></li>
<li><p>Выберите <span class="b">Build Path -&gt; Configure Build Path...</span>.</p></li>
<li><p>Перейдите на вкладку <span class="b">Libraries</span>.</p></li>
<li><p>Нажмите <span class="b">Add External JARs...</span>.</p></li>
<li><p>Найдите файл jar-библиотеки и нажмите <span class="b">Open</span>.</p></li>
</ol>
<p>После этих шагов IDE автоматически добавит JAR-файл в путь компиляции и запуска.</p>
...
<style>
	...
	<div class="nav"><p><a href="./13.1.php">Назад</a><a href="./">Содержание</a><a href="./13.4.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
