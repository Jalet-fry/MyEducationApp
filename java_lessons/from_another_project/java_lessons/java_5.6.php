...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Интерфейсы Comparable и Comparator. Сортировка</h1><div class="date">Последнее обновление: 01.10.2025</div>
	<div class="socBlock">
	...
	<p>Сортировка - одна из распротсраненных задач при работе с данными. В Java в различных структурах данных есть встроенная поддержка сортировки. Однако для поддержки сортировки обычно требуется реализация двух интерфейсов: 
<b>Comparable</b> и <b>Comparator</b></p>
<h2>Интерфейс Comparator</h2>
<p>Многие встроенные коллекции в Java имеют методы сортировки, которые обычно называются <code>sort()</code>. Например, в интерфейсе <code>List</code>, который определяет базовый функционал для коллекций-списков, 
есть подобный метод:</p>
<pre class="brush:java;">
default void sort(Comparator&lt;? super E&gt; c)
</pre>
<p>То есть, чтобы отсортировать список, нам надо в метод <code>sort()</code> передать <span class="b">компаратор</span> - объект типа <b>Comparator</b>, который типизирован типом элементов списка. 
Компаратор позволяет сравнить два объекта.</p>
<p>Интерфейс <b>Comparator</b> содержит ряд методов, ключевым из которых является метод <span class="b">compare()</span>:</p>
<pre class="brush:java;">   
...
<p>Метод <code>compare</code> принимает два сравниваемых объекта и возвращает числовое значение в зависимости от результата сравнения:</p>
<ul>
<li><p>Если объект a предшествует объекту b (грубо говоря, <code>a &lt; b</code>), то возвращается отрицательное значение</p></li>
<li><p>Если объект a идет после объекта b (<code>a &gt; b</code>), то возвращается положительное значение</p></li>
<li><p>Если объекты равны (<code>a == b</code>), то возвращается ноль</p></li>
</ul>
<p>Для применения интерфейса необходимо создать класс компаратора, который реализует этот интерфейс. Например, у нас есть следующий класс Person:</p>
<pre class="brush:java;">
class Person{
...
<p>Допустим, мы хотим сравнивать два объекта Person, благодаря чему будет производиться сортировка. Для этого определим следующий класс компаратора:</p>
<pre class="brush:java;">
...
<p>Здесь в реализации мы сравниваем по полю name. Для этого используем метод <code>compareTo()</code>, который есть у строк и который позволяет сравнить две строки. Это удобно, поскольку нам не надо дополнительно определять логику сравнения символов строк, а можно воспользоваться готовым функционалом. То есть, если <code>a.name</code> условно меньше (в лексекографическом поряке размещается ранее), чем 
<code>b.name</code>, то метод <code>compareTo()</code> возвращает отрицательное число. Если <code>a.name</code> условно больше, чем <code>b.name</code>, то возвращается положительное число. Если строки равны, то возвращается 0.</p>
<p>Хотя в реальности мы могли бы определить и свою логику, например, сравнивать по длине имени:</p>
<pre class="brush:java;">
class PersonNameComparator implements Comparator&lt;Person&gt;{
...
<p>Посмотрим, как все это используется:</p>
<pre class="brush:java;">
...
<p>Здесь мы определяем список объектов Person и используем компаратор PersonNameComparator для сортировки списка:</p>
<pre class="brush:java;">people.sort(new PersonComparator());</pre>
<p>Благодаря этому при выводе на консоль мы увидим список, отсортированный в зависимости от поля name:</p>
<pre class="sh">
Person Name: Bob;  Age: 46;
Person Name: Sam;  Age: 28;
Person Name: Tom;  Age: 41;
</pre>
<p>Аналогично мы могли бы определить компаратор для сравнения по полю age:</p>
<pre class="brush:java;">
import java.util.ArrayList; 
import java.util.Comparator; 
...
<p>Здесь в качестве компаратора применяется класс PersonAgeComparator, логика которого тоже не отличается сложностью: просто возващаем результат выражения <code>a.getAge() - b.getAge()</code>. 
Что фактически эквивалентно, если бы мы расписали это следующим образом:</p>
<pre class="brush:java;">
class PersonAgeComparator implements Comparator&lt;Person&gt;{
...
<p>Но операция вычитания <code>a.getAge() - b.getAge()</code> гораздо проще и производительнее, чем условная конструкция.</p>
<p>В итоге теперь мы получим другой вывод - с сортировкой по полю age:</p>
<pre class="sh">
Person Name: Sam;  Age: 28;
Person Name: Tom;  Age: 41;
Person Name: Bob;  Age: 46;
...
<h3>Сочетание компараторов</h3>
<p>Также мы можем применять сразу несколько компараторов по принципу приоритета. Например, мы хотим, чтобы сначала сортировка шла по полю name, а если есть несколько объектов с одинаковым полем name, то применять сортировку по полю age.</p>
<p>Интерфейс компаратора определяет специальный метод <span class="b">thenComparing()</span>, который позволяет использовать цепочки компараторов для сортировки набора::</p>
<pre class="brush:java;">
import java.util.ArrayList; 
import java.util.Comparator; 
...
<p>В данном случае с помощью вызовов</p>
<pre class="brush:java;">PersonNameComparator().thenComparing(new PersonAgeComparator());</pre>
<p>создаем цепочку компараторов в виде условно общего компаратора totalComparator, который затем используется для сортировки. В итоге теперь мы получим следующий консольный вывод:</p>
<pre class="sh">
...
<h2>Интерфейс Comparable</h2>
<p>Кроме интерфейса <code>Comparator</code> для сравнения объектов применяется другой интерфейс - <span class="b">Comparable</span>. Данный интерфейс довольно прост, определяя один метод <code>compareTo()</code>:</p>
<pre class="brush:java;">
...
<p>Этот метод служит тем же целям, что и метод <code>compare</code> в интерфейсе <code>Comparator</code>. Метод <code>compareTo()</code> принимает объект, сравнивает его с текущим и возвращает числовое 
значение в зависимости от результата сравнения:</p>
<ul>
<li><p>Если текущий объект меньше объекта other, то возвращается отрицательное значение</p></li>
<li><p>Если текущий объект больше объекта other, то возвращается положительное значение</p></li>
<li><p>Если объекты равны, то возвращается ноль</p></li>
</ul>
<p>Некоторые встроенные классы в JDK уже по умолчанию реализуют этот интерфейс. Так, выше в примере с компаратором PersonNameComparator для сравнения строк мы использовали метод <code>compareTo()</code></p>
<pre class="brush:java;">return a.getName().compareTo(b.getName());</pre>
<p>То есть в данном случае мы как раз и использовали реализацию интерфейса <code>Comparable</code> в строках. И мы можем использовать ее при сортировке объектов, например, в том же списке:</p>
<pre class="brush:java;">
import java.util.ArrayList; 
...
<p>Здесь мы определяем и сортируем список строк. Консольный вывод:</p>
<pre class="sh">
Bob
Sam
Tom
</pre>
<p>Почему здесь выполняется сортировка? Если в метод <code>sort()</code> для компаратора передается значение <code>null</code>, то для выполнения сортировки все элементы в этом списке должны 
реализовывать интерфейс <code>Comparable</code>. И эта реализация и будет применяться для сортировки. То есть в случае выше для сортировки строк неявно применяется реализация интерфейса Comparable.</p>

<p>И мы также можем реализовать данный интерфейс и в своих кастомных классах:</p>
<pre class="brush:java;">
...
<p>В данном случае мы не возвращаем явным образом никакое число, а опять же полагаемся на встроенный механизм сравнения, который есть у класса String.</p>
<p>Применение в программе:</p>
<pre class="brush:java;">
import java.util.ArrayList; 
...
<style>
	...
	<div class="nav"><p><a href="./5.2.php">Назад</a><a href="./">Содержание</a><a href="./5.7.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
