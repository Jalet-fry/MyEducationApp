...
<html  lang="ru">
<head>
<title>Java | Введение в аннотации</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в аннотации в языке программирования Java, встроенные аннотации @Override, @Deprecated, @SuppressWarnings, @FunctionalInterface">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Введение в аннотации</h1><div class="date">Последнее обновление: 01.11.2025</div>
	<div class="socBlock">
...
<p>Аннотации в языке программирования Java представляют специальную форму <span class="b">метаданных</span> (данных о данных), которую можно добавлять в исходный код. Они предоставляют дополнительную информацию о программе, которая может использоваться компилятором, инструментами сборки, фреймворками или во время выполнения. 
Аннотации позволяет отделить конфигурацию и метаданные от основной бизнес-логики. Они делают код более чистым, декларативным и легким для поддержки, перенося большую часть "магии" настройки на фреймворки и инструменты. Можно сказать, что аннотации являются основой многих современных Java-технологий, в частности, веб-фреймворков, фреймворков для тестирования, валидации и т.д.</p>
<p>Сами по себе аннотации не выполняют никаких действий. Они лишь "маркируют" классы, методы, поля и другие элементы кода, а уже другой код (обработчик аннотаций) ищет эти маркеры и выполняет на их основе некоторую определенную логику.</p>
<p>Синтаксически аннотация начинается с символа <b>@</b>, за которым следует ее имя, например, <code>@Override</code>.</p>
<p>В языке Java есть ряд встроенных аннотаций, которые используются для стандартных задач. Рассмотрим некоторые наиболее используемые:</p>
<ul>
<li><p><b>@Override</b>: указывает компилятору, что текущий метод <span class="b">переопределяет</span> метод из суперкласса.</p></li>
<li><p><b>@Deprecated</b>: эта аннотация помечает элемент (класс, метод, поле) как <span class="b">устаревший</span></p></li>
<li><p><b>@SuppressWarnings</b>: отключает определенные <span class="b">предупреждения компилятора</span> для аннотированного элемента.</p></li>
<li><p><b>@FunctionalInterface</b>: указывает, что интерфейс предназначен для использования в качестве <span class="b">функционального интерфейса</span> (т.е. он должен иметь ровно один абстрактный метод).</p></li>
</ul>

<h3>Применение аннотаций</h3>
<p>Аннотации объявлений могут применяться ко многим компонентам. И в зависимости от места применения аннотации делятся на 2 группы:</p>
<ul>
<li><p><span class="b">аннотации объявлений</span> (declaration annotation)</p></li>
<li><p><span class="b">аннотации использования типов</span> (type uses annotation)</p></li>
</ul>
<h3>Аннотации объявлений</h3>
<p>Аннотации объявлений содержат некоторую информацию об объявляемом элементе и могут применяться к следующим объявлениям:</p>
<ul>
<li><p>К классам (включая перечисления) и интерфейсам (в том числе к интерфейсам аннотаций)</p></li>
<li><p>К методам</p></li>
<li><p>К конструкторам</p></li>
<li><p>К полям (в том числе к константам перечислений и полям классов record).</p></li>
<li><p>К локальным переменным (в том числе к объявленным в конструкциях <code>for</code> и <code>try-with-resources</code>).</p></li>
<li><p>К параметрам методов</p></li>
<li><p>К параметрам типов</p></li>
<li><p>К пакетам и модулям</p></li>
</ul>
<p>Для классов, интерфейсов и модулей аннотации размещаются перед ключевым словом <b>class</b>, <b>interface</b> или <b>module</b> и любым модификаторам (при его наличии):</p>
<pre class="brush:java;">@MyAnnotation public class Person { }</pre>

<p>Для переменных, параметров, а также поле классов-record аннотации размещаются перед типом:</p>
<pre class="brush:java;">
@MyAnnotation String name = "Tom";
public User getUser(@MyAnnotation int userId)
record Person(@MyAnnotation String name) {}
</pre>
<p>Также аннотация помещается перед параметром типа в обобщенном классе или методе:</p>
<pre class="brush:java;">
class Person&lt;@MyAnnotation T&gt; { }
</pre>
<h3>Аннотации использования типов</h3>
<p>Аннотации использования типов могут появляться в следующих местах:</p>
<ul>
<li><p>С аргументами обобщенного типа:</p>
<pre class="brush:java;">Person&lt;@MyAnnotation String&gt;</pre>
</li>


<li><p>Перед скобками массива:</p>
<pre class="brush:java;">
String[] @MyAnnotation [] data // Применяется к String[]
String @MyAnnotation [][] data // Применяется к String[][]
</pre>
<p>От эих ситуаций надо отличать следующую:</p>
<pre class="brush:java;">@MyAnnotation String[][] data</pre>
<p>Здесь аннотация применяется к String</p>
</li>
<li><p>С суперклассами и реализованными интерфейсами:</p>
<pre class="brush:java;">class Employee extends @MyAnnotation Person.</pre>
</li>

<li><p>С вызовами конструктора</p>
<pre class="brush:java;"> new @MyAnnotation Person();</pre>
</li>
<li><p>С вложенными типами</p>
<pre class="brush:java;"> Map.@MyAnnotation Entry</pre>
</li>

<li><p>С приведениями типов и проверками <code>instanceof</code>:</p>
<pre class="brush:java;">
(@MyAnnotation String) text
if (text instanceof @MyAnnotation String)
</pre>
<p>Аннотации предназначены только для использования внешними инструментами. Они не влияют на поведение приведения типов или проверки <code>instanceof</code>.</p>
</li>

<li><p>С указанием генерируемых исключений</p>
<pre class="brush:java;">void print() throws @MyAnnotation Exception.</pre>
</li>

<li><p>С подстановочными типами и ограничениями типов</p>
<pre class="brush:java;">
List&lt;@MyAnnotation ? extends Person&gt;
List&lt;? extends @MyAnnotation Person&gt;
</pre></li>
<li><p>С ссылками на методы и конструктор:</p>
<pre class="brush:java;">@MyAnnotation Person::getName</pre></li>
</ul>
<p>Аннотации можно помещать до или после других модификаторов, таких как <code>static</code> и <code>public</code>. 
Обычно аннотации использования типа размещаются после других модификаторов, а аннотации объявлений - перед другими модификаторами. Например:</p>
<pre class="brush:java;">
...
<h2>Встроенные (стандартные) аннотации Java</h2>

<h3>@Override</h3>
<p>Аннотация <b>@Override</b> указывает компилятору, что текущий метод <span class="b">переопределяет</span> метод из суперкласса. Если метод с такой сигнатурой в суперклассе не найден, 
компилятор выдаст ошибку. Это помогает избежать опечаток.</p>
<p>Конечно, при переопределении метода суперкласса в производном классе нам необязательно использовать данную аннотацию. Но рассмотрим следующую программу:</p>
...
<p>Класс Animal представляет животное и определяет метод <code>makeSound()</code> для имитации звука животного. Класс Cat переопределяет этот метод. А в методе <code>main()</code> вызываем этот метод. Но посмотрим на 
консольный вывод программы:</p>
<pre class="sh">Некий звук</pre>
<p>Результат, очевидно, не тот, который ожидался. А почему? А потому что я допустил опечатку: вместо "makeSound" в классе Cat определил метод "makeSound<span class="b">s</span>". В итоге я по сути определил новый метод, 
который ничего не переопределяет. Но программа была успешно скомпилирована и успешно выполнила все свои действия! И в том числе для предупреждения подобных ситуаций предназначена аннотация <b>@Override</b>. Применим ее:</p>
<pre class="brush:java;">
class Animal {
    void makeSound() {
        System.out.println("Некий звук");
...
<p>В производном классе Cat попрежнему метод с <b>опечаткой</b>: "makeSound</code><span class="b">s</span>" вместо "makeSound". Однако? применив к методу аннотацию <b>@Override</b>, мы указываем, 
что в суперклассе должен быть метод с подобным именем, и мы его хотим переопределить. Но поскольку в суперклассе нет метода "makeSounds", то при компиляции теперь мы получим ошибку:</p>
<pre class="sh">
...
<p>Таким образом, мы увидим ошибку и сможем своевременно ее исправить, недопустив некорректного выполнения программы:</p>
<pre class="brush:java;">
class Cat extends Animal {
...
<h3>@Deprecated</h3>
<p>Аннотация <b>@Deprecated</b> помечает элемент (класс, метод, поле) как <span class="b">устаревший</span>, что может быть полезно, если после обновления API класса или пакета, мы хотим, что какие-то компоненты больше не использовались.
 Компилятор будет выдавать предупреждение при каждой попытке использовать такой компонент. Например:</p>
 <pre class="brush:java;">
class Cat{
...
<p>Здесь в классе Cat определен метод <code>makeSound()</code>, который с помощью аннотации <code>@Deprecated</code> объявлен как устаревший (допустим, мы хотим, чтобы в будущем разработчики использовали метод 
<code>say()</code>, который дублирует функциональность метода <code>makeSound()</code>). Данная программу успешно скомпилируется и будет выполняться, но при компиляции компилятор выдаст предупреждение:</p>
<pre class="brush:java;">
...
<p>Аннотация <b>@SuppressWarnings</b> отключает определенные <span class="b">предупреждения компилятора</span> для аннотированного элемента. Например:</p>
...
<p>Эта программа успешно скомпилируется и выполняется, однако при компиляции мы получим предупреждение:</p>
<pre class="sh">
eugene@Eugene:/workspace/java$ java Program.java
...
<p>Проблема в том, что нам надо указать тип хранимых данных для ArrayList, наподобие следующего:</p>
<pre class="brush:java;">List&lt;String&gt; names = new ArrayList&lt;String&gt;();</pre>
<p>Однако мы также можем применить аннотацию <b>@SuppressWarnings</b> для отключения подобного (и других) предупреждений. Для этого после названия аннотации в скобках надо передать строку с типом предупреждения. 
В примере выше мы тип предупреждения - "unchecked", соответственно нам надо написать:</p>
<pre class="brush:java;">
...
<p>Аннотация <b>@FunctionalInterface</b> указывает, что интерфейс предназначен для использования в качестве <span class="b">функционального интерфейса</span> (т.е. он должен иметь ровно один абстрактный метод). Компилятор проверяет это условие.</p>
<pre class="brush:java;">
@FunctionalInterface
...
<style>
	...
	<div class="nav"><p><a href="./14.7.php">Назад</a><a href="./">Содержание</a><a href="./15.2.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
