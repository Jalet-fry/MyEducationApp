...
<meta charset="utf-8" />
<meta name="description" content="Синхронизация потоков в языке программирования Java, оператор synchronized, состояние гонки race condition">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Синхронизация потоков. Оператор synchronized</h1><div class="date">Последнее обновление: 22.10.2025</div>
	<div class="socBlock">
...
<p>При работе потоки нередко обращаются к каким-то общим ресурсам, которые определены вне потока, например, обращение к какой-то переменной или даже файлу. 
...
<p>То есть в итоге мы ожидаем, что после выполнения цикла <code>res.x</code> будет равно 4.</p>
<p>В главном классе программы запускается пять потоков. То есть мы ожидаем, что каждый поток будет увеличивать значение <code>res.x</code> с 1 до 4 и так пять раз. Но 
...
<p>То есть пока один поток не окончил работу с полем <code>res.x</code>, с ним начинает работать другой поток. Эта ситуация называется <b>состоянием гонки</b> (<b>race condition</b>), при котором два и более потоков борятся за управление одинм и тем же ресурсом. 
...
подобное состояние гонки может существенно нарушить корректность данных.</p>
...
<span class="b">synchronized</span>. Этот оператор предваряет блок кода или метод, который подлежит синхронизации.</p>
<h2>synchronized-блоки</h2>
<p>synchronized-блок или синхронизированный блок представляет блок кода, к которому одномоментно имеет доступ только один поток. Синхронизированный блок имеет следующую форму:</p>
<pre class="brush:java;">
synchronized (obj)
{
    // критическая секция
...
<p>При создании синхронизированного блока кода после оператора <code>synchronized</code> идет объект-заглушка: <code>synchronized(obj)</code>. Причем в качестве 
объекта может использоваться только объект какого-нибудь класса, но не примитивного типа. Затем идет блок кода, в которую помещается критическая секция - код, к которому в одним момент времени имеет доступ только один поток.</p>
...
управления доступа к объекту. Когда выполнение кода доходит до оператора <code>synchronized</code>, 
монитор объекта блокируется, и на время его блокировки монопольный доступ к блоку кода имеет только один 
поток, который и произвел блокировку. После окончания работы блока кода, монитор объекта освобождается и становится доступным для других потоков.</p>
...
<p>Например изменим предудущий пример, применив синхронизированные блоки для синхронизации потоков:</p>
<pre class="brush:java;">
public class Program {
...
<p>И так, у нас есть некоторый участок кода, к которому надо дать доступ одномоментно только одному потоку. В примере выше это код, связанный с изменением общего ресурса - res. Поэтому эту часть кода оформляем в синхронизированный блок:</p>
<pre class="brush:java;">
synchronized(res){  // определяем синхронизированный блок
...
<p>Первый из потоков, который начнет выполнять этот код, захватит объект <code>res</code> и начнет выполнять код <code>synchronized</code>-блока. Все остальные потоки в это время будут ждать освобождения ресурса res. 
Когда первый поток завершит выполнение кода в блоке, ресурс <code>res</code> освобождается, и его захватывает другой (произвольный) поток. И опять остальные потоки будут ожидать его освобождения. И так далее, пока все потоки не выполнят код 
<code>synchronized</code>-блока. В итоге консольный вывод изменится и станет упорядоченным:</p>
<pre class="sh">
Thread 1 1 
Thread 1 2
Thread 1 3
...
<h2>Синхронизированные методы</h2>
<p>При применении оператора <code>synchronized</code> к методу, как и в случае с блоком, монопольный доступ к методу (выполнять метод в один момент времени) может только один поток - первый, 
который начал его выполнение.  Для применения оператора <b>synchronized</b> к методу, изменим классы программы:</p>
<pre class="brush:java;">
public class Program {
...
<p>Результат работы в данном случае будет аналогичен примеру выше с блоком <b>synchronized</b>. Здесь опять в дело вступает монитор объекта CommonResource - общего объекта для всех потоков. Поэтому синхронизированным объявляется не 
...
<style>
	...
	<div class="nav"><p><a href="./8.12.php">Назад</a><a href="./">Содержание</a><a href="./8.5.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
