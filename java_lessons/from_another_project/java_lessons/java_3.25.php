...
<html  lang="ru">
<head>
<title>Java | Sealed-классы и интерфейсы</title>
<meta charset="utf-8" />
<meta name="description" content="Sealed-классы и интерфейсы в языке программирования Java, ключевое слово sealed, non-sealed, ограничение круга наследования">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Sealed-классы и интерфейсы</h1><div class="date">Последнее обновление: 27.09.2025</div>
	<div class="socBlock">
...
<p><span class="b">sealed-классы</span> (или запечатанные классы) и <span class="b">sealed-интерфейсы</span> (или запечатанные интерфейсы) в языке Java управляют тем, какие именно классы могут расширять данный класс или интерфейс. 
 Эта функция вносит уровень контроля и предсказуемости в иерархии наследования, который ранее был недоступен, кроме как через менее гибкие механизмы, такие как <code>final</code> или ограничения доступа.</p>
<p>Прежде всего встает вопрос: зачем же нужны <span class="b">sealed-классы</span>? До появления <code>sealed-классов</code> в Java иерархия классов могла быть либо полностью открытой для расширения 
(любой класс мог наследоваться), либо быть полностью закрытым для наследования с помощью модификатора <b>final</b>. <code>Sealed-классы</code> предлагают золотую середину. Они позволяют контролировать расширяемость - явно указать, какие классы могут быть прямыми подклассами, 
предотвращая неожиданное и несанкционированное наследование. Кроме того, они предоставляют эффективный способ моделирования ADT (Алгебраических типов данных) в Java, где базовый тип имеет фиксированный набор вариантов.</p>

<p>Чтобы объявить класс или интерфейс как <code>sealed</code>, используются два ключевых слова: <b>sealed</b> и <b>permits</b>:</p>
...
<p>Ключевое слово <span class="b">sealed</span> применяется к суперклассу или интерфейсу для объявления его запечатанным.</p>
<p>После имени класса указывается ключевое слово <span class="b">permits</span>, за которым следует список классов, которым разрешено быть прямыми наследниками. Далее будем также называть эти классы <b>разрешенными классами</b>.</p>
...
<p>В данном случае мы разрешаем классам Sum, Subtract и Multiply быть наследниками класса Operation. Однако для всех остальных классов установлен 
запроет на наследование. Например, в следующем случае мы столкнемся с ошибкой на этапе компиляции:</p>
<pre class="brush:java;">
sealed class Operation
    permits Sum, Subtract, Multiply
{ }
...
<h3>Ограничения для разрешенных подклассов</h3>
<p>Классы, перечисленные после слова <b>permits</b>, также должны следовать определенным правилам. Прежде всего должны соблюдаться правила расположения классов:</p>
<ul>
<li><p>Разрешенные подклассы sealed-класса должны быть доступны. Они не могут быть приватными классами, вложенными в другой класс, или классами, которые доступны только на уровне другого пакета.</p></li>
<li><p>Разрешенные публичные подклассы должны находиться в том же пакете, что и sealed-класс. Если применяются модули, то разрешенные подклассы должны находиться в том же модуле, что и <code>sealed-класс</code>.</p></li>
<li><p>sealed-класс может быть объявлен без оператора <code>permits</code>. Тогда все его прямые подклассы должны быть объявлены в одном файле.</p></li>
</ul>
<p>Кроме того, разрешенные классы должны явно объявлять, как они продолжают (или прекращают) иерархию запечатанного класса. Для этого к разрешенному классу применяется один из следующих трех модификаторов:</p>
<ul>
<li><p><span class="b">final</span>: запрещает любое дальнейшее наследование, завершая ветвь иерархии:</p>
<pre class="brush:java;">
sealed class Operation
    permits Sum, Subtract, Multiply { }
...
<li><p><span class="b">sealed</span>: разрешает ограниченное дальнейшее наследование, но только тем классам, которые идут после <code>permits</code>:</p>
<pre class="brush:java;">
...
<li><p><span class="b">non-sealed</span>: открывает этот подкласс для расширения любыми классами, как обычный класс Java, продолжая ветвь иерархии</p>
<pre class="brush:java;">
sealed class Operation permits UnaryOp, BinaryOp{}
...
<h3>Пример sealed-классов</h3>
<p>Рассмотрим простейший пример с sealed-классами:</p>
<pre class="brush:java;">
class Program{
...
<p>Здесь абстрактный класс Operation определен как запечатанный <code>sealed</code>-класс. Он представляет арифметическую операцию, которая хранит два операнда в виде переменных op1 и op2 и определяет абстрактный 
метод <code>execute()</code> для выполнения этой операции. И этот класс могут наследовать только классы Sum и Subtract.</p>
<p>В классах Sum и Subtract соответствующим образом реализуем метод <code>execute()</code> для выполнения соответствующей операции. Причем стоит отметить, что наследование от этих классов запрещено.</p>
...
<p>Рассмотрим более расширенный пример:</p>
<pre class="brush:java;">
class Program{
...
<p>Здесь у нас есть единый sealed-класс, который представляет операцию - Operation. От него могут наследоваться классы BinaryOperation (операция с двумя операндами) и IfOperation (операция с тремя операндами)</p>
<p>Класс IfOperation определен как <code>final</code> и запрещает дальнейшее наследование, он реализует своего рода тернарную операцию.</p>
<p>А вот класс BinaryOperation позволяет унаследоваться двум классам - Sum и Subtract. Причем Subtract определен с модификатором <code>non-sealed</code>, что позволяет наследоваться от него произвольным классам. Как например, классу 
Negation, который реализует операцию унарного минуса.</p>

<h3>Sealed-Интерфейсы</h3>
<p>Концепция sealed-типов также применима к интерфейсам. <span class="b">Sealed-интерфейс</span> может быть реализован или расширен только теми классами или интерфейсами, которые указаны в его предложении 
<code>>permits</code>. Например:</p>
<pre class="brush:java;">
...
<style>
	...
	<div class="nav"><p><a href="./3.30.php">Назад</a><a href="./">Содержание</a><a href="./3.26.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
