...
<html  lang="ru">
<head>
<title>Java | Многопоточное программирование и класс Thread</title>
<meta charset="utf-8" />
<meta name="description" content="Многопоточное программирование в языке программирования Java, понятие потока, класс Thread, состояние и приоритеты потоков, метод Thread.currentThread и получение текущего потока и его свойств">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Многопоточное программирование и класс Thread</h1><div class="date">Последнее обновление: 14.10.2025</div>
	<div class="socBlock">
...
<p>Стоит отметить, что начиная с версии 21 в языке Java есть два типа потоков: <span class="b">потоки платформы</span> (platform thread) и <span class="b">виртуальные потоки</span> (virtual thread). 
<span class="b">Поток платформы</span> представляет поток, предоставляемый операционной системой, которая планирует его выполнение. 
<span class="b">Виртуальные потоки</span> выполняются в потоках платформы и планируются средой выполнения Java.</p>
<p>Каждый процессор может одновременно выполнять один поток платформы. На машине с несколькими процессорами можно запускать несколько потоков параллельно. 
Если количество потоков платформы больше, чем количество доступных процессоров, то каждому запущенному потоку выделяется определенный промежуток времени для выполнения своей задачи. 
Когда этот промежуток времени исчерпывается, операционная система вытесняет поток и предоставляет возможность другому потоку выполнить его.</p>
<h2>Класс Thread</h2>
<p>В языке Java за работу с потоками отвечает класс <span class="b">Thread</span>. Фактически этот класс и представляет функциональность потока. И чтобы создать новый поток, нам надо создать 
объект этого класса.</p>
<p>Но все потоки необходимо создавать вручую. Когда запускается программа, начинает работать <span class="b">главный поток</span> этой программы, в котором и выполняется метод <code>main()</code>. 
От этого главного потока порождаются все остальные дочерние потоки.</p>
<h3>Получение текущего потока</h3>
<p>С помощью статического метода <b>Thread.currentThread()</b> мы можем получить текущий поток выполнения в виде объекта <code>Thread</code>:</p>
...
<p>В данном случае мы получаем текущий поток, в котором запущен метод <code>main</code>, в переменную <code>t</code> и выводим его на консоль. В итоге мы получим консольный вывод следующего вида:</p>
<pre class="sh">Thread[#3,main,5,main]</pre>

<p>Что значит этот вывод? Этот вывод сформирован с помощью метода <code>toString()</code> и означает следующее:</p>
<pre class="brush:java;">идентификатор_потока, имя_потока, приоритет, группа потока</pre>
<p>То есть мы получаем:</p>
<ul>
<li><p><span class="b">идентификатор потока</span>: 3</p></li>
<li><p><span class="b">имя потока</span>: main</p></li>
<li><p><span class="b">приоритет потока</span>: 5</p></li>
<li><p><span class="b">группа потока</span>: main</p></li>
</ul>
<p>Стоит отметить, что с помощью статического метода <span class="b">Thread.getAllStackTraces()</span> мы можем получить трассировку стека для всех активных потоков в программе в 
виде словаря <code>Map&lt;Thread, StackTraceElement[]&gt;</code>. Например, выведем на консоль все активные потоки:</p>
<pre class="brush:java;">
public class Program {
...
<p>В этом случае мы получим консольный вывод что-то вроде следующего:</p>
<pre class="sh">
...
<h3>Получение информации о потоках</h3>
<p>Класс <b>Thread</b> предоставляет ряд методов для управления и получения состояния потоков. Отмечу некоторые из этих методов:</p>
<ul>
<li><p><span class="b">String getName()</span>: возвращает имя потока</p></li>
<li><p><span class="b">void setName(String name)</span>: устанавливает имя потока</p></li>
<li><p><span class="b">int getPriority()</span>: возвращает приоритет потока</p></li>
<li><p><span class="b">void setPriority(int proirity)</span>: устанавливает приоритет потока. Приоритет является одним из ключевых факторов для выбора 
...
<li><p><span class="b">StackTraceElement[] getStackTrace()</span>: возвращает трассировку стека текущего потока в виде массива объектов <code>StackTraceElement</code></p></li>
<li><p><span class="b">void setDaemon(boolean on)</span>: устанавливает, является ли поток фоновым</p></li>
<li><p><span class="b">boolean isAlive()</span>: возвращает true, если поток активен</p></li>
<li><p><span class="b">boolean isInterrupted()</span>: возвращает true, если поток был прерван</p></li>
<li><p><span class="b">boolean isDaemon()</span>: возвращает true, если поток является фоновым (потоком-демоном)</p></li>
<li><p><span class="b">boolean isVirtual()</span>: возвращает true, если поток является виртуальным</p></li>
<li><p><span class="b">long threadId()</span>: возвращает идентификатор текущего потока</p></li>
</ul>

<p>И также можно выделить ряд методов по управлению потоками:</p>
<ul>
<li><p><span class="b">void join()</span>: ожидает завершение потока</p></li>
<li><p><span class="b">void run()</span>: определяет действия, которые собственно и выполняет поток</p></li>
<li><p><span class="b">static void sleep(long millis)</span>: приостанавливает поток на заданное количество миллисекунд</p></li>
<li><p><span class="b">start()</span>: запускает (или ставит в очередь на запуск) поток, вызывая его метод <code>run()</code></p></li>
<li><p><span class="b">static Thread startVirtualThread(Runnable task)</span>: запускает или ставит в очередь на запуск виртуальный поток</p></li>
</ul>

<p>Например, получим информацию о текущем потоке (главном потоке программы):</p>
<pre class="brush:java;">
public class Program {
...
<p>Рассмотрим некоторые свойства потоков подробнее.</p>
<h3>Состояние потоков</h3>
<p>Потоки могут находиться в следующих 6 состояниях:</p>
<ul>
<li><p>
<li><p><code>NEW</code>: поток создан, но еще не запущен</p></li>
<li><p><code>RUNNABLE</code>: поток выполняется</p></li>
<li><p><code>BLOCKED</code>: поток заблокирован в ожидании блокировки монитора</p></li>
<li><p><code>WAITING</code>: поток неопределенно долго ожидает, пока другой поток выполнит определенное действие</p></li>
<li><p><code>TIMED_WAITING</code>: поток, ожидающий выполнения действия другим потоком в течение заданного времени</p></li>
<li><p><code>TERMINATED</code>: поток завершил выполнение</p></li>
</ul>
<p>В любой момент времени поток может находиться только в одном состоянии. Эти состояния представляют собой состояния виртуальной машины, которые не отражают состояния потоков операционной системы.</p>
<h3>Приоритеты потоков</h3>
<p>Каждый поток имеет приоритет. Приоритеты представляют числа от 1 до 10. Для их описания Java представляет три константы:</p>
<ul>
<li><p><code>MIN_PRIORITY</code>: минимальный приоритет, соответствует числу 1</p></li>
<li><p><code>MAX_PRIORITY</code>: максимальный приоритет, соответствует числу 10</p></li>
<li><p><code>NORM_PRIORITY</code>: стандартный приоритет, соответствует числу 5</p></li>
</ul>
<p>По умолчанию поток наследует приоритет потока, который его создал. Но теоретически можно увеличить или уменьшить приоритет любого потока платформы с помощью метода <code>setPriority()</code>. В частности, можно установить приоритет на любое значение между 
<code>MIN_PRIORITY</code> (1) и MAX_PRIORITY (10).</p>

<p>Всякий раз, когда у планировщика потоков платформы есть возможность выбрать новый поток, он отдает предпочтение потокам с более высоким приоритетом. Однако приоритеты потоков сильно зависят от системы. 
Поскольку виртуальная машина Java опирается на реализацию потоков текущей платформы, приоритеты потоков Java сопоставляются с уровнями приоритета текущей платформы, которая может иметь больше или меньше 
уровней приоритета потоков.</p>
<p>Например, в Windows имеется семь уровней приоритета. Некоторые приоритеты Java будут соответствовать одному и тому же уровню операционной системы. В виртуальной машине OpenJDK для Linux приоритеты 
потоков вообще игнорируются — все потоки имеют одинаковый приоритет. Приоритеты потоков могли быть полезны в ранних версиях Java, которые не использовали потоки операционных систем. Вам не следует использовать их в настоящее время.
<p>Для обратной совместимости все виртуальные потоки имеют приоритет NORM_PRIORITY. Так, у главного потока программы по умолчанию приоритет - 5.  Попытка изменить приоритет не имеет никакого эффекта.</p>
...
<style>
	...
	<div class="nav"><p><a href="./7.4.php">Назад</a><a href="./">Содержание</a><a href="./8.2.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
