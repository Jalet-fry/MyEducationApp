...
<html  lang="ru">
<head>
<title>Java | Словари. Интерфейс Map и класс HashMap</title>
<meta charset="utf-8" />
<meta name="description" content="Словари и хеш-карты в языке программирования Java, интерфейс Map и класс HashMap, их методы, добавление и удаление элементов в словаре">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Словари. Интерфейс Map и класс HashMap</h1><div class="date">Последнее обновление: 12.10.2025</div>
	<div class="socBlock">
	...
	<p>Словари (также называемые картами и отображениями) позволяют сопоставить с определенным ключом некоторое значение. В языке Java для определения базового функционала словарей имеется интерфейс 
<span class="b">Map&lt;K, V&gt;</span>. Он представляет структуру данных, где каждый элемент представляет пару "ключ-значение". При этом в рамках объекта <code>Map</code> 
все ключи уникальные. Такие коллекции облегчают поиск элемента, если нам известен ключ - уникальный идентификатор объекта.</p>
<p>Следует отметить, что в отличие от других интерфейсов, которые представляют коллекции, интерфейс Map НЕ расширяет интерфейс <code>Collection</code>.</p>
...
<li><p><code>boolean isEmpty()</code>: возвращает <code>true</code>, если коллекция пуста</p></li>
...
<li><p><code>Set&lt;K&gt; keySet()</code>: возвращает набор всех ключей словаря</p></li>
<li><p><code>Collection&lt;V&gt; values()</code>: возвращает набор всех значений словаря</p></li>
<li><p><code>void putAll(Map&lt;? extends K, ? extends V&gt; map)</code>: добавляет в коллекцию все объекты из словаря map</p></li>
<li><p><code>V remove(Object k)</code>: удаляет объект с ключом k</p></li>
<li><p><code>boolean remove(Object k, Object v)</code>: удаляет объект с ключом k, если только его значение равно v</p></li>
<li><p><code>V replace(K k, V v)</code>: заменяет значение для указанного ключа k на v</p></li>
<li><p><code>boolean replace(K key, V oldValue, V newValue)</code>: заменяет значение для указанного ключа key на newValue, если только старое значение равно oldValue</p></li>
<li><p><code>int size()</code>: возвращает количество элементов коллекции</p></li>
<li><p><code>static &lt;K,V&gt; Map&lt;K,V&gt; ofEntries(Map.Entry&lt;? extends K, ? extends V&gt;... entries)</code>: возвращает неизменяемые словарь, который содержит элементы из entries</p></li>
</ul>
...
<li><p><code>K getKey()</code>: возвращает ключ объекта словаря</p></li>
<li><p><code>V getValue()</code>: возвращает значение объекта словаря</p></li>
...
<p>При переборе объектов словаря мы будем оперировать этими методами для работы с ключами и значениями объектов.</p>

<p>Например, создадим неизменяемый словарь:</p>
...
<p>В данном случае создается словарь, где ключи представляют тип <code>Integer</code>, а значения - тип <code>String</code>. Для создания словаря применяется мтеод <code>Map.ofEntries()</code>, 
в который передаются объекты <code>Map.Entry</code>, создаваемые с помощью метода <code>Map.entry()</code>. Первый параметр этого метода - ключ, а второй - сопоставляемое с ним значение.</p>
<p>Например, выражение</p>
<pre class="brush:java;">Map.entry(1, "Tom")</pre>
<p>Создает объект <code>Map.Entry</code>, где ключ - число 1, а значение - строка "Tom".</p>

<p>Поскольку результатом метода <code></code> является неизменяемый словарь типа <code>java.util.ImmutableCollections$MapN</code>, то мы не можем изменять его значения. Но посмотрим, какие другие операции мы можем произвести со словарем.</p>
<pre class="brush:java;">
import java.util.Map;
...
<h3>HashMap</h3>
<p>Базовым классом для всех словарей является абстрактный класс <span class="b">AbstractMap</span>, 
который реализует большую часть методов интерфейса Map. Наиболее распространенным классом словарей является <span class="b">HashMap</span>, 
который наследуется от класса <code>AbstractMap</code> и представляет реализацию интерфейса <code>Map</code> на основе хеш-таблицы.</p>
<p>Для создания словаря <code>HashMap</code> применяется ряд конструкторов:</p>
<ul>
<li><p><code>HashMap()</code>: создает пустой<code>HashMap</code>с начальной емкостью по умолчанию (16) и коэффициентом загрузки по умолчанию (0,75)</p></li>

<li><p><code>HashMap(int initialCapacity)</code>: создает пустой<code>HashMap</code>с указанной начальной ёмкостью и коэффициентом загрузки по умолчанию (0,75)</p></li>

<li><p><code>HashMap(int initialCapacity, float loadFactor)</code>: создает пустой<code>HashMap</code>с указанной начальной емкостью и коэффициентом загрузки</p></li>

<li><p><code>HashMap(Map&lt;? extends K, ? extends V&gt; m)</code>: создает новый<code>HashMap</code>с теми же сопоставлениями, что и у заданного Map</p></li>
</ul>
<p>Создание <code>HashMap</code> и добавление в него элементов:</p>
<pre class="brush:java;">
import java.util.Map;
...
<p>В данном случае создаем словарь, где и ключи, и значения представляют строки. Затем с помощью методов <code>put()/putAll()</code> добавляем ряд объектов.</p>
<p>ПРимер получения данных из словаря:</p>
<pre class="brush:java;">
import java.util.Map;
import java.util.HashMap;
...
<p>Пример изменения и удаления элементов словаря:</p>
<pre class="brush:java;">
import java.util.Map;
import java.util.HashMap;
...
<style>
	...
	<div class="nav"><p><a href="./5.5.php">Назад</a><a href="./">Содержание</a><a href="./5.9.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
