...
<html  lang="ru">
<head>
<title>Java | Класс Constructor и cоздание объектов</title>
<meta charset="utf-8" />
<meta name="description" content="Получение конструкторов с помощью рефлексии в программе на языке программирования Java, методы getConstructors и getDeclaredConstructors, создание объекта произвольного класса с помощью метода newInstance">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Класс Constructor и cоздание объектов</h1><div class="date">Последнее обновление: 29.09.2025</div>
	<div class="socBlock">
...
<p>Класс <span class="b">Constructor</span> из пакета <code>java.lang.reflect</code> представляет конструктор класса. Он предоставляет функционал для получения информации о конструкторе, а также возможности для создания новых объектов. 
Отмечу основные его возможности:</p>
<ul>
<li><p><span class="b">String getName()</span>: возвращает имя конструктора</p></li>
...
<li><p><code>Object newInstance(Object... initargs)</code>: создаёт новый экземпляр класса, передавая конструктору аргументы через параметр <code> initargs</code>.</p></li>
<li><p><code>Class[] getParameterTypes()</code>: возвращает массив объектов Class, которые представляют типы параметров.</p></li>
<li><p><code>Class[] getExceptionTypes()</code>: возвращает массив объектов Class, представляющих типы исключений, генерируемых методом.</p></li>
</ul>

<h3>Получение конструктора</h3>
<p>Для получения конструктора класса в виде объекта <code>Constructor</code> применяется ряд методов объекта <code>Class</code>:</p>
<ul>
<li><p><code>Constructor[] getConstructors()</code>:  возвращает массив объектов <code>Constructor</code> для открытых конструкторов (с модификатором <code>public</code>) данного класса</p></li>

<li><p><code>Constructor[] getDeclaredConstructors()</code>: возвращает массив объектов <code>Constructor</code> для всех конструкторов данного класса. Метод возвращают массив нулевой длины, если таких конструкторов нет или если объект 
представляет собой примитивный тип или массив.</p></li>
<li><p><code>Constructor getConstructor(Class... parameterTypes)</code>:  возвращает объект <code>Constructor</code> для публичного конструктора. 
Параметр parameterTypes — это массив объектов Class, которые идентифицируют формальные типы параметров конструктора в порядке их объявления. Благодаря этим определениям параметров Java может выбрать нужную версию, если определено несколько конструкторов. Если подобный публичный конструктор отсутствует, то метод генерирует исключение <code>NoSuchConstructorException </code></p></li>
<li><p><code>Constructor getDeclaredConstructor(String name)</code>: возвращает объект <code>Constructor</code> для любого конструктора класса. Параметр parameterTypes — это массив объектов Class, которые идентифицируют формальные типы параметров конструктора в порядке их объявления. Благодаря этим определениям параметров Java может выбрать нужную версию, если определено несколько конструкторов. Если подобный публичный конструктор отсутствует, то метод генерирует исключение <code>NoSuchConstructorException </code></p></li>
</ul>
<p>Например, возьмем следующий класс:</p>
...
<p>Получим конструкторы этого класса:</p>
<pre class="brush:java;">
...
<p>Теперь получим один конструктор, исходя из его параметров. Параметы передаются в виде набора объектов Class, который представляют типы соответствующих параметров. Например, в случае с классом Person второй конструктор принимает два параметра 
с типами <code>String</code> и <code>int</code>. Получим этот конструктор:</p>
<pre class="brush:java;">
import java.lang.reflect.*;
...
<h3>Получение параметров</h3>
<p>С помощью метода <code>getParameterTypes()</code> можно получить массив параметров, где каждый параметр представляет тип <code>Class</code>:</p>
<pre class="brush:java;">
import java.lang.reflect.*;
...
<p>С помощью метода <code>Object newInstance(Object... initargs)</code> можно создать новый объект класса:</p>
<pre class="brush:java;">
...
<p>Параметры метода представляют набор объектов, которые предоставляют значения параметрам конструктора. Например, создадим объект Person:</p>
<pre class="brush:java;">
import java.lang.reflect.*;
...
<p>Здесь сначала получаем конструктор с двумя параметрами:</p>
<pre class="brush:java;">Constructor ctor = cl.getDeclaredConstructor(String.class, int.class);</pre>
<p>А затем используем полученный конструктор, передавая в метод <code>newInstance()</code> параметры для него</p>
<pre class="brush:java;">Person person = (Person) ctor.newInstance("Sam", 23); </pre>
...
<style>
	...
	<div class="nav"><p><a href="./14.4.php">Назад</a><a href="./">Содержание</a><a href="./14.6.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
