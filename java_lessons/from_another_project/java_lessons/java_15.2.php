...
<html  lang="ru">
<head>
<title>Java | Создание и применение аннотаций</title>
<meta charset="utf-8" />
<meta name="description" content="Создание и применение аннотаций в языке программирования Java, мета-аннотации @Retention и @Target, управление применением с помощью констант ElementType, элементы аннотаций и их значения по умолчанию">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Создание и применение аннотаций</h1><div class="date">Последнее обновление: 01.11.2025</div>
	<div class="socBlock">
	...
	<h3>Создание собственных аннотаций</h3>
<p>Хотя в языке Java, во фреймворках на этом языке есть различные аннотации для разных ситуаций, но Java также позволяет создавать свои собственные аннотации. 
Каждая аннотация определяется с помощью <code>​​интерфейса аннотации</code> - с помощью ключевого слова <code>@interface</code>, после которого указывается имя аннотации. 
<b>@interface</b> создает интерфейс Java:</p>
...
<p>После имени аннотации (как и в случае с интерфейсами) идут фигурные скобки, внутри которых размещаются элементы аннотации. В данном случае аннотация называется "MyAnnotation" и не содержит никаких элементов.</p>
<p>Все интерфейсы аннотаций неявно расширяют интерфейс <b>java.lang.annotation.Annotation</b>, который является обычным интерфейсом. Однако расширять интерфейсы аннотаций или реализовать их в своих классах нельзя. 
Вместо этого инструменты обработки исходного кода и виртуальная машина при необходимости сами автоматически генерируют необходимые прокси-классы и объекты.</p>

<p>При создании аннотации часто используются так называемые <span class="b">мета-аннотации</span> (аннотации для аннотаций):</p>
<ul>
<li><p><b>@Retention</b>: определяет политику хранения.</p></li>
<li><p><b>@Target</b>: указывает, к каким элементам кода можно применять аннотацию (класс, метод, поле, параметр и т.д.).</p></li>
<li><p><b>@Documented</b>: указывает, что аннотация должна быть включена в генерируемую Javadoc-документацию.</p></li>
<li><p><b>@Inherited</b>: указывает, что аннотация, примененная к классу, будет наследоваться его подклассами.</p></li>
<li><p><b>@Repeatable</b>: позволяет применять одну и ту же аннотацию к одному элементу несколько раз.</p></li>
</ul>
<p>Далее мы рассмотрим некоторые из этих мета-аннотаций. А пока посмотрим, как мы можем применить нашу аннотацию.</p>
<pre class="brush:java;">
@interface MyAnnotation{}
...
<p>Здесь аннотация MyAnnotation применяется к объявлениям - к классу Thing и его полю name. Причем аннотация сама по себе и ее применение ничего не делают, мы можем использовать этот класс как обычный класс:</p>
<pre class="brush:java;">
// объявляем аннотацию
@interface MyAnnotation{}
...
<p>Стоит отметить, что по умолчанию аннотация может использоваться с любыми объявлениями, но не с параметрами типа (в обобщениях) и использованием типов. 
Для более детальной настройки применения аннотации применяется мета-аннотация <b>@Target</b>.</p>

<h3>@Target</h3>
<p>Мета-аннотация <b>@Target</b> определяет, к каким элементам кода можно применять аннотацию. 
Ее значение представляет массив объектов <b>ElementType</b>, которые и определяют элементы для применения аннотации:</p>

<ul>
<li><p><code>ElementType.ANNOTATION_TYPE</code>:  объявления типов аннотаций</p></li>
<li><p><code>ElementType.MODULE</code>:  модули</p></li>
<li><p><code>ElementType.PACKAGE</code>: пакеты</p></li>
<li><p><code>ElementType.TYPE</code>:  классы (включая перечисления) и интерфейсы (включая типы аннотаций)</p></li>
<li><p><code>ElementType.METHOD</code>:  методы</p></li>
<li><p><code>ElementType.CONSTRUCTOR</code>:  конструкторы</p></li>
<li><p><code>ElementType.FIELD</code>:  поля (включая константы перечисления)</p></li>
<li><p><code>ElementType.PARAMETER</code>:  параметры метода или конструктора</p></li>
<li><p><code>ElementType.RECORD_COMPONENT</code>:  компоненты record-классов</p></li>
<li><p><code>ElementType.LOCAL_VARIABLE</code>:  локальные переменные</p></li>
<li><p><code>ElementType.TYPE_PARAMETER</code>:  параметры типы</p></li>
<li><p><code>ElementType.TYPE_USE</code>:  использование типа</p></li>
</ul>
<p>Например:</p>
<pre class="brush:java;">
...
<p>Здесь аннотация MyAnnotation может применться к полям, типам (классам и интерфейсам) и компонентам классов record. Например:</p>
<pre class="brush:java;">
@MyAnnotation
class Thing{
    @MyAnnotation String name;
...
<p>Но, например, к методам и другим компонентам такая аннотация применяться не может.</p>
<p>Аннотация без ограничения <code>@Target</code> может использоваться с любыми объявлениями, но не с параметрами типа и использованием типов.</p>
...
<h3>Элементы аннотаций</h3>
<p>Аннотация может содержать элементы, которые фактически являются методами. Причем эти методы не могут иметь параметров, не могут быть обобщенными, а также не могут в объявлении содержать выражения 
<b>throws</b>. Например:</p>
<pre class="brush:java;">
@interface MyAnnotation {
...
<p>В данном случае аннотация MyAnnotation имеет два элемента: <code>value</code> (представляет строку) и <code>count</code> (представляет число).</p>
<p>При применении аннотации нам надо передать этим элементам некоторые значения:</p>
<pre class="brush:java;">
@interface MyAnnotation {
...
<p>Значения для элементов аннотаций передаются после названия аннотации в скобках через запятую в виде <code>название_элемента = значение_элемента</code>. В какой-то степени это похоже на вызов конструктора класса.</p>

<h4>Значения по умолчанию для элементов аннотаций</h4>
<p>Элементы аннотаций могут иметь значение по умолчанию. Для его установки после метода и перед значением по умолчанию указывается ключевое слово <b>default</b>. Например:</p>
<pre class="brush:java;">
@interface MyAnnotation {
...
<p>Если элементы аннотации имеют значения по умолчанию, то при применении аннотации для этих элементов можно не передавать значения:</p>
<pre class="brush:java;">
@interface MyAnnotation {
...
<p>Если элемент аннотации представляет массив, то в качестве значения по умолчанию указываются внутри фигурных скобок:</p>
<pre class="brush:java;">
@interface MyAnnotation {
...
<p>Значения по умолчанию не сохраняются вместе с аннотацией, а вычисляются динамически.</p>
<h3>@Retention</h3>
<p>Мета-аннотация <b>@Retention</b> указывает, где можно получить доступ к аннотации, то есть по сути то, как долго аннотация "живет". Эта аннотация может принимать одно из трех значений перечисления <b>RetentionPolicy</b>:</p>
<ul>
<li><p><b>RetentionPolicy.SOURCE</b>: аннотация существует <span class="b">только в исходном коде</span> и отбрасывается компилятором. Используется, например, для инструментов анализа кода или кодогенерации</p></li>
<li><p><b>RetentionPolicy.CLASS</b>: аннотация сохраняется в файле <code>.class</code> (в байт-коде), но <span class="b">недоступна во время выполнения</span>. Это значение по умолчанию.</p></li>
<li><p><b>RetentionPolicy.RUNTIME</b>: аннотация сохраняется в файле <code>.class</code>, но в отличие от предыдущего варианта аннотация <span class="b">доступна во время выполнения</span> через рефлексию (reflection). (Это самый распространенный тип для фреймворков, например, Spring или JUnit, которые анализируют классы во время работы программы)</p></li>
</ul>
<p>Пример применения мета-аннотации:</p>
<pre class="brush:java;">
...
<style>
	...
	<div class="nav"><p><a href="./15.1.php">Назад</a><a href="./">Содержание</a><a href="./15.3.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
