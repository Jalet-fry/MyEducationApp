...
<html  lang="ru">
<head>
<title>Java | Закрытие потоков и конструкция try-с-ресурсами</title>
<meta charset="utf-8" />
<meta name="description" content="Закрытие файловых потоков в языке программирования Java, интерфейс Closable, конструкция try..with resources (try-с-ресурсами)">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Закрытие потоков и конструкция try-с-ресурсами</h1><div class="date">Последнее обновление: 07.10.2025</div>
	<div class="socBlock">
...
<h3>try-с-ресурсами</h3>
<p>Но язык Java позволяет использовать более удобную конструкцию, котораяя автоматически вызывает метод <code>close()</code> - конструкция 
...
<p>Данная конструкция также не исключает использования блоков <code>catch</code>, которые также могут размещаться после блока <code>try</code>:</p>
<pre class="brush:java;">
try(Resource res = ...){
...
<p>После окончания работы в блоке try у ресурса (в данном случае у объекта <code>FileInputStream</code>) автоматически вызывается метод <code>close()</code>.</p>
...
<p>Также обратите внимание, что в данном случае ресурс определяется непосредственно в заголовке конструкции <code>try</code>:</p>
<pre class="brush:java;">try(FileInputStream fin = new FileInputStream("notes.txt"))</pre>
<p>Однако ресурс может быть создан и вне этой конструкции, но при передаче в заголовок конструкции <code>try</code> после ее завершения этот ресурс все равно будет закрыт:</p>
<pre class="brush:java;">
import java.io.*;
...
<style>
	...
	<div class="nav"><p><a href="./6.3.php">Назад</a><a href="./">Содержание</a><a href="./6.4.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
