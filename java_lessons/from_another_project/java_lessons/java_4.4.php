...
<meta charset="utf-8" />
<meta name="description" content="Оператор Assert и генерация ошибок при разработке и тестировании в языке программирования Java, параметр выполнения -enableassertions, тип ошибок AssertionError">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Assert</h1><div class="date">Последнее обновление: 07.10.2025</div>
	<div class="socBlock">
...
<p><b>assertion</b> (утвержение) представляет распространенный паттерн защитного механизма (guard), который позволяет во время тестирования проверить соблюдение некоторых условий. Для поддержки этого механизма в языке Java есть ключевое слово <b>assert</b>. </p>
<p>Оператор <b>assert</b> используется для проверки таких ошибок, при которых нет смысла продолжать программу. Но обычно этот оператор применяется только во время разработки и тестирования, например, 
для поиска внутренних ошибок программы во время тестирования.</p>
<h3>Оператор assert</h3>
<p>Оператор <b>assert</b> имеет следующие формы:</p>
<pre class="brush:java;">
assert условие;
assert условие : выражение;
</pre>
<p>В обоих формах после оператора <code>assert</code> идет некоторое условие. И среда проверяет истинность этого условия. И если условие не соблюдается, то система генерирует ошибку типа <b>AssertionError</b></p>
<p>Во второй форме после условия через двоеточие указывается выражение, которое передается в конструктор объекта <b>AssertionError</b> и применяется для создания сообщения об ошибке.</p>
<p>Приведу абстрактные примеры. Первая форма:</p>

<pre class="brush:java;">
assert n &gt; 0;
</pre>
<p>Здесь оператор <code>assert</code> проверяет, что некоторая переменная n больше 0. Если переменная равна 0 или меньше, что система генерирует ошибку <b>AssertionError</b></p>
<p>Вторая форма:</p>
<pre class="brush:java;">
assert n &gt; 0 : n;
</pre>
<p>Здесь фактически все то же самое, только теперь значение переменной <code>n</code> передается в конструктор класса <b>AssertionError</b> и добавляется в сообщение об ошибке.</p>
<h3>Включение поддержки assert</h3>
<p>По умолчанию оператор <code>assert</code> не оказывает никакого действия на работу программы. Ошибка типа <b>AssertionError</b> генериурется, если только <span class="b">при выполнении программы</span> 
явным образом включена поддержка <code>assert</code>. Для этого при запуске программы утилите <b>java</b> передается опция <span class="b">-enableassertions</span> или <span class="b">-ea</span>. 
Например:</p>
<pre class="sh">java -enableassertions Program</pre>
<p>Также Java позволяет подключать поддержку <code>assert</code> только для определенных классов и пакетов:</p>
<pre class="sh">java -ea:SomeClass -ea:com.mycompany.somelib... Program</pre>
<p>Подвыражение <code>-ea:SomeClass</code> включает <code>assert</code> для класса SomeClass, а <code>-ea:com.mycompany.somelib...</code> - для пакета "com.mycompany.somelib".</p>
<p>Если пакет не имеет имени (пакет по умолчанию), то для подключения <code>assert</code> можно использовать следующее выражение:</p>
<pre class="sh">-ea:...</pre>
<p>При необходимости также можно отключить <code>assert</code> в определенных классах и пакетах с помощью опции <span class="b">-disablessertions</span> или <span class="b">-da</span>:</p>
<pre class="sh">java -ea:... -da: SomeClass Program</pre>
<h3>Пример использования assert</h3>
<p>Рассмотрим небольшой пример использования оператора <b>assert</b>:</p>
<pre class="brush:java;">
class Program {
...
<p>Здесь определен метод <code>factorial</code>, который вычисляет факториал числа. Само число передается через параметр. Но по условию факториала число должно быть больше 0. И в самом  методе 
мы проверяем это условие с помощью <code>assert</code>:</p>
<pre class="brush:java;">
static int factorial(int n){
...
<p>Для демонстрации в методе <code>main()</code> мы два раза вызываем данный метод, передавая в него корректное и некорректное значения:</p>
<pre class="brush:java;">
int f1 = factorial(4);

int f2 = factorial(-4);
</pre>
<p>Скомпилируем программу и выполним ее с флагом <code>-enableassertions</code>:</p>
<pre class="sh">
eugene@Eugene:/workspace/java$ javac Program.java && java -enableassertions Program
f1: 24
...
<p>И мы видим, что первый вызов метода с корректным значением отработал нормально, а при втором вызове возникла ошибка <code>AssertionError</code> на 14-й строке кода. Поймав 
подобную ошибку при тестировании или разработке, мы увидим, что данные, которые в передаются в метод <code>factorial()</code>, не проверяются, и, возможно, при вызове метода добавим код проверки 
или какой-нибудь другой механизм, который позволит избежать падения программы.</p>

<p>И дополнительно рассмотрим, что будет при использовании другой формы оператора <code>assert</code>. Для этого изменим немного метод <code>factorial()</code></p>
<pre class="brush:java;">
static int factorial(int n){
...
<p>В данном случае выражение <code>assert n &gt; 0 : n</code> указывает, что значение параметра <code>n</code> будет передаваться в сообщение об ошибке. Так, при выполнении программы мы получим немного другое сообщение:</p>
<pre class="sh">
eugene@Eugene:/workspace/java$ javac Program.java && java -enableassertions Program
f1: 24
...
<p>Теперь из сообщения об ошибке мы видим, что стало причиной ошибки. Ну и естественно мы можем определять более подробные сообщения. Например:</p>
<pre class="brush:java;">assert n > 0 : "Параметр меньше 1";</pre>
...
<style>
	...
	<div class="nav"><p><a href="./4.3.php">Назад</a><a href="./">Содержание</a><a href="./5.1.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
