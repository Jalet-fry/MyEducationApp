...
<meta charset="utf-8" />
<meta name="description" content="Создание своих классов исключений в языке программирования Java, наследование от классов Throwable и Exception, примеры использования">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Создание своих классов исключений</h1><div class="date">Последнее обновление: 06.10.2025</div>
	<div class="socBlock">
...
<p>В классе Person в конструкторе получаем имя и возраст пользователя и проверяем их на соответствие некоторым условиям. В частности, если длина имени меньше 2 символов или 
возраст меньше 1 или больше 110, то выбрасываются исключения. В качестве типа генерируемого исключения применяется базовый класс <code>Exception</code>, в конструктор которого передается сообщение об ошибке:</p>
<pre class="brush:java;">
Person(String name, int age) throws Exception{
...
<p>И поскольку конструктор сам не обрабатывает эти исключения, то в его объявлении с помощью оператора <code>throws</code> указываем компилятору, что конструктор генерирует исключения типа Exception.</p>
<p>В методе <code>createPerson()</code> собственно обрабатываем исключения при вызове конструктора и создании объекта Person:</p>
<pre class="brush:java;">
try{
    var person = new Person(name, age);
    person.print();
...
<p>Подобный подход вполне можно использовать. Но иногда удобнее использовать свои классы исключений. Например, в какой-то ситуации мы хотим обработать определенным образом только те исключения, 
которые относятся к классу Person. Или выделить исключительные случаи и ошибки для класса Person в отдельные классы. Для этих целей мы можем сделать специальный класс <code>PersonException</code>:</p>
<pre class="brush:java;">
...
<p>По сути класс кроме пустого конструктора ничего не имеет, и то в конструкторе мы просто обращаемся к конструктору базового класса <code>Exception</code>, передавая в него строку с сообщением об ошибке. Но теперь мы можем изменить класс Person, чтобы он выбрасывал 
исключение именно этого типа и соответственно в основной программе обрабатывать это исключение:</p>
<pre class="brush:java;">
class Program{
...
<pre class="sh">
Некорректный возраст: -10
Недостаточная длина имени: 1
</pre>
...
<p>Но мы можем пойти дальше и создать для отдельных ситуаций свои типы исключений. Например:</p>
<pre class="brush:java;">
class Program{
...
<p>Здесь для ошибок имени определен тип <code>PersonNameException</code>, а для ошибок при указании возраста - тип <code>PersonAgeException</code>. Оба типа наследуются от <code>Exception</code> и 
через конструктор получают данные об ошибке.</p>
<p>Конструктор класса Person генерирует исключения этих типов:</p>
<pre class="brush:java;">
Person(String name, int age) throws PersonNameException, PersonAgeException {
</pre>
<p>А в методе <code>createPerson()</code> при создании объектов Person обрабатываем по отдельности оба этих вида исключений:</p>
<pre class="brush:java;">
catch(PersonNameException ex){
...
<pre class="sh">
Ошибка в возрасте:
Некорректный возраст. Текущее значение: -10. Минимальный возраст: 1. Максимальный возраст: 110
Ошибка в имени:
Недостаточная длина имени. Текущее значение: m. Минимальная длина имени: 2
</pre>
...
<style>
	...
	<div class="nav"><p><a href="./4.1.php">Назад</a><a href="./">Содержание</a><a href="./4.4.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
