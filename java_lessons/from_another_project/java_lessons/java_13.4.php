...
<html  lang="ru">
<head>
<title>Java | Модульный jar-файл</title>
<meta charset="utf-8" />
<meta name="description" content="Модульный jar-файл на языке программирования Java, утилита jar и создание JAR-файлов из модулей">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Модульный jar-файл</h1><div class="date">Последнее обновление: 06.11.2025</div>
	<div class="socBlock">
...
<p>Упаковка функционала пакетов в единый jar-файл также может использоваться для распространения модулей Java. JAR-файл, который по сути представляет скомпилированный модуль, называется 
<span class="b">модульным JAR-файлом</span>.</p>
<p>Для создания модульного JAR-файла аналогичным образом применяется утилита <b>jar</b>. 
...
А при упаковке файлов в <code>jar-файл</code> для перехода в этот каталог применяется опция <span class="b">-C</span> команды <b>jar</b>. Рассмотрим на примере.</p>

<h3>Определение модуля</h3>
<p>Для начала где-нибудь на жестком диске (в моем случае это папка <span class="b">worlspace/java</span>) создадим каталог, который назовем <span class="b">demo</span>. Этот каталог будет представлять 
одноименный модуль. И в этом каталоге <span class="b">demo</span> определим новый файл <span class="b">module-info.java</span> - файл дескриптора модуля  со следующим кодом:</p>
<pre class="brush:java;">
module demo {
}
</pre>
<p>Далее в каталоге <span class="b">demo</span> создадим папку <span class="b">com</span>. В папке com создадим папку <span class="b">metanit</span>, а в папке <span class="b">com/metanit</span> - 
папку <span class="b">hello</span>.</p>
<p>В папке <span class="b">com/metanit/hello</span> определим новый файл <span class="b">Hello.java</span>:</p>
<pre class="brush:java;">
...
<p>Название пакета файла - <span class="b">com.metanit.hello</span> отражает структуру папок, в которых расположен файл. Сам файл определяет класс Hello, 
который в методе main выводит на консоль строку.</p>
<p>В итоге у нас получится следующая стуктура проекта:</p>
<img src="./pics/11.1.png" alt="Модули в Java" />
<p>Теперь скомпилируем все это и упакуем в файл jar.</p>
<h3>Создание jar</h3>
<p>Для создания jar перейдем в командной строке/терминале к папке, где находится папка <span class="b">demo</span> с нашим модулем (в моем случае это папка <span class="b">worlspace/java</span>). 
Затем для компиляции модуля выполним следующую команду:</p>
<pre class="sh">
javac --module-source-path . -d mods demo/module-info.java demo/com/metanit/hello/Hello.java
</pre>
<p>После компиляции модуля в текущей папке появится папка "mods", в которую будут помещаться все скомпилированные классы модуля demo и которая будет иметь следующую структуру:</p>
<pre class="browser">
mods
|_demo
    |_module-info.class
...
<p>Далее для упаковки в <b>jar</b> выполним следующую команду:</p>
<pre class="sh">
jar -c -v -f demo.jar -C mods/demo .
</pre>
<p>Команда принимает следующие параметры</p>
<ul>
<li><p><span class="b">-c</span>: указывает на создание нового архива.</p></li>
<li><p><span class="b">-v</span>: предоставляет подробный вывод выполнения команды</p></li>
<li><p><span class="b">-f demo.jar</span>: задает имя создаваемого JAR-файла. То есть создаваемый файл будет называться "demo.jar"</p></li>
<li><p><span class="b">-C mods/demo .</span>: переходит в папку с скомпилированными классами (mods/имя__модуля) и добавляет все ее содержимое (.) в JAR. В данном случае это папка "mods" (название произвольное) и 
ее подкаталог "demo", название которого должно соответствовать имени модуля, указананного в <span class="b">module-info.java</span> (если указано). В примере выше модуль называется "demo", соответственно 
все компилируемые файлы помещаются в папку "mods/demo"</p></li>
</ul>
<p>Полный консольный вывод:</p>
<pre class="sh">
...
<h3>Создание исполняемого jar</h3>
<p>Но в примере выше класс Hello является исполняемым, то есть он имеет метод <code>main()</code> и может быть запущен на выполнение. И 
как и в обычных JAR-файлах, в модульном JAR-файле можно указать основной класс, который будет выполняться при запуске. 
Для этого предназначена опция <span class="b">-e</span> (полное название опции - <span class="b">--main-class</span>), которой передается название основного класса с учетом его пакетов:</p>
<pre class="sh">
jar -c -v -f demo.jar -e com.metanit.hello.Hello -C mods/demo .
</pre>
<p>В примере выше класс Hello определен в пакете <code>com.metanit.hello</code>, поэтому указываем в качестве полного имени файла "com.metanit.hello.Hello".</p>

<p>После этого можно запустить созданный модульный JAR-файл с помощью команды <span class="b">java</span>:</p>
<pre class="sh">java --module-path demo.jar --module demo</pre>
...
<h3>Компиляция взаимодействующих модулей</h3>
<p>Рассмотрим другую, более сложную ситуацию, где у нас есть несколько модулей, которые взаимодействуют друг с другом. Допустим, у нас есть следующая структура проектов:</p>

<img src="./pics/11.5.png" alt="Взаимодействие модулей в Java" />
<p>Здесь определено два модуля: <span class="b">demo</span> и <span class="b">operations</span>. Каждый располагается в своей одноименной папке.</p>
<p>В папке <span class="b">operations/com/metanit/factorial</span> определен следующий файл <span class="b">Factorial.java</span>:</p>
<pre class="brush:java;">
...
<p>Чтобы сделать этот класс и в целом его пакет доступным для использования в других модулях в папке <span class="b">operations</span> определен следующий файл <span class="b">module-info.java</span>:</p>
<pre class="brush:java;">
module operations {
    exports com.metanit.factorial;
}
</pre>
<p>В папке <span class="b">demo</span> определен следующий файл <span class="b">module-info.java</span>:</p>
<pre class="brush:java;">
module demo {
    requires operations;
}
...
<p>А в папке <span class="b">demo/com/metanit/hello/</span> определим следующий файл <span class="b">Hello.java</span>:</p>
<pre class="brush:java;">
package com.metanit.hello;
...
<p>Теперь нам надо оба модуля скомпилировать в jar-файлы. Стандартный подход предполагает, что оба модуля компилируются по отдельности каждый в свой jar-файл. 
В консоли перейдем к корневой папке, где размещаются оба модуля (в моем случае это папка "/workspace/java") и скомпилируем их. Для этого выполним следующую команду:</p>
<pre class="sh">javac --module-source-path . -d mods demo/module-info.java demo/com/metanit/hello/Hello.java</pre>
<p>Хотя здесь указан только один модуль, зависимый модуль также будет скомпилирован. Оба модуля в данном случае будут помещаться в папку "mods", которая в итоге будет иметь следующую структуру:</p>
<pre class="browser">
mods
|_demo
|   |_module-info.class
...
<p>Далее скомпилируем модуль <b>operations</b>:</p>
<pre class="sh">jar -c -v -f libs/operations.jar -C mods/operations .</pre>
<p>Компилируем из модуля файл "operations.jar" будет помещаться в папку "libs". Далее скомпилируем модуль <b>demo</b>:</p>
<pre class="sh">jar -c -v -f libs/demo.jar -e com.metanit.hello.Hello -C mods/demo .</pre>
<p>В итоге в папке "libs" будут располагаться оба скомпилированных модуля. Теперь запустим главный модуль, передав путь к папке с модульными jar:</p>
<pre class="sh">java --module-path libs --module demo/com.metanit.hello.Hello</pre>
<p>Полный консольный вывод:</p>
<pre class="sh">
eugene@Eugene:/workspace/java$ javac --module-source-path . -d mods demo/module-info.java demo/com/metanit/hello/Hello.java
...
<style>
	...
	<div class="nav"><p><a href="./13.2.php">Назад</a><a href="./">Содержание</a><a href="./13.3.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
