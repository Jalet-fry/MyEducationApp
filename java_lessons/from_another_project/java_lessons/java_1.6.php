...
<html  lang="ru">
<head>
<title>Java | Установка JDK</title>
<meta charset="utf-8" />
<meta name="description" content="Установка Java, JRE, JDK на Windows и Linux, OpenJDK и JDK от Oracle, установка через пакетный менеджер">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Установка JDK</h1><div class="date">Последнее обновление: 16.09.2025</div>
	<div class="socBlock">
...
<p>Для разработки программ на Java нам потребуется специальный комплект для разработки JDK 
...
<p>JDK от Oracle для Windows (x64), Linux (x64, ARM64) и MacOS (x64, ARM64) доступны по адресу <a href="https://www.oracle.com/java/technologies/downloads/" rel="nofollow">https://www.oracle.com/java/technologies/downloads/</a></p>
<p>OpenJDK для Windows (x64), Linux (x64, ARM64) и MacOS (x64, ARM64) доступны по адресу <a href="https://jdk.java.net/25/" rel="nofollow">https://jdk.java.net/25/</a></p>
<p>OpenJDK предоставляет только архивы, Oracle также предоставляет установочные пакеты. Рассмотрим установку JDK для различных систем.</p>

<h3>Установка на Windows</h3>
<h4>Установка с помощью установщика</h4>
<p>Наиболее распространенный вариант установки JDK на Windows - это применение установщика с официального сайта Oracle. В частности, перейдем на страницу: 
<a href="https://www.oracle.com/java/technologies/downloads/#jdk25-windows" rel="nofollow">https://www.oracle.com/java/technologies/downloads/#jdk25-windows</a></p>
<img src="./pics/1.11.png" alt="Установка JDK" style="width:800px; />
<p>Для Windows есть три варианта загрузки: в виде установщика exe (x64 Installer) или msi (x64 MSI Installer), либо в виде архива, который не надо устанавливать. 
Большой разницы между двумя установщиками - "x64 Installer" и "x64 MSI Installer" нет. Например, выберем пункт "x64 Installer". После загрузки запустим программу установки:</p>
<img src="./pics/1.12.png" alt="Установка Java Development Kit" style="width:600px;" />
<p>Нажмем на кнопку Next. На следующем экране необходимо указать, в какую папку будет производиться установка:</p>
<img src="./pics/1.13.png" alt="Установка компонентов Java Development Kit"  style="width:600px;" />
...
<img src="./pics/1.15.png" alt="Завершение установки JDK"  style="width:600px;" />
<p>При установке JDK все файлы по умолчанию помещаются в каталог <span class="ii">C:\Program Files\Java\jdk-[номер_версии]</span> (если при установке не было изменено расположение). 
Так, если мы откроем в нем подкаталог <span class="ii">bin</span>, то мы сможем увидеть в нем ряд утилит. 
Нас прежде всего интересует утилита компилятора <span class="b">javac</span> и утилита <span class="b">java.exe</span>. <span class="b">javac</span> компилирует код на языке Java в бинарный файл, а 
<span class="b">java.exe</span> позволяет запускать скомпилированный файл:</p>
<img src="./pics/1.21.png" alt="Компилятор javac в Java" style="width:600px;"/>


<h4>Установка через пакетный менеджер</h4>
<p>Через пакетный менеджер winget также можно установить OpenJDK от Microsoft. Для этого в командной строке/PowerShell надо выполнить команду:</p>
<pre class="sh">winget install Microsoft.OpenJDK.25</pre>
<p>В данном случае устанавливается 25-версия JDK.</p>

<h3>Установка на Linux</h3>
<p>Если мы используем JDK от Oracle, то в данном случае нам доступен ряд вариантов установки. В частности, перейдем на страницу: 
<a href="https://www.oracle.com/java/technologies/downloads/#jdk25-linux" rel="nofollow">https://www.oracle.com/java/technologies/downloads/#jdk25-linux</a></p>
<img src="./pics/1.29.png" alt="Установка JDK на Linux" style="width:800px;" />
<ul>
<li><p><b>ARM64 Compressed Archive</b>: архив для систем на архитектуре ARM64</p></li>
<li><p><b>ARM64 RPM Package</b>: RPM-пакет для систем на архитектуре ARM64, которые для управления пакетами применяют RPM (например, Fedora, Suse и т.д.)</p></li>
<li><p><b>x64 Compressed Archive</b>: архив для систем на архитектуре Intel x86-64</p></li>
<li><p><b>x64 Debian Package</b>: Debian-пакет для систем на архитектуре Intel x86-64 (например, Ubuntu, Debian и т.д.)</p></li>
<li><p><b>x64 RPM Package</b>: RPM-пакет для систем на архитектуре Intel x86-64, которые для управления пакетами применяют RPM (например, Fedora, Suse и т.д.)</p></li>
</ul>
<p>Если наша операционная система Ubuntu (на платформе Intel x86-64), то выберем пункт <b>x64 Debian Package</b>, и нам загрузится debian-пакет (в моем случае файл "jdk-25_linux-x64_bin.deb"). 
Для его установки выполним команду</p>
<pre class="sh">sudo dpkg -i jdk-25_linux-x64_bin.deb</pre>

<p>Если у нас операционная система, которая для управления пакетами применяет RPM, например, Fedora, Suse и др, на платформе Intel x86-64, 
то необходимо выбрать пункт <b>x64 RPM Package</b>. И затем установить загруженный пакет командой</p>
<pre class="sh">sudo rpm -ivh jdk-25_linux-x64_bin.rpm</pre>

<p>Если у нас ОС Fedora, Suse и др, на платформе ARM64, то необходимо выбрать пункт <b>ARM64 RPM Package</b>. И затем установить загруженный пакет командой</p>
<pre class="sh">sudo rpm -ivh jdk-25_linux-aarch64_bin.rpm</pre>

<p>Также для различных операционных систем в репозиториях могут быть доступны пакеты JDK для установки. Например, установка с помощью пакетного менеджера snap:</p>
<pre class="sh">sudo snap install openjdk</pre>
<p>Установка с помощью пакетного менеджера apt:</p>
<pre class="sh">sudo apt install default-jdk </pre>
<p>Установка определенной версии JDK с помощью пакетного менеджера apt:</p>
<pre class="sh">sudo apt install openjdk-25-jdk-headless</pre>
<p>В случае с установкой архивов все идентично для всех основных систем на базе Linux - загружается архив, распаковывается в определенное место. Затем в переменные среды добавляется путь к папке bin распакованного архива.</p>

<h3>Установка Java на MacOS</h3>
<p>Для установки Java на MacOS перейдем на страницу загрузок на сайте Oracle по адресу <a href="https://www.oracle.com/java/technologies/downloads/#jdk25-mac" rel="nofollow">https://www.oracle.com/java/technologies/downloads/#jdk25-mac</a>:</p>
<img src="./pics/mac_java1.png" alt="Установка Java на MacOS" style="width: 600px;" />
<p>Здесь нам доступно 4 варианта пакетов:</p>
<ul>
<li><p><span class="b">ARM64 Compressed Archive</span>: архивный пакет для MacOS на ARM</p></li>	
<li><p><span class="b">ARM64 DMG Installer</span>: установщик для MacOS на архитектуре ARM</p></li>	
<li><p><span class="b">x64 Compressed Archive</span>: архивный пакет для MacOS на архитектуре Intel x86-64</p></li>	
<li><p><span class="b">x64 DMG Installer</span>: установщик для MacOS на архитектуре Intel x86-64</p></li>	
</ul>
<p>Загрузим и после загрузки запустим пакет установщика. Вначале нам отобразится приветственное окно.</p>
<img src="./pics/mac_java2.png" alt="Установка Java sdk на MacOS" style="width: 600px;" />

<p>Далее на всех последующих шагах оставим все опции по умолчанию. И после установки нам отобразится окно об ее успешном окончании:</p>
<img src="./pics/mac_java3.png" alt="настройка установки Java sdk на MacOS" style="width: 600px;" />


<h3>Проверка установки</h3>
<p>После установки Java перейдем к командной строке/терминалу и для проверки версии введем команду</p>
<pre class="sh">java -version</pre>
<p>Консоль нам должна в ответ ввести номер только что установленной версии JDK (в моем случае 25-й версии):</p>
<pre class="sh">
...
<h3>Документация</h3>
<p>Документация является важным вспомогательным элементом при разработке на любом языке программирования, и Java - не исключения. Всю документацию по Java можно найти на оф. сайте Oracle по адресу 
<a href="https://docs.oracle.com/en/java/javase/25/docs/" rel="nofollow">https://docs.oracle.com/en/java/javase/25/docs/</a> (для 25-й версии JDK, для других версий надо заменить номер на соответствующий).</p>
<p>Также документация доступна офлайн - ее также можно загрузить с оф. сайта в виде архива со страницы <a href="https://www.oracle.com/java/technologies/javase-jdk25-doc-downloads.html" rel="nofollow">https://www.oracle.com/java/technologies/javase-jdk25-doc-downloads.html</a> (для 
25-й версии JDK).</p>
...
<style>
	...
	<div class="nav"><p><a href="./1.1.php">Вперед</a><a href="./">Содержание</a><a href="./1.2.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
