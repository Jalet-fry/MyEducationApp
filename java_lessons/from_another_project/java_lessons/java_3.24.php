...
<html  lang="ru">
<head>
<title>Java | Полиморфизм и динамическая диспетчеризация методов</title>
<meta charset="utf-8" />
<meta name="description" content="Полиморфизм и динамическая диспетчеризация методов в языке программирования Java, динамическое и статическое связывание, dynamic и static binding, использование объектов подкласса вместо суперкласса">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Полиморфизм и динамическая диспетчеризация методов</h1><div class="date">Последнее обновление: 24.09.2025</div>
	<div class="socBlock">
...
<p><b>Полиморфизм</b> является одной из ключевых концепций объектно-ориентированного программирования и подразумевает, что сущность может принимать различные формы. 
В языке Java переменные являются полиморфными. Это значит, что переменная базового класса может ссылаться на объект 
производного класса. Например:</p>
...
является человеком. Поэтому переменной типа Person мы можем присвоить ссылку на объект типа Employee.</p>
...
Данный процесс еще называется <span class="b">dynamic method lookup</span> или динамический поиск метода или динамическая диспетчеризация методов. А установка точной версии метода  называется 
<span class="b">динамическим связыванием</span> (<b>dynamic binding</b>)</p>
<p>Для упрощения поиска виртуальная машина заранее вычисляет таблицу методов для каждого класса (<b>method table</b>). В таблице методов перечисляются все сигнатуры методов 
(названия методов и типы их параметров) и сами вызываемые методы. Виртуальная машина может построить таблицу методов после загрузки класса, объединив методы, найденные в файле класса, 
с таблицей методов базового класса. При фактическом вызове метода виртуальная машина просто выполняет поиск в таблице.</p>

<p>Если же метод определен с модификаторами <code>private</code>, <code>static</code>, <code>final</code> или является конструктором, то компилятор точно знает, какой метод вызывать. 
Это называется <span class="b">статическим связыванием</span> (<b>static binding</b>).</p>
...
<p>Однако обратное не верно: мы НЕ можем присвоить переменной производного класса ссылку на объект базового класса, например, следующим образом:</p>
<pre class="brush:java;">
Employee sam = new Person("Sam");
</pre>
<p>И это логично: не все люди работают и являются работниками некоторого предприятия.</p>
...
<h3>Полиморфизм в параметрах и возвращаемых типам</h3>
<p>То обстоятельство, что любой объект производного класса также рассматривается как объект базового класса, позволяет параметру базового типа передавать объекты производных классов. Например:</p>
<pre class="brush:java;">
public class Program{
...
<p>В данном случае в классе Program метод <code>printPerson</code> принимает объект типа Person:</p>
<pre class="brush:java;">
static void printPerson(Person person){
...
<p>Но поскольку объекты производного типа Employee также являются объектами типа Person, то мы можем передать эти объекты в данный метод:</p>
<pre class="brush:java;">
Employee sam = new Employee("Sam", "Oracle");
printPerson(sam); // параметру типа Person передаем объект Employee
</pre>

<p>То же самое относится к возвращению объекту из метода: если возвращаемый тип представляет базовый класс, то мы можем вернуть из метода объект производного класса. Например:</p>
<pre class="brush:java;">
public class Program{
...
<p>В данном случае в классе Program статический метод <code>defaultPerson</code> возвращает значение типа Person:</p>
<pre class="brush:java;">
static Person defaultPerson(int type){
...
<p>Но в зависимости от переданного параметра это может быть как объект типа Person, так и объект типа Employee (который также является и объектом типа Person)</p>
...
<style>
	...
	<div class="nav"><p><a href="./3.23.php">Назад</a><a href="./">Содержание</a><a href="./3.9.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
