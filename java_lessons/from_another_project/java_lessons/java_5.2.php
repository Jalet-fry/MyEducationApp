...
<meta charset="utf-8" />
<meta name="description" content="Интерфейс List и создание списков в языке программирования Java, класс ArrayList и его методы, добавление в список, изменение и удаление элементов списка">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Класс ArrayList и интерфейс List</h1><div class="date">Последнее обновление: 11.10.2025</div>
	<div class="socBlock">
...
<li><p><span class="b">void addFirst(E obj)</span>: добавляет объект в начало списка</p></li>
<li><p><span class="b">void addLast(E obj)</span>: добавляет объект в конец списка</p></li>
...
<li><p><span class="b">E getFirst()</span>: возвращает первый элемент списка</p></li>
<li><p><span class="b">E getLast()</span>: возвращает последний элемент списка</p></li>
...
<li><p><span class="b">E removeFirst()</span>: удаляет первый элемент из списка</p></li>
<li><p><span class="b">E removeLast()</span>: удаляет последний элемент из списка</p></li>
...
<li><p><span class="b">static &lt;E&gt; List&lt;E&gt;of(E... elements)</span>: возвращает неизменяемый список (тип <code>java.util.ImmutableCollections$ListN</code>), который содержит элементы elements.</p></li>
</ul>
...
<p>Рассмотрим основные операции <code>ArrayList</code>. Создание списка:</p>
...
<p>Поскольку компилятор может сам вывести тип, то мы можем сократить определение:</p>
<pre class="brush:java;">
...
<h3>Размер списка</h3>
<p>Для получения размера списка применяется метод <b>size()</b>, который возвращает значение типа <code>int</code>:</p>
<pre class="brush:java;">
var people = new ArrayList&lt;String&gt;();
System.out.println(people.size()); // 0
</pre>
<h3>Добавление</h3>
<p>Для добавления применяется метод <code>add()</code>, в который передается добавляемый элемент:</p>
<pre class="brush:java;">
var people = new ArrayList&lt;String&gt;();
...
<p>Кроме того, с помощью перегрузки метода <code>add()</code> мы можем добавить элемент на определенную позицию в списке:</p>
<pre class="brush:java;">
var people = new ArrayList&lt;String&gt;();
...
<p>В выражении <code>people.add(1, "Alice")</code> мы добавляем строку "Alice" в список по индексу 1.</p>
<p>Стоит отметить, что при создании <code>ArrayList</code> выделяется начальная область памяти, которая по умолчанию вмещает 10 элементов списка. Например, в коде выше при создании списка для него 
выделяется область памяти, достаточная для вмещения 10 строк String. Если мы будем добавлять большее количество строк, то будут происходить повторные выделения памяти для вмещения новых объектов. 
Поскольку выделения памяти на низком уровне - довольно затратный процесс, то, если мы заранее знаем сколько у нас будет объектов в списке или знаем по крайней мере максимальное количество объектов, 
то рекомендуется это количество передавать в конструктор при создании объекта <code>ArrayList</code>. Например, создадим список, у которого начальная емкость равна 16 объектам:</p>
<pre class="brush:java;">
var people = new ArrayList&lt;String&gt;(16);
System.out.println(people.size()); // 0
</pre>
<p>Причем важно, что емкость списка и его размер - это не одно и то же. Так, в данном случае при создании списка выделяется область для хранения 16 объектов, однако пока она пуста и содержит 0 объектов. Тем не менее, когда мы 
будем добавлять 11-й элемент, не будет происходить дополнительного выделения памяти, потому что необходимая память уже выделена. Что в итоге увеличит производительность.</p>
<p>Либо это можно сделать с помощью метода <code>ensureCapacity</code>:</p>
<pre class="brush:java;">people.ensureCapacity(25);</pre>
...
<h3>Добавление элементов из коллекции</h3>
<p>Добавлять в список по одному элементу может быть не очень удобно, особенно когда надо добавить множество элементов. Но Java предоставляет ряд способов для добавления набора элементов. Прежде всего 
интерфейс <code>List</code> определяет статический метод <code>of()</code>, в который можно передать произвольное количество объектов:</p>
<pre class="brush:java;">
...
<p>В данном случае переменная people представляет интерфейс <code>List&lt;String&gt;</code>. В реальности эта переменная представляет тип 
<code>java.util.ImmutableCollections$ListN</code> - внутренний класс Java, который представляет неизменяемый список фиксированной длины. Подобный список может быть полезен в некоторых ситуациях, но в данном случае 
мы говорим об <code>ArrayList</code> - изменяемом списке произвольного размера. И для его создания мы можем использовать конструктор:</p>
<pre class="brush:java;">
import java.util.List;
...
<p>Из других доступных способов можно отметить статический метод <code>Collections.addAll()</code>:</p>
<pre class="brush:java;">static &lt;T&gt; boolean addAll(Collection&lt;? super T&gt; c, T... elements)</pre>
<p>Первый параметр - коллекция, в которую надо добавить данные, а второй - добавляемые объекты. Применение:</p>
<pre class="brush:java;">
...
<p>В данном случае нам не надо создавать для добавляемых объектов отдельную коллекцию.</p>

<h3>Получение элементов списка</h3>
<p>Для получения элементов списка применяется метод <b>get()</b>, в который передается индекс элемента:</p>
<pre class="brush:java;">
import java.util.ArrayList;
...
<p>Для перебора списка можно использовать традиционный цикл <code>for</code>:</p>
<pre class="brush:java;">
import java.util.ArrayList;
...
<p>Или цикл <code>for</code> в стиле <code>for-each</code>:</p>
<pre class="brush:java;">
import java.util.ArrayList;
...
<p>Для изменения элемента применяется метод <span class="b">set(int index, E obj)</span>, где первый параметр представляет индекс элемента для изменения, а второй - помещаемый по этому индексу объект:</p>
<pre class="brush:java;">
import java.util.ArrayList;
...
<p>Нередко бывает, что нам надо узнать индекс определенного элемента, то применяются методы <span class="b">indexOf(Object obj)</span> (для первого индекса в списке) и 
<span class="b">lastIndexOf(Object obj)</span> для последнего индекса:</p>
<pre class="brush:java;">
import java.util.ArrayList;
...
<h3>Удаление</h3>
<p>Для удаления элементов применяется метод <span class="b">remove(Object obj)</span>, в который передается удаляемый объект:</p>
<pre class="brush:java;">
import java.util.ArrayList;
...
<p>Но метод <span class="b">remove()</span> имеет и другую версию, в которую передается индекс удаляемого элемента - <code>remove(int index)</code>. 
И тут может возникнуть вопрос, что будет, если у нас список чисел, и нам надо удалить какое-либо число. Например:</p>
<pre class="brush:java;">
import java.util.ArrayList;
...
<p>Данная программа завершится с ошибкой, поскольку при передаче числа в метод <code>remove()</code> это число будет восприниматься как индекс, а не как элемент. И при вызове 
<code>numbers.remove(12)</code> программа будет пытаться удалить элемент по индексу 12, которого в принципе не существует. И в этом случае надо будет сначала найти индекс удаляемого значения, а затем удалить его:</p>
<pre class="brush:java;">
numbers.remove(numbers.indexOf(12)); // удаляем элемент 12 
</pre>
<p>Еще один метод - <b>removeIf()</b> позволяет удалить все элементы, которые соответствуют некоторому предикату - условию. Например:</p>
<pre class="brush:java;">
import java.util.Collections;
import java.util.ArrayList;
...
<p>В метод <code>removeIf()</code> передается условие в виде функции, которая получает каждый элемент списка и возвращает <code>true</code>, если этот элемент соответствует некоторому условию. В данном случае удаляем все четные 
числа, то есть если при делении числа на 2 остаток равен 0:</p>
<pre class="brush:java;">numbers.removeIf( num -&gt; num % 2 == 0);</pre>
...
<style>
	...
	<div class="nav"><p><a href="./5.1.php">Назад</a><a href="./">Содержание</a><a href="./5.6.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
