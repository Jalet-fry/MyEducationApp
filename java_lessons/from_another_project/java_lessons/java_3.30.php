...
<html  lang="ru">
<head>
<title>Java | Анонимные классы</title>
<meta charset="utf-8" />
<meta name="description" content="Анонимные классы в языке программирования Java, применение анонимных классов вместо интерфейсов и абстрактных классов, конструкторы и иницализаторы анонимных классов">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Анонимные классы</h1><div class="date">Последнее обновление: 03.10.20125</div>
	<div class="socBlock">
...
<p>Иногда бывает необходимо создать только один объект некоторого класса. Причем его имя даже не важно. Такой класс можно определить как <span class="b">анонимный</span>. 
Часто анонимные классы создаются там, где требуется объект некоторого интерфейс, чтобы не определять отдельный класс.</p>
<p>Общий синтаксис определения анонимного класса следующий:</p>
...
<p>Здесь SuperType может быть интерфейсом, тогда анонимный класс реализует этот интерфейс. SuperType также может быть классом, тогда анонимный класс расширяет этот класс.</p>
<p>Анонимный внутренний класс не может иметь конструкторов, поскольку имя конструктора должно совпадать с именем класса, а у класса нет имени. Вместо этого аргументы конструирования передаются 
конструктору суперкласса (если в суперклассе определен конструктор с параметрами).</p>
<p>Если SuperType представляет интерфейс, то аргументы для конструктора естественно не передаются (ведь у интерфейсов нет конструктора), и просто указываются аустые скобки:</p>
<pre class="brush:java;">
new InterfaceType()
{
    // поля и методы анонимного класса
...
<h3>Анонимный класс вместо интерфейса</h3>
<p>Распространенным вариантом использования анонимных классов является их создание там, где требуется объект интерфейса. Например:</p>
<pre class="brush:java;">
...
<ol>
<li><p>У нас есть интерфейс Movable, в котором определен метод <code>move()</code></p>
<pre class="brush:java;">
interface Movable{
    void move(int distance);
}
...
<li><p>Определяем метод <code>moveObject()</code>, в который передается объект интерфейса Movable. И далее у этого объекта вызывается метод <code>move()</code></p>
<pre class="brush:java;">
static void moveObject(Movable obj, int distance){
    obj.move(distance);
}
...
<li><p>Определяем переменную <code>rectangle</code>, которая по сути представляет объект анонимного класса, который, в свою очередь, реализует интерфейс Movable:</p>
<pre class="brush:java;">
var rectangle = new Movable(){
...
<p>Это похоже на обычную реализацию интерфейса, только в данном случае у класса нет имени - его заменяет имя интерфейса и нет конструктора.</p>
</li>
<li><p>И в конце вызываем метод <code>moveObject()</code>, передавая в него объект анонимного класса:</p>
<pre class="brush:java;">moveObject(rectangle, 54);</pre>
</li>
</ol>
<p>При этом нам необязательно создавать отдельно переменную, которая содержит ссылку на объект анонимного класса. Мы можем создать объект анонимного класса непосредственно при вызове метода:</p>
<pre class="brush:java;">
public class Program{
...
<h3>Анонимный класс вместо класса</h3>
<p>Менее распространенный вариант, но тем не менее возможный - это расширение анонимным классом обычного класса. Например:</p>
<pre class="brush:java;">
public class Program{
...
<ol>
<li><p>Здесь определен абстрактный класс Person, который представляет некоторого человека. Через конструктор он принимает имя и определяет абстрактный метод <code>ptint()</code></p></li>
<li>
<p>Статический метод <code>printPerson()</code> принимает объект Person и вызывает у него метод <code>ptint()</code></p>
<pre class="brush:java;">
static void printPerson(Person person){
    person.print();
}
</pre></li>
<li><p>При вызове метода <code>printPerson()</code> передаем объект анонимного класса. Причем, как мы знаем, в Java мы не можем использовать вызов конструктора для создания объекта абстрактного класса. 
И в данном случае вызов</p>
<pre class="brush:java;">new Person("Tom")</pre>
<p>означает создание объекта анонимного класса, который расширяет класс Person и передает в его конструктор строку "Tom".</p>
<p>Затем в фигурных скобках определяем тело анонимного класса:</p>
<pre class="brush:java;">
{
    private String company = "Yandex";
    @Override
...
<p>В реализации добавляем дополнительное поле (для хранения компании) и определяем реализацию для абстрактного метода <code>print()</code>. Причем в реализации анонимного класса мы можем, как и в производных классах, 
ссылаться на функционал базового класса через ключевое слово <code>super</code></p></li>
</ol>

<h3>Инициализаторы в анонимных классах</h3>
<p>Как выше говорилось, мы не можем определить конструктор в анонимных классах, однако блок инициализатора для начальной инициализации:</p>
<pre class="brush:java;">
printPerson(
    new Person("Tom"){
...
<style>
	...
	<div class="nav"><p><a href="./3.12.php">Назад</a><a href="./">Содержание</a><a href="./3.25.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
