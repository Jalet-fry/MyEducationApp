...
<meta charset="utf-8" />
<meta name="description" content="Параметры методов в языке программирования Java, передача аргументов в метод, формальные и фактические параметры, неопределенное количество параметров">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Параметры методов</h1><div class="date">Последнее обновление: 16.09.2025</div>
	<div class="socBlock">
	...
	<p>С помощью параметров мы можем передать в методы различные данные, которые будут использоваться для вычислений. Например, определим следующую программу:</p>
...
<p>Здесь определен класс Person, в котором есть метод <code>say()</code>. Этот метод имитирует говорение некоторой фразы. В методе main создается один объект класса Person, и у него вызывается два раза метод say. 
И ожидаемо мы увидим на консоли два раза слово "Hello":</p>
<pre class="sh">
Hello
Hello
</pre>
<p>Но сколько бы мы не вызывали метод say, результат не измениться. И чтобы иметь возможность настроить извне поведение метода применяются параметры. 
Параметры определяются через заятую в скобках после названия метода в виде:</p>
<pre class="brush:java;">
...
<p>Так, используем параметры и для этого изменим программу следующим образом:</p>
<pre class="brush:java;">
class Program{
...
<p>Здесь метод <code>say()</code> теперь принимает один параметр, который называется message и имеет тип <code>String</code>.</p>
...
<p>Здесь параметру <code>message</code> передается строка "Hello work". Значения, которые передаются параметрам, еще называются <b>аргументами</b>. То есть передаваемая строка "Hello work" в данном случае 
является аргументом.</p>
...
<p>Рассмотрим другой пример, где у нас есть метод, который складывает два числа:</p>
<pre class="brush:java;">
class Program{
...
<p>Здесь определен класс Calculator  с одним методом sum, имеет два параметра - два числа и выводит на консоль их сумму. Поскольку оба параметра имеют тип <code>int</code>, то на место параметров надо 
...
могут быть автоматически преобразованы в тип <code>int</code>.</p>
<p>Значения передаются параметрам по позиции, то есть первый аргумент первому параметру, второй аргумент - второму параметру и так далее.  
Например, при вызове <code>calc.sum(5, 3)</code> число 5 передается параметру <code>a</code>, а число 3 - параметру <code>b</code>.</p>
...
<style>
	...
	<div class="nav"><p><a href="./2.7.php">Назад</a><a href="./">Содержание</a><a href="./2.17.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
