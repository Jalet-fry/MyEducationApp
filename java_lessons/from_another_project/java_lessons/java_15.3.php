...
<html  lang="ru">
<head>
<title>Java | Обработка аннотаций во время выполнения</title>
<meta charset="utf-8" />
<meta name="description" content="Обработка аннотаций во время выполнения с помощью рефлексии в языке программирования Java, методы getAnnotation, getDeclaredAnnotation, getAnnotationsByType, isAnnotationPresent, создание механизма валидации с помощью аннотаций">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Обработка аннотаций во время выполнения</h1><div class="date">Последнее обновление: 01.11.2025</div>
	<div class="socBlock">
...
<p>Аннотация сама по себе в языке Java ничего делает. Чтобы использовать аннотацию каким-то образом, нам нужно ее обрабатывать. Во время выполнения мы можем получить данные аннотаций с помощью рефлексии. Для этого в пакете <code>java.lang.reflect</code> определен интерфейс <b>AnnotatedElement</b>. ДЛя работы с аннотациями этот интерфейс объявляет ряд методов:</p>
...
<p>Рассматривая эти методы, стоит отметить о различиях в аннотациях:</p>
<li><p><span class="b">аннотация к элементу типа</span> - это аннотация объявления (declaration annotation). 
Для из получения предназначены методы <code>getDeclaredAnnotation() / getDeclaredAnnotations() / getDeclaredAnnotationByType()</code></p></li> , 
<li><p><span class="b">аннотация к типу</span> (type annotation) - это аннотация типа. Для их получения предназначены методы с суффиксом <code>ByType</code>.</p>
<p>Методы <code>getAnnotations()</code> и <code>getDeclaredAnnotations()</code> возвращают вообще все применяемые к типу или его элементу аннотации в виде массива объектов <span class="b">Annotation</span>. Этот тип 
представляет интерфейс, из методов которого можно отметить метод <code>annotationType()</code> - он возвращает класс конкретной аннотации:</p>
<pre class="brush:java;">
Class&lt;? extends Annotation&gt; annotationType()
</pre>

<p>Поскольку классы <code>Class</code>, <code>Field</code>, <code>Parameter</code>, <code>Method</code>, <code>Constructor</code> и <code>Package</code> реализуют этот интерфейс, то, получив с помощью рефлексии 
определенный компонент программы, мы можем получить его аннотации.</p>
...
<p>Здесь сначала определяем переменную класса, к которому применяется аннотация:</p>
<pre class="brush:java;">var obj = new Thing();</pre>
<p>Получаем класс переменной:</p>
<pre class="brush:java;">var cl = obj.getClass();</pre>
<p>И далее получаем нашу аннотацию MyAnnotation:</p>
<pre class="brush:java;">MyAnnotation annot = cl.getAnnotation(MyAnnotation.class);</pre>
<p>Обратите внимание, что в метод передается объект класса аннотации (в данном случае <code>MyAnnotation.class</code>). В качестве результата метод возвращает объект некоторого прокси-класса, 
который реализует интерфейс <code>MyAnnotation</code>.</p>
<p>Затем можно вызвать методы интерфейса. Однако может быть, что у класса отсутствует данная аннотация, и в этом случае метод <code>getAnnotation()</code> возвращает <code>null</code>. 
Соответственно перед вызовом методов аннотации необходимо проверить ее объект на <code>null</code>:</p>
<pre class="brush:java;">
if (annot != null){
...
<p>Метод <code>getAnnotation()</code> возвращает один объект аннотации. Если же к типу или его элементу применяется несколько аннотаций, и все их надо получить, 
то можно использовать метод <code>getAnnotations()</code>, который возвращает массив объектов <code>Annotation</code>:</p>
<pre class="brush:java;">
import java.lang.annotation.ElementType;
import java.lang.annotation.Retention;
import java.lang.annotation.RetentionPolicy;
...
<p>В данном случае к классу Thing применяется две аннотации, и мы получим следующий консольный вывод:</p>
<pre class="sh;">
@Name("Thing")
interface Name
@Description(data="Represents some object with a name")
...
<p>Рассмотрим более практический пример применения аннотаций. Для этого возьмем следующую программу:</p>
<pre class="brush:java;">
import java.lang.annotation.ElementType;
import java.lang.annotation.Retention;
import java.lang.annotation.RetentionPolicy;
...
<p>Данная программа создает механизм, который позволяет декларативно задавать правила для полей, а затем динамически проверять, соответствуют ли реальные объекты этим правилам. 
Рассмотрим пошагово, что здесь происходит:</p>
<ul>
<li><p>Создание аннотации <code>@Range</code><p>
<pre class="brush:java;">
@Retention(RetentionPolicy.RUNTIME) 
@Target(ElementType.FIELD) 
@interface Range {
...
<p>Аннотация имеет два параметра, <code>min</code> и <code>max</code>, которые будут хранить допустимый диапазон значений.</p></li>

<li><p>Класс <code>Person</code> имеет поле <code>age</code>, которое аннотировано созданной нами аннотацией:</p>
<pre class="brush:java;">
class Person{
...
<p>Это "правило" означает, что допустимый возраст для <code>Person</code> должен быть в диапазоне от 1 до 110.</p></li>

<li><p>Класс <code>RangeValidator</code> отвечает за проверку произвольного объекта. Для этого в нем определен метод <code>validate()</code>, который принимает любой объект <code>obj</code> 
и возвращает <code>true</code> (если объект валиден) или <code>false</code> (если нет).</p>
<pre class="brush:java;">
public static boolean validate(Object obj) throws Exception{
        // Получаем класс объекта 
        var cl = obj.getClass();
...
<p>С помощью рефлексии метод получает "описание" класса переданного объекта (например, класса <code>Person</code>). Далее перебираем все поля класса</p>
<p><code>for (Field field : cl.getDeclaredFields())</code></p>
<p>Затем проверяем, есть ли у текущего поля аннотация <code>@Range</code></p>
<p><code>if (field.isAnnotationPresent(Range.class) ...)</code></p>
<p>Если аннотация найдена, то получаем саму аннотацию и ее параметры ( <code>min=1</code> и <code>max=110</code>).</p>
<p><code>Range range = field.getAnnotation(Range.class);</code></p>
<p>Разрешаем доступ к полю, даже если бы оно было <code>private</code>:</p>
<p><code>field.setAccessible(true);</code></p>
<p>С помощью рефлексии получаем реальное значение поля <code>age</code> из конкретного объекта</p>
<p><code>int age = (int)field.get(obj);</code></p> 
<p>И в конце сравниваем значение поля с границами из аннотации (1 и 110). Если значение выходит за рамки, метод немедленно возвращает <code>false</code> (объект невалиден).</p>
<p><code>if(age &gt; range.max() || age &lt; range.min()) return false;</code></p> 
<p>В противном случае возвращается <code>true</code> - объект считается валидным.</p></li>

<li><p>В классе <code>Program</code> для упрощени проверки определяет вспомогательный метод <code>validatePerson</code> и вызываем его, передавая в него пару типовых объектов.</p></li>
</ul>
...
<style>
	...
	<div class="nav"><p><a href="./15.2.php">Назад</a><a href="./">Содержание</a><a href="./16.1.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
