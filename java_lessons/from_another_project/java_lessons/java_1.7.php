...
<meta charset="utf-8" />
<meta name="description" content="Выполнение программ на языке программирования Java в среде JShell и облочка REPL, автодополнение">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>JShell</h1><div class="date">Последнее обновление: 16.09.2025</div>
	<div class="socBlock">
...
<p>Консольная утилита <b>JShell</b> предоставляет функциональность "read-evaluate-print loop" или REPL (цикл "чтение-вычисление-вывод"). В этой программе вы вводите выражение языка Java, и 
JShell оценивает и выполняет это выражение, выводит результат выполнения и ждет следующего ввода. JShell предоставляет отличный способ экспериментировать — гораздо быстрее, чем писать 
полноценную программу.</p>
<p>Для начала работы с JShell введем команды "jshell" в консоли/командной строке:</p>
<pre class="sh">
eugene@Eugene:~$ jshell
|  Welcome to JShell -- Version 22.0.1
|  For an introduction type: /help intro

jshell&gt; 
</pre>
<p>Далее введем простейшее выражение на языке Java - <code>2 + 6</code> (то есть попробуем найти сумму двух чисел):</p>
<pre class="sh">
jshell&gt; 2 + 6
$1 ==&gt; 8
...
<p>JShell возвращает результат — в данном случае число 8 - сумму чисел 2 и 6</p>
<pre class="sh">$1 ==&gt; 8</pre>
<p>Значение <span class="b">$1</span> в выводе означает, что результат (число 8) доступен для дальнейших вычислений через плейсхолдер <code>$1</code>. Например, далее введем выражение <code>5 * $1</code>:</p>
<pre class="sh">
jshell&gt; 5 * $1
$2 ==&gt; 40
...
<p>Фактически выражение <code>5 * $1</code> означает выполнение выражения <code>5 * 8</code>, поэтому в итоге мы получим число 40б и это число сможем использовать через плейсхолдер <code>$2</code></p>
<p>Если какое-то значение используется множество раз, то ей можно дать более запоминающееся имя. Например:</p>
<pre class="sh">
jshell&gt; var sum = 2 + 5
sum ==&gt; 7
...
<p>Здесь с помощью ключевого слова <code>var</code> определяем переменную "sum", которая будет хранить сумму чисел 2 и 5. И далее мы можем использовать эту переменную:</p>
<pre class="sh">
jshell&gt; 4 * sum
$4 ==&gt; 28
...
<p>Еще одна полезная функция — автодополнение с помощью клавиши Tab. Например, введем строку "System." (с точкой в конце) и нажмем клавишу Tab. Из-за большого количества автодополнений 
нам будет предложено нажать клавишу Tab ещё раз. И мы получим список всех методов, которые можно вызвать с помощью класса System:</p>
<pre class="sh">
jshell&gt; System.
...
<p>Далее введем "System.out." и опять же нажмем на Tab:</p>
<pre class="sh">
jshell&gt; System.out.
append(        charset()      checkError()   close()        equals(        
flush()        format(        getClass()     hashCode()     notify()       
notifyAll()    print(         printf(        println(       toString()     
wait(          write(         writeBytes(    
jshell&gt; System.out.
</pre>
<p>Мы видим, какие методы есть у объекта <code>System.out</code>. Допустим, нам нужно вывести строку на консоль. Для этого нам нужен один из методов - printf, print или println. Поэтому 
введем "<code>System.out.p</code> - с буквой "p" на конце и также нажмем на Tab</p>
<pre class="sh">
jshell&gt; System.out.p
print(     printf(    println(   
jshell&gt; System.out.print
</pre>
<p>Программа сразу подставляет общую начальную часть - "print", и нам далее прощее допечатать необходимый код:</p>
<pre class="sh">
...
<h3>Выход из программы</h3>
<p>Для выхода из программы необходимо ввести команду "/exit":</p>
<pre class="sh">
jshell&gt; /exit
|  Goodbye
eugene@Eugene:~$ 
...
<style>
	...
	<div class="nav"><p><a href="./1.4.php">Назад</a><a href="./">Содержание</a><a href="./2.11.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
