...
<html  lang="ru">
<head>
<title>Java | Управление потоками</title>
<meta charset="utf-8" />
<meta name="description" content="Управление потоками в языке программирования Java, ожидание окончания потока с помощью метода join, приостановка текущего потока с помощью метода sleep">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Управление потоками</h1><div class="date">Последнее обновление: 16.10.2025</div>
	<div class="socBlock">
...
<p>Класс <code>Thread</code> предоставляет ряд методов для управления потоками, которые мы можем использовать в программах. Рассмотрим ряд этих методов.</p>
<h3>Метод sleep</h3>
<p>Статический метод <span class="b">Thread.sleep()</span> приостанавливает текущий поток (переводит в спящий режим) на определенный период времени. Этот метод имеет ряд версий:</p>
<pre class="brush:java;">
static void sleep(long millis)
static void sleep(long millis, int nanos)
static void sleep(Duration duration) </pre>
<p>Все  три версии в качестве параметра принимают время приостановки потока. В первой версии - это количество миллисекунд, во второй версии дополнительно передается количество наносекунд, которые добавляются к милллисекундам. 
В третьей версии время передается через объект <code>Duration</code></p>
<p>Стоит учитывать, что этот метод может сгенерировать исключение типа <code>InterruptedException</code>. Соответственно нам его надо обработать или передать обработку внешнему коду.</p>
<p>Применим первую версию:</p>
<pre class="brush:java;">
...
<p>В данном случае в классе MyTask в методе <code>run()</code>, который определяет действия потока, в конструкции <code>try..catch</code> осуществляем задержку на 1000 миллисекунд (1 минуту)</p>
<pre class="brush:java;">
try{
    Thread.sleep(1000); // приостанавливаем поток на 1000 миллисекунд
}
...
<p>Таким образом, когда выполнение дойдет до вызова <code>Thread.sleep(1000)</code> поток приостановит выполнение и возобновит его только через 1 секунду. В итоге мы получим следующий консольный вывод:</p>
<pre class="sh">
Main thread started...
...
<h3>Ожидание завершения потока и метод join</h3>
<p>При запуске потоков в примерах ранее основной поток (Main thread) завершался до дочернего потока. Как правило, более распространенной ситуацией является случай, 
когда основной поток завершается самым последним - после всех своих дочерних потоков. Для этого надо применить метод <span class="b">join()</span>:</p>
<pre class="brush:java;">
void join()
...
<p>Этот метод также имеет несколько версий. Но вне зависимости от версии текущий поток будет ожидать завершения потока, для которого вызван метод <code>join()</code>. 
С помощью дополнительных параметров в метод можно передать время, в течение которго текущий поток будет ожидать будет ожидать завершения потока.</p>
...
<p>Например, изменим предыдущий пример - пусть главный поток теперь ожидает завершения дочернего потока:</p>
<pre class="brush:java;">
class MyTask implements Runnable {
...
<p>Стоит учитывать, что метод <code>join()</code> также генерирует исключение, поэтому его вызов также помещаем в конструкцию <code>try..catch</code> с обработкой исключения <code>InterruptedException</code>.</p>
<pre class="brush:java;">
var task = new MyTask();
var t = new Thread(task, "MyTask");
t.start();
...
<p>В итоге теперь главный поток завершится после потока MyTask:</p>
<pre class="sh">
Main thread started...
MyTask started...
MyTask works
...
<p>Однако при ожидании потока мы можем столкнуться со слишком долгим или даже неограниченным ожиданием завершения потока, особенно если в его коде существует какая-то логическая ошибка. В 
этом случае мы можем ограничить ожидание определенным временем. Например, ограничим ожидание 500 миллисекундами:</p>
<pre class="brush:java;">
var task = new MyTask();
var t = new Thread(task, "MyTask");
t.start();
...
<p>В примере выше поток MyTask выполняется более 1 секунды, тогда как его ожидание длится не более 500 миллисекунд. Поэтому главный поток завершит свою работу, не дожидаясь завершения MyTaskЖ</p>
<pre class="sh">
Main thread started...
MyTask started...
MyTask works
...
<h4>Ождание группы потоков</h4>
<p>Если в программе используется несколько дочерних потоков, и надо, чтобы главный поток (Main thread) завершался после дочерних, то для каждого дочернего потока надо вызвать метод <code>join()</code>. Например:</p>
<pre class="brush:java;">
class MyTask implements Runnable {
...
<p>Здесь запускаем 4 потока и затем ожидаем их завершения. В итоге главный поток будет завершаться только после завершения всех четырех дочерних потоков:</p>
<pre class="sh">
Main thread started...
MyTask_2 started...
MyTask_1 started...
...
<style>
	...
	<div class="nav"><p><a href="./8.2.php">Назад</a><a href="./">Содержание</a><a href="./8.4.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
