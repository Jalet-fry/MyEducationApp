...
<html  lang="ru">
<head>
<title>Java | Foreign Functions и Memory API</title>
<meta charset="utf-8" />
<meta name="description" content="Использование Foreign Functions и Memory API для взаимодействия с нативным кодом на языке Си в программе на языке программирования Java, опция запуска enable-native-access, типы Linker, Arena, MemorySegment, MethodHandle">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Foreign Functions и Memory API</h1><div class="date">Последнее обновление: 21.12.2025</div>
	<div class="socBlock">
...
<p>Хотя подход подключения функционала на Си в программу на Java с помощью <span class="b">JNI</span> продолжает работать и в самых последних версиях Java, тем не менее подключение нативного кода 
с помощью JNI довольно трудоемко, требует написания дополнительного кода на Си, компиляцию соответствующей библиотеки и т.д. Чтобы упростить всю эту работу в последних версиях Java был добавлен новый 
API для доступа к "внешним" функциям и памяти - <span class="b">Foreign Functions and Memory API</span> (<span class="b">FFM</span>), ранее развивавшийся по именем <span class="b">Project Panama</span>. Начиная с версии Java 14 этот API был экспериментальным, а 
в Java 22 данный API достиг состояния релиза и стал полноценной функциональностью платформы. Предполагается, что со временем он вытеснит JNI.</p>
<p>Для примера опять же используем стандартную библиотечную функцию <span class="b">printf</span> из стандартной библиотеки языка Си. И для этого определим файл <span class="b">Program.java</span> со следующим кодом:</p>
...
<p>Разберем, что происходит в коде. Прежде всего создаем <span class="b">линкер</span> (компоновщик) для работы с нативными функциями:</p>
<pre class="brush:java;">
Linker linker = Linker.nativeLinker();
</pre>
<p>Линкер по сути представляет связующее звено между Java и нативной архитектурой текущей операционной системы.</p>

<p>Далее определяем дескриптор для вызываемыой функции в виде объекта <code>MethodHandle</code></p>
<pre class="brush:java;">
MethodHandle printf = linker.downcallHandle(...)
</pre>
<p><code>MethodHandle printf</code> представляет своего рода указатель на нативную функцию, который Java может вызвать как обычный метод.</p></li>

<p>Для создания этого объекта применяется метод <code>linker.downcallHandle()</code>, который принимает два параметра:</p>
<ul>
<li><p>Дескриптор внешней функции. Чтобы найти эту функцию, вызывается последовательность методов:</p>
<pre class="brush:java;">linker.defaultLookup().findOrThrow("printf")</pre>
<p>Здесь выражение <code>linker.defaultLookup()</code> выполняет поиск символов (функций) в стандартных библиотеках C, которые загружены по умолчанию. А выражение 
<code>findOrThrow("printf")</code> указывает имя искомой функции (то есть в нашем случае <code>printf</code>)</p>
</li>
<li><p>Параметры линкера. Применяются для описания сигнатуры функции</p>
<pre class="brush:java;">
FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS)
</pre>
<p>В данном случае указываем, что <code>printf()</code> возвращает <code>int</code> (<code>ValueLayout.JAVA_INT</code>) и принимает указатель - <code>ValueLayout.ADDRESS</code> (<code>char*</code>).</p>
</li>
</ul>

<p>Следующий важный момент - выделение памяти для строки, которая будет передаваться в вызываемую функцию в качестве параметра:</p>
<pre class="brush:java;">
try (Arena arena = Arena.ofConfined()) {
       MemorySegment str = arena.allocateFrom("Hello METANIT.COM\n");
</pre></p>
<p>Поскольку нативные функции не умеют работать с объектами Java (такими как <code>String</code>), данные нужно разместить в "неуправляемой" памяти (вне кучи Java), для чего применяются следующие действия:</p>
<ol>
<li><p><code>Arena.ofConfined()</code>: создает область памяти (арену), которая автоматически освобождается при закрытии блока <code>try</code>. Это предотвращает утечки памяти.</p></li>
<li><p><code>arena.allocateFrom("...")</code>: выделяет память для переданной строки Java и копирует эту строку в выделенную нативную память, добавляя в конец нулевой байт (<code>\0</code>), чтобы формат 
соответствовал стандарту C-строк. Возвращает объект <code>MemorySegment</code> - по сути, безопасный указатель для работы с сегментом памяти.</p></li>
</ol>
<p>И далее собственно идет вызов гативной функции <code>printf()</code> и получение результата:</p>
<pre class="brush:java;">
int result = (int) printf.invoke(str);
</pre>
<p>В итоге мы имеем следующий общий алгоритм работы программы:</p>
<ol>
<li><p><span class="b">Поиск</span>. Находит функцию <code>printf</code> в системных библиотеках.</p></li>
<li><p><span class="b">Выделение памяти</span>. Резервирует блок памяти вне Java для строки.</p></li>
<li><p><span class="b">Выполнение</span>. Вызывает нативную функцию, передавая ей адрес этого блока.</p></li>
<li><p><span class="b">Освобождение</span>. После выполнения блока <code>try</code> память, выделенная под строку, мгновенно освобождается.</p></li>
</ol>

<p>Стоит отметить, что как и в случае с <span class="b">JNI</span>, для запуска программы необходимо передать утилите java дополнительную опцию, которая разрешает использование нативного кода в программе на Java. Если модули не используются, то программа запускается следующим образом:</p>
...
<p>Протестируем программу, запустив ее на выполнение:</p>
<pre class="sh">
