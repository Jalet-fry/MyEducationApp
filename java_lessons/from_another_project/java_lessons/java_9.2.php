...
<meta charset="utf-8" />
<meta name="description" content="Лямбды как параметры методов в языке программирования Java, возвращение из метода лямбда-выражения">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Лямбды как параметры и результаты методов</h1><div class="date">Последнее обновление: 02.10.2025</div>
	<div class="socBlock">
...
<p>Одним из преимуществ лямбд в языке Java является то, что их можно передавать в качестве параметров в методы. Рассмотрим пример:</p>
...
<p>Здесь для представления лямбда-выражений применяется функциональный интерфейс <code>Operation</code> с методом <code>execute()</code>, который принимает два числа, должен выполнять над ними некоторые вычисления и 
возвращает число - результат вычислений:</p>
<pre class="brush:java;">
interface Operation{
    int execute(int a, int b);
}
</pre>
<p>В программе определен метод doOperation, который принимает два числа и объект интерфейса Operation - то есть некоторую операцию над двумя числами:</p>
<pre class="brush:java;">
static void doOperation(int a, int b, Operation op)
{
    System.out.println(op.execute(a, b));
...
<p>То есть на момент определения метода мы точно не знаем, что это будет за операция. Мы только знаем, что она принимает два числа и возвращает число.</p>

<p>В методе <code>main</code> вызываем несколько раз метод doOperation, передавая в него через лямбда-выражения разные операции. При этом можно не определять переменную интерфейса, а сразу передать в метод лямбда-выражение:</p>
<pre class="brush:java;">
doOperation(10, 4, (a, b) -&gt; a + b);  // 14
doOperation(10, 4, (a, b) -&gt; a - b);  // 6
doOperation(10, 4, (a, b) -&gt; a * b);  // 40
</pre>
<p>Это особенно удобно тем, что нам не надо дополнительно создавать переменную для хранения лямбда-выражения, которое, возможно, использовалось бы один раз. Хотя при необходимости также можно было бы создать переменную 
интерфейса Operation и передать ее в метод.</p>
...
sum.</p>
<p>В методе main применяем метод sum, передавая в него лямбда-выражение:</p>
<pre class="brush:java;">
int[] nums = { 1, 2, 3, 4, 5, 6, 7, 8, 9 };
