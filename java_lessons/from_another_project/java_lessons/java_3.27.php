...
<html  lang="ru">
<head>
<title>Java | Pattern мatching. record-паттерн</title>
<meta charset="utf-8" />
<meta name="description" content="Pattern мatching. record-паттерн в языке программирования Java, проверка типа класса record и сопоставление его полей с переменными с помощью оператора instanceof и конструкции switch, оператор when, защитные условия guards, проверка на null">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Pattern мatching. record-паттерн</h1><div class="date">Последнее обновление: 27.09.2025</div>
	<div class="socBlock">
...
<p>Начиная с версии Java 21 мы можем использовать так называемый <span class="b">record pattern</span> - паттерн, который позволяет проверить на соответствие типу классу record и использовать его поля. 
Для применения паттерна record можно применять как оператор <code>instanceof</code>, так и конструкции/выражения <code>switch</code>.</p>
<h3>instanceof</h3>
<p>Пример <code>record patter</code> с применением оператора <code>instanceof</code>:</p>
...
<p>В данном случае с помощью выражения <code>tom instanceof Person(var pName, var pAge)</code> мы не только проверяем, что переменная tom представляет тип Person, но и объявляем переменные name и age, которые получают значения 
одноименных полей класса Person. Затем внутри блока <code>if</code> мы можем использовать эти переменные.</p>
<p>Причем мы явным образом можем указать тип переменных</p>
<pre class="brush:java;">
if(tom instanceof Person(String pName, int pAge)){
    System.out.printf("Person. Name: %s;  Age: %d\n", pName, pAge);
}
</pre>
<p>Если нам какое-то поле не нужно, мы можем его опустить и вместо переменной указать прочерк. Например, нам нужно только поле age:</p>
<pre class="brush:java;">
...
<h3>switch</h3>
<p>Конструкции/выражения <code>switch</code> также позволяют использовать данный паттерн:</p>
<pre class="brush:java;">
Person tom = new Person("Tom", 22);
...
<h3>Константы и защитные условия guards</h3>
<p>Стоит учитывать, что в record pattern мы не можем использовать константы. Напимер, нам нужно отдельно отследить ситуацию, когда имя пользователя - "Tom". Но мы не можем написать так:</p>
<pre class="brush:java;">
if(tom instanceof Person("Tom", _)){  // Ошибка: "Tom" - константа
    System.out.printf("Person Tom");
}
</pre>
<p>Однако мы можем использовать защитные условия, где переменные сравниваются с определенными константами:</p>
<pre class="brush:java;">
public class Program{
...
<p>Таким образом, если имя  "Tom", то срабатывает блок <code>if</code>:</p>
<pre class="brush:java;">
if(person instanceof Person(var pName, _) && pName == "Tom"){
    System.out.println("Hello Tom");
}
</pre>
<p>Если же у пользователя другое имя, то срабатывает блок <code>else if</code></p>
<pre class="brush:java;">
else if(person instanceof Person(_, _) ){
    System.out.println("Hello");
}
...
<p>При использовании <code>switch</code> для создания защитных условий применяется оператор <code>when</code>, после которого идет условие:</p>
<pre class="brush:java;">
public class Program{
...
<p>Шаблоны могут быть вложенными:</p>
<pre class="brush:java;">
public class Program{
...
<p>В данном случае второе поле класса Company представляет тип Person. А с помощью выражения <code>mycorp instanceof Company(_, Person(var name, _))</code> получаем поле name у класса Person в одноименную переменную name.</p>
<p>Аналогичный пример с <code>switch</code>:</p>
<pre class="brush:java;">
public class Program{
...
<p>Обратите внимание на последние два блока сопоставления:</p>
<pre class="brush:java;">
case null -&gt; {}
default -&gt; {}
</pre>
<p>При определении конструкции следует учитывать, что варианты в ней должны быть "exhaustive" (исчерпывающими), то есть охватывать все возможные варианты. В нашем случае у нас компания может быть равна null:</p>
<pre class="brush:java;">Company mycorp = null;</pre>
<p>Этот вариант отслеживается блоком</p>
<pre class="brush:java;">
case null -&gt; {}
</pre>
<p>Однако у нас может быть вариант, что Person равен null:</p>
<pre class="brush:java;">
Company mycorp = new Company("MyCorp", null);
</pre>
<p>Этот вариант отслеживается блоком <code>default</code>, который отслеживает все остальные варианты:</p>
<pre class="brush:java;">
default -&gt; {}
</pre>
...
<style>
	...
	<div class="nav"><p><a href="./3.26.php">Назад</a><a href="./">Содержание</a><a href="./2.10.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
