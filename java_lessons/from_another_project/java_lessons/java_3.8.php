...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Перечисления enum</h1><div class="date">Последнее обновление: 26.09.2025</div>
	<div class="socBlock">
...
<p>После названия перечисления в фигурных скобках через запятую идет список элементов перечисления. Например:</p>
<pre class="brush:java;">
enum DayTime{
...
<p>Здесь определено перечисление DayTime, которое имеет четыре значения: MORNING, AFTERNOON, EVENING, NIGHT. Названия констант произвольны, но в Java часто определяются в вернем регистре.</p> 
...
<p>Обратите внимание, что при использовании констант нам надо указывать их полное имя с учетом перечисления:</p>
<pre class="brush:java;">DayTime current = DayTime.MORNING;</pre>
<p>При выводе на консоль константа перечисления преобразуется в строку, которая фактически представляет название этой константы, например, для <code>DayTime.MORNING</code> это "MORNING".</p>
<h3>Перечисления в конструкциях/выражениях switch</h3>
<p>Перечисления удобно использовать в конструкциях/выражениях <code>switch</code>, которые позволяют не указывать название перечисления при сопоставлении с константами:</p>
<pre class="brush:java;">
class Program{
...
<h3>Перечисления для хранения состояния</h3>
<p>Перечисления удобно использовать в классах для хранения состояния:</p>
<pre class="brush:java;">
class Program{
...
<p>Здесь перечисление OpType представляет тип операции. Класс Operation представляет арифметическую операцию и через конструктор устанавливает ее тип, а также два операнда. А в 
методе execute в зависимости от типа операции выполняет те или иные действия.</p>
...
<p>Данный метод удобен для создания карт сопоставления:</p>
<pre class="brush:java;">
class Program{
...
<h2>Конструкторы, поля и методы перечисления</h2>
...
<p>Перечисление DayTome определяет приватное поле message для хранения сообщения, связанного со значением перечисления, а с помощью метода getMessage оно возвращается. 
Через конструктор передается для него значение.</p>
<p>Следует отметить, что конструктор 
по умолчанию приватный, то есть имеет модификатор <code>private</code>. Любой другой модификатор будет считаться ошибкой. Поэтому создать константы перечисления с помощью 
конструктора мы можем только внутри перечисления.</p>
<h4>Определение методов</h4>
...
<style>
	...
	<div class="nav"><p><a href="./3.16.php">Назад</a><a href="./">Содержание</a><a href="./3.11.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
