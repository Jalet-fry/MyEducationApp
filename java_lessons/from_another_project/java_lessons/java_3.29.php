...
<html  lang="ru">
<head>
<title>Java | Множественная реализация и наследование интерфейсов</title>
<meta charset="utf-8" />
<meta name="description" content="Множественная реализация интерфейсов в языке программирования Java как решение проблемы множественного наследования, конфликты методов разных интерфейсов, наследование интерфейсов">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Множественная реализация и наследование интерфейсов</h1><div class="date">Последнее обновление: 30.09.2025</div>
	<div class="socBlock">
...
<p>В ряде языков программирования доступно множественное наследование. Например, в языке C++ можно наследовать один класс от нескольких базовых классов. В Java множественное наследование недоступно, 
однако мы можем использовать множественную реализацию интерфейсов, когда один класс применяет сразу несколько интерфейсов.</p>
...
<p>В данном случае класс TextFile реализует два интерфейса: File и Printable, поэтому класс обязан реализовать методы обоих этих интерфейсов. Это также означает, что 
объект класса TextFile может рассматриваться и как объект File, и как объект Printable:</p>
<pre class="brush:java;">
public class Program{
...
<p>В данном случае метод <code>printFileName</code> принимает объект интерфейса File, соответвенно мы можем передать в метод объект любого класса, который реализует этот интерфейс. Аналогично метод 
<code>printObj()</code> принимает объект интерфейса Printable, вместо которого можно передать объект класса TextFile, реализующего данный интерфейс.</p>
<h4>Конфликты методов по умолчанию</h4>
<p>Что будет, если один и тот же метод определен как метод по умолчанию в одном интерфейсе, и как метод суперкласса или другого интерфейса? В этом случае:</p>
<ol>
<li><p>Суперклассы имеют преимущество. Если суперкласс предоставляет конкретный метод, методы по умолчанию с теми же именами и типами параметров просто игнорируются.</p></li>
<li><p>Конфликт интерфейсов. Если интерфейс предоставляет метод по умолчанию, а другой интерфейс содержит метод с теми же именами и типами параметров (стандартными или нет), то необходимо разрешить конфликт, переопределив этот метод.</p></li>
</ol>
<p>С первым пунктом в принципе все должно быть ясно. ПОэтому посмотрим на второй вариант.</p>
<pre class="brush:java;">
public class Program{
...
<p>Здесь класс EBook представляет класс электронной книги. Он реализует два интерфейса: интерфейс Printable представляет печать на принтере с помощью метода print и интерфейс Console, который представляет работу с консолью, 
в частности вывод на консоль с помощью метода print. Применение двух интерфейсов логично: мы хотим, чтобы электронную книгу можно было распечатать на принтере, а ее текст можно было бы вывести на консоль. 
Но поскольку в обоих интерфейсах определен метод с одним и тем же именем, мы столкнемся с ошибкой на этапе компиляции:</p>
<pre class="sh">
...
<p>Посмотрим, какие есть варианты для разрешения этой двойственности</p>
<h4>Явное определение метода в классе</h4>
<p>В этом случае мы можем предоставить реализацию метода print в классе EBook:</p>
<pre class="brush:java;">
class EBook implements Printable, Console{ 
...
<p>Причем в данном случае неважно, как мы будем рассматривать объект EBook - как объект Printable, или как объект Console, метод print все равно будет работать:</p>
<pre class="brush:java;">
...
<h4>Выбор реализации определенного интерфейса</h4>
<p>В качестве альтернативы мы можем выбрать реализацию из определенного интерфейса:</p>
<pre class="brush:java;">
public class Program{
...
<p>С помощью выражения <code>Printable.super.print()</code> в методе print класса EBook мы выбираем реализацию из интерфейса Printable. ПРичем это будет работать вне зависимости, какой интерфейс представляет переменная, даже если это 
переменная интерфейса Console:</p>
<pre class="brush:java;">
Console book = new EBook();
...
<p>В данном случае интерфейс File наследует интерфейс Printable. А это значит, что класс, который реализует интерфейс File, должен будет реализовать как методы интерфейса File, так и методы базового 
интерфейса Printable. Например:</p>
<pre class="brush:java;">
public class Program{
...
<style>
	...
	<div class="nav"><p><a href="./3.28.php">Назад</a><a href="./">Содержание</a><a href="./3.16.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
