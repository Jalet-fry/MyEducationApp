...
<html  lang="ru">
<head>
<title>Java | Executor - исполнитель задач</title>
<meta charset="utf-8" />
<meta name="description" content="Интерфейс Executor как исполнитель задач в языке программирования Java, ExecutorService и пул потоков, методы Executors.newFixedThreadPool, newSingleThreadExecutor, newVirtualThreadPerTaskExecutor и newCachedThreadPool">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Executor - исполнитель задач</h1><div class="date">Последнее обновление: 20.10.2025</div>
	<div class="socBlock">
...
<p><b>Executor</b> или исполнитель представляет объект, который выполняет полученные задачи. Исполнитель обычно используется вместо явного создания потоков и позволяет отделить отправку задачи от механизма ее выполнения.</p>
<p><b>Executor</b> представляет функциональный интерфейс с одним методом <code>execute()</code>:</p>
<pre class="brush:java;">
void execute(Runnable command)
</pre>
<p>Предполагается, что данный метод принимает задачу Runnable и выполняет ее в некоторый момент в будущем. Простейший пример:</p>
<pre class="brush:java;">
...
<p>В данном случае определяем класс TaskExecutor, который реализует интерфейс <code>Executor</code> и в котором метод <code>execute()</code> просто запускает переданную задачу на выполнение.</p>
<p>В методе <code>main</code> создаем объект этого класса и вызываем его метод <code>execute()</code>, передавая в него простешую задачу Runnable. Таким образом выполняется переданная задача.</p>
<p>В таком запуске задачи, конечно, нет большого смысла, так как мы просто могли бы запустить ту же задачу Runnable в том же методе <code>main()</code> без реализации интерфейса <code>Executor</code>. 
Но тем не менее интерфейс <code>Executor</code> не требует строго асинхронного выполнения и может выполняться синхронно, как в примере выше, где задача выполнялась непосредственно в потоке метода <code>main()</code></p>
<p>Однако чаще всего задачи выполняются в потоке, отличном от потока вызывающего объекта. Например, изменим код исполнителя, чтобы он создавал новый поток для каждой задачи:</p>
<pre class="brush:java;">
class TaskExecutor implements Executor{
...
<p>При запуске программы мы увидим, что теперь задача выполняется в отдельном потоке:</p>
<pre class="sh">
Main thread started...
Main thread finished...
Executing task in Thread[#34,Thread-0,5,main]
</pre>
<p>Нередко реализации Executor используются для координации выполнения задач. И в большинстве случаев нам не потребуется создавать свои собственные реализации интерфейса <code>Executor</code>, 
так как в Java есть встроенные реализации этого интерфейса, например, класса <b>ThreadPoolExecutor</b>, который для управления выполнением задач 
использует <b>пул потоков</b>, позволяя оптимизировать выполнение программы.</p>

<h2>ExecutorService</h2>
<p>Интерфейс <b>ExecutorService</b> расширяет интерфейс <code>Executor</code>, предоставляя методы управления завершением работы, а также методы создания объекта <code>Future</code> для отслеживания хода выполнения одной или нескольких 
асинхронных задач. Этот интерфейс упрощает применение исполнителей <code>Executor</code> для управления асинхронными задачами. Выше упомянутый класс <b>ThreadPoolExecutor</b> как раз реализует данный интерфейс 
<code>ExecutorService</code> (а через него и <code>Executor</code>). Рассмотрим, как использовать <code>ExecutorService</code>.</p>
<h4>Создание исполнителя</h4>
<p>Для создания исполнителей применяется один из методов класса <b>Executors</b>. Подобные методы в основном создают <b>пулы потоков</b>. <b>Пул потоков</b> представляет технику для оптимизации работы с платформенными потоками. Поскольку создание платформенных потоков довольно затратно и требует больших ресурсов, 
...
в потоке какую-нибудь задачу <code>Runnable</code> или <code>Callable</code>, то программа берет из пула один из потоков и использует его для выполнения задачи. После выполнения задачи поток не завершается, 
а возвращается обратно в пул для обслуживания следующего запроса.</p>
<p>Некоторые из них:</p>
<ul>
<li><p><code>Executors.newCachedThreadPool()</code></p>
<p>Создаёт пул потоков, который создаёт новые потоки по мере необходимости, и повторно использует ранее созданные потоки, когда они становятся доступны. Бездействующие потоки сохраняются в течение 60 секунд</p></li>

<li><p><code>Executors.newFixedThreadPool()</code></p>
<p>Создает пул из фиксированного количества потоков; бездействующие потоки сохраняются неограниченное время. Если число отправленных задач превышает число бездействующих потоков, необработанные задачи помещаются в очередь. Они запускаются после завершения других задач.</p></li>

<li><p><code>Executors.newSingleThreadExecutor()</code></p>
<p>Создает объект <code>Executor</code> (фактически пул из одного потока) с одним рабочим потоком, который последовательно выполняет отправленные задачи.</p></li>

<li><p><code>Executors.newVirtualThreadPerTaskExecutor()</code></p>
<p>Создает объект <code>Executor</code>, который запускает каждую задачу в новом виртуальном потоке.</p></li>
</ul>
<p>Все эти методы возвращают объекты классов, которые реализуют интерфейс <b>ExecutorService</b>, который в свою очередь расширяет интерфейс <code>Executor</code>.</p>

<h3>Отправка задачи исполнителю</h3>
<p>Для отправки задачи <code>Runnable</code> или <code>Callable</code> в ExecutorService применяется один из следующих методов:</p>
<pre class="brush:java;">
Future&lt;T&gt; submit(Callable&lt;T&gt; task)
Future&lt;?&gt; submit(Runnable task)
Future&lt;T&gt; submit(Runnable task, T result)
</pre>
<p>При вызове метода <b>submit()</b> исполнитель - объект <code>Executor</code> планирует выполнение отправленной задачи.</p>
<p>В качестве результата методы возвращают объект <code>Future</code>, который можно использовать для получения результата или отмены задачи.</p>
<p>Вторая версия метода <code>submit</code> возвращает объект <code>Future&lt;?&gt;</code>. Его метод <code>get()</code> блокирует выполнение до завершения задачи, а затем возвращает значение <code>null</code>.</p>
<p>Третья версия метода <code>submit</code> возвращает объект <code>Future</code>, у которого метод <code>get()</code> после завершения задачи возвращает объект, переданный через второй параметр.</p>
...
<h3>Завершение исполнителя</h3>
<p>После завершения работы исполнителя для освобождения ресурсов вызывается метод <code>close()</code>:</p>
<pre class="brush:java;">void close()</pre>
<p>Закрытый исполнитель не принимает новых задач. Метод <code>close()</code> блокирует выполнение до завершения всех отправленных задач. 
(Если поток, вызывающий close, прерывается, отправленные задачи отменяются.)  Для более удобного неявного вызова метода <code>close()</code> можно использовать конструкцию <code>try-with-resources</code> 
(try с ресурсами):</p>
<pre class="brush:java;">
...
<p>В итоге общий план применения объекта <b>Executor</b> для выполнения задачи:</p>
<ol>
<li><p>С помощью одного из статических методов класса <code>Executors</code> (<code>newCachedThreadPool</code>, <code>newFixedThreadPool</code> и т.д) создается объект <b>ExectorService</b> </p></li>
<li><p>У объекта <code>ExectorService</code> для отправки задач <code>Callable</code> или <code>Runnable</code> вызывается метод <b>submit()</b></p></li>
<li><p>Метод <code>submit()</code> возвращает объект <b>Future</b>. Этот объект можно сохранить возвращаемые, чтобы иметь возможность запрашивать состояния задач, получать результат задачи или отменять ее.</p></li>
<li><p>В конце, если больше не планируется отправлять задачи на выполнение, у объекта <code>ExectorService</code> вызывается метод <b>close()</b>, который завершает работу исполнителя.</p></li>
</ol>
...
<p>Здесь ExecutorService создает новый виртуальный поток для каждой задачи <code>Runnable</code>:</p>
<pre class="brush:java;">try (ExecutorService executor = Executors.newVirtualThreadPerTaskExecutor()) {</pre>
<p>Через метод <code>submit()</code> передаем на выполнение задачу в виде лямбда-выражения:</p>
<pre class="brush:java;">executor.submit(() -&gt; {</pre>
<p>Для имитации работы останавливаем каждую задачу на 1 секунду. В итоге мы получим консольный вывод наподобие следующего:</p>
<pre class="sh">
Main thread started...
Executing task 2 in VirtualThread[#39]/runnable@ForkJoinPool-1-worker-2
Executing task 3 in VirtualThread[#42]/runnable@ForkJoinPool-1-worker-5
Executing task 1 in VirtualThread[#37]/runnable@ForkJoinPool-1-worker-1
Executing task 0 in VirtualThread[#35]/runnable@ForkJoinPool-1-worker-4
Executing task 4 in VirtualThread[#46]/runnable@ForkJoinPool-1-worker-3
Main thread finished...
</pre>
...
<style>
	...
	<div class="nav"><p><a href="./8.13.php">Назад</a><a href="./">Содержание</a><a href="./8.15.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
