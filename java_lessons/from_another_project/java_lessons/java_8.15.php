...
<html  lang="ru">
<head>
<title>Java | Координация выполнения асинхронных задач</title>
<meta charset="utf-8" />
<meta name="description" content="Координация выполнения асинхронных задач в языке программирования Java, ExecutorService и его методы inVokeAll и invokeAny, ожидание завершения всех задач и только одной задачи">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Координация выполнения асинхронных задач</h1><div class="date">Последнее обновление: 20.10.2025</div>
	<div class="socBlock">
...
<p>Нередко требуется запустить несколько задач одновременно и получить результат первой успешно выполненной задачи или объединить результаты всех выполненных задач. Для этого класс 
<code>ExecutorService</code> предоставляет два метода: <b>inVokeAll()</b> и <b>invokeAny()</b></p>


<h3>invokeAll. Ожидание завершения всей группы задач</h3>
<p>Метод <b>invokeAll()</b> отправляет на выполнение все задачи из коллекции объектов <code>Callable</code>, блокирует их выполнение до завершения всех задач и возвращает список объектов <b>Future</b>, 
которые представляют результаты всех задач, в том же порядке, в котором эти задачи были отправлены. Соответственно в соответствии с порядком можно совместить задачи и их результаты.</p>
<p>Недостатком данного метода в некоторых ситуациях может быть то, что он ждет завершения всех задач, даже если какие-то из них уже завершились с ошибкой.</p>
<p>Этот метод также имеет две версии:</p>
<pre class="brush:java;">
&lt;T&gt; List&lt;Future&lt;T&gt;&gt; invokeAll(Collection&lt;? extends Callable&lt;T&gt;&gt; tasks)
&lt;T&gt; List&lt;Future&lt;T&gt;&gt; invokeAll(Collection&lt;? extends Callable&lt;T&gt;&gt; tasks, long timeout, TimeUnit unit)
</pre>
<p>Первая версия принимает коллекцию задач, которые надо выполнить, и возвращает список Future-объектов со статусом задач и их результатами.</p>
<p>Вторая версия также дополнительно принимает параметры тайм-аута, до истечения которого должны быть выполнены все задачи.</p>

<p>Рассмотрим на примере вычисления факториалов в следующей программе:</p>
<pre class="brush:java;">
...
<p>Здесь для представления результата задачи определен класс FactorialResult:</p>
<pre class="brush:java;">record FactorialResult(int number, int factorial){}</pre>
<p>Зднсь поле number представляет число, для которого вычисляется факториал, а поле factorial - сам вычисленный факториал числа.</p>
<p>Для представления задачи определен класс FactorialTask, который реализует интерфейс <code>Callable</code> - в методе <code>call</code> вычисляет факториал числа, послученного через конструктор, и возвращает результа - 
объект FactorialResult.</p>
<pre class="brush:java;">
public FactorialResult call() throws Exception{
...
<p>Допустим, нам надо вычислить факториалы от -1 до 4 (для теста выбираем ряд чисел заведо некорректных). Для этого создаем объект <code>ExecutorService</code>, который будет выполнять задачи на виртуальных потоках, и собственно список задач <code>Callable</code>:</p>
<pre class="brush:java;">
try (ExecutorService executor = Executors.newVirtualThreadPerTaskExecutor()) {
...
<p>Затем вызываем все задачи с помощью метода <code>executor.invokeAll()</code> и ждем их завершения:</p>
<pre class="brush:java;">List&lt;Future&lt;FactorialResult&gt;&gt; results = executor.invokeAll(tasks);</pre>
<p>На этой строке метод main приостанавливает выполнение и ждет завершения всех задач. И когда от них станут доступны результаты, проходим в цикле по все результатам и обрабатываем их:</p>
<pre class="brush:java;">
for (Future&lt;FactorialResult&gt; result : results) {
    if (result.state() == Future.State.SUCCESS){
        FactorialResult factorialResult =  result.resultNow();
...
<p>Каждый результат представляет объект <code>Future</code>, у которого с помощью метода <code>state()</code> можно проверить состояние выполнения: если задача выполнена успешно, то используем 
метод <code>resultNow()</code> для получения непосредственно самого результата в виде объекта <code>FactorialResult</code>. Если выполнение завершилось неудачно, то с помощью метода 
<code>exceptionNow()</code> получаем возникшее исключение и выводим его сообщение на консоль.</p>
...
<h3>invokeAny. Ожидание завершения одной задачи их группы</h3>

<p>Метод <b>invokeAny()</b> отправляет на выполнение все задачи из коллекции объектов <code>Callable</code> и блокирует выполнение до тех пор, пока одна из них не завершится успешно, вернув свой результат. Этот метод имеет две версии:</p>
<h3>invokeAny. Ожидание завершения одной задачи их группы</h3>
...
<p>Первая версия метода принимает коллекцию задач, из которых надо завершить одну. Результатом метода является результат первой завершенной задачи.</p>
<p>Вторая версия дополнительно принимает параметры тайм-аута - времени, до истечения которого должна быть выполнена одна из задач.</p>
<p>При этом все незавершенные задачи отменяются. Если ни одна задача не была выполнена успешно, генерируется исключение <b>ExecutionException</b>.</p>
<p>Изменим предыдущий код - пусть теперь хотя бы одна из задач по вычислению факториала будет выполнена:</p>
<pre class="brush:java;">
import java.util.concurrent.*;
import java.util.List;
import java.util.ArrayList;
...
<p>Большая часть логики здесь та же, что и в предыдущем примере. Отличие только в том что ждем выполнения одной задачи с помощью <code>invokeAny()</code>:</p>
<pre class="brush:java;">
FactorialResult factorialResult = executor.invokeAny(tasks);
System.out.printf("Factorial of %d:  %d\n",  factorialResult.number(), factorialResult.factorial());
</pre>
<p>И в данном случае вывод может быть следующим (консольный вывод не детерминирован):</p>
<pre class="sh">
Main thread started...
Factorial of 3:  6
Main thread finished...
</pre>
<p>Если при выполнении задачи возникнет ошибка, то будет сгенерировано исключение, которое будет обработано в блоке <code>catch</code>:</p>
<pre class="sh">
Main thread started...
...
<style>
	...
	<div class="nav"><p><a href="./8.14.php">Назад</a><a href="./">Содержание</a><a href="./8.18.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
