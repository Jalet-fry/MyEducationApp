...
<html  lang="ru">
<head>
<title>Java | Запрет наследования и переопределения методов</title>
<meta charset="utf-8" />
<meta name="description" content="Запрет наследования классов и переопределения методов в языке программирования Java, роль модификатора final, закрытые запечатнные sealed классы и методы">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Запрет наследования и переопределения методов</h1><div class="date">Последнее обновление: 24.09.2025</div>
	<div class="socBlock">
...
<p>Кроме запрета наследования можно также запретить переопределение отдельных методов. Например, в примере выше переопределен метод <code>print()</code>, 
...
<p>В этом случае класс-наследник не сможет переопределить метод print:</p>
<pre class="brush:java;">
...
<p>Запрет наследования класса или переопределения метода с помощью <code>final</code> может быть полезным, если мы хотим гарантировать, что семантика класса или метода не будет изменена в производном 
...
<p>В конструкторе класса Person вызывается метод <code>print()</code> для вывода информации о созданном объекте. Но в классе Employee переопределяем этот метод, чтобы также вывести значение поля company.</p>
<p>Поскольку в конструкторе Employee мы уже вызываем конструктор базового класса Person, нам не нужно второй раз вызывать метод print:</p>
<pre class="brush:java;">
Employee(String name, String company){
...
<p>Однако посмотрим на консольный вывод программы:</p>
<pre class="sh">
Name: Bob
Company: null
</pre>
<p>Конструктор базового класса Person, где вызывается метод print, отработал <span class="b">ДО</span> установки поля company. В итоге при выводе на консоль выводится некорректная информация.</p>
...
<p>Вызов метода базового класса в конструкторе - это одна из распространенных причин запрета переопределения метода. И в этом случае метод print в классе Person мы можем определить как <code>final</code>:</p>
<pre class="brush:java;">
public class Program{ 
...
<p>В этом случае мы получим корректный вывод:</p>
<pre class="sh">
Name: Bob
Company: Google
</pre>
<p>Стоит отметить, что проблема выше была связана прежде всего с тем, что вызов конструктора базового класса выполнялся до остального кода (установки поля company). И в до версии Java 25 мы могли вызывать конструктор 
базового класса только в самом начале конструктоора производного класса до всего остального кода:</p>
<pre class="brush:java;">
Employee(String name, String company){
...
<p>Однако начиная с Java 25 можно вызывать конструктор базового класса в любом месте конструктора производного класса:</p>
<pre class="brush:java;">
Employee(String name, String company){
...
<p>Конкретно эта деталь в данном случае бы решила проблему, тем не менее в более сложных программах нам надо учитывать все возможные зависимости, которые могут повлять на создание объекта базового класса. 
Поэтому в конструкторе лучше вызывать только методы с модификаторами <code>final</code> или <code>private</code>.</p>
...
<style>
	...
	<div class="nav"><p><a href="./3.5.php">Назад</a><a href="./">Содержание</a><a href="./3.24.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
