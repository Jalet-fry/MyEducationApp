...
<html  lang="ru">
<head>
<title>Java | Введение в рефлексию. Класс Class</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в рефлексию в языке программирования Java, класс Class и его методы, поле class, метод getClass">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Введение в рефлексию. Класс Class</h1><div class="date">Последнее обновление: 29.09.2025</div>
	<div class="socBlock">
	...
	<p><span class="b">Рефлексия</span> представляет мощный механизм, который позволяет динамически во время выполнения анализировать возможности классов и исследовать их и их функционал. Рефлексия может быть полезна при создании различных плагинов, 
анализаторов и других типов приложений, когда динамически надо получать информацию о программе, ее типах данных и при возможности даже взаимодействовать с ними, даже если эти типы данных определяли не вы, и их код вам не доступен.</p>
<h2>Class</h2>
<p>Ключевым классом для исследования типов является класс <span class="b">Class</span> в языке Java. Рассмотрим его основные методы:</p>
<ul>
<li><p><code>Field[] getFields()</code>:  возвращает массив объектов <code>Field</code> для открытых полей данного класса или его суперклассов. Метод возвращают массив нулевой длины, если таких полей нет или если объект 
представляет собой примитивный тип или массив</p></li>

<li><p><code>Field[] getDeclaredFields()</code>: возвращает массив объектов <code>Field</code> для всех полей данного класса. Метод возвращают массив нулевой длины, если таких полей нет или если объект 
представляет собой примитивный тип или массив.</p></li>

<li><p><code>Method[] getMethods()</code>: возвращает открытые методы в виде объектов <code>Method</code> (в том числе и унаследованные методы)</p></li>

<li><p><code>Method[] getDeclaredMethods()</code>: возвращает все методы данного класса или интерфейса, но не включает унаследованные методы</p></li>

<li><p><code>Constructor[] getConstructors()</code>: возвращает массив объектов <code>Constructor</code>, которые представляют все открытые конструкторы</p></li>

<li><p><code>Constructor[] getDeclaredConstructors()</code>: возвращает массив объектов <code>Constructor</code>, которые представляют все конструкторы, определенные непосредственно в этом классе</p></li>

<li><p><code>boolean isAnonymousClass()</code>: возвращает <code>true</code>, если тип является анонимным классом</p></li>
<li><p><code>boolean isArray()</code>: возвращает <code>true</code>, если тип представляет массив</p></li>
<li><p><code>boolean isPrimitive()</code>: возвращает <code>true</code>, если тип является примитивным типом</p></li>
<li><p><code>boolean isInterface()</code>: возвращает <code>true</code>, если объект представляет интерфейс</p></li>
<li><p><code>boolean isEnum()</code>: возвращает <code>true</code>, если объект представляет перечисление</p></li>
<li><p><code>boolean isRecord()</code>: возвращает <code>true</code>, если объект представляет класс record</p></li>
<li><p><code>boolean isInstance(Object obj)</code>: возвращает <code>true</code>, если объект-параметр является экземпляром этого типа</p></li>
<li><p><code>boolean isSealed()</code>: возвращает <code>true</code>, если класс определен с модификатором <code>sealed</code></p></li>
<li><p><code>Class getSuperclass()</code>: возвращает суперкласс для текущего типа</p></li>
<li><p><code>RecordComponent[] getRecordComponents()</code>: возвращает массив объектов RecordComponent, которые описывают поля объекта, или null, если этот объект не представляет класс record.</p></li>

<li><p><code>String getPackageName()</code>: получает имя пакета, содержащего этот тип, или пакета типа элемента, если этот тип является массивом, или "java.lang", если этот тип является примитивным типом.</p></li>
</ul>

<h2>Получение класса</h2>
<p>Чтобы исследовать типы, нам прежде всего надо получить соответствуюзий типу объект <code>Class</code>. Для этого есть три способа:</p>
<ul>
<li><p>Метод <code>getClass()</code> в классе <code>Object</code> (а, значит, и у всех остальных классов) возвращает экземпляр типа <code>Class</code></p></li>
<li><p>Статический метод <code>Class.forName()</code>  принимает полное (квалифицированное) имя типа с учетом пакетов и 
возвращает объект <code>Class</code>, который представляет этот тип</p></li>
<li><p>Поле <code>class</code> в классе <code>Object</code> (а, значит, и у всех остальных классов) возвращает экземпляр типа <code>Class</code></p></li>
</ul>
<h3>Метод getClass</h3>
<p>Самый простой способ получить связанный с типом объект <code>Class</code> - это  метод <span class="b">getClass()</span>, который есть у всех типов в языке Java:</p>
...
<p>Здесь получаем информацию о классе Person, а консоль выведет нам строку "class Person", что вообщем-то очевидно.</p>
<p>Если тип расположен в другом пакете, то выводится полное (квалифицированное) имя класса с учетом пакетов. Например:</p>
<pre class="brush:java;">
public class Program{
...
<p>Так, для строки (объекта String) полное имя класса будет "java.lang.String"</p>
<h3>Метод forName</h3>
<p>Cтатическоий метод <code>Class.forName()</code> принимает полное (квалифицированное) имя типа с учетом пакетов и 
возвращает объект <code>Class</code>, который представляет этот тип:</p>
<pre class="brush:java;">
public class Program{
...
<p>При использовании этого метода стоит учитывать, что если Java не найдет класс по переданному имени, то будет сгенерировано исключение <code>ClassNotFoundException</code>, поэтому на уровне кода надо обрабатывать это исключение.</p>


<h3>Поле class</h3>
<p>Cтатическое поле <code>class</code>, которое есть у всех типов, возвращает объект <code>Class</code>, который представляет этот тип:</p>
<pre class="brush:java;">
public class Program{
...
<style>
	...
	<div class="nav"><p><a href="./13.3.php">Назад</a><a href="./">Содержание</a><a href="./14.2.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
