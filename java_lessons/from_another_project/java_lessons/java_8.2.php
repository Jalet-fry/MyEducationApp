...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Создание и выполнение потоков</h1><div class="date">Последнее обновление: 16.10.2025</div>
	<div class="socBlock">
	...
	<p>Для создания нового потока (потока платформы) в Java мы можем использовать следующие варианты:</p>
<ul>
<li><p>Реализуация интерфейса <span class="b">Runnable</span></p></li>
<li><p>Создание класса, который наследуется от <span class="b">Thread</span></p></li>
</ul>
<p>Рассмотрим оба варианта.</p>
<h3>Реализация интерфейса Runnable</h3>
<p>Интерфейс <span class="b">Runnable</span> представляет операцию, которая не возвращает никакого результата. Этот интерфейс имеет один метод <span class="b">run</span>:</p>
...
<p>Общий процесс создания и запуска потока выглядит следующим образом:</p>
<ol>
<li><p>Определяем реализацию интерфейса <b>Runable</b>. Например:</p>
<pre class="brush:java;">
Runnable task = ()-&gt;{
...
<p>Поскольку <b>Runnable</b> представляет функциональный интерфейс, который определяет один метод, то объект этого интерфейса мы можем представить в виде лямбда-выражения.</p>
</li>

<li><p>Создаем объект <b>Thread</b> с помощью <b>Runable</b>:</p>
<pre class="brush:java;">
var t = new Thread(task);
</pre>
<p>Класс <code>Thread</code> имеет несколько конструкторов. В данном случае используем тот, который принимает объект <code>Runable</code></p>
</li>


<li><p>Запускаем поток <code>Thread</code> с помощью метода <b>start()</b>:</p>
<pre class="brush:java;">
t.start();
</pre>
</li>
...
<p>Здесь в создаваемом потоке просто выводим два сообщения на консоль. Консольный вывод в этом случае может быть следующим:</p>
<pre class="sh">
Main thread started...
...
<p>По консольному выводу видно, что, несмотря на то, что поток запускается до окончания метода <code>main()</code>, но его работа выполняется после последней инструкции в методе <code>main()</code>.</p>

<p>При создании потока - объекта <code>Thread</code> с помощью оператора <code>new</code> и вызова конструктора:</p>
<pre class="brush:java;">var t = new Thread(task);</pre>
<p>поток автоматически не запускается. Он лишь переходит в состояние <span class="b">NEW</span>, а программа ещё не начала выполнять его код.</p>
<p>После вызова метода <code>start()</code> поток переходит в состояние <b>RUNNABLE</b> - готовый к запуску. Однако в реальности такой поток еще не выполняется: оне может быть запущен, а может и не 
запущен. Планировщик потоков решает, предоставить ли потоку время для выполнения. После того, как поток запущен, он может периодически приостанавливаться, чтобы дать другим потокам возможность выполниться. 
Однако точные детали планирования потоков зависят от операционной системы и от природы потока.</p>

<p>Вместо лямбда-выражения мы можем избрать более долгий путь - определить отдельный класс, который реализует интерфейс <code>Runable</code>. Например:</p>
<pre class="brush:java;">
...
<p>Другой способ создания потока представляет наследование от класса <b>Thread</b>. Например:</p>
<pre class="brush:java;">
...
<p>Класс потока называется MyThread. В конструкторе класса вызываем один из конструкторов базового класса <code>Thread</code>. 
И также в MyThread переопределяется метод <span class="b">run()</span>, код которого собственно и будет представлять весь тот код, который 
...
<p>В данном случае для разграничения потоков в класс MyTask через конструктор передается имя потока. А в цикле при запуске потока формируем имя на основе номера потока.</p>
<p>Консольный вывод в данном случае недетерминирован. Например, он может быть следующим:</p>
<pre class="sh">
Main thread started...
Main thread finished...
MyTask_2 started...
...
<p>Но по данному выводу мы видимо, что потоки вне зависимости от запуска работают фактически одновременно, как бы параллельно</p>

<h3>Конструкторы Thread</h3>
<p>Для данного класса <b>Thread</b> определенно несколько конструкторов, которые можно использовать. Но я отмечу один из них:</p>
<pre class="brush:java;">
Thread(ThreadGroup group, Runnable task, String name)
</pre>
<p>Параметры конструктора:</p>
<ul>
<li><p><code>group</code>: группа потока. Если значение равно <code>null</code>, то в качестве группы устанавливается группа текущего потока.</p></li>
<li><p><code>task</code>: объект <code>Runnable</code>, который собственно и определяет выполняемые в потоке действия. 
Если значение равно <code>null</code>, то вызывается метод <code>run()</code> этого потока.</p></li>
<li><p><code>name</code>: имя создаваемого потока</p></li>
</ul>
<p>Я взял именно этот вариант конструктора, потому что большинство других неявно вызывают его. Например, следующий вариант конструктора</p>
<pre class="brush:java;">Thread(Runnable task, String name)</pre>
<p>Фактически имеет тот же эффект, что и вызов</p>
<pre class="brush:java;">Thread (null, task, name)</pre>
<p>А вариант конструктора</p>
<pre class="brush:java;">Thread(Runnable task)</pre>
<p>Фактически аналогичен вызову</p>
<pre class="brush:java;">Thread (null, task, gname)</pre>
<p>Здесь <code>gname</code> - автоматически генерируемое имя в виде "Thread-"+n, где n - целое число.</p>
<p>Например, модифицтруем предыдущую программу:</p>
<pre class="brush:java;">
class MyTask implements Runnable {
...
<p>Здесь в классе MyTask в методе <code>run()</code> с помощью вызова <code>Thread.currentThread().getName()</code> получаем имя потока, которое автоматически ему присваивается. Далее на консоли мы увидим автосгенерированные имена потоков:</p>
<pre class="sh">
Main thread started...
Main thread finished...
Thread-2 started...
...
<p>Такие имена могут быть неудобны, или мы захотим подобрать более описательные имена для различения потоков. В этом случае мы можем передавать имена через второй параметр конструктора <code>Thread</code>:</p>
<pre class="brush:java;">
for(int i=1; i &lt; 6; i++){
...
<p>При консольном выводе мы увидим назначенные вручную потокам имена:</p>
<pre class="sh">
Main thread started...
Main thread finished...
MyTask_1 started...
...
<style>
	...
	<div class="nav"><p><a href="./8.1.php">Назад</a><a href="./">Содержание</a><a href="./8.11.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
