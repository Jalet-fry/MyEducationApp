...
<html  lang="ru">
<head>
<title>Java | Инкапсуляция</title>
<meta charset="utf-8" />
<meta name="description" content="Инкапсуляция в приложении на языке программировании Java, методы доступа, геттеры и сеттеры, защита и сокрытие данных">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Инкапсуляция</h1><div class="date">Последнее обновление: 17.09.2025</div>
	<div class="socBlock">
...
<p><b>Инкапсуляция</b> (иногда называемая сокрытием информации) — ключевая концепция работы с объектами. Формально инкапсуляция — это просто объединение данных и поведения в одну единицу кода (пакет/модуль/класс и т.д.) и 
сокрытие деталей реализации (в нашем случае реализации полей и методов) от пользователей объекта. Конкретный объект, являющийся экземпляром класса, будет иметь определённые значения своих полей. 
Набор этих значений представляет собой текущее состояние объекта. При каждом вызове метода объекта его состояние может измениться.</p>
<p>Обычно ключ к эффективной инкапсуляции при работе с классами состоит в том, чтобы запретить напрямую обращаться к полям объекта класса и разрешить обращаться к полям класса только через его методы. 
То есть программы должны взаимодействовать с данными объекта только через методы этого объекта. Инкапсуляция — это способ придать объекту поведение "черного ящика", что является ключом к повторному 
использованию и надежности.</p>
<p>Почему это важно? Рассмотрим следующую программу:</p>
...
<p>Здесь в классе Person определены два поля - name и age, которые представляют соответственно имя и возраст и которые доступны в любой точке программы. 
Если другой класс имеет прямой доступ к этому полю (как, например, в коде выше класс Program), то есть вероятность, что в процессе работы 
программы ему будет передано некорректное значение, например, отрицательное число:</p>
<pre class="brush:java;">tom.age = -18;</pre>
<p>Подобное изменение данных не является желательным. Либо же мы хотим, чтобы некоторые данные были достуны напрямую, 
чтобы их можно было вывести на консоль или просто узнать их значение. В этой связи рекомендуется 
как можно больше ограничивать доступ к данным, чтобы защитить их от нежелательного доступа извне (как для получения значения, так и для его изменения).</p>
<p>Использование различных модификаторов гарантирует, что данные не будут искажены или изменены не надлежащим образом:</p>
<pre class="brush:java;">
public class Program {
...
<h3>Методы доступа</h3>
<p>Однако возникает другая проблема: если мы все таки захотим изменить значения полей на корретные значения, то мы не сможем это сделать. 
И для этой цели используют методы доступа. Например:</p>
<pre class="brush:java;">
public class Program {
...
<p>Теперь вместо непосредственной работы с полями name и age в классе Person мы будем работать с методами, которые устанавливают и возвращают 
значения этих полей.</p>
<p>Для изменения возраста предназначен метод <code>setAge()</code>, который принимает новое значение и устанавливает его, если оно соответствует некоторым ограничениям:</p>
<pre class="brush:java;">
public void setAge(int age){
    if(age &gt; 0 && age &lt; 110)
        this.age = age;
...
<p>Как правило, методы, которые предназначены для изменения поля, называются по шаблону <code>set[имя_поля]</code> (имя поля с большой буквы как в "setAge") и называются мьютейтерами (mutator) или сеттерами, так как они изменяют значения поля. 
И если мы хотим изменить значение поля, то используем эти методы:</p>
<pre class="brush:java;">
tom.setAge(22);
tom.setAge(-123); 
</pre>
<p>Для поля name такой метод отсутствует, так как я предполагаю, что в программе не требуется менять имя. Однако при необходимости и для поля name можно было бы добавить аналогичный метод.</p>
<p>Для полученияз начений полей предназначены методы <code>getName()</code> и <code>getAge()</code></p>
<pre class="brush:java;">
public String getName(){
    return this.name;
}
...
<p>Такие методы еще называют аксессерами (accessor) или геттерами, так как с их помощью мы получаем значение поля. Они обычно называются по шаблону <code>get[Имя_поля]</code>. И в программе мы можем их использовать для 
получения значений полей:</p>
<pre class="brush:java;">
String name = tom.getName();
int age = tom.getAge();
</pre>
...
<h3>Обращение к полям в конструкторе</h3>
<p>Выше добавленные методы позволяют обезопасить доступ к полям класса, однако тем не менее у нас продолжает иметься проблема, так как мы можем передать в конструктор некорректные данные:</p>
<pre class="brush:java;">Person tom = new Person("Tom", -123);</pre>
<p>И здесь нет единого решения. Можно, например, в этом случае генерировать ошибку (в последующих статьях будет показано, как это делать), либо можно использовать какие-то другие способы. 
Но одно из решений является - определение единой точки доступа к полям:</p>
<pre class="brush:java;">
public class Program {
...
<p>Теперь даже внутри класса для установки полей надо обращаться к методам setName/setAge, а для получения - к методам getName/getAge. В случае, если полю передано некорректное значение, 
мы можем установить значение по умолчанию, как в примере выше число 1 для поля age.</p>
<h3>Инкапсуляция и статические компоненты</h3>
<p>В примере выше рассматривались нестатические компоненты - поля и методы. Но все то же самое касается и статических полей и методов. Например:</p>
<pre class="brush:java;">
public class Program {
...
<p>Здесь в классе Person определена статическая переменная count, которая хранит количество созданных объектов Person. Поскольку она относится ко всему классу в целом, она определена как статическая. Она приватная, поэтому мы не можем обратиться к ней вне класса. Если бы мы могли бы это сделать, то ей можно было бы присвоить любое произвольное значение. 
Соответственно тогда корректность счетчика была бы под вопросом. В данном же случае она приватная, а для ее получения ее значения определен специальный статический метод getCount</p>
...
<style>
	...
	<div class="nav"><p><a href="./3.19.php">Назад</a><a href="./">Содержание</a><a href="./3.5.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
