...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Условные конструкции</h1><div class="date">Последнее обновление: 16.09.2025</div>
	<div class="socBlock">
	...
	<p>Одним из фундаментальных элементов многих языков программирования являются <span class="b">условные конструкции</span>. Данные конструкции 
позволяют направить работу программы по одному из путей в зависимости от определенных условий.</p>
<p>В языке Java для этой цели имеется условная конструкция <span class="b">if..else</span> и тернарный оператор <span class="b">?:</span></p>
<h2>Конструкция if/else</h2>
<p>Выражение <span class="b">if..else</span> проверяет истинность некоторого условия и в зависимости от результатов проверки выполняет определенный код. Ее простейшая форма состоит из блока <b>if</b>:</p>
...
<p>После ключевого слова <b>if</b> идет условие. Условие должно представлять значение типа <b>boolean</b>. Это может быть непосредственно значение типа <code>boolean</code> или результат условного выражения 
или другого выражения, которое возвращает значение типа <code>boolean</code>. И если это условие истинно (равно <code>true</code>), то срабатывает код, который помещен далее после условия внутри фигурных скобок.</p>
...
конкретно метод <code>System.out.printf</code>, который выводит строку на консоль. Если бы первое число оказалось бы меньше второго или равно ему, то инструкции в блоке <code>if</code> не выполнялись бы.</p>
...
<h3>Блок else</h3>
<p>Но что, если мы захотим, чтобы при несоблюдении условия также выполнялись какие-либо действия? В этом случае мы можем добавить блок <span class="b">else</span>:</p>
<pre class="brush:java;">
class Program {
...
<p>Поскольку здесь число num1 НЕ больше числа num2, то условие <code>num1 &gt; num2</code> не выполняется и возвращает <code>false</code>, поэтому будет выполняться блок инструкций после <code>else</code>.</p>
<p>Опять же если блок <code>else</code> содержит толко одну инструкцию, то оять же мы можем его сократить, убрав фигурные скобки:</p>
<pre class="brush:java;">
int num1 = 6;
int num2 = 40;
if(num1 &gt; num2)
...
С помощью блоков <code>else if</code> мы можем обрабатывать дополнительные условия:</p>
<pre class="brush:java;">
class Program {
...
<p>При необходимости можно добавить несколько выражений else if::</p>
<pre class="brush:java;">
class Program {
...
<p>Тернарную операция имеет следующий синтаксис:</p>
<pre class="brush:java;">[первый операнд - условие] ? [второй операнд] : [третий операнд]</pre>
<p>Таким образом, в этой операции участвуют сразу три операнда. 
...
</fieldset></div>	<div class="nav"><p><a href="./2.4.php">Назад</a><a href="./">Содержание</a><a href="./2.6.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
