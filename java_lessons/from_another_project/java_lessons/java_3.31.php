...
<html  lang="ru">
<head>
<title>Java | Type Erasure (Стирание типов)</title>
<meta charset="utf-8" />
<meta name="description" content="Type Erasure (Стирание типов) в generics при работе с обобщениями - универсальными классами и методами в языке программирования Java">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Type Erasure (Стирание типов)</h1><div class="date">Последнее обновление: 10.10.2025</div>
	<div class="socBlock">
...
<p>Хотя язык Java позволяет создавать обобщенные типы и методы, которые используют параметры типа, однако на уровне виртуальной машины Java в реальности 
не существует никаких обобщенных типов и методов — все объекты принадлежат обычным классам. При компиляции параметры типа удаляются и заменяются либо типом <code>Object</code> для параметров типа без ограничений, либо их типами-ограничениями. 
Данный процесс называется <b>type erasure</b> (стирание типов).</p>

<p>Например, возьмем следующий обобщенный класс:</p>
...
<p>Здесь поскольку для параметра <code>T</code> не установлено ограничений, то он просто заменяется на <code>Object</code>. То есть в итоге мы получим следующий класс:</p>
<pre class="brush:java;">
class Person{
...
<p>Причем без разницы, какими типами типизируется объект Person в программе:</p>
<pre class="brush:java;">
Person tom = new Person&lt;Integer&gt;(456, "Tom");
Person bob = new Person&lt;String&gt;("qwert", "Bob");
</pre>
<p>В обоих этих случаях для среды выполнения Java параметр T будет представлять тип <code>Object</code>.</p>
<p>Если результат метода или поле класса представляет параметр типа и этот результат метода или поле присваиваются внешней переменной, то среда выполняет необходимые преобразования. Например, если мы имеет следующий код:</p>
<pre class="brush:java;">
var tom = new Person&lt;Integer&gt;(456, "Tom");
int tomId = tom.getId();
System.out.println(tomId);
</pre>
<p>То среда выполнения трактует этот код наподобие следующего:</p>
<pre class="brush:java;">
var tom = new Person(456, "Tom");
int tomId = (int) tom.getId();
System.out.println(tomId);
</pre>
<h3>Параметры типа с ограничениями</h3>
<p>Теперь рассмотрим другой пример - применение ограничений типа:</p>
<pre class="brush:java;">
class Message{
...
<p>В данном случае класс Messenger типизируется параметром <code>T</code>, для которого установлено ограничение в виде типа <code>Message</code>, поэтому внутри класса Messenger тип <code>T</code> 
заменяется на <code>Message</code>:</p>
<pre class="brush:java;">
class Messenger {
...
<p>Если для одного параметра типа установлено несколько ограничений, то параметр тпиа заменяется на тип из первого ограничения. Например:</p>
<pre class="brush:java;">
...
<p>Здесь в классе <code>Messenger</code> параметр типа <code>T</code> должен представлять одновременно класс Message и интерфейс Printable (класс, который наследуется от Message и реализует интерфейс Printable). 
В итоге для виртуальной срежы Java этот класс будет выглядеть наподобие следующего:</p>
<pre class="brush:java;">
class Messenger {
...
<p>То есть параметр типа T заменяется на первое ограничение - тип Message, а если используется функционал из второго ограничения, то выполняется преобразование.</p>
...
<style>
	...
	<div class="nav"><p><a href="./3.15.php">Назад</a><a href="./">Содержание</a><a href="./3.32.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
