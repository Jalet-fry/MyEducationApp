...
<html  lang="ru">
<head>
<title>Java | StringBuffer и StringBuilder</title>
<meta charset="utf-8" />
<meta name="description" content="Создание строк с помощью классов StringBuffer и StringBuilder в языке программирования Java, неизменяемая природа строк">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>StringBuffer и StringBuilder</h1><div class="date">Последнее обновление: 16.09.2025</div>
	<div class="socBlock">
...
<p>Рассмотрим работу этих классов на примере функциональности StringBuilder.</p>
...
<p>Так как в самом начале StringBuilder инициализируется строкой "Java", то его емкость составляет 4 + 16 = 20 символов. Затем мы увеличиваем емкость буфера 
с помощью вызова <code>strBuilder.ensureCapacity(32)</code> повышаем минимальную емкость буфера до 32 символов. Однако финальная емкость может отличаться в большую сторону. 
...
<p>Но в любом случае вне зависимости от емкости длина строки, которую можно получить с помощью метода <code>length()</code>, в StringBuilder остается прежней - 4 символа (так как в "Java" 4 символа).</p>
<p>Чтобы получить строку, которая хранится в StringBuilder, мы можем использовать стандартный метод <code>toString()</code>:</p>
<pre class="brush:java;">
String str = "Java";
StringBuilder strBuilder = new StringBuilder(str);
...
<p>Метод <span class="b">charAt()</span> получает, а метод <span class="b">setCharAt()</span> устанавливает символ по определенному индексу:</p>
<pre class="brush:java;">
...
<p>Метод <span class="b">append()</span> добавляет подстроку в конец StringBuilder:</p>
<pre class="brush:java;">
StringBuilder strBuilder = new StringBuilder("hello");
strBuilder.append(" world");
System.out.println(strBuilder.toString()); // hello world
</pre>
<p>Метод <span class="b">insert()</span> добавляет строку или символ по определенному индексу в StringBuilder:</p>
<pre class="brush:java;">
...
<p>Для изменения длины StringBuilder (не емкости буфера символов) применяется метод <span class="b">setLength()</span>. Если StringBuilder 
...
<p>Для замены подстроки между определенными позициями в StringBuilder на другую подстроку применяется метод <span class="b">replace()</span>:</p>
<pre class="brush:java;">
...
<p>Метод <span class="b">reverse()</span> меняет порядок в StringBuilder на обратный:</p>
<pre class="brush:java;">
StringBuilder strBuilder = new StringBuilder("assembler");
strBuilder.reverse();
