...
<meta charset="utf-8" />
<meta name="description" content="Ограничения обобщений (generics type bounds) в языке программирования Java, ограничения типа класса и метода, интерфейсы в качестве ограничений, примеры использования">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Ограничения обобщений</h1><div class="date">Последнее обновление: 09.10.2025</div>
	<div class="socBlock">
	...
	<p>Когда мы указываем универсальный параметр у обобщений, то по умолчанию он может представлять любой тип. Однако иногда необходимо, чтобы параметр соответствовал только некоторому ограниченному набору типов. В этом случае применяются <b>ограничения обобщений</b> (type bound) - они позволяют указать базовый класс, которому должен соответствовать параметр.</p>
<p>Например, у нас есть следующий класс Message, который представляет некоторое сообщение, и его производные классы, которые представляют email-сообщение и sms-сообщение:</p>
...
<p>И, допустим, мы хотим определить функционал для отправки сообщений. И для этого создадим следующий класс Messenger:</p>
<pre class="brush:java;">
class Messenger {
...
<p>Класс Messenger через конструктор принимает сообщение и через метод <code>send()</code> условно отправляет. С помощью метода <code>getMessage()</code> мы можем получить отправленное сообщение. 
И чтобы класс Messenger был предельно универсален, в качестве типа сообщений используем базовый тип Message.</p>
<p>Используем выше определенные классы:</p>
<pre class="brush:java;">
class Program {
...
<p>B все вроде прекрасно работает, и никаких проблем нет, поскольку класс Messenger принимает объект Message и соответственно также и объекты производных классов. Но посмотрим с какими проблемами мы можем столкнуться.</p>
<h4>Преобразования типов</h4>
<p>Однако при попытке получить из объекта Messenger отправленное сообщение мы можем столкнуться с необходимостью преобразования типов:</p>
<pre class="brush:java;">
class Program {
...
<p>Здесь мы извлекаем с помощью вызова <code>simpleMessenger.getMessage()</code> отправленное сообщение и преобразуем в <code>EmailMessage</code>, чтобы иметь обращаться к методам, определенным непосредственно в 
классе EmailMessage.</p>
<h4>Типобезопасность</h4>
<p>Вторая проблема, которая здесь возможна - это проблема типобезопасности, если мы точно не знаем, какой тип скрывает объект Message и захотим преобразовать этот объект к несовместимому типу:</p>
<pre class="brush:java;">
class Program {
...
<p>Здесь при попытке преобразовать к типу <code>SmsMessage</code> мы столкнемся с ошибкой, поскольку преобразуемый объект представляет тип EmailMessage. И проблема усугубляется тем, 
что эту ошибку мы можем получить только в процессе выполнения. А компиляция пройдет успешно.</p>

<h4>Использование обобщений</h4>
<p>На первый взгляд, чтобы избежать преобразований и проблем с типобезопасностью, мы могли бы применить обобщения:</p>
<pre class="brush:java;">
class Messenger&lt;T&gt; {
...
<p>Теперь в класс Messenger мы можем передать сообщения любого типа, и объект Messenger будет строго использовать этот тип. Преобразования не потребуются, а ошибки можно будет отловить на этапе компиляции. 
Но теперь мы сталкиваемся с другой проблемой - универсальный параметр <code>T</code> подразумевает любой тип. Но не любой тип 
имеет метод <code>getText()</code>. Соответственно метод <code>getText()</code> для объекта типа <code>T</code> не определен и мы не можем этот метод использовать. Более того для объекта <code>T</code> по умолчанию 
достуны только методы типа <code>Object</code>.</p>

<h4>Определение ограничений обобщений</h4>
...
функционалу класса Message. И <b>ограничения обобщений</b> позволяют решить эту проблему.</p>

<p>Для установки ограничения после универсального параметра ставится слово <b>extends</b>, после которого указывается тип ограничения:</p>
<pre class="brush:java;">&lt;T extends тип_ограничения&gt;</pre>

<h3>Ограничения обобщений в типах</h3>
<p>Ограничения обобщений можно применять на уровне типов:</p>
<pre class="brush:java;">class имя_класса&lt;T extends тип_ограничения&gt;</pre>
<p>В качестве примера изменим класс мессенджера:</p>
<pre class="brush:java;">
...
<p>Выражение <code>&lt;T extends Message&gt;</code> в определении класса говорит, что через универсальный параметр <code>T</code> будут передаваться объекты класса Message и 
производных классов. Благодаря этому компилятор будет знать, что T будет иметь функционал класса Message, и соответственно мы сможем  без проблем обратиться к методам класса Message внутри класса Messenger.</p>
...
<p>Стоит отметить, что при определении переменной класса мы можем сократить ее определение:</p>
<pre class="brush:java;">var mailClient = new Messenger&lt;EmailMessage&gt;(email);</pre>
<p>Либо так:</p>
<pre class="brush:java;">Messenger&lt;EmailMessage&gt; mailClient = new Messenger&lt;&gt;(email);</pre>
<p>Если же мы попробуем получить объект несовместимого типа:</p>
<pre class="brush:java;">
...
<p>то мы получим ошибку на этапе компиляции.</p>
...
<p>Также можно установить, чтобы для одного параметра действовало сразу несколько ограничений. В этом случае ограничения объединяются с помощью оператора <b>&amp;</b>:</p>
<pre class="brush:java;">
public class Program{
...
<p>В данном случае класс Messenger использует параметр типа <code>T</code>, который оновременно должен представлять кдасс Message и интерфейс Printable:</p>
<pre class="brush:java;">class Messenger&lt;T extends Message & Printable&gt; {</pre>
<p>В качестве теста определяем подобный класс - PrintedMessage и затем передаем его объект в метод sendMessage:</p>
<pre class="brush:java;">
var icq = new Messenger&lt;PrintedMessage&gt;();
var hello = new PrintedMessage("Hello METANIT.COM");
icq.sendMessage(hello);     // Текст сообщения: Hello METANIT.COM
...
<p>Аналогично классам ограничения типизированных параметров можно можно применять и к методам. Например:</p>
<pre class="brush:java;">
public class Program{
...
<style>
	...
	<div class="nav"><p><a href="./3.11.php">Назад</a><a href="./">Содержание</a><a href="./3.15.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
