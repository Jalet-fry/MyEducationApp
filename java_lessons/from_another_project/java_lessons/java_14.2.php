...
<html  lang="ru">
<head>
<title>Java | Исследование типов</title>
<meta charset="utf-8" />
<meta name="description" content="Рефлексия и исследование типов в программе на языке программирования Java, класс Class и его методы, классы Field, Method, Constructor, поление полей, методов и конструкторов классов">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Исследование типов</h1><div class="date">Последнее обновление: 29.09.2025</div>
	<div class="socBlock">
...
<p>Для исследования типов в программе кроме класса <code>Class</code> язык Java предоставляет еще ряд классов, которые определены главным образом в пакете <span class="b">java.lang.reflect</span>:</p>
<ul>
<li><p><span class="b">Field</span>: описывает поле класса</p></li>
<li><p><span class="b">Method</span>: описывает метод типа</p></li>
<li><p><span class="b">Constructor</span>: описывает конструктор класса</p></li>
</ul>
<p>Для получения этих компонентов класса применяются соответственно следующие методы класса <code>Class</code>:</p>
<ul>
<li><p><code>Field[] getFields() / getDeclaredFields()</code>: возвращает поля класса</p></li>

<li><p><code>Method[] getMethods() / getDeclaredMethods()</code>: возвращает методы класса</p></li>

<li><p><code>Constructor[] getConstructors() / getDeclaredConstructors()</code>: возвращает конструкторы класса</p></li>

</ul>
<p>Методы <code>getFields()</code>, <code>getMethods()</code> и <code>getConstructors()</code> возвращают массивы общедоступных полей, методов и конструкторов (определенных с модификатором <code>public</code>), 
поддерживаемых классом, в том числе и поля и методы, унаследованные от суперклассов. Методы <code>getDeclaredFields()</code>, <code>getDeclaredMethods()</code> и 
<code>getDeclaredConstructors()</code> возвращают массивы из всех полей, методов и конструкторов, объявленных непосредственно в классе, в том числе и закрытые, приватные  компоненты. Но эти массивы НЕ включают компоненты суперклассов.</p>
<p>Таким образом, в зависимости от уровня доступа компонентов а также от того, хотим мы получить также компоненты суперклассов или хотим ограничиться компонентами, которые определены непосредственно в классе, зависит выбор конкретного метода для получения компонентов.</p>
<p>Допустим, у нас есть следующая иерархия классов:</p>
...
<p>В данном случае используем методы <code>getFields()</code>, <code>getMethods()</code> и <code>getConstructors()</code> для получения всех публичных компонентов класса, в том числе унаследованных от класса Person. 
...
<p>Преимуществом классов <code>Field/Method/Constructor</code> является то, что в Java уже по умолчанию реализован полноценный вывод компонента, что очень удобно при выводе на консоль. И в данном случае мы видим, что методы вывели только 
публичные компоненты, в том числе и унаследованные от класса Object.</p>
...
<p>Теперь применим другой набор методов для получения информации о классе - <code>getDeclaredFields()/getDeclaredMethods()/getDeclaredConstructors()</code>:</p>
<pre class="brush:java;">
import java.lang.reflect.*;
...
<p>Подобным образом мы можем исследовать и встроенные типы данных. Например, исследуем тип String (ограничимся конструкторами):</p>
<pre class="brush:java;">
import java.lang.reflect.*;
...
<h3>Проверка типа</h3>
<p>Ряд методов класса <code>Class</code> позволяют проверить исследуемый тип:</p>
<ul>
...
<p>Пример применения некоторых из этих методов:</p>
<pre class="brush:java;">
import java.lang.reflect.*;
...
<style>
	...
	<div class="nav"><p><a href="./14.1.php">Назад</a><a href="./">Содержание</a><a href="./14.3.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
