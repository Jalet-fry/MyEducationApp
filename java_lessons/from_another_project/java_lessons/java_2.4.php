...
<meta charset="utf-8" />
<meta name="description" content="Использование массивов в Java для хранения однотипных значений, одномерные и многомерные массивами, класс Arrays, перебор и длина массивов">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Массивы</h1><div class="date">Последнее обновление: 16.09.2025</div>
	<div class="socBlock">
...
<p>Нередко разработчикам приходятся иметь дело не с единичнами данными, а с наборами данных. Самым простым видом наборов данных является массив. <b>Массив</b> в Java представляет набор однотипных значений.</p>
<h3>Объявление и определение массива</h3>
<p>Объявление массива похоже на объявление обычной переменной, которая хранит одиночное 
...
<p>Если надо объявить несколько массивов, можно указать квадратные скобки с типом только один раз:</p>
<pre class="brush:java;">
int[] arr1, arr2, arr3;
</pre>
...
<h3>Индексы и обращение к элементам массива</h3>
...
<p>Индексация элементов массива начинается с 0. В данном случае выражение <code>nums[1]</code> представляет обращение ко второму элементу (индекс 1).</p>
А чтобы обратиться к четвертому элементу в массиве, нам надо использовать индекс 3 и  выражение <code>nums[3]</code>.</p>
<p>По умолчанию, если мы явным образом не указали элементы массива, то все элементы равны значеню по умолчанию - для чисел это число 0. Поэтому при выполнении выражения <code>System.out.println(nums[1])</code> 
на консоль будет выведено число 0.</p>
<p>Подобным образом мы можем использовать индексы для установки элементов массива или их получения:</p>
<pre class="brush:java;">
int[] nums = new int[4];
...
<p>И так как у нас массив определен только для 4 элементов, то мы не можем обратиться, например, к шестому элементу:</p>
<pre class="brush:java;">
int[] nums = new int[4];
nums[5] = 5;  // Ошибка
</pre> 
<p>Если мы так попытаемся сделать, то мы получим ошибку.</p>
<h3>Длина массива</h3>
...
<p>Для перебора массивов можно использовать стандартный цикл <b>for</b>:</p>
<pre class="brush:java;">
class Program{
...
<p>В данном случае в качестве счетчика используется переменная i, которая с каждым витком цикла увеличивается на 1 от 0 до <code>array.length</code> (то есть до длины массива). И эту переменную используем в качестве индекса для получения элемента 
массива для вывода на консоль.</p>

<p>Однако в Java есть также специальная версия цикла <code>for</code> предназначена для перебора элементов в наборах, например, в массивах и коллекциях. 
...
<p>В то же время стандартная версия цикла <code>for</code> более гибкая по сравнению <code>for (int i : array)</code>. В частности, в этой версии мы можем изменять элементы:</p>
<pre class="brush:java;">
int[] array =  { 1, 2, 3, 4, 5 };
for (int i=0; i&lt;array.length;i++){
    array[i] = array[i] * 2;
...
<p>Также аналогично можно перебрать зубчатые массивы:</p>
<pre class="bnush:java;">
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i &lt; nums.length; i++){
            for(int j=0; j &lt; nums[i].length; j++){
                ...
                nums[i][j] = i + j;     // устанавливаем значение элемента массива
                System.out.printf("%d ", nums[i][j]);   // выводим это значение на консоль
            }
            System.out.println();
        }
    }
...
</fieldset></div>	<div class="nav"><p><a href="./2.6.php">Назад</a><a href="./">Содержание</a><a href="./2.20.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
