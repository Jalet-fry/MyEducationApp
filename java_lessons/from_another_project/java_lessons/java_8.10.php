...
<meta charset="utf-8" />
<meta name="description" content="Условия в блокировках ReentrantLock в приложения на языке программирования Java, класс Condition и его применение в потоках, методы signalAll и await">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Условия в блокировках</h1><div class="date">Последнее обновление: 22.10.2025</div>
	<div class="socBlock">
...
<span class="b">Condition</span> из пакета <span class="b">java.util.concurrent.locks</span>.</p>
...
Применение аналогично использованию методу <code>notify</code> класса Object.</p>
<p>Поток для разблокировки выбирается из множества ожидающих случайным образом. С одной стороны, это эффективнее, чем разблокировать все потоки. Но если случайно выбранный поток обнаруживает, что он все 
еще не может продолжить выполнение, он снова блокируется. Если ни один другой поток снова не вызовет сигнал, программа блокируется.</p></li></ul>
...
<p>Для примера возьмем задачу производителей и потребителей: производитель должен произвести 5 товаров, а покупатель должен эти товары купить. Однако на складе можно одновременно разместить не более 3 товаров. 
В то же время покупатель не может купить товар, если на складе нет никаких товаров. То есть покупатель должен ждать, пока производитель не произведет хотя бы один товар. А производитель должен ждать, пока покупатель не купит хотя бы один товар, если на складе 3 товара. 
И для решения этой задачи используем блокировки:</p>
<pre class="brush:java;">
...
<p>Разберем пошагово, что происходит. Итак, у нас есть следующие компоненты:</p> 
<ul>
<li><p><code>Store</code> (Магазин): общий ресурс (буфер), к которому обращаются два потока. У него есть ограничение: он может хранить <span class="b">не более 3</span> товаров (<code>product >= 3</code>) и <span class="b">не менее 0</span> (<code>product &lt; 1</code>).</p></li>
<li><p><code>Producer</code> (Производитель): поток (<code>Runnable</code>), который пытается 5 раз добавить товар в <code>Store</code> (вызывая метод <code>put()</code>).</p></li>
<li><p><code>Consumer</code> (Потребитель): поток (<code>Runnable</code>), который пытается 5 раз забрать товар из <code>Store</code> (вызывая метод <code>get()</code>).</p></li>
<li><p><code>Program</code>: uлавный класс создает <code>Store</code>, <code>Producer</code> и <code>Consumer</code>, а затем запускает <code>Producer</code> и <code>Consumer</code> в двух отдельных потоках.</p></li>
</ul>
<p>Ключевая логика находится в классе <code>Store</code>, который управляет доступом к переменной <code>product</code>.
<ul>
<li><p><code>ReentrantLock (locker)</code>: бокировка. Прежде чем производитель или потребитель смогут изменить количество товаров (</code>product</code>), они должны "захватить" эту блокировку. Это гарантирует, что только один поток (либо <code>Producer</code>, либо <code>Consumer</code>) может одновременно выполнять код внутри блоков <code>try...finally</code> в методах <code>get()</code> и <code>put()</code>.</p></li>
<li><p><code>Condition (condition)</code>: условие, которое реализует "механизм ожидания", связанный с блокировкой. Оно позволяет потоку, который уже захватил блокировку, временно "уснуть" (освободив блокировку), пока не выполнится какое-то условие.</p></li>
</ul>
<p>Метод <code>put()</code> в классе Producer (Производитель) выполняет следующие действия:</p>
<ul>
<li><p><code>locker.lock()<code>: Поток производителя захватывает блокировку.</p></li>
<li><p><code>while (product >= 3)<code>: Он проверяет, полон ли склад.</p></li>
<li><p><code>condition.await()<code>: Если склад полон (3 товара), производитель <span class="b">засыпает</span> и освобождает блокировку. Он будет ждать, пока его не "разбудит" потребитель.</p></li>
<li><p><code>product++<code>: Если склад не полон (меньше 3), производитель добавляет 1 товар.</p></li>
<li><p><code>condition.signalAll()<code>: Производитель "будит" все ожидающие потоки (в данном случае, он будит потребителя, если тот уснул из-за пустого склада).</p></li>
<li><p><code>locker.unlock()<code>: Производитель отпускает блокировку.</p></li>
</ul>

<p>Метод <code>get()</code> класса Consumer (Потребитель) выполняет следующие действия:</p>
<ul>
<li><p><code>locker.lock()<code>: Поток потребителя захватывает блокировку.</p></li>
<li><p><code>while (product &lt; 1)<code>: Он проверяет, пуст ли склад.</p></li>
<li><p><code>condition.await()<code>: Если склад пуст (0 товаров), потребитель <span class="b">засыпает</span> и освобождает блокировку. Он будет ждать, пока его не "разбудит" производитель.</p></li>
<li><p><code>product--<code>: Если на складе есть товары (1 или больше), потребитель забирает 1 товар.</p></li>
<li><p><code>condition.signalAll()<code>: Потребитель "будит" все ожидающие потоки (производителя, если тот уснул из-за полного склада).</p></li>
<li><p><code>locker.unlock()<code>: Потребитель отпускает блокировку.</p></li>
</ul>
<p>Выполнение программы:</p>
<ul>
<li><p>Два потока (Producer и Consumer) запускаются одновременно.</p></li>
<li><p>Они начинают "бороться" за блокировку <code>locker</code>.</p></li>
<li><p>Producer (скорее всего) захватит блокировку первым, добавит 3 товара (вызывая <code>put()</code> 3 раза) и на четвертой попытке уснет (т.к. <code>product</code> станет равен 3).</p></li>
<li><p>Consumer тем временем будет забирать товары. Как только он заберет хотя бы один товар, он вызовет <code>signalAll()</code>, что разбудит потребителя - объекта Producer.</p></li>
<li><p>Producer снова попытается добавить товар.</p></li>
<li><p>Процесс будет повторяться. Потоки будут поочередно производить и потреблять товары, при этом количество товаров на складе (</code>product</code>) никогда не будет больше 3.</p></li>
<li><p>В итоге Producer 5 раз добавит товар, а Consumer 5 раз его заберет, после чего оба потока завершат свою работу.</p></li>
</ul>
...
<style>
	...
	<div class="nav"><p><a href="./8.9.php">Назад</a><a href="./">Содержание</a><a href="./8.16.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
