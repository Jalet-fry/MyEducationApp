...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Иерархия наследования и преобразование типов</h1><div class="date">Последнее обновление: 24.09.2025</div>
	<div class="socBlock">
...
<h3>upcasting или восходящие преобразования</h3>
...
<p>Это так называемое <span class="b">восходящее преобразование</span> (от подкласса внизу к суперклассу вверху иерархии) или <span class="b">upcasting</span>. Такое преобразование осуществляется 
автоматически.</p>
<h3>downcasting или нисходящее преобразование </h3>
<p>Обратное не всегда верно. Например, объект Person не всегда является объектом Employee или Client. Поэтому <span class="b">нисходящее преобразование</span> или <span class="b">downcasting</span> 
от суперкласса к подклассу автоматически не выполняется. В этом случае нам надо использовать операцию преобразования типов. Например:</p>
<pre class="brush:java;">
Object sam = new Employee("Sam", "Yandex");
sam.print(); // ! Ошибка
</pre>
<p>Здесь хотя переменная <code>sam</code> и хранит ссылку на объект типа Employee, но является переменной типа <code>Object</code>. Поэтому при компиляции мы столкнемся с ошибкой - поскольку объект типа Object 
не обязательно представляет тип Employee или Person, у него может и не быть метода <code>print()</code>. 
И в этом случае нам надо применить <span class="b">нисходящее преобразование</span> от типа Object к типу Employee или Person:</p>
<pre class="brush:java;">
Object sam = new Employee("Sam", "Yandex");
...
<p>В данном случае переменная <code>kate</code>, которая имеет тип Person, хранит ссылку на объект Client. Мы можем без ошибок привести этот объект к типу Client. Но при попытке преобразования 
к типу Employee мы получим ошибку во время выполнения. Так как <code>kate</code> НЕ представляет объект типа Employee.</p>
...
<p>В методе <code>printPerson</code> выражение <code>person instanceof Employee</code> проверяет, представляет ли параметр person объект типа Employee. И если не является, то такая 
проверка вернет значение <code>false</code>, и преобразование не сработает, и выполняется блок <code>else</code>:</p>
<pre class="brush:java;">
else{
...
<p>Если же параметр <code>person</code> представляет тип Employee, то выполняется блко <code>if</code>, где преобразуем в тип Employee и выводим на консоль информацию об объекте:
<pre class="brush:java;">
if(person instanceof Employee){
...
<style>
	...
	<div class="nav"><p><a href="./3.6.php">Назад</a><a href="./">Содержание</a><a href="./3.7.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
