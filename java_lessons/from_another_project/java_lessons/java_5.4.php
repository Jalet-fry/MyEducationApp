...
<html  lang="ru">
<head>
<title>Java | Интерфейс Set и хеш-таблицы HashSet</title>
<meta charset="utf-8" />
<meta name="description" content="Интерфейс Set и хеш-таблицы HashSet в языке программирования Java, неупорядоченная коллекция уникальных элементов , операции с хэш-таблицей">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Интерфейс Set и хеш-таблицы HashSet</h1><div class="date">Последнее обновление: 11.10.2025</div>
	<div class="socBlock">
	...
	<p>Интерфейс <span class="b">Set</span> расширяет интерфейс Collection и представляет набор уникальных элементов. <code>Set</code> почти не добавляет 
новых методов, только вносит изменения в унаследованные. В частности, метод <span class="b">add()</span> добавляет элемент в коллекцию и возвращает true, 
если в коллекции еще нет такого элемента.</p>
<p>Добавленных методов в <code>Set</code> следует отметить следующие два статических метода:</p>
<ul>
<li><p><code>static &lt;E&gt; Set&lt;E&gt;of(E... elements)</code>: возвращает неизменяемый набор из элементов <code>elements</code></p></li>
<li><p><code>static &lt;E&gt; Set&lt;E&gt;copyOf(Collection&lt;? extends E&gt; col)</code>: возвращает неизменяемый набор, в которую добавляет все элементы коллекции col</p></li>
</ul>
<p>Например, используем метод <code>of()</code> для создания набора:</p>
...
<p>Метод <code>Set.of()</code> создает неизменяемый набор типа <code>class java.util.ImmutableCollections$SetN</code>.</p>
<p>Отличительной особенностью наборов является то, что все элементы в нем должны быть уникальными. Для сравнения на равенстро применяется метод <code>equals()</code>, который имеют все классы. Так, если мы попробуем добавить в набор два одинаковых значения, то при выполнении будет сгенерирована ошибка:</p>
<pre class="brush:java;">
import java.util.Set;
...
<h2>HashSet</h2>
<p>Расспространенной реализацией интерфейса <code>Set</code> является обобщенный класс <span class="b">HashSet</span>, который представляет <b>хеш-таблицу</b>. Если быть точнее, он наследует свой 
функционал от класса <code>AbstractSet</code>, который, в свою очередь, реализует интерфейс <code>Set</code>.</p>
...
таблице. Хеш-код представляет целое число, которое хеш-таблица вычисляет для каждого добавляемого объекта с помощью его метода <code>hashCode()</code>. Хеш-код обычно выводится из полей экземпляра объекта, предпочтительно таким образом, 
чтобы объекты с разными данными давали разные коды. И также важно, чтобы хэш-коды вычислялись быстро, и чтобы вычисление зависело только от состояния объекта, который необходимо хэшировать.</p>
...
<p>Создание хеш-таблицы по имеющейся коллекции (из неизменяемого набора):</p>
<pre class="brush:java;">
import java.util.Set;
import java.util.HashSet;
...
<p>Класс <code>HashSet</code> в основном реализует те методы, которые объявлены в родительских классах и применяемых интерфейсах. Приведу основные методы <code>HashSet</code>:</p>
<ul>
...
<li><p><code>Object clone()</code>: Возвращает поверхностную копию данного экземпляра HashSet: сами элементы не клонируются</p></li>
<li><p><code>boolean contains(Object o)</code>: Возвращает true, если этот набор содержит указанный элемент</p></li>
<li><p><code>boolean isEmpty()</code>: Возвращает true, если этот набор не содержит элементов</p></li>
<li><p><code>Iterator&lt;E&gt; iterator()</code>: Возвращает итератор по элементам этого набора</p></li>
<li><p><code>static &lt;T&gt; HashSet&lt;T&gt; newHashSet(int numElements)</code>: Создаёт новый пустой HashSet, подходящий для ожидаемого количества элементов</p></li>
<li><p><code>boolean remove(Object o)</code>: Удаляет указанный элемент из этого набора, если он присутствует</p></li>
<li><p><code>int size()</code>: Возвращает количество элементов в этом наборе</p></li>
<li><p><code>Object[] toArray()</code>: Возвращает массив, содержащий все элементы этой коллекции</p></li>
</ul>



<p>С помощью метода <code>add()</code> можно добавить новый объект в хеш-таблицу. Если такого элемента еще нет, то объект добавляется, а метод возвращает <code>true</code>. Если элемент уже есть, ничего не происходит, а метод возвращает 
<code>false</code>:</p>
<pre class="brush:java;">
import java.util.HashSet;
...
<p>Метод <code>remove()</code> удаляет объект из хеш-таблицы. ПРи успешном удалении метод возвращает <code>true</code>. Если объект отсутствует, ничего не происходит, а метод возвращает 
<code>false</code>. Для полного же удаления всех элементов применяется метод <code>clear()</code>. Пример удаления:</p>
<pre class="brush:java;">
import java.util.Set;
import java.util.HashSet;
...
<style>
	...
	<div class="nav"><p><a href="./5.3.php">Назад</a><a href="./">Содержание</a><a href="./5.5.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
