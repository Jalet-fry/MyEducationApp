...
<meta charset="utf-8" />
<meta name="description" content="Наследование в языке программирования Java и его основные моменты, базовые и производные классы, суперклассы и подклассы, расширение базовых классов, ключевое слово super, пример наследования классов, переопределение методов, атрибут @Override">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Наследование</h1><div class="date">Последнее обновление: 24.09.2025</div>
	<div class="socBlock">
...
<p>Однако так как этот класс реализует тот же функционал, что и класс Person, поскольку сотрудник - это также и человек, то было бы целесообразнее не определять повторно один и тот же функционал, а как-то передать этот функционал 
из класса Person в класс Employee. И для этого можно <span class="b">унаследовать</span> класс Employee от класса Person:</p>
<pre class="brush:java;">
class Employee extends Person { }  
</pre>
<p>Чтобы унаследовать один класс от другого, надо использовать после имени класса-наследника ключевое слово <span class="b">extends</span>, 
...
<p>Таким образом, наследование реализует отношение <b>is-a</b> (является). То есть объект класса Employee также является объектом класса Person. В этом отношении 
класс Employee еще называют <span class="b">производным классом</span> (а также <span class="b">дочерним классом</span>, <span class="b">классом-наследником</span>, <span class="b">подклассом</span>) 
а класс Person - <span class="b">базовым классом</span> (а также <span class="b">родительским классом</span> или <span class="b">суперклассом</span>).</p>

<p>Затем мы можем использовать функционал базового класса (Person), обращаясь к объекту производного класса (Employee):</p>
<pre class="brush:java;">
...
<p>То есть в данном случае благодаря наследованию у объекта класса Employee мы можем обратиться и к полю name, и к методу print, хотя они определены в классе Person.</p>

<h3>Наследование конструкторов</h3>
...
помощью ключевого слова <span class="b">super</span>, в который передаются значения для параметров конструкторв:</p>
<pre class="brush:java;">
super(аргументы);
</pre>
<p>Например, определим к классе Person конструктор:</p>
<pre class="brush:java;">
class Person {
...
<p>Здесь конструктор класса Person принимает один параметр. Поэтому в классе 
...
<p>Стоит отметить, что при определении конструктора в производном классе до версии Java 25 вызов конструктора суперкласса должны был выполняться ДО всех остальных инструкций. Например:</p>
<pre class="brush:java;">
Employee(String name){
...
<p>Начиная с версии Java 25 вызов конструктора суперкласса может располагаться в любом месте конструктора производного класса. Например:</p>
<pre class="brush:java;">
Employee(String name){
...
<p>Производный класс не только наследует функционал от базового класса, но и также может расширять его - добавлять свои поля и методы. Например, класс Employee представляет работника некоторой компании, и 
соответственно нам бы хотелось хранить в нем данные об этой компании:</p>
<pre class="brush:java;">
public class Program{ 
...
<p>В данном случае класс Employee добавляет поле company, которое хранит место работы сотрудника, а также метод work для вывода названия компании.</p>



<h3>Доступ к функциональности базового класса</h3>
...
Так в примере выше мы сделали поле name в классе Person приватным (определенным с модификатором <code>private</code>). Поэтому класс Employee не имеет к нему доступа:</p>
<pre class="brush:java;">
class Employee extends Person{
...
<p>При попытке скомпилировать этот класс мы столкнемся с ошибкой, так как в методе <code>work</code> идет обращение к переменной name базового класса Person, которая является приватной.</p>
<p>Определение полей приватными представляет стандартную практику лучше инкапсулировать и защитить данные от некорретного вмешательства извне. 
...
<span class="b">protected</span>. Например, если суперкласс Person объявляет поле name с модификатором <code>protected</code>, а не приватным, то методы класса Employee могут обращаться к нему напрямую:</p>
<pre class="brush:java;">
public class Program{ 
...
<p>В Java поле с модификатором <code>protected</code> доступно любому классу в том же пакете. Поэтому для надлежащей инкапсуляции нам надо поместить класс Person в отдельный пакет. В этом случае поля и методы с модификатором <code>protected</code> 
будут доступны только классу Employee и другим унаследованным классам.</p>


<h3>Переопределение методов</h3>
<p>Производный класс может определять свои методы, а может переопределять методы, которые унаследованы от базового класса.  При переопределении метода производный класс определяет метод с тем же именем, что и базовый класс. 
Например:</p>
<pre class="brush:java;">
public class Program{ 
...
<p>Здесь производный класс Employee переопределяет метод print из базового класса. Перед переопределяемым методом указывается аннотация <span class="b">@Override</span>. Данная аннотация в принципе необязательна.</p>
...
<p>В классе Employee  реализация метода print выводит значение поля company на консоль. Но что делать, если мы хотим, чтобы переопределенный метод print также выводил и значение поля name из базового класса? 
Напрямую в классе Employee мы не можем обращаться к полю name, потому что оно приватное. Однако мы можем вызвать в производном классе реализацию метода базового класса  с помощью слова <span class="b">super</span>:</p>
<pre class="brush:java;">
...
<p>Здесь в методе print в классе Employee помощью ключевого слова <span class="b">super</span> мы обращаемся к реализации метода print из базового класса, которая выводит имя:</p>
<pre class="brush:java;">super.print();</pre>
...
<style>
	...
	<div class="nav"><p><a href="./3.22.php">Назад</a><a href="./">Содержание</a><a href="./3.23.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
