...
<html  lang="ru">
<head>
<title>Java | Pattern мatching. Паттерн типов</title>
<meta charset="utf-8" />
<meta name="description" content="Pattern мatching. Паттерн типов (type pattern) в языке программирования Java, проверка типа с помощью оператора instanceof и конструкции switch, оператор when, защитные условия guards">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h1>Pattern мatching. Паттерн типов</h1><div class="date">Последнее обновление: 27.09.2025</div>
	<div class="socBlock">
...
<p><span class="b">Паттерн типов</span> (<span class="b">type pattern</span>) позволяет сопоставить значение или результат выражения с определенным типом. Для применения этого паттерна можно использовать оператор <span class="b">instanceof</span> 
или выражения <b>switch</b></p>
<h3>instanceof</h3>

<p>Традиционно оператор <b>instanceof</b> позволял проверить принадлежность объекта определенному типа. Но в последних версиях Java при его применении также можно  автоматически преобразовать объект в сопоставленный тип и 
передать преобразованное значение в определенную переменную. Например, пусть у нас есть следующая система типов:</p>
...
<p>Рассмотрим применение паттерна типов с помощью оператора <code>instanceof</code>:</p>
<pre class="brush:java;">
class Program{
...
<p>В статическом методе <code>printPerson</code> получаем объект Person и с помощью оператора <code>instanceof</code> проверяем принадлежность объекта определенному типу. Так, выражение</p>
<pre class="brush:java;">person instanceof Employee empl</pre>
<p>проверяет, представляет ли параметр <code>person</code> класс <code>Employee</code>, и если представляет (то есть оператор <code>instanceof</code> 
возвращает <code>true</code>), то передает ссылку на этот объект переменной <code>empl</code> типа <code>Employee</code>. И в дальнейшем мы можем использовать эту переменную empl и производить 
с ней различные операции.</p>

<p>Также можно использовать паттерн типов с <code>instanceof</code> в тернарном операторе:</p>
<pre class="brush:java;">
class Program{
...
<h3>Выражения/конструкции switch</h3>
<p>Конструкции/выражения <span class="b">switch</span> позволяют упростить паттерн типов и применение проверки типов:</p>
<pre class="brush:java;">
class Program{
...
<p>Выражения типа <code>case Employee empl</code> аналогично оператору <code>instaceof</code> проверяют принадлежность значения типу Employee и при положительном результате передаются ссылку на объект в переменную empl.</p>

<p>Стоит отметить, что если сопоставляем с типом, то нам надо обязательно указать переменную. Например, мы не можем написать следующим образом:</p>
<pre class="brush:java;">
switch(person){
    case Employee -&gt;
        System.out.println("Person is Employee");
</pre>
<p>После типа <code>Employee</code> нам обязательно надо указать переменную. Однако в случае выше переменная не требуется - даже если мы ее определим, мы ее никак не используем. И в этом случае можно 
указать прочерк _ (так называемая "неименованная переменная"):</p>
<pre class="brush:java;">
static void printPerson(Person person){
    switch(person){
        case Employee _ -&gt;
...
<p>Теоретически да и практически переменная может хранить значение <b>null</b>. При использовании оператора <code>instanceof</code> мы не столкнемся с проблемами, так как этот оператор возвращает 
<code>true</code>, если выражение не равно <code>null</code>:</p>
<pre class="brush:java;">
class Program{
...
<p>В любом случае мы можем добавить проверку на <code>null</code></p>
<pre class="brush:java;">
else if(person == null){
    System.out.println("Person Undefined");
}
</pre>
<p>Но в любом случае, даже если выражение представляет тип Person, может иметь смысл сравнить его тип с типом Person, чтобы отсеить возможность нулевых ссылок:</p>
<pre class="brush:java;">else if(person instanceof Person){ ....</pre>
<h4>switch</h4>
<p>Классический оператор <b>switch</b> генерирует исключение NullPointerException, если выражение, которое передается оператору <code>switch</code>, равно <code>null</code>. 
Этого можно избежать, добавив <code>case null</code>:</p>
<pre class="brush:java;">
class Program{
...
<h3>Защитные условия паттерна или Guards</h3>
<p>При сопоставления паттернов мы можем добавлять дополнительные условия или <span class="b">guards</span>, которым также должны соответствовать выражения.</p>
<h4>instanceof</h4>
<p>Так, при использовании оператора <code>instanceof</code> мы можем по цепочке добавить условные выражения:</p>
<pre class="brush:java;">
class Program{
...
<p>указывает, что объект person не только должен представлять тип Employee, но и его компания должна быть "Yandex". Таким образом, подвыражение <code>empl.getCompany() == "Yandex"</code> и 
есть то, что называется "guard" или условие паттерна. И блок <code>if</code> выполняется, если верно это условие. Если условие не верно, то выполняется следующий блок:</p>
<pre class="brush:java;">
else if(person instanceof Employee){
...
<h4>switch</h4>
<p>ПРи использовании <b>switch</b> для ввода условий применяется оператор <span class="b">when</span>:</p>
<pre class="brush:java;">
class Program{
...
<p>То есть в данном случае выражение</p>
<pre class="brush:java;">when empl.getCompany() == "Yandex"</pre>
<p>И есть защитное условие паттерна.</p>
<p>При этом защитные условия могут быть более сложными по структуре, объединяя несколько под условий:</p>
<pre class="brush:java;">
switch(person){
    case Employee empl 
...
<p>В данном случае блок case выполняется, если объект person представляет тип Employee, но при этом также его имя - "Bob", а компания - "Yandex"</p>
...
<style>
	...
	<div class="nav"><p><a href="./3.25.php">Назад</a><a href="./">Содержание</a><a href="./3.27.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
