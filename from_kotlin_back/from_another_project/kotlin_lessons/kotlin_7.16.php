...
<meta charset="utf-8" />
<meta name="description" content="Массивы в языке программирования Kotlin, одномерные и многомерные массивы, функция arrayOf и arrayOfNulls, получение и изменение элементов массива, перебор массивов и функции forEach и forEachIndexed">
...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Массивы</h2><div class="date">Последнее обновление: 04.03.2024</div>
	<div class="socBlock">
	...
	<p>Массивы во многом аналогичны коллекциям. Массивы характеризуются постоянной длиной, соответственно 
мы не можем динамически добавлять или удалять элементы из массива. Но мы можем изменять значение уже имеющихся элементов.</p>
<p>Многие общие для коллекций стандартные методы реализованы для класса Array в виде функций расширения.</p>

<p>Массивы представляют тип <code>Array</code>, который является обобщенным. Например, определим пару массивов:<p>
<pre class="brush:kt;">
val numbers: Array&lt;Int&gt;       // массив чисел
val people: Array&lt;String&gt;     // массив строк
</pre>
<p>Для создания массива применяется функция <span class="b">arrayOf()</span>:</p>
<pre class="brush:kt;">
val numbers = arrayOf(1, 2, 3, 4, 5)     // объект Array&lt;Int&gt;
val people = arrayOf("Tom", "Sam", "Kate", "Bob", "Alice")   // объект Array&lt;String&gt;
</pre>
<p>В этом случае тип элементов массива будет выводиться из типа значений из функции <code>arrayOf()</code></p>
<p>Встроенная функция <b>arrayOfNulls()</b> позволяет создать массив из элементов, каждый из который имеет значение 
<span class="b">null</span> - то есть фактически не установленное значение. В эту функцию передается количество элементов:</p>
<pre class="brush:kt;">
val numbers = arrayOfNulls&lt;Int&gt;(3)  // [null, null, null]
</pre>
<p>После названия функции в угловых скобках указывается тип элементов массива. Так, в примере выше это тип <span class="b">Int</span>. В данном случае создается массив из трех элементов, 
каждый из которых равен по умолчанию <b>null</b>, то есть не имеет никакого значения.</p>


<p>Также имеется еще один способ создать массив и инициализировать его значениями:</p>
<pre class="brush:kt;">
val numbers = Array(3, {5})	// [5, 5, 5]
</pre>
<p>Здесь применяется конструктор класса Array. В этот конструктор передаются два значения. 
...
<p>Также подобный метод может быть полезен, если массив изначально создан с помощью функции <span class="b">arrayOfNulls</span> и может содержать null, и мы хотим в этом случае возвратить некоторое значение по умолчанию</p>
<pre class="brush:kt;">
fun main() {
...
<h4>Установка данных</h4>
<p>Для установки значений элементов можно применять метод <code>set(index, value)</code>, который принимает индекс элемента и новое значение, или просто индексы:</p>
<pre class="brush:kt;">
...
<p>Для работы с массивами класс Array предоставляет три свойства:</p>
<ul>
<li><p><span class="b">size</span>: размер массива</p></li>
<li><p><span class="b">lastIndex</span>: индекс последнего элемента массива, тоже самое, что и <code>size - 1</code></p></li>
<li><p><span class="b">indices</span>: диапазон индексов элементов массива</p></li>
</ul>
<p>Каждый массив имеет свойство <span class="b">size</span>, которое хранит количество элементов массива:</p>
<pre class="brush:kt;">
val people = arrayOf("Tom", "Bob", "Sam")
...
<p>Также при переборе мы можем использовать индексы для обращения к элементам массива:</p>
<pre class="brush:kt;">
fun main(){
    val people = arrayOf("Tom", "Sam", "Bob")
...
<p>В данном случае мы перебираем не сам массив people, а последовательность чисел от 0 до people.lastIndex, то есть фактически от 0 до 2. Каждое число из этой последовательности попадает 
в переменную i. Затем получаем каждый элемент массива с помощью выражения <code>people[i]</code></p>
<p>Может показаться, что последний способ перебора избыточен - мы же напрямую можем перебрать массив, использовав выражение <code>for(person in people)</code>. Однако что, если мы хотим 
изменить все элементы массива. Например, следующим образом:</p>
<pre class="brush:kt;">
...
<p>В данном случае мы хотим, чтобы в массиве каждое число было заменено квадратом этого числа. Однако написать, как в примере выше мы не можем.</p>
<p>Теперь изменим подход:</p>
<pre class="brush:kt;">
fun main(){
    val numbers = arrayOf(2, 3, 4)
...
<p>Для упрощения перебора индексов элементов массива каждый массив предоставляет свойство <span class="b">indices</span>, который представляет последовательность всех индексов начиная с 0 и 
которое мы также можем перебрать:</p>
<pre class="brush:kt;">
...
<p>Также для перебора можно использовать функцию расширения <span class="b">forEach()</span>. Она принимает функцию-коллбек, параметр которой представляет каждый элемент массива:</p>
<pre class="brush:kt;">
fun main() {
...
<p>Для перебора вместе с индексами можно использовать функцию <span class="b">forEachIndexed()</span>. Она принимает функцию с двумя параметрами: индексом элемента и самим элементом:</p>
<pre class="brush:kt;">
fun main() {
...
<style>
	...
	<div class="nav"><p><a href="./7.5.php">Назад</a><a href="./">Содержание</a><a href="./7.6.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
