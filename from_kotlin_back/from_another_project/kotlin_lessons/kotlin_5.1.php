...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Null и nullable-типы</h2><div class="date">Последнее обновление: 11.12.2024</div>
	<div class="socBlock">
...
<p>Но здесь мы также можем избежать финального возвращения null и присвоить значение по умолчанию:</p>
...
<p>Например, следующий код компилятор не пропустит:</p>
<pre class="brush:kt;">
var name : String? = "Tom"
val id: String = name       // Ошибка
</pre>
<p>Так как name потенциально может быть равно null, поэтому мы не можем присвоить это значение переменной id. Но здесь по коду очевидно, что name НЕ равно null. Возможно, где-то дальше в программе оно может измениться на null, 
но здесь мы уверены, что оно не равно null. Поэтому для утверждения, что nullable-переменная не равна null, мы как раз можем использовать оператор <span class="b">!!</span>:</p>
<pre class="brush:kt;">
fun main() {
    var name : String? = "Tom"
    val id: String = name!!
...
<p>То есть данный оператор применяется, когда мы уверены, что значение НЕ равно <span class="b">null</span>. Если же мы применим этот оператор к значению null, то программа сгенерирует исключение:</p>
<pre class="brush:kt;">
fun main() {
    try{
        var name : String? = null
...
<style>
	...
	<div class="nav"><p><a href="./5.2.php">Назад</a><a href="./">Содержание</a><a href="./5.3.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
