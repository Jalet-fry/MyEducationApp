...
<div class="innercontainer">
   ...
   <div class="item center menC">
     <h2>Конструкторы</h2><div class="date">Последнее обновление: 26.03.2024</div>
	<div class="socBlock">
...
который мы можем использовать. Но также мы можем определять свои собственные конструкторы. Классы в Kotlin могут иметь один <span class="b">первичный конструктор</span> (primary constructor) и один или несколько <span class="b">вторичных конструкторов</span> (secondary constructor).</p>
<h3>Вторичные конструкторы</h3>
<p>Вторичные конструкторы определяются в теле класса с помощью ключевого слова <span class="b">constructor</span>:</p>
<pre class="brush:kt;">
fun main() {
...
<p>Конструкторы во многом похожи на функциии, как и обычные функции, могут иметь параметры. Здесь в классе Person определен конструктор, принимает два параметра: _name и _age - условно говоря имя и возраст человека. Внутри конструктора эти значения передаются переменным name и age:</p>
<pre class="brush:kt;">
constructor(_name: String, _age: Int){
    name = _name
    age = _age
...
<p>В функции main создаются два объекта Person. Для их создания применяется вторичный конструктор, параметрам которого передаются некоторые значения:</p>
<pre class="brush:kt;">
val tom = Person("Tom", 39)
val bob = Person("Bob", 45)
</pre>
...
<p>Аналогичным образом мы можем определять и большее количество конструкторов:</p>
<pre class="brush:kt;">
fun main() {
...
<p>Теперь в классе Person также определен еще один конструктор, который принимает только один параметр - имя человека и передает его значение переменной name. Переменная age 
в данном случае оставляет значение по умолчанию - число 0.</p>
<p>В функции main вызываем оба конструктора:</p>
<pre class="brush:kt;">
val tom = Person("Tom")
val bob = Person("Bob", 45)
</pre>
<p>Для создания первого объекта вызывается первый конструктор, а для второго - второй конструктор. Консольный вывод:</p>
<pre class="sh">
Name: Tom  Age: 0
Name: Bob  Age: 45
</pre>
<p>Однако в данном случае мы сталкиваемся с дублированием кода - установкой переменной name. В данном случае это может быть не актуально. Но что, если мы захотим добавить какую-то более сложную 
...
Для этого применяется ключевое слово <span class="b">this</span>:</p>
<pre class="brush:kt;">
class Person{
    val name: String
    var age: Int = 0
...
<p>Здесь выражение <code>this(_name)</code> как раз и представляет вызов первого конструктора, где и устанавливается имя.</p>
...
<p>Значения параметров первичного конструктора можно использовать внутри класса, например, передать их значения переменным класса:</p>
<pre class="brush:kt;">
fun main() {
...
<p>Стоит отметить, что класс может иметь только один первичный конструктор. И также в классе могут быть одновременно и первичный, и вторичные конструкторы. Однако 
...
<p>Обратите внимание, что если в классе определен только первичный конструктор, то фигурные скобки, которые оформляют тело класса, использовать необязательно. Хотя мы могли бы написать и так:</p>
<pre class="brush:kt;">class Person(val name: String, var age: Int){ }</pre>


<h3>Инициализатор</h3>
<p>Кроме конструкторов для инициализации объектов мы можем использовать блоки инициализаторов. Они представляют блок кода в фигурных скобках, перед которым идет слово <span class="b">init</span> :</p>
<pre class="brush:kt;">
fun main() {
...
<p>Здесь в классе Person определен следующий блок инициализатора:</p>
<pre class="brush:kt;">
init{
        name = _name
        if(_age &gt;0 && _age &lt; 110) age = _age
...
<p>В данном случае блок инициализатора применяется для установки переменных. Так, мы можем определить здесь проверку входных значений, как в случае с параметром _age. Можно определить какую-то другую логику, которая 
должна выполняться при инициализации объекта.</p>
...
<style>
	...
	<div class="nav"><p><a href="./4.1.php">Назад</a><a href="./">Содержание</a><a href="./4.5.php">Вперед</a></p></div>
	<div class="socBlock">
	<div class="share soctop">
	<ul>
	<li><a title="Поделиться в Вконтакте" rel="nofollow" class="fa fa-lg fa-vk"></a></li>
